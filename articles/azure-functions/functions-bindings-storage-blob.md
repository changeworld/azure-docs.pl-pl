...
* Konta magazynu tylko dla obiektów blob
* Wysoka skalowalność
* Opóźnienie zimnego startu

### <a name="blob-only-storage-accounts"></a>Konta magazynu tylko dla obiektów blob

[Konta magazynu tylko dla obiektów blob](../storage/common/storage-create-storage-account.md#blob-storage-accounts) są obsługiwane dla obiektu blob danych wejściowych i wyjściowych powiązania, ale nie dla wyzwalacze obiektów blob. Wyzwalaczy magazynu obiektów blob wymagają konta magazynu ogólnego przeznaczenia.
...
W poniższym przykładzie pokazano obiekt blob wyzwalacza powiązanie w *function.json* pliku i [skrypt języka C# (csx)](functions-reference-csharp.md) kodu korzystającego z powiązania. Jeśli obiekt blob jest dodane lub zaktualizowane w funkcji zapisuje dziennik `samples-workitems` kontenera.
...
> [Przejdź do przewodnika Szybki Start, korzystającą z magazynu obiektów Blob wyzwalacza](functions-create-storage-blob-triggered-function.md)
