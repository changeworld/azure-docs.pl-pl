...
2. W **ustawienia aplikacji** kartę, Znajdź `FUNCTIONS_EXTENSION_VERSION` ustawienie i zmień wartość na prawidłową wersję 1.x środowiska uruchomieniowego lub `~2` dla wersji 2.0. Tylda za pomocą wersji głównej oznacza, użyj najnowszej wersji tej wersji głównej (na przykład, "~ 1"). 
...
```
W tym kodzie Zastąp `<function_app>` nazwę aplikacji funkcji. Zastąp również `<my_resource_group>` nazwą grupy zasobów dla aplikacji funkcji. Zastąp `<version>` przy użyciu prawidłowej wersji 1.x środowiska uruchomieniowego lub `~2` wersji 2.x. 
