...
| Wątkowe interfejsy API | W ramach trwałej struktury zadań działa kod programu Orchestrator w pojedynczym wątku i nie może współdziałać z żadnymi innymi wątkami. Wprowadzenie nowych wątków do wykonywania aranżacji może spowodować niedeterministyczne wykonanie lub zakleszczenia. | Funkcje programu Orchestrator powinny prawie nigdy nie używać interfejsów API wątków. Na przykład w programie .NET Unikaj używania `ConfigureAwait(continueOnCapturedContext: false)`; gwarantuje to, że kontynuacje zadań zostanie uruchomione na oryginalnym `SynchronizationContext`funkcji programu Orchestrator. Jeśli takie interfejsy API są niezbędne, należy ograniczyć ich użycie tylko do funkcji działania. |
