...
> [!NOTE]
> Wersja 2. x nie tworzy tematu lub subskrypcji skonfigurowanej w tym `ServiceBusTrigger` wystąpieniu. Wersja 2. x jest oparta na [Microsoft. Azure. ServiceBus](https://www.nuget.org/packages/Microsoft.Azure.ServiceBus) i nie obsługuje zarządzania kolejki.
...
Powiązanie ServiceBus jest zdefiniowane w *funkcji Function. JSON* , w której *Typ* jest `serviceBusTrigger`ustawiony na.
...
Kod w `func.ServiceBusMessage`   *_\_init_.PRdeklarujeparametr,któryumożliwiaodczytywaniekomunikatuwkolejcewfunkcji.\_*
...
Definicja powiązania ServiceBue jest zdefiniowana w *funkcji Function. JSON* , w której *Typ* jest `serviceBus`ustawiony na.
...
`set` W   *_init\__ .PRmożnanapisaćkomunikatdokolejki\_* , przekazując wartość do metody.
