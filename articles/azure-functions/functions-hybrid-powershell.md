---
title: Zarządzanie zdalnymi zasobami lokalnymi przy użyciu Azure Functions programu PowerShell
...
Połączenia hybrydowe Azure App Service umożliwia dostęp do zasobów w innych sieciach. Więcej informacji na temat tej możliwości można znaleźć w dokumentacji [połączenia hybrydowe](../app-service/app-service-hybrid-connections.md) . Poniższe informacje opisują, jak używać tej funkcji do uruchamiania funkcji programu PowerShell przeznaczonych dla serwera lokalnego. Tego serwera można następnie użyć do zarządzania wszystkimi zasobami w środowisku lokalnym przy użyciu funkcji Azure PowerShell.
...
Poniższy skrypt włącza funkcję komunikacji zdalnej programu PowerShell, tworzy nową regułę zapory i odbiornik https usługi WinRM. Do celów testowych jest używany certyfikat z podpisem własnym. Zalecane jest używanie podpisanego certyfikatu dla środowiska produkcyjnego.
...
App Service Połączenia hybrydowe są dostępne tylko w podstawowych, standardowych i izolowanych planach cenowych. Podczas tworzenia aplikacji funkcji przy użyciu programu PowerShell Utwórz lub wybierz jeden z tych planów.
...
1. Wybierz **przycisk OK** , aby utworzyć plan, a następnie użyj pozostałych ustawień aplikacji funkcji, jak określono w tabeli poniżej obrazu. 
    ...
    | **Storage** |  Nazwa unikatowa w skali globalnej |  Utwórz konto magazynu używane przez aplikację funkcji. Nazwy kont usługi Storage muszą mieć długość od 3 do 24 znaków i mogą zawierać tylko cyfry i małe litery. Możesz także użyć istniejącego konta.
...
Połączenia hybrydowe są konfigurowane z sekcji sieci aplikacji funkcji.

1. Wybierz kartę platforma aplikacji funkcji i wybierz pozycję Sieć.
...
1. Wprowadź informacje dotyczące połączenia hybrydowego, jak pokazano poniżej. Host punktu końcowego może opcjonalnie dopasować nazwę hosta serwera lokalnego, aby ułatwić zapamiętanie serwera w przyszłości przy uruchamianiu poleceń zdalnych. Port jest zgodny z domyślnym portem usługi zdalnego zarządzania systemem Windows zdefiniowanym na serwerze.
![Dodaj połączenie hybrydowe](./media/functions-hybrid-powershell/add-hybrid-connection.png)  
    ...
    **Nazwa połączenia hybrydowego** ContosoHybridOnPremisesServer
    ...
    Finance1 **hosta punktu końcowego**
    
    **Port punktu końcowego** 5986
    
    **ServiceBus przestrzeń nazw** Utwórz nowy
    
    **Lokalizacja** Wybierz dostępną lokalizację
    
    Contosopowershellhybrid **nazwy**

5. Kliknij przycisk OK, aby utworzyć połączenie hybrydowe

## <a name="download-and-install-the-hybrid-connection"></a>Pobierz i zainstaluj połączenie hybrydowe

1. Wybierz ikonę Pobierz Menedżera połączeń, aby zapisać plik msi lokalnie na komputerze.
...
1. Uruchom Instalatora połączenia hybrydowego, aby zainstalować usługę na serwerze lokalnym.
![Zainstaluj połączenie hybrydowe](./media/functions-hybrid-powershell/hybrid-installation.png)  
1. W portalu Otwórz połączenie hybrydowe i skopiuj parametry połączenia bramy do Schowka.
...
1. Uruchom ponownie Menedżera połączeń hybrydowych z programu PowerShell, jeśli nie zostanie on wyświetlony jako połączony.
```powershell
...
1. Wybierz kartę platformy z aplikacji funkcji
1. Wybierz konfigurację z sekcji ![ustawienia ogólne wybierz pozycję Konfiguracja platformy](./media/functions-hybrid-powershell/select-configuration.png)  
1. Wybierz pozycję nowe ustawienie aplikacji, aby utworzyć nowe ustawienie dla hasła
1. Nazwij ustawienie ContosoUserPassword i wprowadź hasło
1. Wybierz przycisk OK, a następnie Zapisz, aby zapisać hasło w aplikacji ![funkcji Dodaj ustawienie aplikacji dla hasła](./media/functions-hybrid-powershell/add-appsetting-password.png)  

## <a name="create-a-function-http-trigger-to-test"></a>Tworzenie wyzwalacza http funkcji do testowania

1. Utwórz nową funkcję wyzwalacza http z aplikacji ![funkcji Utwórz nowy wyzwalacz http](./media/functions-hybrid-powershell/create-http-trigger-function.png)  
...
Zamień te zmienne w powyższym skrypcie na wartości znajdujące się w danym środowisku:
* $HybridEndpoint
* $RemoteServer

Powyższe dwa scenariusze umożliwiają łączenie środowisk lokalnych i zarządzanie nimi przy użyciu programu PowerShell w Azure Functions i Połączenia hybrydowe. Dodatkowe informacje na temat [połączenia hybrydowe](../app-service/app-service-hybrid-connections.md) lub [programu PowerShell](./functions-reference-powershell.md) można znaleźć w temacie funkcje w dokumentacji.
