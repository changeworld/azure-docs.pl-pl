---
title: Urządzenia Intel Edison w chmurze (Node.js) — urządzenia Intel Edison połączyć z usługą Azure IoT Hub | Dokumentacja firmy Microsoft
...
Teraz jest składany tablicy.
   ...
   ![zmontowanych tablicy](media/iot-hub-intel-edison-kit-node-get-started/5_assembled_board.jpg)
...
1. Umieść tarczy Base Grove do tablicy. Upewnij się, że wszystkie kody PIN ściśle są podłączone do tablicy.
   ...
   ![Grove podstawowy Shield](media/iot-hub-intel-edison-kit-node-get-started/6_grove_base_sheild.jpg)

2. Łączenie czujników temperatury Grove na tarczy Base Grove przy użyciu kabla Grove **A0** portu.

   ![Nawiązać połączenie z czujnika temperatury](media/iot-hub-intel-edison-kit-node-get-started/7_temperature_sensor.jpg)

   ![Połączenie Edison i czujnik](media/iot-hub-intel-edison-kit-node-get-started/16_edion_sensor.png)
...
2. Podłącz wczesnych kabla USB do górnej wczesnych portu USB.
   ...
   ![Najważniejsze wczesnych port USB](media/iot-hub-intel-edison-kit-node-get-started/10_top_usbport.jpg)
...
> Upewnij się, że Edison jest podłączony do sieci z komputera. Komputer łączy się z Edison przy użyciu adresu IP.
   ...
   ![Nawiązać połączenie z czujnika temperatury](media/iot-hub-intel-edison-kit-node-get-started/12_configuration_tool.png)
...
1. Aby nawiązać połączenie z urządzeniem Intel Edison, użyj jednej z następujących klientów SSH z komputera hosta. Adres IP jest za pomocą narzędzia konfiguracji i hasło zostało ustawione w tym narzędziu.
    ...
    - Wbudowany klient SSH w systemie Ubuntu lub z systemem macOS.
...
1. Otwórz plik konfiguracji, uruchamiając następujące polecenia:
   ...
   Istnieją dwa makra w tym pliku mogą configurate. Pierwsza z nich jest `INTERVAL`, która definiuje odstęp czasu między dwa komunikaty, które wysyłają do chmury. Drugi `simulatedData`, który jest wartość logiczna wskazująca, czy używać danych z czujników symulowanych lub nie.
...
![Dane wyjściowe — dane czujników wysyłanych z urządzenia Intel Edison do Centrum IoT hub](media/iot-hub-intel-edison-kit-node-get-started/15_message_sent.png)
