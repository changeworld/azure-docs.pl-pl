...
## <a name="register-with-the-sql-vm-resource-provider"></a>Rejestrowanie przy użyciu dostawcy zasobów maszyny wirtualnej SQL
Jeśli [rozszerzenie agenta SQL Server IaaS](virtual-machines-windows-sql-server-agent-extension.md) jest już zainstalowane na maszynie wirtualnej, rejestrowanie przy użyciu dostawcy zasobów maszyny wirtualnej SQL po prostu tworzy zasób metadanych typu Microsoft. SqlVirtualMachine/SqlVirtualMachines. 
...
Jeśli SQL Server rozszerzenie IaaS nie jest zainstalowane na maszynie wirtualnej, można zarejestrować się w dostawcy zasobów maszyny wirtualnej SQL, określając tryb zarządzania uproszczonego. W trybie uproszczonego zarządzania dostawca zasobów maszyny wirtualnej SQL automatycznie zainstaluje rozszerzenie SQL Server IaaS w [trybie uproszczonym](virtual-machines-windows-sql-server-agent-extension.md#install-in-lightweight-mode) i zweryfikuje metadane wystąpienia SQL Server. Nie spowoduje to ponownego uruchomienia usługi SQL Server. Musisz podać typ licencji SQL Server, która ma być zarejestrowana u dostawcy zasobów maszyny wirtualnej SQL: płatność zgodnie z rzeczywistym użyciem (`PAYG`) lub korzyść użycia hybrydowego platformy Azure (`AHUB`). 

Zarejestruj SQL Server maszynę wirtualną w trybie uproszczonego zarządzania przy użyciu następującego fragmentu kodu programu PowerShell:
...
Zarejestrowanie się w [trybie uproszczonym](virtual-machines-windows-sql-server-agent-extension.md#install-in-lightweight-mode) przy użyciu dostawcy zasobów maszyny wirtualnej SQL zapewni zgodność i umożliwi Elastyczne licencjonowanie, a także w miejscu SQL Server aktualizacji wersji. Wystąpienia klastra trybu failover i wdrożenia z wieloma wystąpieniami mogą być rejestrowane przy użyciu dostawcy zasobów maszyny wirtualnej SQL tylko w trybie uproszczonym. W dowolnym momencie można postępować zgodnie z instrukcjami znajdującymi się w Azure Portal, aby przeprowadzić uaktualnienie do [trybu w trybie pełnym](virtual-machines-windows-sql-server-agent-extension.md#install-in-full-mode) i włączyć kompleksowy zestaw funkcji zarządzania przy użyciu SQL Server ponownego uruchomienia. 
...
SQL Server 2008 i 2008 R2 zainstalowane w systemie Windows Server 2008 można zarejestrować przy użyciu dostawcy zasobów maszyny wirtualnej SQL w trybie ([no-Agent](virtual-machines-windows-sql-server-agent-extension.md)). Ta opcja zapewnia zgodność i umożliwia monitorowanie SQL Server maszyny wirtualnej w Azure Portal z ograniczoną funkcjonalnością.
...
Aby zarejestrować wystąpienie SQL Server 2008 lub 2008 R2 w systemie Windows Server 2008, użyj następującego fragmentu kodu programu PowerShell:  
...
### <a name="azure-cli"></a>Interfejs wiersza polecenia platformy Azure
  ...
  ```
`ProvisioningState`będzie wyświetlana `Succeeded` , jeśli rejestracja zakończyła się pomyślnie. 
...
Aplikacja konsolowa zostanie zamknięta po sprawdzeniu, czy na maszynie wirtualnej jest uruchomione co najmniej jedno wystąpienie SQL Server. Rejestracja przy użyciu dostawcy zasobów maszyny wirtualnej SQL w trybie uproszczonym nie spowoduje ponownego uruchomienia SQL Server i nie spowoduje utworzenia agenta na maszynie wirtualnej.
