---
title: Obsługa powiadomień dotyczących konserwacji maszyn wirtualnych z systemem Linux na platformie Azure | Microsoft Docs
description: Wyświetl powiadomienia o konserwacji dla maszyn wirtualnych z systemem Linux działających na platformie Azure i uruchom konserwację samoobsługową.
...
ms.locfileid: "70018839"
---
# <a name="handling-planned-maintenance-notifications-for-linux-virtual-machines"></a>Obsługa powiadomień dotyczących planowanej konserwacji dla maszyn wirtualnych z systemem Linux
...
- Jeśli nieprzerwana utrata pewnej pojemności (1/aktualizacja liczby domen) jest istotna, można ją łatwo wyrównać przez przydzielenie wystąpień dodawania w okresie konserwacji. 
...
## <a name="find-vms-scheduled-for-maintenance-using-cli"></a>Znajdowanie maszyn wirtualnych zaplanowanych do konserwacji przy użyciu interfejsu wiersza polecenia

Informacje o planowanej konserwacji można zobaczyć przy użyciu [usługi Azure VM Get-instance-View](/cli/azure/vm?view=azure-cli-latest).
 
Informacje o konserwacji są zwracane tylko wtedy, gdy jest planowana konserwacja. Jeśli nie zaplanowano konserwacji, która ma wpływ na maszynę wirtualną, polecenie nie zwraca żadnych informacji o obsłudze. 
...
| MaintenanceWindowStartTime            | Początek zaplanowanego okna konserwacji, w którym platforma Azure inicjuje konserwację na maszynie wirtualnej |
...
## <a name="start-maintenance-on-your-vm-using-cli"></a>Rozpocznij konserwację na maszynie wirtualnej przy użyciu interfejsu wiersza polecenia

Następujące wywołanie spowoduje zainicjowanie konserwacji na maszynie wirtualnej `IsCustomerInitiatedMaintenanceAllowed` , jeśli jest ustawiona na wartość true.
