...
* W przypadku RHEL 7,7 (wersja zapoznawcza) pakiet Cloud-init jest obsługiwanym pakietem: *18.5-3.el7*
...
Jeśli nie dołączysz przełącznika interfejsu wiersza `--custom-data` polecenia platformy Azure w czasie aprowizacji, wala przyjmuje minimalne parametry inicjowania obsługi maszyny wirtualnej wymagane do zainicjowania obsługi maszyny wirtualnej i ukończą wdrożenie przy użyciu ustawień domyślnych.  Jeśli odwołujesz się do przełącznika `--custom-data` Cloud-init, niezależnie od tego, jakie są zawarte w danych niestandardowych (poszczególne ustawienia lub pełny skrypt), zastępuje domyślne wartości wala. 
