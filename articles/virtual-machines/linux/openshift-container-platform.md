---
title: Wdrażanie platformy kontenera OpenShift na platformie Azure | Microsoft Docs
description: Wdróż platformę kontenera OpenShift na platformie Azure.
...
ms.locfileid: "70091776"
---
# <a name="deploy-openshift-container-platform-in-azure"></a>Wdrażanie platformy kontenera OpenShift na platformie Azure

Do wdrożenia platformy kontenera OpenShift na platformie Azure można użyć jednej z kilku metod:
...
## <a name="deploy-using-the-openshift-container-platform-resource-manager-template"></a>Wdrażanie przy użyciu szablonu Menedżer zasobów platformy kontenera OpenShift
...
| `infraVmSize` | Rozmiar infrastruktury maszyny wirtualnej. Wybierz jeden z dozwolonych rozmiarów maszyn wirtualnych wymienionych w pliku azuredeploy. JSON |  | Standard_D4s_v3 |
| `nodeVmSize` | Rozmiar maszyny wirtualnej węzła aplikacji. Wybierz jeden z dozwolonych rozmiarów maszyn wirtualnych wymienionych w pliku azuredeploy. JSON |  | Standard_D4s_v3 |
...
| `virtualNetworkNewOrExisting` | Wybierz, czy chcesz użyć istniejącego Virtual Network, czy utworzyć nowy Virtual Network | istniejącego <br> nowy | nowy |
...
| `routerClusterType` | Określ, czy klaster ma używać prywatnych, czy publicznych węzłów infrastruktury. W przypadku wybrania opcji prywatne węzły infrastruktury nie będą ujawniane w Internecie za pośrednictwem publicznego adresu IP. Zamiast tego będzie używany prywatny adres IP określony w`routerPrivateClusterIp` | publiczny <br> prywatna | publiczny |
