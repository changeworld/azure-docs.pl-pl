...
description: Jak zarejestrować Azure Backup SQL Server uruchomione na maszynie wirtualnej platformy Azure
services: backup
...
Azure Backup, wśród innych ofert zapewnia obsługę tworzenia kopii zapasowych obciążeń, takich jak SQL Server uruchomione na maszynach wirtualnych platformy Azure. Ponieważ aplikacja SQL jest uruchomiona na maszynie wirtualnej platformy Azure, usługa kopia zapasowa musi mieć uprawnienia dostępu do aplikacji i pobrać niezbędne szczegóły.
...
Azure Backup obsługuje Tagi sieciowej grupy zabezpieczeń, wdrażając serwer proxy lub wymienione zakresy adresów IP; Aby uzyskać szczegółowe informacje na temat każdej z tych metod [](https://docs.microsoft.com/azure/backup/backup-sql-server-database-azure-vms#establish-network-connectivity), zapoznaj się z tym artykułem.

## <a name="extension-schema"></a>Schemat rozszerzenia

Wartości konfiguracyjne schematu i właściwości (ustawienia środowiska uruchomieniowego) są przekazywane do interfejsu API usługi CRP. Te wartości konfiguracji są używane podczas rejestracji i uaktualniania. Rozszerzenie **AzureBackupWindowsWorkload** również używa tego schematu. Schemat jest wstępnie ustawiony; w polu objectStr można dodać nowy parametr
