...
2. Później aplikacja żąda od usługi Azure AD „tokenu dostępu”. Token dostępu jest wystawiany dla określonego zestawu uprawnień dla konkretnego identyfikatora usługi/interfejsu API (https://api.timeseries.azure.com), określanych również jako „odbiorcy” tokenu. Uprawnienia tokenu są wystawiane w imieniu zalogowanego użytkownika. Identyfikator usługi/interfejsu API występuje jeszcze w innej właściwości zawartej w rejestracji aplikacji w usłudze Azure AD. Gdy biblioteka ADAL zwraca token dostępu do aplikacji, jest on przekazywany jako „token okaziciela” podczas uzyskiwania dostępu do interfejsów API usługi TSI.
