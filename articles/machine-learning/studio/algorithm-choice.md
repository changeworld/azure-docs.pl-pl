...
Uczenie nadzorowane to popularny i przydatny typ uczenia maszynowego. Po jednym wyjątku wszystkie moduły w Azure Machine Learning Studio (klasyczny) są nadzorowanymi algorytmami uczenia. Istnieje kilka rodzajów nadzorowanych szkoleń, które są reprezentowane w Azure Machine Learning Studio (klasyczny): Klasyfikacja, regresja i wykrywanie anomalii.
...
Alternatywnie istnieje blok modułu służący do [czyszczenia parametrów](algorithm-parameters-optimize.md) w Azure Machine Learning Studio (klasyczny), który automatycznie podejmuje próbę wykonania wszystkich kombinacji parametrów przy wybranej szczegółowości. Chociaż jest to świetny sposób, aby upewnić się, że połączono miejsce parametru, czas wymagany do uczenia modelu rośnie wykładniczo z liczbą parametrów.
...
| **Algorytm** | **Accuracy** | **Czas uczenia** | **Liniowość** | **Parametry** | **Uwagi** |
| --- |:---:|:---:|:---:|:---:| --- |
...
| [one-v-all](/azure/machine-learning/studio-module-reference/one-vs-all-multiclass) |- |- |- |- |Zobacz właściwości wybranej metody dwuklasowej |
| **Regression** | | | | | |
...
Metody bayesowskie mają wysoce pożądaną jakość: unikają zamontowania. W tym celu należy przede wszystkim założeń na temat możliwie najłatwiejszej dystrybucji odpowiedzi. Innym ubocznym tego podejścia jest to, że mają one kilka parametrów. Azure Machine Learning Studio (klasyczny) ma algorytmy bayesowskie dla obu klasyfikacji ([Bayesa "punkt" z dwiema klasami](/azure/machine-learning/studio-module-reference/two-class-bayes-point-machine)) i regresji ([bayesowskie regresja liniowa](/azure/machine-learning/studio-module-reference/bayesian-linear-regression)).
...
Azure Machine Learning Studio (klasyczny) obejmuje również dostęp do zaawansowanej platformy uczenia maszynowego pod tytułem [Vowpal Wabbit](/azure/machine-learning/studio-module-reference/train-vowpal-wabbit-version-7-4-model).
...
* Aby zapoznać się z pełną alfabetyczną listą algorytmów i modułów w Machine Learning Studio (klasyczny), zobacz [listę a-z dla modułów Machine Learning Studio (klasycznych)](/azure/machine-learning/studio-module-reference/a-z-module-list) w temacie Machine Learning Studio (klasyczny) i pomoc dotyczącą modułów.
