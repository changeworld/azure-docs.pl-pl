...
2. Utwórz obszar roboczy Azure Machine Learning i Pobierz jego plik konfiguracji, wykonując poniższe kroki.


### <a name="create-a-workspace"></a>Tworzenie obszaru roboczego

Obszar roboczy Azure Machine Learning to podstawowe zasoby w chmurze, za pomocą których można eksperymentować, uczeniować i wdrażać modele uczenia maszynowego. Wiąże się to z łatwym w użyciu zestawem SDK i grupą zasobów platformy Azure. Jeśli masz już obszar roboczy Azure Machine Learning, [Przejdź do następnej sekcji](#config). W przeciwnym razie utwórz je teraz.

[!INCLUDE [aml-create-portal](../../../includes/aml-create-in-portal.md)]

### <a name="config"></a>Pobierz plik config. JSON

1. W górnej części strony obszaru roboczego wybierz pozycję **Pobierz plik config. JSON**.

    ![Pobierz plik config. JSON](media/tutorial-1st-r-experiment/download-config.png)  

1. Dodaj ten plik do katalogu **samouczków** .

Teraz można przystąpić do uruchamiania samouczka.

Zalecamy używanie RStudio do uruchamiania tego samouczka. W RStudio wybierz pozycję Plik > Nowy projekt > istniejący katalog i wybierz utworzony powyżej katalog **samouczków** .

> [!Important]
> Pozostała część tego artykułu zawiera tę samą zawartość, która jest widoczna w części **uczenie i wdrażanie-do-ACI. Plik RMD** .  
...
* Połącz z obszarem roboczym, aby komputer lokalny mógł komunikować się z zasobami zdalnymi
...
Dzięki modelowi można przewidzieć niebezpieczeństwo zgonu z kolizji. Użyj platformy Azure ML do wdrożenia modelu jako usługi predykcyjnej. W tym samouczku zostanie wdrożona usługa sieci Web w [Azure Container Instances](https://docs.microsoft.com/en-us/azure/container-instances/) (ACI).
