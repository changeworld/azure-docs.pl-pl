...
[![Wykres eksperymentu](media/how-to-ui-sample-classification-predict-credit-risk-cost-sensitive/graph.png)](media/how-to-ui-sample-classification-predict-credit-risk-cost-sensitive/graph.png#lightbox)
...
4. Wybierz przycisk **Otwórz** dla przykładowego eksperymentu 4:
    ...
    ![Otwieranie eksperymentu](media/how-to-ui-sample-classification-predict-credit-risk-cost-sensitive/open-sample4.png)
...
W tym doświadczeniu porównano dwa różne podejścia do generowania modeli w celu rozwiązania tego problemu:

- Szkolenie z oryginalnym zestawem danych.
- Szkolenie z replikowanym zestawem danych.

Oba podejścia umożliwiają ocenę modeli przy użyciu zestawu danych testowych z replikacją w celu zapewnienia, że wyniki są wyrównane przy użyciu funkcji koszt. Przetestuj dwa klasyfikatory z obu metod: **Dwie klasy obsługują maszyny wektorowe** i **dwuklasowe drzewo decyzyjne**.
...
![Wykres eksperymentu](media/how-to-ui-sample-classification-predict-credit-risk-cost-sensitive/score-part.png)

W fazie oceny eksperymentu jest obliczana dokładność każdego z czterech modeli. Na potrzeby tego eksperymentu należy użyć **Oceń model** , aby porównać przykłady, które mają ten sam koszt niesklasyfikowany.
