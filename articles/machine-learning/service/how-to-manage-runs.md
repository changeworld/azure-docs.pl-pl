...
Jeśli zakończy się przebieg, ale zawiera błąd (na przykład skrypt szkoleniowy niepoprawne użyto), możesz użyć [ `fail()` ](https://docs.microsoft.com/python/api/azureml-core/azureml.core.run(class)#fail-error-details-none--error-code-none---set-status-true-) metodę, aby oznaczyć go jako nie powiodła się.
