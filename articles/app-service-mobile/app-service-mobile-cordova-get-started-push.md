...
Otwórz pliku config.xml w aplikacji Cordova. Następnie zastąp `Your_Project_ID` liczbowym identyfikator projektu dla aplikacji na podstawie [konsoli dewelopera Google][18].
...
Aby zapewnić obsługę powiadomień wypychanych w swoich kompilacjach domyślne (debugowanie), otwórz plik build.json. Następnie skopiuj konfigurację "wersja" konfiguracji debugowania.
...
Po zainstalowaniu aktualizacji plik build.json powinien zawierać następujący kod:
