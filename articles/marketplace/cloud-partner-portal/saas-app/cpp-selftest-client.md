---
title: Samodzielnie przetestować klienta w celu wstępnego zweryfikowania aplikacji SaaS usługi Azure | Dokumentacja firmy Microsoft
description: Jak utworzyć samodzielnie przetestować klienta wstępnie weryfikowania aplikacji SaaS usługi Azure.
...
---
# <a name="create-a-self-test-client-to-pre-validate-an-azure-saas-application"></a>Tworzenie klienta samodzielnie przetestować w celu wstępnego zweryfikowania aplikacji SaaS usługi Azure
...
Interfejs API ma następującą strukturę.
...
|          DNSName          |         Nazwa DNS maszyny Wirtualnej, który będzie do testowania.          |
|          Użytkownik          |          Nazwa użytkownika do nawiązywania połączenia i zalogowanie się do maszyny wirtualnej.         |
|           Hasło         |         Hasło do nawiązywania połączenia i zalogowanie się do maszyny Wirtualnej.          |
|          System operacyjny          |         System operacyjny maszyny wirtualnej "Windows" lub "Linux".          |
|         PortNo           |         Liczba otwartego portu do nawiązywania połączenia z maszyną Wirtualną. Numer portu to zazwyczaj "22" do "5986" dla Windows i Linux.          |
...
Poniższy zrzut ekranu przedstawia przykład JSON wyniki testów wyświetlane w trybie online przeglądarkę kodu JSON.
...
Do wywołania interfejsu API za pomocą programu cURL, wykonaj następujące kroki:
...
3. Na pasku nawigacyjnym po lewej stronie wybierz **wszystkich usług** , a następnie wybierz **usługi Azure Active Directory**.
   ...
   W poniższych krokach możesz potrzebować nazwa dzierżawy (lub nazwę katalogu) lub identyfikator dzierżawy (lub identyfikator katalogu).
...
20. Utwórz klucz tajny, wybierając klawisz **opis** pola tekstowego. Określ następujące pola:
    ...
    - W obszarze **wartość**, skopiuj klucz.
     ...
     >[!Important]
     >Nie można wyświetlić wartość klucza po wyjściu z formularza kluczy.
...
Po pomyślnym przetestowaniu aplikacji SaaS usługi Azure, możesz [publikowanie oferty](./cpp-publish-offer.md)