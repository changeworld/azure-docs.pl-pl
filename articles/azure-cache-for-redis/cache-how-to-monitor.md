...
| Podłączeni klienci |Liczba połączeń klienta z pamięcią podręczną w określonym interwale raportowania. To mapuje `connected_clients` z Redis INFO. Po osiągnięciu [limitu połączenia](cache-configure.md#default-redis-server-configuration) kolejne próby połączenia z pamięcią podręczną zakończą się niepowodzeniem. Należy pamiętać, że nawet jeśli nie ma aktywnych aplikacji klienckich, nadal może istnieć kilka wystąpień połączonych klientów z powodu wewnętrznych procesów i połączeń. |
