...
| Metryka | Opis |
| --- | --- |
| Trafienia pamięci podręcznej |Liczba pomyślnych wyszukiwań kluczy w określonym interwale raportowania. To mapuje `keyspace_hits` do programu z Redis [info](https://redis.io/commands/info) . |
...
| Używana pamięć |Ilość pamięci podręcznej używana dla par klucz/wartość w pamięci podręcznej w MB w określonym interwale raportowania. Ta wartość jest mapowana `used_memory` na podstawie polecenia Redis info. Nie obejmuje to metadanych ani fragmentacji. |
