...
* Wychodząca łączność sieciowa z *ocsp.msocsp.com,* *mscrl.microsoft.com*i *crl.microsoft.com*. Ta łączność jest potrzebna do obsługi funkcji SSL.
...
Łączenie się z pamięcią podręczną platformy Azure dla wystąpienia Redis z aplikacji lokalnej przy użyciu usługi ExpressRoute nie jest typowym scenariuszem użycia ze względu na wydajność (dla najlepszej wydajności usługa Azure Cache dla klientów Redis powinna znajdować się w tym samym regionie co pamięć podręczna Azure dla redis) .
