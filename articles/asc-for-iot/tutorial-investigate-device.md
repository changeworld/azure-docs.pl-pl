---
title: ASC samouczek badania urządzenia IoT (wersja zapoznawcza) | Dokumentacja firmy Microsoft
description: W tym artykule opisano sposób ASC IoT umożliwia zbadanie podejrzanych urządzeń IoT za pomocą usługi Log Analytics.
services: ascforiot
...
ms.locfileid: "58541581"
---
# <a name="tutorial-investigate-a-suspicious-iot-device"></a>Samouczek: Zbadanie podejrzanych urządzeń IoT
...
W tym samouczku Użyj sugestie badania, pod warunkiem, aby ustalić potencjalne zagrożenia dla Twojej organizacji, należy zdecydować, jak rozwiązania i najlepszych sposobów, aby zapobiec atakom podobne w przyszłości.  
...
## <a name="where-can-i-access-my-data"></a>Gdzie mogę uzyskać dostęp Moje dane
...
Aby uzyskać dostęp do szczegółowych informacji i nieprzetworzonych danych dotyczących urządzeń IoT, przejdź do obszaru roboczego usługi Log Analytics [gdzie dostępu Moje dane](#where-can-i-access-my-data).
...
Aby dowiedzieć się, należy użyć tych danych: 
  ...
  2. Użytkownicy z dostępem mają poziomy uprawnień, oczekiwano? 

- Aby dowiedzieć się, jak użytkownicy będą mogli uzyskać dostęp do urządzenia, użyj następującego zapytania kql:
  ...
  2. Nasłuchiwania gniazd, które są obecnie powinien być aktywny?

- Aby dowiedzieć się, który zalogował się do urządzenia, użyj następującego zapytania kql: 
  ...
  ~~~
    ...
    Aby dowiedzieć się, należy użyć wyników zapytania:
  1. Użytkowników, którzy zalogowany do urządzenia?
  2. Czy użytkownicy, którzy zalogowali połączyć z oczekiwanych lub nieoczekiwany adresów IP?
  
- Aby dowiedzieć się, jeśli urządzenie zachowuje się zgodnie z oczekiwaniami, użyj następującego zapytania kql:
...
Po zbadaniu urządzenia i uzyskania lepszego zrozumienia ryzyka, warto wziąć pod uwagę [Konfigurowanie alertów niestandardowych](quickstart-create-custom-alerts.md) do zwiększenia poziomu bezpieczeństwa rozwiązań IoT. Jeśli nie masz jeszcze agent urządzenia, należy wziąć pod uwagę [wdrażanie agenta zabezpieczeń](select-deploy-agent.md) lub [zmiana konfiguracji istniejącego agenta urządzenia](concept-agent-configuration.md) aby poprawić wyniki. 
