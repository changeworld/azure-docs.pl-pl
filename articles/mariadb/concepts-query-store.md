---
title: Query Store w usłudze Azure Database dla serwera MariaDB
description: W tym artykule opisano funkcję Store zapytania w usłudze Azure Database dla serwera MariaDB
author: ajlam
ms.author: andrela
ms.service: mariadb
ms.topic: conceptual
...
1. Zaloguj się do witryny Azure portal i wybierz usługi Azure Database dla serwera MariaDB.
...
Użyj [witryny Azure portal](howto-server-parameters.md) można pobrać lub ustawić inną wartość dla parametru.

## <a name="views-and-functions"></a>Widoki i funkcje

Wyświetlanie i zarządzanie nimi Query Store przy użyciu następujących widoki i funkcje. Każda osoba w [wybierz uprawnienia roli publicznej](howto-create-users.md#create-additional-admin-users) użyć tych widoków, aby wyświetlić dane w Query Store. Widoki te są dostępne tylko w **mysql** bazy danych.
...
- Jeśli dany serwer MariaDB parametru `default_transaction_read_only` , Query Store nie mogą przechwytywać dane.
