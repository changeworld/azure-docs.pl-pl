...
Azure Event Grid to w pełni zarządzana usługa routingu zdarzeń, która zapewnia jednorodne użycie zdarzeń przy użyciu modelu publikowania/subskrybowania. W tym przewodniku szybki start utworzysz rejestr kontenerów za pomocą interfejsu wiersza polecenia platformy Azure, zasubskrybujesz zdarzenia rejestru, a następnie wdrożono przykładową aplikację sieci Web do odbierania zdarzeń. Na koniec Wyzwalasz `push` obrazu kontenera i zdarzenia `delete` i wyświetlaj ładunek zdarzenia w przykładowej aplikacji.
...
Następnie wdróż rejestr kontenerów w grupie zasobów przy użyciu następujących poleceń. Przed uruchomieniem polecenia [AZ ACR Create][az-acr-create] należy ustawić `ACR_NAME` na nazwę rejestru. Nazwa musi być unikatowa w ramach platformy Azure i jest ograniczona do 5-50 znaków alfanumerycznych.
...
Wykonaj następujące polecenie interfejsu wiersza polecenia platformy Azure, aby skompilować obraz kontenera z zawartości repozytorium GitHub. Domyślnie zadania ACR automatycznie wypchnięcia pomyślnie skompilowanego obrazu do rejestru, co generuje zdarzenie `ImagePushed`.
...
Teraz Wygeneruj zdarzenie `ImageDeleted`, usuwając obraz za pomocą polecenia [AZ ACR Repository Delete][az-acr-repository-delete] :
...
Teraz można wypchnąć obraz do rejestru, a następnie go usunąć. Przejdź do aplikacji sieci Web w przeglądarce Event Grid i zobacz `ImageDeleted` i `ImagePushed` zdarzenia. Możesz również zobaczyć zdarzenie weryfikacji subskrypcji wygenerowane przez wykonanie polecenia w sekcji [subskrybowanie zdarzeń rejestru](#subscribe-to-registry-events) .

Poniższy zrzut ekranu przedstawia przykładową aplikację z trzema zdarzeniami, a zdarzenie `ImageDeleted` zostanie rozwinięte, aby wyświetlić jego szczegóły.

![Przeglądarka sieci Web przedstawiająca przykładową aplikację ze zdarzeniami ImagePushed i ImageDeleted][sample-app-03]

Gratulacje! Jeśli widzisz zdarzenia `ImagePushed` i `ImageDeleted`, rejestr wysyła zdarzenia do Event Grid i Event Grid przekazuje te zdarzenia do punktu końcowego aplikacji sieci Web.
