...
> [!NOTE]
> Jeśli konieczne jest dystrybuowanie identycznych obrazów kontenerów w wielu regionach platformy Azure, Azure Container Registry obsługuje również [replikację geograficzną](container-registry-geo-replication.md). Dzięki replikacji geograficznej rejestru (wymagana jednostka SKU Premium) można obsłużyć wiele regionów z identycznymi nazwami obrazów i tagów z jednego rejestru.
>
...
Na przykład zaimportuj najnowszy obraz systemu Windows Server Core z repozytorium `windows` w programie Microsoft Container Registry.
...
Na przykład zaimportuj obraz `aci-helloworld:latest` ze źródłowego rejestru *mysourceregistry* do *rejestru* w tej samej subskrypcji platformy Azure.
