...
Użytkownicy rozpoczynający korzystanie z usługi Azure SQL Data Warehouse nie powinni przerażać się bogactwem informacji zawartych w tym artykule.  Zasadniczo tematy zostały uporządkowane według ważności.  Na początek warto skupić się choćby na kilku pierwszych koncepcjach — pozwoli to uzyskać znaczne korzyści.  Jak można uzyskać bardziej znanych i doświadczenia w używaniu SQL Data Warehouse, możesz wrócić i przyjrzyj się kilka koncepcji więcej.  Wkrótce wszystko zacznie składać się w spójną całość.
...
Zobacz też [Zarządzanie statystykami tabel][Manage table statistics], [CREATE STATISTICS][CREATE STATISTICS], [UPDATE STATISTICS][UPDATE STATISTICS]
...
Zobacz też [ładowanie danych][Load data], [Guide for using PolyBase][Guide for using PolyBase], [Azure SQL Data Warehouse strategii i wzorców ładowania][Azure SQL Data Warehouse strategii i wzorców ładowania], [ Ładowanie danych za pomocą usługi Azure Data Factory][Load Data with Azure Data Factory], [Move data with Azure Data Factory][Move data with Azure Data Factory], [CREATE EXTERNAL FILE FORMAT][CREATE EXTERNAL FILE FORMAT], [Tworzenie tabeli przy użyciu instrukcji select (CTAS)][Create table as select (CTAS)]
...
Zobacz też [Omówienie tabel][Table overview], [Table distribution][Table distribution], [wybór dystrybucji tabel][wybór dystrybucji tabel], [CREATE TABLE][CREATE TABLE], [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]
...
Zobacz też [opis transakcji][Understanding transactions], [Optimizing transactions][Optimizing transactions], [Partycjonowanie tabel][Partycjonowanie tabel], [TRUNCATE TABLE][TRUNCATE TABLE] , [ALTER TABLE][ALTER TABLE], [Tworzenie tabeli przy użyciu instrukcji select (CTAS)][Tworzenie tabeli przy użyciu instrukcji select (CTAS)]
...
Zobacz też [Omówienie tabel][Table overview], [Table data types][Table data types], [CREATE TABLE][CREATE TABLE]
...
Zobacz też [indeksy tabel][Table indexes], [Columnstore indexes guide][Columnstore indexes guide], [ponowne tworzenie indeksów magazynu kolumn][ponowne tworzenie indeksów magazynu kolumn]
...
Zobacz też [klasy zasobów do zarządzania obciążeniem](resource-classes-for-workload-management.md), [sys.dm_pdw_waits][sys.dm_pdw_waits]
