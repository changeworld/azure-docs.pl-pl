...
Można utworzyć kopię dowolnej aplikacji, minus wszystkie wystąpienia urządzeń, historię danych urządzeń i dane użytkowników. Kopia jest aplikacją standardową, za którą zostanie naliczona stawka. Nie można utworzyć aplikacji próbnej przez skopiowanie aplikacji.

Wybierz pozycję **Kopiuj**. W oknie dialogowym Wprowadź szczegóły nowej aplikacji standardowej. Następnie wybierz pozycję **Kopiuj** , aby potwierdzić, że chcesz kontynuować. Aby dowiedzieć się więcej na temat pól w formularzu, zobacz [Tworzenie aplikacji](quick-deploy-iot-central.md) — Szybki Start.
...
> [!NOTE]
> Po utworzeniu aplikacji przy użyciu **linku** możliwego do udostępniania dostępne lokalizacje to **Europa** i **US**.
