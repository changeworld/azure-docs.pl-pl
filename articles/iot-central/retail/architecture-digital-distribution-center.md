...
Kamery wideo są podstawowymi czujnikami w tym połączonym cyfrowo ekosystemie korporacyjnym. Postępy w uczeniu maszynowym i sztucznej inteligencji, które umożliwiają przekształcanie wideo w uporządkowane dane i przetwarzanie ich na urządzeniach brzegowych przed wysłaniem do chmury. Możemy użyć kamer IP do przechwytywania obrazów, kompresji ich w aparacie, a następnie wysyłania skompresowanych danych na podstawie danych obliczeniowych do potoku analizy wideo lub użyć kamer wizyjnych GigE do przechwytywania obrazów na czujniku, a następnie wysyłania tych obrazów bezpośrednio do usługi Azure IoT Edge, która następnie kompresuje przed przetworzeniem w potoku analizy wideo. 
