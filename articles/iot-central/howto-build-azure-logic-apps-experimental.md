...
1. Rozpocznij od [tworzenia reguły w IoT Central](howto-create-telemetry-rules-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json). Po zapisaniu warunki reguły kliknij **usługi Azure Logic Apps** Kafelek jako nową akcję. Kliknij przycisk **Utwórz w witrynie Azure portal**. Nastąpi przekierowanie do witryny Azure portal utworzyć nową aplikację logiki. Może być konieczne Zaloguj się do konta platformy Azure.
...
1. Wprowadź wymagane informacje, aby utworzyć nową aplikację logiki. Możesz wybrać subskrypcję platformy Azure, aby zainicjować obsługę nowej aplikacji logiki do. Nie ma być ta sama subskrypcja, użytym do utworzenia aplikacji IoT Central. Kliknij pozycję **Utwórz**.
    ...
    ![Tworzenie aplikacji logiki w witrynie Azure portal](./media/howto-build-azure-logic-apps-experimental/createinazureportal.png)

1. Po pomyślnym utworzeniu aplikacji logiki są automatycznie wyświetlana strona wskazywana Projektant aplikacji logiki. Kliknij przycisk **pusta aplikacja logiki**. 

    ![Tworzenie pustej aplikacji logiki](./media/howto-build-azure-logic-apps-experimental/blanklogicapp.png)
...
1. W projektancie, wyszukaj "iot central", a następnie wybierz **po wyzwoleniu reguły** wyzwalacza. Zaloguj się do łącznika przy użyciu konta, którego zalogujesz się do Twojej aplikacji IoT Central, za pomocą.
    ...
    ![Zaloguj się do łącznika IoT Central](./media/howto-build-azure-logic-apps-experimental/addtrigger.png)

1. Po pomyślnym zalogowaniu powinny być widoczne pola są wyświetlane. Wybierz **aplikacji** i **reguły** z list rozwijanych.

    ![Wybierz aplikację i reguły](./media/howto-build-azure-logic-apps-experimental/pickappandrule.png)
...
1. W akcji, wybierz **zespołu** i **kanału**. Wypełnij **komunikat** pole co chcesz każdą wiadomość mówią. Możesz uwzględnić *zawartości dynamicznej* z reguły IoT Central, przekazując wzdłuż ważne informacje, takie jak nazwa urządzenia i sygnatura czasowa do powiadomienia.
    ...
    ![Otwórz edycji akcji aplikacji logiki za pomocą okienka dynamicznej](./media/howto-build-azure-logic-apps-experimental/buildworkflow.png)
...
![Identyfikator urządzenia w Eksploratorze urządzenia IoT Central](./media/howto-build-azure-logic-apps-experimental/iotcdeviceid.png)
