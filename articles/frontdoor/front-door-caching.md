...
Usługa drzwiami frontowymi zapewnia duże pliki bez limitu rozmiaru pliku. Drzwiami frontowymi wykorzystuje technikę o nazwie obiekt segmentu. Jeśli wymagane są duże pliki, drzwiami frontowymi pobiera mniejsze kawałki pliku z wewnętrznej bazy danych. Po otrzymaniu żądania pliku full lub zakres bajtów, środowisko drzwiami frontowymi żąda pliku z wewnętrznej bazy danych we fragmentach po 8 MB.
...
- "application/x-host z wieloma adresami cgi"
...
- "tekst/js", "text/plain"
- "tekst/richtext"
- "tekst/kartę zawierającego wartości rozdzielane przecinkami"
- "text/xml"
- "tekst/x skrypt"
- "tekst/x-component"
- "tekst/x-java-source"
...
1. **Pojedynczy adres URL przeczyszczania**: przeczyszczania poszczególnych zasobów, określając pełny adres URL z rozszerzeniem pliku, na przykład /pictures/strasbourg.png;
2. **Symbol wieloznaczny przeczyszczania**: gwiazdki (\*) może być używana jako symbol wieloznaczny. Przeczyść wszystkich folderów, podfolderów i plików w ramach punktu końcowego za pomocą /\* w ścieżce lub Przeczyść wszystkie podfoldery i pliki w określonym folderze, określając folder następuje /\*, na przykład /zdjęcia/miasto.PNG\*.
