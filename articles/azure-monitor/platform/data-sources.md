...
| Centrum zdarzeń | Przesyłanie strumieniowe dzienników usługi Azure AD do innych lokalizacji przy użyciu Event Hubs. | [Samouczek: Przesyłanie strumieniowe dzienników Azure Active Directory do centrum zdarzeń platformy Azure (](../../active-directory/reports-monitoring/tutorial-azure-monitor-stream-logs-to-event-hub.md)wersja zapoznawcza). |
...
| Dzienniki Azure Monitor | Skonfiguruj dzienniki Azure Monitor w celu zebrania dziennika aktywności w celu przeanalizowania go przy użyciu innych danych monitorowania. | [Zbieranie i analizowanie dzienników aktywności platformy Azure w obszarze roboczym Log Analytics w Azure Monitor](activity-log-collect.md) |
...
| Miejsce docelowe | Opis | Tematy pomocy |
|:---|:---|:---|
| Dziennik aktywności<br>Dzienniki Azure Monitor | Rekordy Service Health są przechowywane w dzienniku aktywności platformy Azure, dzięki czemu można je przeglądać w Azure Portal lub wykonywać inne czynności, które można wykonać przy użyciu dziennika aktywności. | [Wyświetlanie powiadomień dotyczących kondycji usługi za pomocą witryny Azure Portal](service-notifications.md) |


## <a name="azure-resources"></a>Zasoby platformy Azure
Metryki i dzienniki diagnostyczne na poziomie zasobów zawierają informacje o _wewnętrznej_ operacji zasobów platformy Azure. Są one dostępne dla większości usług platformy Azure, a monitorowanie rozwiązań i szczegółowych informacji zbiera dodatkowe dane dotyczące określonych usług.
...
| Dzienniki Azure Monitor | Kopiuj metryki platformy do dzienników dla trendów i innych analiz przy użyciu Log Analytics. | [Diagnostyka Azure bezpośrednio do Log Analytics](diagnostic-logs-stream-log-store.md) |
...
[Dzienniki diagnostyczne](diagnostic-logs-overview.md) zapewniają wgląd w wewnętrzną operację zasobu platformy Azure.  Dzienniki diagnostyczne nie są domyślnie włączone. Należy je włączyć i określić miejsce docelowe dla każdego zasobu. 

Wymagania dotyczące konfiguracji i zawartość dzienników diagnostycznych różnią się w zależności od typu zasobu, a nie wszystkie usługi jeszcze nie tworzą dzienników diagnostycznych. Zobacz [obsługiwane usługi, schematy i kategorie dla dzienników diagnostycznych platformy Azure](diagnostic-logs-schema.md) , aby uzyskać szczegółowe informacje na temat każdej usługi i linki do szczegółowych procedur konfiguracyjnych. Jeśli usługa nie jest wymieniona w tym artykule, ta usługa nie zapisuje obecnie w dziennikach diagnostycznych.

| Miejsce docelowe | Opis | Tematy pomocy |
|:---|:---|:---|
| Dzienniki Azure Monitor | Wyślij dzienniki diagnostyczne, aby Azure Monitor dzienniki analizy z innymi zebranymi danymi dzienników. Niektóre zasoby mogą zapisywać bezpośrednio do Azure Monitor podczas gdy inne zapisują dane na koncie magazynu przed ich zaimportowaniem do Log Analytics obszaru roboczego. | [Strumieniowe przesyłanie dzienników diagnostycznych platformy Azure do obszaru roboczego Log Analytics w Azure Monitor](diagnostic-logs-stream-log-store.md)<br>[Używanie Azure Portal do zbierania dzienników z usługi Azure Storage](azure-storage-iis-table.md#use-the-azure-portal-to-collect-logs-from-azure-storage)  |
| Magazyn | Wyślij dzienniki diagnostyczne do usługi Azure Storage w celu archiwizacji. | [Archiwizowanie dzienników diagnostycznych platformy Azure](archive-diagnostic-logs.md) |
| Event Hubs | Strumieniowe przesyłanie dzienników diagnostycznych do innych lokalizacji przy użyciu Event Hubs. |[Przesyłanie strumieniowe dzienników diagnostycznych platformy Azure do centrum zdarzeń](diagnostic-logs-stream-event-hubs.md) |
...
| Miejsce docelowe | Opis | Tematy pomocy |
|:---|:---|:---|
| Magazyn | Włączenie rozszerzenia Diagnostyka spowoduje domyślne zapisanie konta magazynu. | [Przechowywanie i przeglądanie danych diagnostycznych w usłudze Azure Storage](diagnostics-extension-to-storage.md) |
...
| Event Hubs | Skonfiguruj rozszerzenie diagnostyki, aby przesyłać strumieniowo dane do innych lokalizacji przy użyciu Event Hubs.  | [Przesyłanie strumieniowe danych Diagnostyka Azure w ścieżce gorąca przy użyciu Event Hubs](diagnostics-extension-stream-event-hubs.md) |
...
| Miejsce docelowe | Opis | Tematy pomocy |
|:---|:---|:---|
| Dzienniki Azure Monitor | Agent Log Analytics łączy się Azure Monitor bezpośrednio lub za pośrednictwem System Center Operations Manager i umożliwia zbieranie danych ze źródeł danych skonfigurowanych lub z rozwiązań monitorowania, które zapewniają dodatkowe informacje o aplikacjach uruchomione na maszynie wirtualnej. | [Źródła danych agentów w Azure Monitor](agent-data-sources.md)<br>[Połącz Operations Manager z Azure Monitor](om-agents.md) |
...
| Miejsce docelowe | Opis | Tematy pomocy |
|:---|:---|:---|
| Dzienniki Azure Monitor | Przechowuje dane dotyczące procesów i zależności w agencie. | [Korzystanie z mapy Azure Monitor dla maszyn wirtualnych (wersja zapoznawcza) w celu zrozumienia składników aplikacji](../insights/vminsights-maps.md) |
...
| Miejsce docelowe | Opis | Tematy pomocy |
|:---|:---|:---|
| Dzienniki Azure Monitor | Dane operacyjne dotyczące aplikacji, w tym widoki stron, żądania aplikacji, wyjątki i ślady. | [Analizowanie danych dziennika w Azure Monitor](../log-query/log-query-overview.md) |
...
| Miejsce docelowe | Opis | Tematy pomocy
|:---|:---|:---|
| Dzienniki Azure Monitor | Monitorowanie rozwiązań zbiera dane do dzienników Azure Monitor, które mogą być analizowane przy użyciu języka zapytań lub [widoków](view-designer.md) , które są zwykle dołączone do rozwiązania. | [Szczegóły zbierania danych na potrzeby monitorowania rozwiązań na platformie Azure](../insights/solutions-inventory.md) |
...
|:---|:---|:---|:---|
| Dzienniki Azure Monitor | Interfejs API modułu zbierającego dane | Zbieranie danych dziennika z dowolnego klienta REST i przechowywanie w Log Analytics obszarze roboczym. | [Wysyłanie danych dziennika do Azure Monitor za pomocą interfejsu API modułu zbierającego dane HTTP (publiczna wersja zapoznawcza)](data-collector-api.md) |
