...
- Otwórz blok "Skalowanie automatyczne" w Azure Monitor i wybierz zasób, który chcesz skalować
  > Uwaga: W poniższych krokach użyto planu usługi App Service skojarzonego z aplikacją sieci Web, dla której skonfigurowano usługę App Insights.
...
- Ustaw limity wystąpień. Na przykład jeśli chcesz skalować między 2-5 wystąpieniami w zależności od niestandardowych fluktuacji metryk, ustaw wartość "minimum" na "2", "maksimum" na "5" i "domyślne" na "2"
  > Uwaga: W przypadku gdy występuje problem z odczytem metryk zasobów i aktualna pojemność jest mniejsza niż domyślna pojemność, to aby zapewnić dostępność zasobu, Skalowanie automatyczne zostanie skalowane do wartości domyślnej. Jeśli bieżąca pojemność jest już wyższa niż domyślna pojemność, Skalowanie automatyczne nie będzie skalowane.
...
> Uwaga: Te same kroki mają zastosowanie do rozpoczęcia pracy z rolą usługi VMSS lub w chmurze.
