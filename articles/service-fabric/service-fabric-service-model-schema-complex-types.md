---
title: Typy złożone platformy Azure Service Fabric service model XML schematu | Dokumentacja firmy Microsoft
description: W tym artykule opisano typy złożone w nim schemat XML modelu usługi Service Fabric.
...
ms.locfileid: "58884197"
---
<!-- This article was generated by the Python script found in the service-fabric-service-model-schema.md file -->

# <a name="service-model-xml-schema-complex-types"></a>Typy złożone schemat XML modelu usług 
...
Nazwa aplikacji, które ma zostać utworzony.
...
Określa, czy traktować ostrzeżenia kondycji zgłasza jako błędy podczas oceny kondycji. Domyślnie: false.
...
Określa maksymalną wartość procentową tolerowana wdrożone aplikacje, które mogą być złej kondycji, zanim aplikacja zostanie uznane za błąd. To jest obliczana przez podzielenie liczbę wdrożonych aplikacji w złej kondycji przez liczbę węzłów, które aplikacje są aktualnie wdrożonych w klastrze. Obliczenia zaokrągla w górę do tolerowanie awarii jednego w małej liczby węzłów. Domyślnej wartości procentowej: 0.
...
W tym artykule opisano, w magazynie diagnostyki na koncie usługi Azure storage.
...
#### <a name="version"></a>Wersja
Ciąg wersji zdefiniowany przez użytkownika dokumentu manifestu klastra.
...
#### <a name="nodetypes"></a>Elementy NodeType
...
Wskazuje, jeśli zawartość tego pakietu kodu są udostępniane przez inne pakiety kodu. W przypadku opcji true w przypadku uaktualnienia tego pakietu kodu, wszystkie pakiety kodu zostanie ponownie uruchomiona. Ten atrybut nie jest obecnie obsługiwane, a jego wartość zostanie zignorowana.
...
#### <a name="name"></a>Name (Nazwa)
Nazwa pakietu konfiguracji w usłudze manifestu, który zawiera ustawienia do zastąpienia.
...
|Atrybut|Wartość|
|---|---|
|content|0 następującą liczbę elementów, atrybutów 8|
|Definicja|Globalnie|
...
Nazwa magazynu dla X509 certyfikatu.
...
Wersja pakietów danych, które znajdują się pliki certyfikatu.
...
Hasło i prywatnego klucza certyfikatu.
...
W przypadku wartości true wartość hasło są szyfrowane.
...
#### <a name="name"></a>Name (Nazwa)
Identyfikator informacji określonego certyfikatu. Ta nazwa jest używana do ustawiania zmiennej środowiskowej w kontenerze.
...
Jeśli kontener ma włączone test kondycji i docker raporty health_status zdarzenia dla tego kontenera, Usługa Service Fabric będzie zawierać jako część systemu zgłosił kondycji. Gdy health_status jest w złej kondycji usługi Service Fabric zgłosi ostrzeżenie kondycji. Domyślnie jest ustawiony na wartość true.
...
Jeśli kontener ma włączone test kondycji i docker raporty health_status zdarzenia dla tego kontenera, Usługa Service Fabric zostanie uruchomiony ponownie kontenera podczas health_status zgłoszone przez platformę docker jest w złej kondycji.
Domyślnie jest ustawiona na wartość false.
...
|Atrybut|Wartość|
|---|---|
|content|10 następującą liczbę elementów, atrybutów 8|
|Definicja|Globalnie|
...
Użyj poświadczeń repozytorium domyślne, które są określone w clusterManifest.xml zamiast poświadczeń określonych w tagu "RepositoryCredentials" wewnątrz ApplicationManifest.xml.
...
Użyj tokenu uwierzytelniania tożsamości usługi Zarządzanej (lub niestandardowego określonego punktu końcowego) do uzyskania tokenu używany do uwierzytelniania.
...
Tryb izolacji dla kontenerów. Prawidłowe wartości to domyślnie procesu oraz funkcji Hyper-v (obsługiwana tylko w przypadku kontenerów systemu windows).
...
Liczba kontenerów na potrzeby pozostawione, gdy kontener wielokrotnie śmierci. Domyślnie ta jest równa 0, dzięki czemu zostaną pozostawione nie kontenerów.
...
Flaga wskazująca, czy usuwanie kontenerów, będą automatycznie występują (PRAWDA) czy nie (false), jeśli kontener jest zakończone.  Jeśli FAŁSZ SF ręcznie usunąć kontenerów.
...
Uruchom kontener z włączoną flagą interaktywne. Prawidłowe wartości to PRAWDA/FAŁSZ. Wartość false, domyślnie.
...
NetworkType. Obecnie obsługiwane typy to "Otwarte" i "Isolated".
...
Nazwa punktu końcowego, który musi być zadeklarowana w sekcji zasobów w manifeście usługi.
...
W tym artykule opisano skojarzenie między pakietu usług oraz sieci kontenera i opcjonalnie skojarzenie pakietu z punktów końcowych z sieci kontenera.
...
Nazwa sieci kontenera (bez uwzględniania wielkości liter). Musi być nazwą kontenera sieci uprzednio utworzono lub zarezerwowanych nazw, w tym "Open" lub "Translatora adresów Sieciowych".
...
Lokalizacja źródła jest mapowany do uruchomionego kontenera. W związku z tym w lokalizacji docelowej nie może być lokalizacji, która już istnieje w kontenerze.
...
#### <a name="driver"></a>Sterownik
Nazwa sterownika dla wtyczki woluminu plików platformy Azure jest "sfazurefile".
...
|Atrybut|Wartość|
|---|---|
|content|4 elementy, atrybuty (10)|
|Definicja|Globalnie|
...
Nieobsługiwane, nie należy używać. Definiuje zasady izolacji dla niezarządzanych bibliotek DLL i zarządzane zestawy, które są ładowane w DllHost. 
...
#### <a name="managedassembly"></a>Biblioteki ManagedAssembly
...
Opcje sterownika, które zostaną przekazane do sterownika. Wtyczka woluminu plików platformy Azure obsługuje następujące opcje sterownika: shareName (Azure Files udziału plików który zawiera wolumin w kontenerze), storageAccountName (Azure konto magazynu zawierające udział plików usługi Azure Files), storageAccountKey () Klucz dostępu dla konta usługi Azure storage, które zawiera udział plików usługi Azure Files).
Te opcje sterownika trzech są wymagane.
...
#### <a name="certificateref"></a>CertificateRef
Nazwa certyfikatu punktu końcowego jest zadeklarowana w sekcji certyfikaty w celu zwrócenia jej klientowi. 
...
#### <a name="port"></a>Port
Numer portu dla punktu końcowego. Zastępuje wartości w manifeście usługi.
...
Protokół dla punktu końcowego. Zastępuje wartości w manifeście usługi. Punkty końcowe HTTPS musi mieć również EndpointCertificate i EndpointBindingPolicy zadeklarowane w manifeście aplikacji. Nie można zmienić Protokół w dalszej części uaktualnienie aplikacji. 
...
#### <a name="type"></a>Type
Typ punktu końcowego. Zastępuje wartości w manifeście usługi. Wejściowe punkty końcowe są używane do udostępniania portów na zewnątrz wewnętrznych punktów końcowych są używane do komunikacji w obrębie aplikacji.
...
Schemat identyfikatora URI. Zastępuje wartości w manifeście usługi. Na przykład "http", "https" lub "ftp".
...
Sufiks ścieżki. Na przykład "/ myapp1". Zastępuje wartości w manifeście usługi.
...
#### <a name="protocol"></a>Protokół
Protokół dla punktu końcowego: http, https, tcp lub udp. Punkty końcowe HTTPS musi mieć również EndpointCertificate i EndpointBindingPolicy zadeklarowane w manifeście aplikacji. Nie można zmienić Protokół w dalszej części uaktualnienie aplikacji. 
...
#### <a name="type"></a>Type
Typ punktu końcowego.  Wejściowe punkty końcowe są używane do udostępniania portów na zewnątrz wewnętrznych punktów końcowych są używane do komunikacji w obrębie aplikacji.
...
#### <a name="codepackageref"></a>CodePackageRef
Nazwa kodu pakiet, który będzie używał tego punktu końcowego.
...
#### <a name="port"></a>Port
Port zostaną zastąpione przez port określane przez usługę Microsoft Azure Service Fabric po zarejestrowaniu Http.sys lub BFE.
...
Wartość true, jeśli plik wykonywalny znajduje się poza pakietem aplikacji. Sprawdzanie istnienia program w pakiecie aplikacji zostanie pominięte.
...
To jest używana tylko wtedy, gdy X509 poświadczeń. Określa, jak można znaleźć certyfikatu przez nazwę lub odcisk palca 
...
#### <a name="x509findvalue"></a>X509FindValue
To jest używana tylko wtedy, gdy X509 poświadczeń. Jest to rzeczywista nazwa lub odcisk palca certyfikatu.
...
|Atrybut|Wartość|
|---|---|
|content|21 elementy, atrybuty (0)|
|Definicja|Globalnie|
...
Adres IP lub nazwę FQDN komputera, na którym ma zostać umieszczony ten węzeł.
...
#### <a name="nodetyperef"></a>NodeTypeRef
Nazwa elementu nodetype zdefiniowane w sekcji elementy NodeType. 
...
Określa, czy hasło jest zaszyfrowanym lub niezaszyfrowanym tekstu.
...
|---|---|
|name|Image (Obraz)|
|type|[Typ obrazu](#imagetype-complextype)|
|minOccurs|0|
|maxOccurs|niepowiązane|
...
Tworzenie wersji systemu operacyjnego. Na przykład wersji kompilacji systemu Windows Server w wersji 1709 — 16299.
...
Obsahuje informace infrastruktury dla tego klastra usługi Microsoft Azure Service Fabric.
...
|Atrybut|Wartość|
|---|---|
|content|2 następującą liczbę elementów, atrybutów 7|
|Definicja|Globalnie|
...
Nazwa roli, który stanowi łącze do węzła typu ref, która jest zdefiniowana w sekcji elementy NodeType.
...
#### <a name="nodetyperef"></a>NodeTypeRef
Nazwa typu węzła, który jest zdefiniowany w sekcji elementy NodeType.
...
#### <a name="name"></a>Name (Nazwa)
Unikatowy identyfikator dla metryki w ramach klastra z punktu widzenia Menedżer zasobów klastra.
...
Domyślna ilość obciążenia, który tworzy tę usługę bezstanową, dla tej metryki.
...
Domyślna ilość obciążenia, które ta usługa będzie wywierać dla tej metryki po repliki podstawowej.
...
Domyślna ilość obciążenia, które ta usługa będzie wywierać dla tej metryki, gdy jest ona tylko do repliki pomocniczej.
...
Określa waga metryki względem innych metryk, które są skonfigurowane dla tej usługi. Podczas wykonywania Jeśli dwie metryki znajdą się w konflikcie, Menedżer zasobów klastra preferuje Metryka z wyższa waga. Zero powoduje wyłączenie równoważenia obciążenia dla tej metryki.
...
W tym artykule opisano zasady sieciowe, łącznie z kontenera zasad sieci dla pakietu z.
...
Nazwa pakietu kodu, konfiguracji lub danych do udostępnienia. Musi odpowiadać nazwie pakietu zdefiniowany w manifeście usługi.
...
## <a name="parametertype-complextype"></a>Zgodność complexType
...
|---|---|
|name|Parametr|
|type|[Zgodność](#parametertype-complextype)|
|minOccurs|1|
|maxOccurs|niepowiązane|
...
#### <a name="passwordencrypted"></a>PasswordEncrypted
Określa, czy hasło jest zaszyfrowanym lub niezaszyfrowanym tekstu. Ten atrybut jest przestarzały. Użyj atrybutu "Type", aby wskazać, zaszyfrowane hasło.
...
|Atrybut|Wartość|
|---|---|
|content|0 następującą liczbę elementów, atrybutów 12|
|Definicja|Globalnie|
...
Limity pamięci w Megabajtach. Wartość musi być dodatnią liczbą całkowitą. Pakiety kodu (kontenery lub procesy) nie mogą przydzielać pamięci ponad ten limit. Podjęcie próby takiego przydzielenia spowoduje wyjątek braku pamięci.
...
Całkowita ilość pamięci wymiany, które mogą być używane w MB. Wartość musi być dodatnią liczbą całkowitą.
...
Miękki limit (w MB) dla nadzoru pamięci, który jest wymuszany, tylko wtedy, gdy wykryto rywalizacji o zasoby pamięci w węźle. Wartość musi być dodatnią liczbą całkowitą.
...
Można używać procent dostępne procesory CPU (tylko Windows). Wartość musi być dodatnią liczbą całkowitą. Jeśli podano limity procesora CPU dla pakietu z tego parametru skutecznie jest ignorowana.
...
Maksymalna we/wy (Odczyt i zapis) pod kątem operacji We/Wy, które mogą być używane. Wartość musi być dodatnią liczbą całkowitą.
...
Maksymalna operacji wejścia/wyjścia (w bajtach na sekundę) mogą być używane (Odczyt i zapis). Wartość musi być dodatnią liczbą całkowitą.
...
Waga we/wy bloku względem innych pakietów kodu. Musi być dodatnią liczbą całkowitą od 10 do 1000.
...
Limity pamięci jądra w bajtach.  Wartość musi być dodatnią liczbą całkowitą.  Należy pamiętać, jest określone w systemie Linux i docker w systemie windows zgłosi błąd, jeśli ta opcja jest ustawiona.
...
Rozmiar/dev/shm w bajtach. Jeśli argument jest pominięty, wówczas system używa 64MB.  Wartość musi być dodatnią liczbą całkowitą.  Należy zauważyć, że to Linux określonych, jednak będą platformy docker nie generowanie błędu po podjęciu określony.  Jest po prostu ignorowane.
...
#### <a name="codepackageref"></a>CodePackageRef
Nazwa pakietu kodu. Musi odpowiadać nazwie CodePackage określonego w manifeście usługi.
...
Konto użytkownika, których będzie używać pakietu kodu usługi.  Konto użytkownika musi być zadeklarowana w sekcji podmiotów zabezpieczeń. Często jest uruchomienie punktu wejścia instalacji przy użyciu konta systemu lokalnego, a nie konta administratorów.
...
#### <a name="entrypointtype"></a>EntryPointType
Instalator jest SetupEntryPoint zadeklarowane w manifeście usługi, punkt wejścia uprzywilejowanych, który jest uruchamiany przed innymi punktu wejścia.  Główny jest punkt wejścia zadeklarowane w manifeście usługi zwykle długotrwałych hosta usługi. Obejmuje wszystkie punkty wejścia.
...
Zasób, zostanie im przyznany dostęp, deklarowane i skonfigurowane w manifeście usługi.
...
Użytkownik lub grupa przypisania praw dostępu do zasobu, musi być zadeklarowana w sekcji podmiotów zabezpieczeń.
...
Prawa do udzielania: Odczyt, zmiany lub pełne. Wartość domyślna jest do odczytu.
...
Typ zasobu, zdefiniowane w sekcji zasobów w manifeście usługi. Punkt końcowy lub certyfikatów.
...
Wskazuje, jeśli środowisko uruchomieniowe Service Fabric określone informacje mają być widoczne pakiety kodu w pakiecie usługi.
...
Wskazuje, jeśli usługi Service Fabric replikowane blockstore powinny być używane do zapewnienia wolumin kontenerów w pakiecie usługi.
...
Typ elementu członkowskiego z grupy usługi.
...
#### <a name="name"></a>Name (Nazwa)
Nazwa elementu członkowskiego grupy usługi względem nazwy grupy usługi.
...
Typ podstawowy, który opisuje stanowe i bezstanowe ServiceGroupType.

|Atrybut|Wartość|
|---|---|
|content|4 elementy, atrybuty (2)|
|Definicja|Globalnie|
...
Identyfikator typu zdefiniowanego przez użytkownika grupy usług, na przykład "ActorQueueSGType". Ta wartość jest używana w pliku ApplicationManifest.xml, aby zidentyfikować grupę usługi.
...
Określa, czy wystąpienie grupy usługi jest tworzony przez fabrykę niejawne. Jeśli wartość false (domyślnie), jeden z pakietów kodu należy zarejestrować fabryki grupy usług
...
Ograniczenia w przypadku umieszczania usług, które są częścią tego pakietu.
...
|Atrybut|Wartość|
|---|---|
|content|11 elementy, atrybuty (0)|
|Definicja|Globalnie|
...
Identyfikator tego manifestu usługi ciągiem bez struktury.
...
#### <a name="isolation"></a>Izolacja
Tryb izolacji dla kontenerów. Prawidłowe wartości to "default", "procesu" lub "funkcji Hyper-v".
...
Procesor CPU limit liczby rdzeni logicznych. Wartość musi być dodatnią liczbą całkowitą.
...
#### <a name="memoryinmb"></a>MemoryInMB
Limity pamięci w Megabajtach. Wartość musi być dodatnią liczbą całkowitą.
...
|Atrybut|Wartość|
|---|---|
|content|11 następującą liczbę elementów, atrybutów 4|
|Definicja|Globalnie|
...
#### <a name="contentchecksum"></a>ContentChecksum
Wartość sumy kontrolnej to pakiet ServicePackage zawartości.
...
#### <a name="servicetypename"></a>ServiceTypeName
Nazwa typu usługi zadeklarowane w manifeście usługi, który zostanie uruchomiony.
...
Określa maksymalną wartość procentową tolerowana usługi w złej kondycji, zanim aplikacja jest uznawana za złą. Domyślnej wartości procentowej: 0.
...
Określa maksymalną wartość procentową tolerowana partycje w złej kondycji, zanim usługa jest uznawana za złą. Domyślnej wartości procentowej: 0.
...
Określa maksymalną wartość procentową tolerowana repliki w złej kondycji, zanim partycji jest określana jako zła. Domyślnej wartości procentowej: 0.
...
Typ podstawowy, który opisuje stanowe i bezstanowe ServiceType.
...
#### <a name="statelessservicetype"></a>StatelessServiceType
W tym artykule opisano ServiceType bezstanowe.
...
Zawiera opis ustawień zdefiniowanych przez użytkownika ServiceComponent lub aplikacji. Składa się z co najmniej jedna sekcja par klucz wartość.
...
|Atrybut|Wartość|
|---|---|
|content|0 następującą liczbę elementów, atrybutów 11|
|Definicja|Globalnie|
...
Jest to pełna ścieżka do lokalizacji pliku MSI.
...
Ta lokalizacja manifest infrastruktury, który jest generowany wewnętrznie.
...
Jest wersją docelową wdrożenia.
...
#### <a name="nodename"></a>NodeName
Jest to nazwa węzła, do którego jest uaktualnienia sieci szkieletowej do wykonania
...
Flaga wskazująca, jeśli po usunięciu konfiguracji węzła powinna być wywoływana RemoveNodeState interfejsu Api.
...
Nazwa pliku exe używane przez usługę Instalatora do uaktualnienia 
...
Nazwa pliku exe, używany przez usługę Instalator do cofania uaktualnienia
