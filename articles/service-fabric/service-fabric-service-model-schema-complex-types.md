---
title: Typy złożone Azure usługi sieć szkieletowa usług modelu XML schematu | Dokumentacja firmy Microsoft
description: W tym artykule opisano typy złożone w schemacie XML modelu usługi sieć szkieletowa usług.
...
Określa, czy traktować ostrzeżenie kondycji raportuje jako błędy podczas oceny kondycji. Domyślnie: false.
|Atrybut|Wartość|
|---|---|
...
Określa maksymalny udział procentowy tolerowaną wdrożone aplikacje, które mogą być zła, zanim aplikacja zostanie uznane za błąd. To jest obliczany przez podzielenie liczba zła wdrożonej aplikacji przez liczbę węzłów, które aplikacje są obecnie wdrożona w klastrze. Obliczenia Zaokrągla wartość w do tolerowanie awarii jednego w małej liczby węzłów. Domyślna wartość procentowa: 0.
|Atrybut|Wartość|
|---|---|
...
Wartość sumy kontrolnej tej zawartości elementu ApplicationPackage
|Atrybut|Wartość|
|---|---|
...
#### <a name="version"></a>Wersja
Ciąg wersji zdefiniowanej przez użytkownika na potrzeby dokumentu manifestu klastra.
|Atrybut|Wartość|
|---|---|
|name|Wersja|
|Użyj|wymagane|
...
Wskazuje, czy zawartość tego pakietu kodu są współużytkowane przez inne pakiety kodu. Jeśli PRAWDA, podczas uaktualniania tego pakietu kodu wszystkie pakiety kodu zostanie uruchomiona ponownie. Ten atrybut nie jest obecnie obsługiwany, a jego wartość zostanie zignorowana.
|Atrybut|Wartość|
|---|---|
|name|IsShared|
|type|xs:Boolean|
...
Nazwa pakietu danych, której pliki certyfikatu.
|Atrybut|Wartość|
|---|---|
|name|DataPackageRef|
|type|xs:String|
...
Wersja pakietu danych, której pliki certyfikatu.
|Atrybut|Wartość|
|---|---|
...
W przypadku wartości PRAWDA wartość hasło jest szyfrowane.
|Atrybut|Wartość|
|---|---|
...
#### <a name="name"></a>Name (Nazwa)
Identyfikator informacji określonego certyfikatu. Ta nazwa jest używana do ustawienia zmiennej środowiskowej w kontenerze.
|Atrybut|Wartość|
|---|---|
|name|Name (Nazwa)|
|type|xs:String|
...
Kontener ma włączone test kondycji i docker raporty health_status zdarzenia dla tego kontenera, Service Fabric zostaną uwzględnione w jako część systemu zgłoszony kondycji. Gdy health_status jest w złej kondycji sieci szkieletowej usług zgłosi ostrzeżenie kondycji. Domyślnie jest ustawiona na true.
|Atrybut|Wartość|
|---|---|
...
Jeśli kontener ma włączone test kondycji i docker raporty health_status zdarzenia dla tego kontenera, sieci szkieletowej usług uruchomi kontener, po health_status zgłoszone przez docker jest zła.
...
Tryb izolacji dla kontenera. Prawidłowe wartości to domyślny, proces lub funkcji Hyper-v (obsługiwane tylko dla kontenerów systemu Windows) w systemie Windows 10 hostów, tylko tryb izolacji funkcji Hyper-V jest obsługiwana, dlatego kontenera działa w trybie izolacji funkcji Hyper-V niezależnie od jego ustawienie trybu izolacji.
|Atrybut|Wartość|
|---|---|
|name|Izolacja|
|Użyj|opcjonalne|
...
Liczba kontenerów do pozostawione po wielokrotnie zaniku kontenera. Domyślnie ta jest równa 0, Brak kontenerów zostanie pozostać.
|Atrybut|Wartość|
|---|---|
...
Uruchom kontenera z włączoną flagą interaktywnego. Prawidłowe wartości to PRAWDA i FAŁSZ. Wartość false, domyślnie.
|Atrybut|Wartość|
|---|---|
|name|RunInteractive|
|Użyj|opcjonalne|
...
NetworkType. Obecnie jedynym obsługiwanym typem jest "Open".
|Atrybut|Wartość|
|---|---|
|name|NetworkType|
|Użyj|wymagane|
...
Folder źródłowy, który może być folderem w maszynie Wirtualnej, która obsługuje kontenery lub trwałego magazynu zdalnego.
|Atrybut|Wartość|
|---|---|
...
Lokalizacja zamapowanego źródła w kontenerze uruchomione. W związku z tym folderu docelowego nie może być już istnieje w kontenerze Twojej lokalizacji.
|Atrybut|Wartość|
|---|---|
...
#### <a name="driver"></a>Sterownik
Nazwa sterownika dla wtyczki woluminu pliki Azure jest "sfazurefile".
|Atrybut|Wartość|
|---|---|
|name|Sterownik|
|Użyj|opcjonalne|
...
Nieobsługiwane, nie należy używać. Definiuje politykę izolacji niezarządzanych bibliotek DLL i zarządzanych zestawów załadowanych do elementu Dllhost. 
|Atrybut|Wartość|
|---|---|
...
Sterownik opcje do przekazania do sterownika. Dodatek woluminu plików Azure obsługuje następujące opcje sterownika: shareName (pliki Azure udziału plików, który zawiera wolumin kontenera), storageAccountName (konto magazynu Azure zawierający pliki Azure udział plików), storageAccountKey ( Klucz dostępu dla konta magazynu Azure, zawierającego pliki Azure udział plików).
Te opcje sterownika trzy są wymagane.
...
#### <a name="port"></a>Port
Port zostanie zastąpiona w elemencie Service Manifest
|Atrybut|Wartość|
|---|---|
|name|Port|
|type|xs:String|
...
Protokół dla punktu końcowego. Punktów końcowych HTTPS musi mieć również EndpointCertificate i EndpointBindingPolicy zadeklarowane w manifeście aplikacji. Protokół nie można zmienić później w przypadku uaktualnienia aplikacji. 
|Atrybut|Wartość|
|---|---|
|name|Protokół|
|type|xs:String|
...
#### <a name="type"></a>Typ
Typ punktu końcowego.  Wejściowych punktów końcowych są używane do udostępnienia portu na zewnątrz wewnętrznych punktów końcowych są używane do komunikacji wewnątrz aplikacji.
|Atrybut|Wartość|
|---|---|
|name|Typ|
|type|xs:String|
...
#### <a name="codepackageref"></a>CodePackageRef
Nazwa kodu pakietu, który będzie używany przez ten punkt końcowy.
|Atrybut|Wartość|
|---|---|
|name|CodePackageRef|
|Użyj|opcjonalne|
...
#### <a name="port"></a>Port
Port zostanie zastąpione portem określonym przez usługi sieć szkieletowa usług Microsoft Azure po zarejestrowaniu przy użyciu rozwiązania Http.sys lub BFE.
|Atrybut|Wartość|
|---|---|
|name|Port|
...
Wartość true, jeśli plik wykonywalny jest zewnętrznych do pakietu aplikacji. Pominięto sprawdzanie istnienia program w pakiecie aplikacji.
|Atrybut|Wartość|
|---|---|
...
To jest używana tylko wtedy, gdy X509 poświadczeń. Określa sposób wyszukiwania według nazwy lub odcisku palca certyfikatu 
|Atrybut|Wartość|
|---|---|
|name|X509FindType|
|default|Postać FindByThumbprint|

#### <a name="x509findvalue"></a>X509FindValue
To jest używana tylko wtedy, gdy X509 poświadczeń. Jest to faktyczna nazwa lub odcisk palca certyfikatu.
|Atrybut|Wartość|
|---|---|
|name|X509FindValue|
|Użyj|wymagane|
...
Adres IP lub nazwę FQDN komputera, na której ma zostać umieszczony ten węzeł.
|Atrybut|Wartość|
|---|---|
...
#### <a name="nodetyperef"></a>wartość nodetyperef równą
Nazwa elementu nodetype zdefiniowane w sekcji elementów NodeType. 
|Atrybut|Wartość|
|---|---|
|name|wartość nodetyperef równą|
|type|xs:String|
...
Określa, czy hasło jest szyfrowane lub zwykłego tekstu.
|Atrybut|Wartość|
|---|---|
...
Tworzenie wersji systemu operacyjnego. Na przykład numer kompilacji systemu Windows Server w wersji 1709 jest 16299.
|Atrybut|Wartość|
|---|---|
...
#### <a name="name"></a>Name (Nazwa)
Unikatowy identyfikator dla metryki w klastrze z punktu widzenia menedżera zasobów klastra s.
|Atrybut|Wartość|
|---|---|
|name|Name (Nazwa)|
|Użyj|wymagane|
...
Domyślna ilość obciążenia, które tworzy tej usługi bezstanowej dla tej metryki.
|Atrybut|Wartość|
|---|---|
|name|DefaultLoad|
|type|xs:Long|
...
Domyślna ilość obciążenia, które tej usługi będą powodowały dla tej metryki po repliki podstawowej.
|Atrybut|Wartość|
|---|---|
...
Domyślna ilość obciążenia, które tej usługi będą powodowały dla tej metryki, gdy replika pomocnicza nie jest.
|Atrybut|Wartość|
|---|---|
...
Określa waga metryki względem innych metryk, które są skonfigurowane dla tej usługi. W czasie wykonywania Jeśli dwa metryki przechodzili powoduje konflikt, Menedżer zasobów klastra preferuje Metryka z wagą wyższy. Zero spowoduje wyłączenie równoważenia obciążenia dla tej metryki.
|Atrybut|Wartość|
|---|---|
|name|Waga|
...
Nazwa pakietu kodu, konfiguracji lub danych do udostępnienia. Musi odpowiadać nazwie pakietu zdefiniowane w manifeście usługi.
|Atrybut|Wartość|
|---|---|
|name|PackageRef|
...
#### <a name="isencrypted"></a>isEncrypted
W przypadku wartości PRAWDA wartość tego parametru jest zaszyfrowana
|Atrybut|Wartość|
|---|---|
|name|isEncrypted|
|type|xs:String|
...
Ogranicza zasobów, których można użyć na hoście i deklaruje limity zasobów pakietu kodu usługi.

|Atrybut|Wartość|
|---|---|
|content|0 elementy, atrybuty 9|
|Definicja|Globalny|
...
Miękki limit (w MB) dla ładu pamięci, które są wymuszane tylko w przypadku wykrycia rywalizacji pamięci w węźle. Musi być dodatnią liczbą całkowitą.
|Atrybut|Wartość|
|---|---|
...
Względną wagę procesora CPU. Musi być dodatnią liczbą całkowitą.
|Atrybut|Wartość|
|---|---|
|name|CpuShares|
|type|xs:String|
...
Można używać procent dostępne procesory CPU (tylko system Windows). Musi być dodatnią liczbą całkowitą. Limity CPU są określone dla pakietu usługi, ten parametr skutecznie jest ignorowana.
|Atrybut|Wartość|
|---|---|
|name|CpuPercent|
|type|xs:String|
...
Maksymalna szybkość we/wy (odczytu i zapisu) pod względem IOps, który może służyć. Musi być dodatnią liczbą całkowitą.
|Atrybut|Wartość|
|---|---|
|name|MaximumIOps|
|type|xs:String|
...
Maksymalna we/wy (w bajtach na sekundę), które mogą być używane (odczytu i zapisu). Musi być dodatnią liczbą całkowitą.
|Atrybut|Wartość|
|---|---|
...
Waga we/wy bloku względem inne pakiety kodu. Musi być dodatnią liczbą całkowitą od 10 do 1000.
|Atrybut|Wartość|
|---|---|
|name|BlockIOWeight|
|type|xs:String|
...
#### <a name="codepackageref"></a>CodePackageRef
Nazwa pakietu kodu. Musi odpowiadać nazwie elementu CodePackage, określona w manifeście usługi.
|Atrybut|Wartość|
|---|---|
|name|CodePackageRef|
|Użyj|wymagane|
...
Konto użytkownika, który zostanie uruchomiony pakiet kodu usługi.  Konto użytkownika musi być zadeklarowana w sekcji podmiotów zabezpieczeń. Często zaleca się uruchomienie Instalatora punktu wejścia przy użyciu konta systemu lokalnego, a nie konta administratorów.
|Atrybut|Wartość|
|---|---|
|name|UserRef|
|Użyj|wymagane|
...
#### <a name="entrypointtype"></a>EntryPointType
Instalator jest SetupEntryPoint zadeklarowane w manifeście usługi, punkt wejścia uprzywilejowane, który jest uruchamiany przed innymi punktu wejścia.  Main jest zadeklarowane w manifeście usługi, zwykle hosta usługi długotrwałe punktu wejścia. Wszystko to wszystkich punktów wejścia.
|Atrybut|Wartość|
|---|---|
|name|EntryPointType|
|Użyj|opcjonalne|
...
## <a name="scalingpolicytype-complextype"></a>Element complexType ScalingPolicyType
...
Zasób zostanie im przyznany dostęp do zadeklarowany i skonfigurowany w manifeście usługi.
|Atrybut|Wartość|
|---|---|
|name|ResourceRef|
|Użyj|wymagane|
...
Użytkownik lub grupa przypisywane prawa dostępu do zasobu, musi być zadeklarowana w sekcji podmiotów zabezpieczeń.
|Atrybut|Wartość|
|---|---|
|name|PrincipalRef|
|Użyj|wymagane|
...
Uprawnienia można przyznać, domyślny jest do odczytu.
|Atrybut|Wartość|
|---|---|
|name|GrantRights|
|default|Odczyt|
...
Typ zasobu, zdefiniowane w manifeście usługi, punkt końcowy lub certyfikatu.
|Atrybut|Wartość|
|---|---|
|name|ResourceType|
|Użyj|opcjonalne|
...
Określa, czy wystąpienie grupy usług jest tworzony przez fabrykę niejawnej. Jeśli wartość FAŁSZ (ustawienie domyślne), jeden z pakietów kodu musi zarejestrować fabryki grupy usług
|Atrybut|Wartość|
|---|---|
...
Identyfikator tego manifestu usługi ciągu bez struktury.
|Atrybut|Wartość|
|---|---|
|name|ManifestId|
|Użyj|opcjonalne|
...
Procesora CPU liczba rdzeni logicznych. Musi być dodatnią liczbą całkowitą.
|Atrybut|Wartość|
|---|---|
|name|CpuCores|
|type|xs:String|
...
#### <a name="contentchecksum"></a>ContentChecksum
Wartość sumy kontrolnej tej zawartości elementu ServicePackage.
|Atrybut|Wartość|
|---|---|
|name|ContentChecksum|
|type|xs:String|
...
#### <a name="servicetypename"></a>ServiceTypeName
Nazwa deklaracji w manifeście usługi, który będzie można utworzyć wystąpienia typu usługi.
|Atrybut|Wartość|
|---|---|
|name|ServiceTypeName|
|Użyj|wymagane|
...
Określa domyślny koszt przeniesienia tej usługi.
|Atrybut|Wartość|
|---|---|
...
Określa maksymalny procent tolerowaną usługi w złej kondycji, zanim aplikacja jest określana jako zła. Domyślna wartość procentowa: 0.
|Atrybut|Wartość|
|---|---|
...
Określa maksymalny procent tolerowaną zła partycji, zanim usługa jest określana jako zła. Domyślna wartość procentowa: 0.
|Atrybut|Wartość|
|---|---|
...
Określa maksymalny udział procentowy tolerowaną repliki w złej kondycji, przed partycji jest określana jako zła. Domyślna wartość procentowa: 0.
|Atrybut|Wartość|
|---|---|
...
|Atrybut|Wartość|
|---|---|
|content|0 elementy, atrybuty 11|
|Definicja|Globalny|
...
To jest docelowe wystąpienie węzła.
|Atrybut|Wartość|
|---|---|
...
Ta lokalizacja manifestu infrastruktury generowana wewnętrznie.
|Atrybut|Wartość|
|---|---|
...
#### <a name="nodename"></a>nodeName
To jest nazwa węzła, do którego jest uaktualnianie sieci szkieletowej
|Atrybut|Wartość|
|---|---|
|name|nodeName|
|type|xs:String|
...
Flaga wskazująca, czy interfejs Api RemoveNodeState powinna być wywoływana po usunięciu konfiguracji węzła.
|Atrybut|Wartość|
|---|---|
...
Nazwa pliku exe używanego przez usługę Instalatora do cofania operacji uaktualniania
|Atrybut|Wartość|
|---|---|
