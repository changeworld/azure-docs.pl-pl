...
Liczba partycji, przez którą przeprowadzić upshift/redukcję.
|Atrybut|Wartość|
|---|---|
|name|ScaleIncrement|
|type|xs:String|
...
Domyślny administrator roli certyfikat klienta używany do zabezpieczania komunikacji klient serwer.
...
Deklaruje folderem o nazwie określonej przez atrybut "Name", który zawiera pliki danych statycznych. Usługa Service Fabric będzie Odtwórz wszystkich plików exe i DLLHOSTs określone w pakietach hosta i pomocy technicznej, po uaktualnieniu żadnych pakietów danych, wymienione w manifeście usługi.
...
|---|---|
|type|anonimowe complexType|
|content|5 elementów, atrybutów 2|
...
|content|0 elementy, atrybuty (0)|
|Definicja|lokalnie w [EnvironmentOverridesType complexType](service-fabric-service-model-schema-complex-types.md#environmentoverridestype-complextype), [EnvironmentVariablesType complexType](service-fabric-service-model-schema-complex-types.md#environmentvariablestype-complextype)|
...
Deklaruje zestaw grup jako zabezpieczenie podmiotów zabezpieczeń, które mogą być przywoływane w zasadach.
...
#### <a name="scaleincrement"></a>ScaleIncrement
Liczba wystąpień, przez którą przeprowadzić upshift/redukcję.
|Atrybut|Wartość|
|---|---|
|name|ScaleIncrement|
|type|xs:String|
...
Wskazuje, pakiet kodu, konfiguracji lub danych powinny być udostępniane.
...
W przypadku wartości true wartość tego parametru jest zaszyfrowany. Deweloper aplikacji jest odpowiedzialny za tworzenie certyfikatu i za pomocą polecenia cmdlet Invoke-ServiceFabricEncryptSecret do szyfrowania poufnych informacji. Informacje o certyfikacie, który będzie używany do szyfrowania wartość jest określone w sekcji certyfikaty.
|Atrybut|Wartość|
|---|---|
|name|IsEncrypted|
|type|xs:Boolean|
...
Używane do kontrolowania węzły, które w klastrze usługi można uruchomić na. Para klucza i wartości, która zawiera opis wymagań s dla wartości i nazwa właściwości węzła. Pojedyncze instrukcje mogą być grupowane razem z prostego logiczny, aby utworzyć ograniczenie niezbędne. Na przykład "(FirmwareVersion > 12 & & InDMZ == True)".
...
Określa zasobu punktu końcowego, który chcesz powiązać ujawnionych portów kontenera.
...
W tym artykule opisano zastąpienia zasobów dla punktów końcowych w zasoby manifestu usługi.
...
Punkt wejścia uprzywilejowane, która domyślnie jest uruchamiany z tymi samymi poświadczeniami co Usługa Service Fabric (zazwyczaj konto NetworkService) przed innymi punktu wejścia. Obecność punktu wejścia Instalatora oddzielne pozwala na uniknięcie konieczności uruchamiania hosta usługi z wysokim poziomem uprawnień na dłuższy czas. W manifeście aplikacji możesz zmienić uprawnienia zabezpieczeń do uruchamiania skryptu uruchamiania w ramach konta systemu lokalnego lub konta administratora.
...
Domyślna rola certyfikatu klienta użytkownika używany do zabezpieczania komunikacji klient serwer.
