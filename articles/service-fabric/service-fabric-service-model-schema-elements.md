...
Określa informacje o X509 certyfikatu, który ma zostać uwidoczniona dla środowiska kontenera. Certyfikat musi być zainstalowany w magazynie LocalMachine wszystkich węzłów klastra.
Podczas uruchamiania aplikacji, środowisko uruchomieniowe odczytuje certyfikatu i generuje plik PFX oraz hasła (w systemie Windows) lub pliku PEM (w systemie Linux).
Plik PFX oraz hasła są dostępne w kontenerze, za pomocą zmiennych środowiskowych Certificates_ServicePackageName_CodePackageName_CertName_PFX i Certificates_ServicePackageName_CodePackageName_CertName_Password. Plik PEM jest dostępny w kontenerze, za pomocą zmiennych środowiskowych Certificates_ServicePackageName_CodePackageName_CertName_PEM i Certificates_ServicePackageName_CodePackageName_CertName_PrivateKey.
...
## <a name="certificates-element-type-certificatestype"></a>Element certyfikatów (typ CertificatesType) 
...
|content|0 elementy, atrybuty (0)|
|Definicja|lokalnie w NodeType element [InfrastructureNodeType complexType](service-fabric-service-model-schema-complex-types.md#infrastructurenodetype-complextype)|
|name|Certyfikaty|
|minOccurs|0|
...
## <a name="certificates-element-defined-in-applicationmanifesttype"></a>Element certyfikatów (zdefiniowany w ApplicationManifestType) 
...
## <a name="certificates-element-defined-in-clustermanifesttype"></a>Element certyfikatów (zdefiniowany w ClusterManifestType) 
|Atrybut|Wartość|
|---|---|
|type|anonimowe complexType.|
|content|1 elementy, atrybuty (0)|
...
Domyślny admin roli certyfikat klienta używany do zabezpieczania komunikacji klient serwer.
...
Określa w debugerze do dołączenia podczas aktywowania pakietu kodu informacje.
...
## <a name="destinations-element-defined-in-crashdumpsource"></a>Element miejsc docelowych (zdefiniowany w CrashDumpSource) 
Miejsca docelowe, do których mają zostać przeniesione zrzuty awaryjne.
...
## <a name="destinations-element-defined-in-etwsource"></a>Element miejsc docelowych (zdefiniowany w ETWSource) 
Miejsca docelowe, do których mają zostać przeniesione zrzuty awaryjne.
...
## <a name="destinations-element-defined-in-foldersource"></a>Element miejsc docelowych (zdefiniowany w FolderSource) 
...
## <a name="endpoint-element-type-endpointoverridetype"></a>Elementu punktu końcowego (typ EndpointOverrideType) 
|Atrybut|Wartość|
|---|---|
|type|[EndpointOverrideType](service-fabric-service-model-schema-complex-types.md#endpointoverridetype-complextype)|
|content|0 elementy, atrybuty (0)|
|Definicja|lokalnie w elemencie punkty końcowe|
|name|Endpoint|
|maxOccurs|bez ograniczeń|
...
## <a name="endpoint-element-type-endpointtype"></a>Elementu punktu końcowego (typ EndpointType) 
...
## <a name="endpoints-element-defined-in-resourceoverridestype"></a>Element punktów końcowych (zdefiniowany w ResourceOverridesType) 
...
## <a name="endpoints-element-defined-in-resourcestype"></a>Element punktów końcowych (zdefiniowany w ResourcesType) 
Definiuje punkty końcowe dla usługi.
...
## <a name="entrypoint-element-type-xsstring"></a>EntryPoint — element (xs:string typu) 
|Atrybut|Wartość|
|---|---|
|type|xs:String|
|content|0 elementy, atrybuty (0)|
...
Zmienne środowiskowe należy przekazać do kontenera lub exe. 
...
## <a name="exehost-element-type-exehostentrypointtype"></a>Element elemencie ExeHost (typ ExeHostEntryPointType) 
|Atrybut|Wartość|
|---|---|
...
## <a name="exehost-element-defined-in-entrypointdescriptiontype"></a>Element elemencie ExeHost (zdefiniowany w EntryPointDescriptionType) 
|Atrybut|Wartość|
|---|---|
|type|anonimowe complexType.|
|content|0 elementy, atrybuty (0)|
...
Opisuje zasady oceniania zdarzeń kondycji dla różnych jednostek powiązanych aplikacji. Jeśli żadne zasady nie zostanie określony, jednostki zakłada się, że zła, jeśli raport o kondycji jest ostrzeżenia lub błędu.
...
Obraz kontenera odpowiadający numeru wersji kompilacji mają zostać uruchomione. Jeśli nie określono atrybutu systemu operacyjnego, obraz kontenera założono, że działa we wszystkich wersjach systemu operacyjnego i przesłania obrazu określonego w manifeście usługi.
...
Repozytorium i obrazu na https://hub.docker.com lub rejestru kontenera platformy Azure.
...
Kontenery systemu Windows Server nie może być zgodne w różnych wersjach systemu operacyjnego.  Można określić wiele obrazów systemu operacyjnego na kontenera i oznacz je za pomocą wersji kompilacji systemu operacyjnego. Pobierz wersję kompilacji systemu operacyjnego, uruchamiając "polecenie winver" w wierszu polecenia systemu Windows. Jeśli podstawowy system operacyjny jest kompilacji wersji 16299 (z systemem Windows Server w wersji 1709), sieci szkieletowej usług wybiera obrazu kontenera z systemu operacyjnego = "16299". Obraz nieoznakowanego kontenera założono, że działa we wszystkich wersjach systemu operacyjnego i przesłania obrazu określonego w manifeście usługi.
...
|content|0 elementy, atrybuty (0)|
|Definicja|lokalnie w LoadMetrics element, LoadMetrics element, LoadMetrics element, LoadMetrics element, LoadMetrics element|
|name|LoadMetric|
|maxOccurs|bez ograniczeń|
...
## <a name="loadmetrics-element-defined-in-servicegrouptypetype"></a>Element LoadMetrics (zdefiniowany w ServiceGroupTypeType) 
Załaduj metryki zgłoszone przez tę usługę.
...
## <a name="loadmetrics-element-defined-in-servicetypetype"></a>Element LoadMetrics (zdefiniowany w ServiceTypeType) 
Załaduj metryki zgłoszone przez tę usługę.
...
## <a name="loadmetrics-element-defined-in-servicetype"></a>Element LoadMetrics (zdefiniowany w ServiceType) 
Załaduj metryki zgłoszone przez tę usługę, używany dla usług równoważenia zasobów.
...
Może dziedziczyć właściwości i ustawienia zabezpieczeń tej grupy członkostwa można dodać użytkowników z żadną istniejącą grupą członkostwa. Grupy członkostwa mogą służyć do zabezpieczenia zasobów zewnętrznych, które muszą być dostępne dla różnych usług lub tej samej usługi (na innym komputerze).
...
## <a name="members-element-defined-in-statefulservicegrouptype"></a>Element elementów członkowskich (zdefiniowany w StatefulServiceGroupType) 
...
## <a name="members-element-defined-in-statelessservicegrouptype"></a>Element elementów członkowskich (zdefiniowany w StatelessServiceGroupType) 
Usługi dla członków tej grupy usług
...
## <a name="ntlmauthenticationpolicy-element-defined-in-group"></a>Element elementu NTLMAuthenticationPolicy (zdefiniowaną w grupie) 
|Atrybut|Wartość|
|---|---|
|type|anonimowe complexType.|
|content|elementy 0, 1 atrybutów|
...
## <a name="ntlmauthenticationpolicy-element-defined-in-user"></a>Element elementu NTLMAuthenticationPolicy (zdefiniowany w użytkownika) 
|Atrybut|Wartość|
|---|---|
|type|anonimowe complexType.|
...
|content|0 elementy, atrybuty (0)|
|Definicja|lokalnie w elemencie wstawienia, wstawienia elementu|
|name|Węzeł|
|maxOccurs|bez ograniczeń|
...
## <a name="nodelist-element-defined-in-infrastructureinformationtype"></a>Element wstawienia (zdefiniowany w InfrastructureInformationType) 
|Atrybut|Wartość|
|---|---|
|type|anonimowe complexType.|
|content|1 elementy, atrybuty (0)|
...
## <a name="nodelist-element-defined-in-linuxinfrastructuretype"></a>Element wstawienia (zdefiniowany w LinuxInfrastructureType) 
|Atrybut|Wartość|
|---|---|
|type|anonimowe complexType.|
|content|1 elementy, atrybuty (0)|
...
## <a name="nodelist-element-defined-in-windowsinfrastructuretype"></a>Element wstawienia (zdefiniowany w WindowsInfrastructureType) 
|Atrybut|Wartość|
|---|---|
|type|anonimowe complexType.|
|content|1 elementy, atrybuty (0)|
...
## <a name="parameter-element-type-parametertype"></a>Parameter — element (typ parametru) 
|Atrybut|Wartość|
|---|---|
...
## <a name="parameter-element-defined-in-parameters"></a>Parameter — element (zdefiniowany w parametrach) 
...
W przypadku wartości PRAWDA wartość tego parametru jest zaszyfrowany. Deweloper aplikacji jest odpowiedzialny za tworzenie certyfikatu i za pomocą polecenia cmdlet Invoke-ServiceFabricEncryptSecret do szyfrowania informacji poufnych. Informacje o certyfikacie, który będzie używany do szyfrowania wartość jest określone w sekcji certyfikatów.
|Atrybut|Wartość|
|---|---|
|name|isEncrypted|
|type|xs:Boolean|
...
## <a name="parameters-element-defined-in-appinstancedefinitiontype"></a>Element parametrów (zdefiniowany w AppInstanceDefinitionType) 
...
## <a name="parameters-element-defined-in-applicationmanifesttype"></a>Element parametrów (zdefiniowany w ApplicationManifestType) 
...
Określa, który zasób punktu końcowego powiązać narażonych port kontenera.
...
## <a name="property-element-defined-in-servicetypeextensionpolicypropertiestype"></a>Property — element (zdefiniowany w ServiceTypeExtensionPolicyPropertiesType) 
|Atrybut|Wartość|
|---|---|
|type|anonimowe complexType.|
|content|0 elementy, atrybuty (2)|
...
Określa limity zasobów pakietu kodu.
...
Definiuje operacje zastępowania zasobów dla punktów końcowych w zasobach manifestu usługi.
...
## <a name="roles-element-defined-in-blackbird"></a>Elementu role (zdefiniowanego w Blackbird) 
|Atrybut|Wartość|
|---|---|
|type|anonimowe complexType.|
|content|1 elementy, atrybuty (0)|
...
## <a name="roles-element-defined-in-paas"></a>Elementu role (zdefiniowanego w PaaS) 
|Atrybut|Wartość|
|---|---|
|type|anonimowe complexType.|
|content|1 elementy, atrybuty (0)|
...
## <a name="roles-element-defined-in-windowsazure"></a>Elementu role (zdefiniowanego w WindowsAzure) 
|Atrybut|Wartość|
|---|---|
|type|anonimowe complexType.|
|content|1 elementy, atrybuty (0)|
...
|type|anonimowe complexType.|
|content|elementy 0, 1 atrybutów|
|Definicja|lokalnie w elemencie ExeHost — element|
|name|RunFrequency|
|minOccurs|0|
...
Skalowania opis zasad, składające się z wyzwalacza i mechanizm skalowania. 
...
|type|[FabricCertificateType](service-fabric-service-model-schema-complex-types.md#fabriccertificatetype-complextype)|
|content|0 elementy, atrybuty (0)|
|Definicja|lokalnie w element certyfikaty, element certyfikaty, DigestedCertificates — element|
|name|Element|
|minOccurs|0|
...
## <a name="section-element-defined-in-settingstype"></a>Element Section (zdefiniowany w SettingsType) 
...
## <a name="serviceplacementpolicies-element-defined-in-servicetypetype"></a>Element ServicePlacementPolicies (zdefiniowany w ServiceTypeType) 
...
## <a name="serviceplacementpolicies-element-defined-in-servicetype"></a>Element ServicePlacementPolicies (zdefiniowany w ServiceType) 
Deklaruje zasady umieszczania dla usługi.  Przydatne w przypadku klastra obejmuje odległości geograficzne lub i/lub geograficznymi regionów.
...
## <a name="servicetypes-element-type-serviceandservicegrouptypestype"></a>Element Serivcetype (typ ServiceAndServiceGroupTypesType) 
...
## <a name="servicetypes-element-type-servicetypestype"></a>Element Serivcetype (typ ServiceTypesType) 
|Atrybut|Wartość|
|---|---|
|type|[ServiceTypesType](service-fabric-service-model-schema-complex-types.md#servicetypestype-complextype)|
|content|0 elementy, atrybuty (0)|
...
Punkt wejścia uprzywilejowane, która domyślnie jest uruchamiana z tymi samymi poświadczeniami, jak usługa sieci szkieletowej (zazwyczaj konto Usługa sieciowa) przed innymi punktu wejścia. Obecność punktu wejścia instalacji oddzielnych pozwala uniknąć konieczności uruchamiania usługi hosta z wysokiego poziomu uprawnień przez dłuższy czas. W manifeście aplikacji można zmienić uprawnienia zabezpieczeń do uruchamiania skryptu uruchomienia w ramach lokalnego konta systemowego lub konta administratora.
...
Określonego identyfikatora GUID do użycia jako identyfikator dziennika udostępniony.
...
|type|[StatefulServiceGroupType](service-fabric-service-model-schema-complex-types.md#statefulservicegrouptype-complextype)|
|content|0 elementy, atrybuty (0)|
|Definicja|lokalnie w [ServiceTemplatesType complexType](service-fabric-service-model-schema-complex-types.md#servicetemplatestype-complextype), element grupy usług|
|name|StatefulServiceGroup|
...
## <a name="systemgroup-element-defined-in-memberof"></a>Element SystemGroup (zdefiniowany w MemberOf) 
Aby dodać użytkownika do grupy system.  Grupa systemu muszą być zdefiniowane w sekcji grupy.
...
## <a name="systemgroup-element-defined-in-membership"></a>Element SystemGroup (zdefiniowany w członkostwie) 
|Atrybut|Wartość|
|---|---|
|type|anonimowe complexType.|
|content|elementy 0, 1 atrybutów|
...
Domyślny użytkownik roli certyfikat klienta używany do zabezpieczania komunikacji klient serwer.
