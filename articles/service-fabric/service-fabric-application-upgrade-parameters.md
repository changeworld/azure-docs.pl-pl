---
title: 'Uaktualnianie aplikacji: parametry uaktualnienia | Microsoft Docs'
...
UpgradeMode | LOKALNE | Dozwolone wartości to **monitorowane** (ustawienie domyślne), **UnmonitoredAuto**lub **UnmonitoredManual**. Aby uzyskać szczegółowe informacje, zobacz parametry programu PowerShell dla każdego trybu w tym artykule. |
...
| WhatIf | PS | Dozwolone wartości to **true** i **false**. Pokazuje, co się stanie po uruchomieniu polecenia cmdlet. Polecenie cmdlet nie jest uruchamiane. |
...
Wyst | Dozwolone wartości to **monitorowanie**, **upgrademode**, **UnmonitoredAuto**, **UnmonitoredManual**. Wartość domyślna to **UnmonitoredAuto**. Opisy tych wartości można znaleźć w sekcji Visual Studio i PowerShell *Required Parameters* .|
