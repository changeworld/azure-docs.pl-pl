---
title: Samouczek dotyczący dodawania wypowiedzi do aplikacji LUIS przy użyciu języka Ruby | Microsoft Docs
...
ms.locfileid: "36266060"
---
# <a name="tutorial-add-utterances-to-app-using-ruby"></a>Samouczek: Dodawanie wypowiedzi do aplikacji przy użyciu języka Ruby 
W tym samouczku pokazano, jak napisać program służący do dodawania wypowiedzi do intencji za pomocą interfejsów API tworzenia w języku Ruby.
...
* Twój istniejący [**identyfikator aplikacji**](./luis-get-started-create-app.md) usługi LUIS. Identyfikator aplikacji jest wyświetlany na pulpicie nawigacyjnym aplikacji. Aplikacja LUIS z intencjami i jednostkami użyta w pliku `utterances.json` musi istnieć przed uruchomieniem tego kodu w pliku `add-utterances.rb`. Za pomocą kodu znajdującego się tym artykule nie są tworzone intencje ani jednostki. Służy on jedynie do dodawania wypowiedzi do istniejących intencji i jednostek. 
* **Identyfikator wersji** w aplikacji, w której odbierane są wypowiedzi. Domyślny identyfikator to „0.1”.
* Utwórz nowy plik o nazwie `add-utterances.rb` w programie VSCode.

> [!NOTE] 
> Kompletny plik `add-utterances.cs` i przykładowy plik `utterances.json` są dostępne w [**repozytorium Github** LUIS-Samples](https://github.com/Microsoft/LUIS-Samples/blob/master/documentation-samples/authoring-api-samples/ruby/).


## <a name="write-the-ruby-code"></a>Tworzenie kodu w języku Ruby
...
Uruchom aplikację z poziomu wiersza polecenia za pomocą języka Ruby.

Wywołanie pliku `add-utterances.rb` tylko z plikiem utterance.json jako argumentem powoduje dodanie nowych wypowiedzi, ale nie uczy aplikacji usługi LUIS za ich pomocą.
````
> ruby add-utterances.rb ./utterances.json
````

Powoduje to wyświetlenie wyników wywołania interfejsu API dodawania wypowiedzi. Pole `response` jest w tym formacie na potrzeby dodanych wypowiedzi. Właściwość `hasError` ma wartość false, co oznacza, że wypowiedź została dodana.  
...
Wywołaj przykład z argumentem `-status`, aby sprawdzić stan uczenia.
