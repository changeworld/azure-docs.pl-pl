...
ms.locfileid: "73828527"
---
# <a name="tutorial-extract-structured-data-with-machine-learned-entities-in-language-understanding-luis"></a>Samouczek: wyodrębnianie danych strukturalnych za pomocą jednostek z obsługą maszynową w Language Understanding (LUIS)
...
W praktyce można utworzyć jednostkę uczenia maszynowego do reprezentowania zamówienia dla aplikacji Pizza. Kolejność powinna zawierać wszystkie części, które są niezbędne do fullfil zamówienia. Aby rozpocząć, jednostka będzie zawierać cały tekst związany z kolejnością, a w odróżnieniu od rozmiaru i ilości. 

Wypowiedź dla `deliver one large cheese pizza` powinien wyodrębnić cały Wypowiedź jako kolejność, a następnie wyodrębnić `1` i `large`. 

Istnieje dalsze dekompozycja, którą można wykonać, np. toppings lub crust. Po tym samouczku należy zaczuć się, że należy dodać te podskładniki do istniejącej jednostki `Order`.
...
1. W sekcji **Zarządzanie** na karcie **wersje** wybierz wersję, a następnie wybierz pozycję **Klonuj** , aby sklonować wersję, a następnie nadaj jej nazwę `mach-learn`. Następnie wybierz pozycję **gotowe** , aby zakończyć proces klonowania. Ponieważ nazwa wersji jest używana jako część trasy adresu URL, nie może ona zawierać żadnych znaków, które są nieprawidłowe w adresie URL.

    > [!TIP] 
    > Klonowanie jest najlepszym rozwiązaniem Przed zmodyfikowaniem aplikacji. Po zakończeniu pracy z wersją, wyeksportuj wersję, jako plik JSON lub lu, i sprawdź, czy w kontroli źródła.

1. Wybierz opcję Kompiluj **, a następnie** zapoznaj się z głównymi blokami konstrukcyjnymi aplikacji Luis.
...
1. Na liście przykład wyrażenia długości wybierz następujące wypowiedź. 
    ...
    > [!NOTE]
    > Jednostka nie zawsze będzie całą wypowiedź. W tym konkretnym przypadku `pickup` wskazuje, w jaki sposób zamówienie ma zostać odebrane, więc powinno być częścią oznaczonej jednostki dla zamówienia. 

1. W polu **Wybierz typ jednostki** wybierz pozycję **Dodaj strukturę** , a następnie wybierz przycisk **dalej**. Struktura jest niezbędna do zezwalania na podskładniki, takie jak rozmiar i ilość.
...
1. W polu **deskryptor Utwórz nową listę fraz** wpisz nazwę `SizeDescriptor` następnie wprowadź wartości: `small`, `medium`i `large`. Po wypełnieniu pola **sugestie** wybierz pozycję `extra large`i `xl`. Wybierz pozycję **gotowe** , aby utworzyć nową listę fraz. 
    ...
    Ten deskryptor listy wyrazów pomaga `Size` podskładnikom Znajdź wyrazy związane z rozmiarem, dostarczając go za pomocą przykładowego wyrazu. Ta lista nie musi zawierać każdego wyrazu o rozmiarze, ale powinna zawierać słowa, które powinny wskazywać rozmiar. 
...
1. Oznacz jednostkę `Order` w pozostałej wyrażenia długości wraz z jednostką rozmiar. Nawiasy kwadratowe w tekście wskazują `Order` jednostki i jednostki `Size` w elemencie.
    ...
    > [!CAUTION]
    > Jak traktuje się implikowane dane, takie jak litera `a` implikuje pojedynczy Pizza? Lub brak `pickup` i `delivery`, aby wskazać, gdzie oczekiwany jest Pizza? Lub brak rozmiaru do wskazania domyślnego rozmiaru lub małego lub dużego? Rozważ traktowanie domniemanej obsługi danych jako części reguł firmy w aplikacji klienckiej. 
...
1. Po przeprowadzeniu szkolenia Dodaj nowy przykład wypowiedź, aby zrozumieć, jak dobrze LUIS rozumie jednostkę uczenia maszynowego. 
    ...
    Link kropkowany wskazuje prognozę. 

1. W obszarze Zmiana przewidywania na jednostkę oznaczoną etykietą zaznacz wiersz, a następnie wybierz pozycję **Potwierdź przewidywania jednostek**.
    ...
    W tym momencie jednostka uczenia maszynowego działa, ponieważ może znaleźć jednostkę w ramach nowego przykładu wypowiedź. Po dodaniu przykładu wyrażenia długości, jeśli jednostka nie jest przewidywalna prawidłowo, Oznacz jednostkę i podskładniki. Jeśli jednostka jest przewidywalna prawidłowo, upewnij się, że potwierdzają przewidywania. 

## <a name="add-prebuilt-number-to-app-to-help-extract-data"></a>Dodawanie wstępnie skompilowanego numeru do aplikacji w celu ułatwienia wyodrębnienia danych

Informacje o zamówieniu powinny również zawierać liczbę elementów w podanej kolejności, takich jak liczba pizzami. Aby wyodrębnić te dane, należy dodać nowy podskładnik maszynowy do `Order`, a ten składnik wymaga ograniczenia wstępnie skompilowanego numeru. Ograniczając jednostkę do wstępnie skompilowanego numeru, jednostka znajdzie i wyodrębni liczbę, czy tekst jest cyfrą, `2`lub tekstem, `two`.

Zacznij od dodania wstępnie skompilowanego numeru do aplikacji. 
...
## <a name="label-example-utterance-with-subcomponent-for-quantity-to-teach-luis-about-the-entity"></a>Etykieta przykład wypowiedź z podskładnikem dla ilości, aby nauczyć LUIS o jednostce
