---
title: Uzyskiwanie wyników automatycznego sugerowania za pomocą API automatycznego sugerowania usługi Bing (kognitywnych usług Azure) | Dokumentacja firmy Microsoft
description: Przedstawia sposób użycia automatycznego sugerowania usługi Bing.
services: cognitive-services
author: v-jaswel
manager: kamrani
ms.assetid: 2575A80C-FC74-4631-AE5D-8101CF2591D3
ms.service: cognitive-services
ms.component: bing-autosuggest
ms.topic: article
ms.date: 09/28/2017
ms.author: v-jaswel
ms.openlocfilehash: f66af61ed06f4c6a7fe36cd44aa7781b71bb18ea
ms.sourcegitcommit: 95d9a6acf29405a533db943b1688612980374272
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 06/23/2018
ms.locfileid: "35349008"
---
# <a name="build-a-web-page-autosuggest-client"></a>Tworzenie klienta Autosugerowanie strony sieci Web

W tym samouczku firma Microsoft będzie tworzenia strony sieci Web, który umożliwia użytkownikom zapytania API automatycznego sugerowania usługi Bing.

Ten samouczek przedstawia sposób wykonania następujących czynności:

> [!div class="checklist"]
> - Wprowadź prostego zapytania do interfejsu API automatycznego sugerowania usługi Bing
> - Wyświetl wyniki zapytania

## <a name="prerequisites"></a>Wymagania wstępne

Aby skorzystać z tego samouczka, należy klucza subskrypcji dla interfejsu API automatycznego sugerowania usługi Bing. Jeśli nie masz, [utworzyć konto bezpłatnej wersji próbnej](https://azure.microsoft.com/try/cognitive-services/?api=autosuggest-api).

## <a name="create-a-new-web-page"></a>Tworzenie nowej strony sieci Web

Otwórz Edytor tekstu. Utwórz nowy plik o nazwie, na przykład autosuggest.html.

## <a name="html-header"></a>Nagłówek HTML

Dodaj informacje o nagłówku HTML i rozpocząć sekcji skrypt w następujący sposób.

```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Bing Autosuggest</title>

<style type="text/css">
    html, body, div, p, h1, h2 {font-family: Verdana, "Lucida Sans", sans-serif;}

    html, body, div, p  {font-weight: normal;}
    h1, h2 {font-weight: bold;}
    sup {font-weight: normal;}

    html, body, div, p  {font-size: 12px;}
    h1 {font-size: 20px;}
    h2 {font-size: 16px;}
    h1, h2 {clear: left;}

    img#logo {float: right;
</style>

<script type="text/javascript">
```

## <a name="getsubscriptionkey-function"></a>Funkcja getSubscriptionKey

Funkcja getSubscriptionKey zwraca klucz API automatycznego sugerowania usługi Bing. Go pobiera go z magazynu lokalnego (plik cookie) albo monituje użytkownika, jeśli to konieczne.

Funkcja getSubscriptionKey BEGIN i deklarować nazwy pliku cookie w następujący sposób.

```html
getSubscriptionKey = function() {

    var COOKIE = "bing-autosuggest-api-key";   // name used to store API key in key/value storage
```

Funkcja pomocnika findCookie zwraca wartość określonego cookie; Jeśli plik cookie nie zostanie znaleziony, zwraca pusty ciąg.

```html
    function findCookie(name) {
        var cookies = document.cookie.split(";");
        for (var i = 0; i < cookies.length; i++) {
            var keyvalue = cookies[i].split("=");
            if (keyvalue[0].trim() === name) {
                return keyvalue[1];
            }
        }
        return "";
        }
```

Funkcja pomocnika getSubscriptionKeyCookie monituje użytkownika o wartości klucza API automatycznego sugerowania usługi Bing i zwraca wartość klucza.

```html
    function getSubscriptionKeyCookie() {
        var key = findCookie(COOKIE);
        while (key.length !== 32) {
            key = prompt("Enter Bing Autosuggest API subscription key:", "").trim();
            var expiry = new Date();
            expiry.setFullYear(expiry.getFullYear() + 2);
            document.cookie = COOKIE + "=" + key.trim() + "; expires=" + expiry.toUTCString();
        }
        return key;
    }
```

Funkcja pomocnika getSubscriptionKeyLocalStorage najpierw próbuje pobrać klucz API automatycznego sugerowania usługi Bing wyszukując odpowiedniego pliku cookie. Jeśli plik cookie nie zostanie znaleziony, monituje użytkownika o wartości klucza. Zwraca wartość klucza.

```html
    function getSubscriptionKeyLocalStorage() {
        var key = localStorage.getItem(COOKIE) || "";
        while (key.length !== 32)
            key = prompt("Enter Bing Autosuggest API subscription key:", "").trim();
        localStorage.setItem(COOKIE, key)
        return key;
    }
```

Funkcja pomocnika getSubscriptionKey przyjmuje jeden parametr **unieważnienie**. Jeśli **unieważnienie** jest **true**, getSubscriptionKey usuwa plik cookie, który zawiera klucz API automatycznego sugerowania usługi Bing. Jeśli **unieważnienie** jest **false**, getSubscriptionKey zwraca wartość klucza API automatycznego sugerowania usługi Bing.

```html
    function getSubscriptionKey(invalidate) {
        if (invalidate) {
            try {
                localStorage.removeItem(COOKIE);
            } catch (e) {
                document.cookie = COOKIE + "=";
            }
        } else {
            try {
                return getSubscriptionKeyLocalStorage();
            } catch (e) {
                return getSubscriptionKeyCookie();
            }
        }
    }
```

Zwraca funkcję pomocnika getSubscriptionKey jako wynik funkcji getSubscriptionKey zewnętrzne. Zamknij definicji funkcji getSubscriptionKey zewnętrzne.

```html
    return getSubscriptionKey;

}();
```

## <a name="helper-functions"></a>Funkcje pomocy

Funkcja pomocnika sprzed zwraca określony tekst wstępnie sformatowany z [sprzed](https://www.w3schools.com/tags/tag_pre.asp) tagu HTML.

```html
function pre(text) {
    return "<pre>" + text.replace(/&/g, "&amp;").replace(/</g, "&lt;") + "</pre>"
}
```

Funkcja renderSearchResults wyświetla wyniki określonej API automatycznego sugerowania usługi Bing, przy użyciu pretty drukowania.

```html
function renderSearchResults(results) {
    document.getElementById("results").innerHTML = pre(JSON.stringify(results, null, 2));
}
```

Funkcja renderErrorMessage Wyświetla określony komunikat o błędzie i kod błędu.

```html
function renderErrorMessage(message, code) {
    if (code)
        document.getElementById("results").innerHTML = "<pre>Status " + code + ": " + message + "</pre>";
    else
        document.getElementById("results").innerHTML = "<pre>" + message + "</pre>";
}
```

## <a name="bingautosuggest-function"></a>Funkcja bingAutosuggest

Funkcja bingAutosuggest jest wywoływana za każdym razem, użytkownik wprowadza tekst w polu formularza HTML.
Trwa dwa parametry: zawartość HTML tworzą pola i klucz API automatycznego sugerowania usługi Bing.

```html
function bingAutosuggest(query, key) {
```

Określ punkt końcowy API automatycznego sugerowania usługi Bing i zadeklarować obiektu XMLHttpRequest, który zostanie wykorzystany do wysyłania żądań do punktu końcowego.

```html
    var endpoint = "https://api.cognitive.microsoft.com/bing/v7.0/Suggestions";

    var request = new XMLHttpRequest();

    try {
        request.open("GET", endpoint + "?q=" + encodeURIComponent(query));
    }
    catch (e) {
        renderErrorMessage("Bad request");
        return false;
    }
```

Ustaw **Ocp-Apim-subskrypcji — klucz** nagłówka do wartości klucza API automatycznego sugerowania usługi Bing.

```html
    request.setRequestHeader("Ocp-Apim-Subscription-Key", key);
```

Obsługa odpowiedzi z punktu końcowego. Jeśli stan jest 200 (OK), Wyświetl wyniki; w przeciwnym wypadku Wyświetl informacje o błędzie.

```html
    request.addEventListener("load", function() {
        if (this.status === 200) {
            renderSearchResults(JSON.parse(this.responseText));
        }
        else {
            if (this.status === 401) getSubscriptionKey(true);
            renderErrorMessage(this.statusText, this.status);
        }
    });
```

Również obsługi zdarzeń może zawierać błąd z obiektu XMLHttpRequest.

```html
    request.addEventListener("error", function() {
        renderErrorMessage("Network error");
    });

    request.addEventListener("abort", function() {
        renderErrorMessage("Request aborted");
    });
```

Wyślij żądanie. Close — funkcja bingAutosuggest **skryptu** tagu i **head** tagu.

```html
    request.send();
    return false;
}
// --></script>

</head>
```

## <a name="html-body"></a>Treść HTML

Podczas ładowania strony sieci Web, upewnij się, że istnieje klucz API automatycznego sugerowania usługi Bing monitowania użytkownika dla niego, jeśli to konieczne.

```html
<body onload="document.forms.bing.query.focus(); getSubscriptionKey();">
```

Wyświetlanie Bing logo.

```html
<img id="logo" align=base src="data:image/png;base64,">
```

Tworzenie formularza HTML z pola tekstowego. Obsługa **oninput** zdarzeń i wywołania funkcji bingAutosuggest przekazywanie zawartości pola tekstowego i klucz API automatycznego sugerowania usługi Bing.

```html
<form name="bing" oninput="return bingAutosuggest(this.query.value, getSubscriptionKey())">
    <h2>Autosuggest</h2>
    <input type="text" name="query" size="80" placeholder="Autosuggest" autocomplete=off>
</form>
```

Dodaj kod HTML **div** tag używanego do wyświetlania wyników. JavaScript wcześniej zdefiniowanego odwołuje się do tego **div** tagu.

```html
<h2>Results</h2>
<div id="results">
<p>None yet.</p>

</div>

</body>
</html>
```

Zapisz plik.

## <a name="display-results"></a>Wyświetl wyniki

Otwórz stronę sieci Web w przeglądarce. W wierszu polecenia wprowadź klucz API automatycznego sugerowania usługi Bing subskrypcji. Następnie wprowadź zapytania (na przykład "kierowania") w **Autosugerowanie** pola tekstowego. Podczas wpisywania strony sieci Web jest automatycznie aktualizowany, aby wyświetlić wyniki Autosugerowanie.

```json
{
  "_type": "Suggestions",
  "queryContext": {
    "originalQuery": "sail"
  },
  "suggestionGroups": [
    {
      "name": "Web",
      "searchSuggestions": [
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=vheQSvKZylM3dlX_B9bQ8-hQEsEJo8zDD2y7H1nsBjE&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailor%2bbrinkley%2bcook%26FORM%3dUSBAPI&p=DevEx,5003.1",
          "displayText": "sailor brinkley cook",
          "query": "sailor brinkley cook",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=EStLqAfxGCa44Ur3jEMXBv-Qp-lXUSFJbkBfnUdKKDg&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailor%2bbrinkley%26FORM%3dUSBAPI&p=DevEx,5004.1",
          "displayText": "sailor brinkley",
          "query": "sailor brinkley",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=gvtP9TS9NwhajSapY2Se6y1eCbP2fq_GiP2n-cxi6OY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailrite%26FORM%3dUSBAPI&p=DevEx,5005.1",
          "displayText": "sailrite",
          "query": "sailrite",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=c0QOA_j6swCZJy9FxqOwke2KslJE7ZRmMooGClAuCpY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailboats%2bfor%2bsale%26FORM%3dUSBAPI&p=DevEx,5006.1",
          "displayText": "sailboats for sale",
          "query": "sailboats for sale",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=mnMdREUH20SepmHQH1zlh9Hy_w7jpOlZFm3KG2R_BoA&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailing%2banarchy%26FORM%3dUSBAPI&p=DevEx,5007.1",
          "displayText": "sailing anarchy",
          "query": "sailing anarchy",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=0udadFl0gCTKCp0QmzQTXS3_y08iO8FpwsoKPHPS6kw&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailboatdata%26FORM%3dUSBAPI&p=DevEx,5008.1",
          "displayText": "sailboatdata",
          "query": "sailboatdata",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=BTS0G6AakxntIl9rmbDXtk1n6rQpsZZ99aQ7ClE7dTY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsail%2bsand%2bpoint%26FORM%3dUSBAPI&p=DevEx,5009.1",
          "displayText": "sail sand point",
          "query": "sail sand point",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=quBMwmKlGwqC5wAU0K7n416plhWcR8zQCi7r-Fw9Y0w&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailflow%26FORM%3dUSBAPI&p=DevEx,5010.1",
          "displayText": "sailflow",
          "query": "sailflow",
          "searchKind": "WebSearch"
        }
      ]
    }
  ]
}
```

## <a name="next-steps"></a>Kolejne kroki

> [!div class="nextstepaction"]
> [Wymagania dotyczące użycia i wyświetlania](../UseAndDisplayRequirements.md)