...
Po uzyskaniu tych wartości Utwórz nowy plik o nazwie _ENV_i wklej do niego następujący kod, dostarczając wartości właściwości niestandardowych z powyższych.
...
Otwórz plik _routes\index.js_ i następujący import w górnej części pliku:

```javascript
var request = require('request');
```

Następnie Dodaj poniższy kod bezpośrednio poniżej tego wiersza. Ten kod tworzy punkt końcowy interfejsu API, który uzyskuje token uwierzytelniania usługi Azure AD przy użyciu hasła nazwy głównej usługi, a następnie zwraca ten token. Istnieje również drugi punkt końcowy do pobierania domeny podrzędnej.
...
1. W _views\index.Pug_Zastąp wywołanie `ImmersiveReader.launchAsync(token, content)` poniższym kodem.
