...
**A**: Nie. Jeśli aplikacja korzysta z ogólnego, codzienne języka, nie trzeba dostosować model. Jeśli aplikacja jest używana w środowisku, w przypadku niewielkiego lub żadnego hałas w tle, nie trzeba dostosować model.
...
**A**: Nie ma żadnego limitu liczbę modeli, jaką może mieć w kolekcji.
...
**A**: Możesz skalować w górę w przyrostach co 20 równoczesnych żądań modelu.

Skontaktuj się z nami, jeśli potrzebujesz większej skali.

**Pyt.: Czy mogę pobrać swój model i uruchomić go lokalnie?**

**A**: Modele nie pobierane i wykonywany lokalnie.
...
**A**: Interfejs API REST ogranicza żądania do 25 na 5 sekund. Szczegółowe informacje można znaleźć w naszych stron [zamiana mowy na tekst](speech-to-text.md).
...
**A**: Nie. Obecnie dozwolone są tylko pliki nieskompresowanych tekstu.
...
**A**: Można tworzenie autonomicznych aplikacji zbierania danych lub użyć oprogramowania nagrywania dźwięku off-the-shelf. Można również utworzyć wersję aplikacji, który rejestruje dane audio, a następnie używa danych. 
...
**A**: Tak, po prostu wybierz język niestandardowy model w menu rozwijanym podczas konfigurowania testu w trybie offline.
...
**A**: Wyniki pokazują porównania między modelem linii bazowej i modeli, które zostały dostosowane. Należy dążyć do zapewniała modelu odniesienia umożliwiają dostosowanie cenna.
