---
title: 'Szybki Start: niestandardowy asystent głosowy, Java (Android) — usługa mowy'
...
Przewodnik Szybki Start jest również dostępny dla [zamiany mowy na tekst](~/articles/cognitive-services/Speech-Service/quickstarts/speech-to-text-from-microphone.md?pivots=programming-language-java&tabs=android) i [zamiany tekstu na mowę](~/articles/cognitive-services/Speech-Service/quickstarts/text-to-speech.md?pivots=programming-language-java&tabs=android).

W tym artykule opisano tworzenie asystenta głosowego za pomocą języka Java dla systemu Android przy użyciu [zestawu Speech SDK](speech-sdk.md). Ta aplikacja będzie łączyć się z bot, który został już utworzony i skonfigurowany za pomocą [kanału bezpośredniej linii mowy](https://docs.microsoft.com/azure/bot-service/bot-service-channel-connect-directlinespeech). Następnie wyśle żądanie głosowe do bot i zaprezentowanie działania odpowiedzi z obsługą głosu.
...
1. Otwórz `MainActivity.java`i Zastąp zawartość następującym kodem:
   ...
   * Metoda `onCreate` obejmuje kod, który żąda dostępu do mikrofonu i Internetu.
   ...
   * Metoda `registerEventListeners` ilustruje zdarzenia używane przez `DialogServiceConnector` i podstawową obsługę działań przychodzących.

1. W tym samym pliku Zastąp ciągi konfiguracyjne, aby były zgodne z zasobami:

    * Zastąp `YourChannelSecret` za pomocą bezpośredniego wpisu tajnego kanału mowy dla bot.
