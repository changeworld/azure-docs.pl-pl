...
* Robota utworzonej wcześniej skonfigurowano [kanału Direct mowy wiersza](https://docs.microsoft.com/azure/bot-service/bot-service-channel-connect-directlinespeech.md)
...
1. Następnie utworzysz `SpeechBotConnector` informacje o subskrypcji. Dodaj następujący kod do treści metody `InitializeBotConnector`, zastępując ciągi `YourChannelSecret`, `YourSpeechSubscriptionKey`, i `YourServiceRegion` własnymi wartościami dla bota subskrypcji mowy i [region](regions.md).
    ...
    > [!NOTE]
    > Informacje na temat konfigurowania bota i pobierania klucza tajnego kanału, zobacz dokumentację platformy Bot Framework [kanału Direct mowy wiersza](https://docs.microsoft.com/azure/bot-service/bot-service-channel-connect-directlinespeech.md).
