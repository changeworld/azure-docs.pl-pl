...
|:---|:---|:---|:---|:---|
|wersja interfejsu API|1.0|Wersja interfejsu API zażądane przez klienta. Dozwolone wartości to: `1.0`.|query   |string|
|z|(puste)   |Określa język przychodzących mowy. Wartość jest jeden z identyfikatorów języka z `speech` zakresu w odpowiedzi z interfejsu API języków.|query|string|
|na|(puste)|Określa język do tłumaczenia uzyskanego tekstu do. Wartość jest jeden z identyfikatorów języka z `text` zakresu w odpowiedzi z interfejsu API języków.|query|string|
...
|Autoryzacja|(puste)  |Określa wartość klienta tokenu elementu nośnego. Użyj prefiksu `Bearer` następuje wartość `access_token` wartości zwracanej przez usługę tokenu uwierzytelniania.|nagłówek   |string|
|OCP-Apim-Subscription-Key|(puste)|Jeśli wymagane `Authorization` nagłówka nie jest określony.|nagłówek|string|
