...
W klasie Utwórz funkcję asynchroniczną o nazwie `TransliterateTextRequest()`. `Program` Ta funkcja przyjmuje cztery argumenty: `subscriptionKey`, `host`, `route`i `inputText`.
...
Zauważysz, że w `Main`, `host`deklarujesz `subscriptionKey` `route`,, i skrypt do transliteracji `textToTransliterate`.
