...
> [!IMPORTANT]
> Ten przewodnik Szybki Start używa interfejsu API aparatu rozpoznawania w wersji 1.0. Jeśli Twoja subskrypcja znajduje się w regionie `West US 2` lub `West Europe`, należy postępować zgodnie z instrukcjami[interfejsu API szybkiego startu](./curl-train-extract.md) w wersji 2,0.
...
Otrzymasz odpowiedź na `200 (Success)` z następującymi danymi wyjściowymi JSON:
...
Zanotuj wartość `"modelId"`. Będzie ona potrzebna w poniższych krokach.
...
1. Zastąp `<modelID>` IDENTYFIKATORem modelu otrzymanym w poprzedniej sekcji.
...
> [Dokumentacja interfejsu API REST](https://westus2.dev.cognitive.microsoft.com/docs/services/form-recognizer-api/operations/AnalyzeWithCustomModel)