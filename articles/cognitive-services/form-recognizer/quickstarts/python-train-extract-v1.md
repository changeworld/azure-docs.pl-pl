...
1. W monicie użyj polecenia `python`, aby uruchomić próbkę. Na przykład `python form-recognize-train.py`.

Otrzymasz odpowiedź `200 (Success)`ową z tym wyjściem JSON:
...
Zanotuj wartość `"modelId"`. Będzie on potrzebny do wykonania następujących kroków.
...
1. Otwórz okno wiersza polecenia.
1. W monicie użyj polecenia `python`, aby uruchomić próbkę. Na przykład `python form-recognize-analyze.py`.
