...
description: Dowiedz się więcej na temat parametrów interfejsu API REST używanych w interfejs API wyszukiwania wizualnego Bing.
...
Obiekt `imageInfo` musi zawierać pole `url` lub pole `imageInsightsToken`, ale nie oba jednocześnie. `url` Ustaw pole na adres URL obrazu dostępnego z Internetu. Maksymalny obsługiwany rozmiar obrazu wynosi 1 MB.
...
Poniżej przedstawiono nagłówki, które należy określić w żądaniu. `User-Agent` `X-MSEdge-ClientID` `X-MSEdge-ClientIP` `X-Search-Location`Nagłówki i sąjedynymiwymaganyminagłówkami,alenależyrównieżuwzględnić,,,i.`Ocp-Apim-Subscription-Key` `Content-Type`
