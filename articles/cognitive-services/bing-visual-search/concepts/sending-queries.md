...
Obiekt `imageInfo` musi zawierać pole `url` lub pole `imageInsightsToken`, ale nie oba jednocześnie. Ustaw wartość pola `url` na adres URL obrazu dostępnego z Internetu. Maksymalny obsługiwany rozmiar obrazu wynosi 1 MB.
...
Każde żądanie musi zawierać nagłówek `Content-Type`. Nagłówek musi być ustawiony na: `multipart/form-data; boundary=\<boundary string\>`, gdzie \<ciąg graniczny\> jest unikatowym, nieprzezroczystym ciągiem, który identyfikuje granicę danych formularza. Na przykład `boundary=boundary_1234-abcd`.

W przypadku wysłania wyszukiwanie wizualne tokenu lub adresu URL obrazu Poniższy fragment kodu przedstawia dane formularza, które należy uwzględnić w treści wpisu. Dane formularza muszą zawierać `Content-Disposition` nagłówka i należy ustawić jego parametr `name` na wartość "knowledgeRequest". Aby uzyskać szczegółowe informacje na temat obiektu `imageInfo`, zobacz żądanie.
...
Opcjonalnie można ustawić atrybut `enableEntityData` w nagłówku, aby `true` w celu uzyskania szczegółowych informacji dotyczących głównej jednostki w przekazywanym obrazie, w tym linków do sieci Web i informacji o przypisywaniu. To pole jest domyślnie `false`.
...
Jeśli dla obrazu są dostępne szczegółowe informacje, odpowiedź obejmuje co najmniej jeden obiekt `tags` zawierający te szczegółowe informacje. Pole `image` zawiera token Insights dla obrazu wejściowego:
...
Jeśli obraz zawiera tekst, który usługa rozpoznaje, jeden tag będzie zawierać szczegółowe informacje TextResults (akcja). `displayName` szczegółowych informacji zawiera rozpoznany tekst:
...
Jeśli obraz zawiera rozpoznaną jednostkę, taką jak dobrze znana/popularna osoba, miejsce lub rzecz, jeden z tagów może zawierać szczegółowe informacje o jednostce. Pola `mainEntity` i `data` są dostępne tylko wtedy, gdy atrybut `enableEntityData` w nagłówku `Content-Type` jest ustawiony na `true`.
