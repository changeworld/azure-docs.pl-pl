...
ms.locfileid: "68357001"
---
# <a name="install-and-run-text-analytics-containers"></a>Instalowanie i uruchamianie kontenerów analiza tekstu

Kontenery analiza tekstu zapewniają zaawansowane przetwarzanie języka naturalnego w przypadku nieprzetworzonego tekstu i zawierają trzy główne funkcje: tonacji Analysis, wyodrębnianie kluczowych fraz i wykrywanie języka. Łączenie jednostek nie jest obecnie obsługiwane w kontenerze.
...
|-------------|-------|
|{API_KEY} | Ten klucz jest używany do uruchamiania kontenera i jest dostępny na stronie `Text Analytics` klucze Azure Portal. |
|{ENDPOINT_URI} | Wartość identyfikatora URI punktu końcowego rozliczenia jest dostępna na `Text Analytics` stronie Przegląd platformy Azure. |
...
Więcej [przykładów](../text-analytics-resource-container-config.md#example-docker-run-commands) `docker run` polecenia jest dostępnych. 
