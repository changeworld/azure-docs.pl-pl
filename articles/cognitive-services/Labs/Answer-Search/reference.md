...
ms.locfileid: "79220821"
---
# <a name="project-answer-search-v7-reference"></a>Odwołanie do wersji 7 wyszukiwania odpowiedzi projektu

Odpowiedź Bing SearchAPI przyjmuje parametr zapytania i zwraca `searchResponse` z `answerType`: `facts` lub `entities`. 

Aplikacje korzystające z interfejsu API wyszukiwania odpowiedzi wysyłają żądania do punktu końcowego z adresem URL, aby wyświetlić podgląd w parametrze zapytania.  Żądanie musi zawierać parametr `q=searchTerm` i wartość *OCP-APIM-Subscription-Key* .   
...
|Nagłówek|Opis|  
|------------|-----------------|  
|Akceptuj|Opcjonalny nagłówek żądania.<br /><br /> Domyślnym typem nośnika jest Application/JSON. Aby określić, że odpowiedź ma używać [JSON-LD](https://json-ld.org/), ustaw nagłówek Accept na Application/LD + JSON.|  
...
|----------|-----------|----------|--------------|  
|<a name="mkt" />mkt|Rynek, z którego pochodzą wyniki. <br /><br />Aby uzyskać listę możliwych wartości rynkowych, zobacz Kody rynku.<br /><br /> **Uwaga:** Interfejs API podglądu adresów URL obecnie obsługuje tylko rynek en-us i język.<br /><br />|Ciąg|Yes|  
...
|Błędy <a name="errors" />|Lista błędów opisujących przyczyny niepowodzenia żądania.|[Porn](#error)|  
...
|Name (Nazwa)|Wartość|Typ|  
|----------|-----------|----------|  
|_type|Wskazówka dotycząca typu, która jest ustawiona na LicenseAttribution.|Ciąg|  
|licencjonowan|Licencja, pod którą można użyć zawartości.|[Licencjonowan](#license)|  
|licenseNotice|Licencja, która ma być wyświetlana obok pola Target. Na przykład "tekst w obszarze licencji CC-BY-SA".<br /><br /> Użyj nazwy i adresu URL licencji w polu `license`, aby utworzyć hiperłącze do witryny sieci Web, która opisuje szczegóły licencji. Następnie zastąp nazwę licencji w ciągu `licenseNotice` (na przykład CC-BY-SA) utworzonym hiperłączem.|Ciąg|  
|mustBeCloseToContent|Wartość logiczna określająca, czy zawartość reguły musi być umieszczona w pobliżu pola, do którego odnosi się reguła. W przypadku **wartości true**zawartość musi być umieszczona w pobliżu sąsiedztwa. Jeśli **wartość jest równa false**, lub to pole nie istnieje, zawartość może być umieszczona według uznania obiektu wywołującego.|Wartość logiczna|  
...
|Name (Nazwa)|Wartość|Typ|  
|----------|-----------|----------|  
|_type|Wskazówka dotycząca typu, która jest ustawiona na LinkAttribution.|Ciąg|  
|mustBeCloseToContent|Wartość logiczna określająca, czy zawartość reguły musi być umieszczona w pobliżu pola, do którego odnosi się reguła. W przypadku **wartości true**zawartość musi być umieszczona w pobliżu sąsiedztwa. Jeśli **wartość jest równa false**, lub to pole nie istnieje, zawartość może być umieszczona według uznania obiektu wywołującego.|Wartość logiczna|  
...
|tekst|Tekst przypisywany.|Ciąg|  
|url|Adres URL witryny sieci Web dostawcy. Użyj `text` i adresu URL, aby utworzyć hiperłącze.|Ciąg|  
...
|Name (Nazwa)|Wartość|Typ|  
|----------|-----------|----------|  
|_type|Wskazówka dotycząca typu, która jest ustawiona na MediaAttribution.|Ciąg|  
|mustBeCloseToContent|Wartość logiczna określająca, czy zawartość reguły musi być umieszczona w pobliżu pola, do którego odnosi się reguła. W przypadku **wartości true**zawartość musi być umieszczona w pobliżu sąsiedztwa. Jeśli **wartość jest równa false**, lub to pole nie istnieje, zawartość może być umieszczona według uznania obiektu wywołującego.|Wartość logiczna|  
|targetPropertyName|Nazwa pola, do którego odnosi się reguła.|Ciąg|  
...
|primaryImageOfPage/contentUrl|Adres URL reprezentatywnego obrazu do uwzględnienia w wersji zapoznawczej|Ciąg| 
...
|tekst|Tekst przypisywany.<br /><br /> Przypisanie tekstu ma zastosowanie do jednostki jako całości i powinno być wyświetlane bezpośrednio po prezentacji jednostki. Jeśli istnieje wiele reguł autorstwa tekstu lub łącza, które nie określają celu, należy je połączyć i wyświetlić przy użyciu etykiety "dane z:".|Ciąg| 
