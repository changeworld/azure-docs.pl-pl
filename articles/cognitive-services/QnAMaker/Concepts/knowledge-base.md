...
## <a name="key-knowledge-base-concepts"></a>Najważniejsze pojęcia dotyczące bazy wiedzy

* **Pytania**: pytanie zawiera tekst, który najlepiej reprezentuje zapytanie użytkownika. 
* **Odpowiedzi**: odpowiedź jest odpowiedzią zwracaną po dopasowaniu zapytania użytkownika do powiązanego pytania.  
...
Pojedyncza QnA reprezentowana przez liczbowy identyfikator QnA ma wiele wariantów pytania (pytań alternatywnych), które są mapowane na jedną odpowiedź. Ponadto każda para może mieć skojarzone wiele pól metadanych: jeden klucz i jedna wartość.
...
Po pozyskaniu rozbudowanej zawartości do bazy wiedzy QnA Maker próbuje skonwertować zawartość do promocji. Przeczytaj [ten blog](https://aka.ms/qnamaker-docs-markdown-support) , aby dowiedzieć się więcej o formatach promocji, które są zrozumiałe dla większości klientów rozmowy.

Pola metadanych zawierają pary klucz-wartość oddzielone dwukropkiem, takie jak produkt: Shredder. Zarówno klucz, jak i wartość muszą być tylko do tekstu. Klucz metadanych nie może zawierać spacji. Metadane obsługują tylko jedną wartość na klucz.
...
|5|QnA Maker stosuje zaawansowane cechowania, aby określić poprawność pobranych wyników wyszukiwania dla kwerendy użytkownika. |
|6|Przeszkolony model rangi używa wyniku funkcji, od kroku 5, do określania rangi wyników Wyszukiwanie poznawcze platformy Azure.|
...
*Baza wiedzy testowej* to wersja, która jest edytowana, zapisywana i przetestowana pod kątem dokładności i kompletności odpowiedzi. Zmiany wprowadzone w bazie wiedzy testowej nie mają wpływu na użytkownika końcowego aplikacji lub rozmowy bot. Baza wiedzy testowej jest znana jako `test` w żądaniu HTTP. 
