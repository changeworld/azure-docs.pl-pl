...
Usługa Azure Blockchain jest przeznaczona do obsługi wielu protokołów księgi. Obecnie zapewnia obsługę księgi [Kworum](https://www.goquorum.com/) Ethereum przy użyciu mechanizmu konsensusu [IBFT.](https://github.com/jpmorganchase/quorum/wiki/Quorum-Consensus)
...
Jako usługa zarządzana usługa Azure Blockchain Service zapewnia, że węzły członka łańcucha bloków są załatane za pomocą najnowszych aktualizacji systemu operacyjnego hosta i stosu oprogramowania księgi, skonfigurowanych pod kątem wysokiej dostępności (tylko warstwa standardowa), eliminując większość devops wymagane dla tradycyjnych węzłów łańcucha bloków IaaS.  Aby uzyskać więcej informacji na temat poprawek i aktualizacji, zobacz [obsługiwane wersje księgi usługi Azure Blockchain Service](ledger-versions.md).
