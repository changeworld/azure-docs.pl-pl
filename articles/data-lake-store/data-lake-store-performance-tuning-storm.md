---
title: Usługa Azure Data Lake Storage Gen1 Storm wydajności, wytyczne dotyczące dostosowywania | Dokumentacja firmy Microsoft
description: Usługa Azure Data Lake Storage Gen1 Storm wydajności, wytyczne dotyczące dostosowywania
...
* **Uruchomiony klaster systemu Storm na Data Lake Storage Gen1**. Aby uzyskać więcej informacji, zobacz [systemu Storm w HDInsight](https://docs.microsoft.com/azure/hdinsight/hdinsight-storm-overview).
...
Dzięki temu można zwiększyć wydajność, zwiększając równocześnie z operacji We/Wy do i z Data Lake Storage Gen1. Topologii Storm ma zestaw konfiguracji, które określają równoległości:
...
### <a name="get-the-best-performance-from-data-lake-storage-gen1"></a>Dla uzyskania optymalnej wydajności z Data Lake Storage Gen1

Podczas pracy z Data Lake Storage Gen1, możesz uzyskać najlepszą wydajność, jeśli Wykonano następujące czynności:
* COALESCE małe dołącza do większych rozmiarów (najlepiej 4 MB).
...
Domyślny element bolt Data Lake Storage Gen1 Storm ma rozmiar synchronizacji zasad parametr (fileBufferSize) można dostrajanie tego parametru.
...
W Storm spout utrzymuje krotki aż jawnie zostało potwierdzone przez element bolt. Jeśli krotka został odczytany przez elementy bolt, ale nie został jeszcze potwierdzony, spout może nie zostały utrwalone w Data Lake Storage Gen1 zaplecza. Po krotki zostało potwierdzone, spout można zagwarantować stanu trwałego przez elementy bolt i następnie można usunąć źródła danych z dowolnego źródła go odczytywać.  
...
* Posiadanie zasad na podstawie czasu, lub na podstawie liczby, gdzie jest hflush() wyzwolone co x opróżnienia, lub co milisekund y, a krotek do tej pory zgromadzonych uznanych Wstecz.

Pamiętaj, że przepływność w tym przypadku jest niższy, ale z powolnego częstość występowania zdarzeń, maksymalna przepływność nie jest celem największych mimo to. Pomagają one środki zaradcze, można zmniejszyć całkowity czas potrzebny krotki przepływ do magazynu. Może być niezależnie od tego, jeśli chcesz, aby potok w czasie rzeczywistym, nawet w przypadku występowania zdarzeń niski. Należy również zauważyć, jeśli szybkość krotki przychodzących jest niska, Dostosuj parametr topology.message.timeout_secs tak kolekcje nie przekraczają limit czasu, gdy są one objęte buforowane lub przetworzone.
...
### <a name="data-lake-storage-gen1-throttling"></a>Data Lake Storage Gen1 ograniczania
Jeśli napotkasz limity przepustowości, dostarczone przez Data Lake Storage Gen1, można napotkać niepowodzeń zadań. Sprawdź dzienniki zadania, aby błędy ograniczania przepływności. Aby zmniejszyć równoległości, zwiększenie rozmiaru kontenera.    
...
2. Monitorowanie topologii systemu Storm dzienniki na węzłach procesu roboczego (w obszarze /var/log/storm/worker-artifacts /&lt;TopologyName&gt;/&lt;portu&gt;/worker.log) for Data Lake Storage Gen1 ograniczania wyjątków.
