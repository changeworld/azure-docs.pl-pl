...
3. Wprowadź lub wybierz wartości dla następujących ustawień, a następnie wybierz **Utwórz**:
    ...
    |Subnet|Yes|Wybierz podsieć w sieci wirtualnej, który wybrano. Można zmienić podsieci, w której interfejs sieciowy jest przypisany do, po jego utworzeniu.|
    ...
    |Resource group|Tak|Wybierz istniejącą [grupy zasobów](../azure-glossary-cloud-terminology.md?toc=%2fazure%2fvirtual-network%2ftoc.json#resource-group) lub utworzyć nowe. Interfejs sieciowy może znajdować się w grupie zasobów w tej samej lub innej niż maszyna wirtualna, które można dołączyć, lub sieć wirtualną można ją połączyć.|
    |Location|Tak|Maszyna wirtualna, możesz dołączyć do interfejsu sieciowego i sieci wirtualnej, połącz go musi istnieć w tym samym [lokalizacji](https://azure.microsoft.com/regions), nazywany również region.|
