...
Dowiedz się, jak utworzyć, zmienić lub usunąć sieci wirtualnej komunikacji równorzędnej. Sieci wirtualnej komunikacji równorzędnej umożliwia łączenie z sieciami wirtualnymi w sieci Azure sieci szkieletowej. Po połączyć za pomocą, sieci wirtualne są nadal zarządzane jako oddzielne zasoby. Jeśli jesteś nowym użytkownikiem sieci wirtualnej komunikacji równorzędnej, możesz dowiedzieć się więcej o w [sieci wirtualnej komunikacji równorzędnej omówienie](virtual-network-peering-overview.md) lub wykonując [samouczek](tutorial-connect-virtual-networks-portal.md).
...
1. W polu wyszukiwania w górnej części portalu Azure, wprowadź *sieci wirtualnych* w polu wyszukiwania. Gdy **sieci wirtualnych** są wyświetlane w wynikach wyszukiwania, wybierz ją. Nie zaznaczaj **sieci wirtualnych (klasyczne)** Jeśli wygląda na to, na liście, ponieważ nie można utworzyć komunikacji równorzędnej z sieci wirtualnej wdrożone za pośrednictwem klasycznego modelu wdrażania.
...
6. <a name="add-peering"></a>Wprowadź lub wybierz wartości poniższych ustawień:
    ...
    Tylko jeden komunikacji równorzędnej dla tej sieci wirtualnej może mieć to ustawienie jest włączone.
        ...
        Nie można używać zdalnego bramy, jeśli masz już skonfigurowano w Twojej sieci wirtualnej bramę. Nie można włączyć tę opcję, jeśli w przypadku komunikacji równorzędnej sieci wirtualnych w różnych regionach. Aby dowiedzieć się więcej na temat korzystania z bramy do przesyłania, zobacz [konfigurowania bramy sieci VPN dla przesyłanych w sieci wirtualnej komunikacji równorzędnej](../vpn-gateway/vpn-gateway-peering-gateway-transit.md?toc=%2fazure%2fvirtual-network%2ftoc.json)
...
1. W polu wyszukiwania w górnej części portalu wprowadź *sieci wirtualnych* w polu wyszukiwania. Gdy **sieci wirtualnych** są wyświetlane w wynikach wyszukiwania, wybierz ją. Nie zaznaczaj **sieci wirtualnych (klasyczne)** Jeśli wygląda na to, na liście, ponieważ nie można utworzyć komunikacji równorzędnej z sieci wirtualnej wdrożone za pośrednictwem klasycznego modelu wdrażania.
...
Konta używanego do pracy z sieci wirtualnej komunikacji równorzędnej musi być przypisany do następujących ról:

- [Sieci współautora](../role-based-access-control/built-in-roles.md?toc=%2fazure%2fvirtual-network%2ftoc.json#network-contributor): sieci wirtualnej wdrożone za pomocą Menedżera zasobów.
- [Klasyczne sieci współautora](../role-based-access-control/built-in-roles.md?toc=%2fazure%2fvirtual-network%2ftoc.json#classic-network-contributor): sieci wirtualnej wdrożone za pośrednictwem klasycznego modelu wdrażania.

Jeśli Twoje konto nie jest przypisany do jednej z poprzednich ról, musi być przypisany do [niestandardowej roli zabezpieczeń](../role-based-access-control/custom-roles.md?toc=%2fazure%2fvirtual-network%2ftoc.json) przypisany odpowiednie działania w poniższej tabeli:

| Akcja | Name (Nazwa) |
|---|---|
| Microsoft.Network/virtualNetworks/virtualNetworkPeerings/write  | Wymagane do utworzenia komunikacji równorzędnej z siecią wirtualną A siecią wirtualną B. wirtualnych sieci A musi być sieć wirtualną (Resource Manager)                            |
| Microsoft.Network/virtualNetworks/peer/action                   | Wymagane do utworzenia komunikacji równorzędnej z siecią wirtualną B (Resource Manager) do sieci wirtualnej, A                                                                                |
| Microsoft.ClassicNetwork/virtualNetworks/peer                   | Wymagane do utworzenia komunikacji równorzędnej z siecią wirtualną B (klasyczne) do sieci wirtualnej, A                                                                                    |
| Microsoft.Network/virtualNetworks/virtualNetworkPeerings/read   | Przeczytaj sieci wirtualnej komunikacji równorzędnej   |
| Microsoft.Network/virtualNetworks/virtualNetworkPeerings/delete | Usunąć sieci wirtualnej komunikacji równorzędnej |
...
* Tworzenie sieci wirtualnej komunikacji równorzędnej przy użyciu [PowerShell](powershell-samples.md) lub [interfejsu wiersza polecenia Azure](cli-samples.md) przykładowe skrypty lub przy użyciu usługi Azure [szablonów Resource Manager](template-samples.md)
