...
|Action     | Zezwolenie lub zablokowanie        |
...
Poniżej wymienione tagi usługi są dostępne do użycia w [sieciowych grup zabezpieczeń reguły](https://docs.microsoft.com/azure/virtual-network/security-overview#security-rules). Tagi usługi gwiazdką na końcu (czyli AzureCloud *) można również w [reguł sieci zapory usługi Azure](https://docs.microsoft.com/azure/firewall/service-tags). 

* **VirtualNetwork** (Resource Manager) (**VIRTUAL_NETWORK** — model klasyczny): Ten znacznik obejmuje przestrzeń adresową sieci wirtualnej (wszystkie zakresy CIDR zdefiniowane dla sieci wirtualnej), wszystkie połączone lokalne przestrzenie adresowe [skomunikowane równorzędnie](virtual-network-peering-overview.md) sieci wirtualne lub sieć wirtualna połączona [wirtualny Brama sieci](../vpn-gateway/vpn-gateway-about-vpngateways.md?toc=%2fazure%2fvirtual-network%2ftoc.json) i eliminowania prefiksy używane na [trasy definiowane przez użytkownika](virtual-networks-udr-overview.md). Należy pamiętać, że ten tag może zawierać domyślną trasę. 
...
* **AzureCloud*** (tylko model Resource Manager): ten tag określa przestrzeń adresów IP dla platformy Azure, w tym wszystkie [publiczne adresy IP centrum danych](https://www.microsoft.com/download/details.aspx?id=41653). W przypadku określenia wartości *AzureCloud* dozwolony lub blokowany jest ruch do publicznych adresów IP platformy Azure. Jeśli chcesz zezwolić na dostęp do AzureCloud w konkretnym [region](https://azure.microsoft.com/regions), możesz określić region, w następującym formacie AzureCloud. [ Nazwa regionu]. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **AzureTrafficManager*** (tylko model Resource Manager): ten tag określa przestrzeń adresów IP dla adresów IP sondy usługi Azure Traffic Manager. Więcej informacji na temat adresów IP sondy usługi Traffic Manager można znaleźć w temacie [Usługa Azure Traffic Manager — często zadawane pytania](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs). Ten tag jest zalecana dla reguły zabezpieczeń ruchu przychodzącego.  
* **Magazyn*** (tylko model Resource Manager): ten tag określa przestrzeń adresów IP dla usługi Azure Storage. W przypadku określenia wartości *Storage* dozwolony lub blokowany jest ruch do usługi Storage. Jeśli chcesz zezwolić na dostęp do magazynu w konkretnym [region](https://azure.microsoft.com/regions), możesz określić region, w następującym formacie magazynu. [ Nazwa regionu]. Tag reprezentuje usługę, ale nie konkretne wystąpienia usługi. Na przykład tag reprezentuje usługę Azure Storage, ale nie konkretne konto usługi Azure Storage. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **SQL*** (tylko model Resource Manager): Ten tag określa prefiksy adresów usługi Azure SQL Database, Azure Database for MySQL, — Azure Database for PostgreSQL i usługi Azure SQL Data Warehouse. W przypadku określenia wartości *Sql* dozwolony lub blokowany jest ruch do usługi Sql. Jeśli chcesz zezwolić na dostęp do programu Sql w konkretnym [region](https://azure.microsoft.com/regions), możesz określić region, w następującym formacie Sql. [region name]. Tag reprezentuje usługę, ale nie konkretne wystąpienia usługi. Na przykład tag reprezentuje usługę Azure SQL Database, ale nie konkretną bazę danych lub serwer SQL. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **AzureCosmosDB*** (tylko model Resource Manager): ten tag określa prefiksy adresów usługi Azure Cosmos Database. W przypadku określenia wartości *AzureCosmosDB* dozwolony lub blokowany jest ruch do usługi AzureCosmosDB. Jeśli chcesz zezwolić na dostęp do usługi AzureCosmosDB w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: AzureCosmosDB.[nazwa regionu]. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **AzureKeyVault*** (tylko model Resource Manager): ten tag określa prefiksy adresów usługi platformy Azure KeyVault. W przypadku określenia wartości *AzureKeyVault* dozwolony lub blokowany jest ruch do usługi AzureKeyVault. Jeśli chcesz zezwolić na dostęp do usługi AzureKeyVault w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: AzureKeyVault.[nazwa regionu]. Ten tag ma zależności na **usługi AzureActiveDirectory** tagu. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego.  
* **Centrum EventHub*** (tylko model Resource Manager): ten tag określa prefiksy adresów usługi Azure EventHub. W przypadku określenia wartości *EventHub* dozwolony lub blokowany jest ruch do usługi EventHub. Jeśli chcesz zezwolić na dostęp do usługi EventHub w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: EventHub.[nazwa regionu]. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **Usługi ServiceBus*** (tylko model Resource Manager): Ten tag określa prefiksy adresów usługi platformy Azure, magistrali usług przy użyciu warstwy Premium. W przypadku określenia wartości *ServiceBus* dozwolony lub blokowany jest ruch do usługi ServiceBus. Jeśli chcesz zezwolić na dostęp do usługi ServiceBus w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: ServiceBus.[nazwa regionu]. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **MicrosoftContainerRegistry*** (tylko model Resource Manager): ten tag określa prefiksy adresów usługi Microsoft Container Registry. W przypadku określenia wartości *MicrosoftContainerRegistry* dozwolony lub blokowany jest ruch do usługi MicrosoftContainerRegistry. Jeśli chcesz zezwolić na dostęp do usługi MicrosoftContainerRegistry w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: MicrosoftContainerRegistry.[nazwa regionu]. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **AzureContainerRegistry*** (tylko model Resource Manager): ten tag określa prefiksy adresów usługi Azure Container Registry. W przypadku określenia wartości *AzureContainerRegistry* dozwolony lub blokowany jest ruch do usługi AzureContainerRegistry. Jeśli chcesz zezwolić na dostęp do usługi AzureContainerRegistry w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: AzureContainerRegistry.[nazwa regionu]. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **AppService*** (tylko model Resource Manager): ten tag określa prefiksy adresów usługi Azure AppService. W przypadku określenia wartości *AppService* dozwolony lub blokowany jest ruch do usługi AppService. Jeśli chcesz zezwolić na dostęp do usługi AppService w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: AppService.[nazwa regionu]. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego frontonów aplikacji internetowych.  
* **AppServiceManagement*** (tylko model Resource Manager): Ten tag określa prefiksy adresów ruchu administracyjnego wdrożeń App Service Environment w wersji dedykowanej. W przypadku określenia wartości *AppServiceManagement* dozwolony lub blokowany jest ruch do usługi AppServiceManagement. Ten tag jest zalecana dla reguły zabezpieczeń dla ruchu przychodzącego/wychodzącego. 
* **ApiManagement*** (tylko model Resource Manager): Ten tag określa prefiksy adresów ruchu administracyjnego dla usługi APIM dedykowane wdrożeń. W przypadku określenia wartości *ApiManagement* dozwolony lub blokowany jest ruch do usługi ApiManagement. Ten tag jest zalecana dla reguły zabezpieczeń dla ruchu przychodzącego/wychodzącego. 
* **AzureConnectors*** (tylko model Resource Manager): Ten tag określa prefiksy adresów łączników aplikacji logiki dla połączeń sondy/wewnętrznej bazy danych. W przypadku określenia wartości *AzureConnectors* dozwolony lub blokowany jest ruch do usługi AzureConnectors. Jeśli chcesz zezwolić na dostęp do usługi AzureConnectors w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: AzureConnectors.[nazwa regionu]. Ten tag jest zalecana dla reguły zabezpieczeń ruchu przychodzącego. 
* **GatewayManager** (tylko usługa Resource Manager): Ten tag określa prefiksy adresów ruchu administracyjnego wdrożeń dedykowanej bramy sieci VPN/aplikacji. W przypadku określenia wartości *GatewayManager* dozwolony lub blokowany jest ruch do usługi GatewayManager. Ten tag jest zalecana dla reguły zabezpieczeń ruchu przychodzącego. 
* **AzureDataLake*** (tylko model Resource Manager): ten tag określa prefiksy adresów usługi Azure Data Lake. W przypadku określenia wartości *AzureDataLake* dozwolony lub blokowany jest ruch do usługi AzureDataLake. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **Usługi AzureActiveDirectory*** (tylko model Resource Manager): ten tag określa prefiksy adresów usługi AzureActiveDirectory. W przypadku określenia wartości *AzureActiveDirectory* dozwolony lub blokowany jest ruch do usługi AzureActiveDirectory. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego.
* **AzureMonitor*** (tylko model Resource Manager): Ten tag określa prefiksy adresów usługi Log Analytics, usługi App Insights, AzMon i metryki niestandardowe (punkty końcowe GB). Jeśli określisz *AzureMonitor* wartości, ruch jest dozwolony lub zablokowany do AzureMonitor. Ten tag usługi Log Analytics ma zależności na **magazynu** tagu. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego.
* **ServiceFabric*** (tylko model Resource Manager): Ten tag określa prefiksy adresów usługi ServiceFabric. Jeśli określisz *ServiceFabric* wartości, ruch jest dozwolony lub zablokowany do ServiceFabric. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **AzureMachineLearning*** (tylko model Resource Manager): Ten tag określa prefiksy adresów usługi AzureMachineLearning. Jeśli określisz *AzureMachineLearning* wartości, ruch jest dozwolony lub zablokowany do AzureMachineLearning. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **BatchNodeManagement*** (tylko model Resource Manager): Ten tag określa prefiksy adresów ruchu administracyjnego, w przypadku wdrożeń usługi Azure Batch w wersji dedykowanej. Jeśli określisz *BatchNodeManagement* wartości, ruch jest dozwolony lub zablokowany z usługą Batch w węzłach obliczeniowych. Ten tag jest zalecana dla reguły zabezpieczeń dla ruchu przychodzącego/wychodzącego. 
* **AzureBackup*** (tylko model Resource Manager): Ten tag określa prefiksy adresów usługi AzureBackup. Jeśli określisz *AzureBackup* wartości, ruch jest dozwolony lub zablokowany do AzureBackup. Ten tag ma zależności na **magazynu** i **usługi AzureActiveDirectory** tagu. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **AzureActiveDirectoryDomainServices*** (tylko model Resource Manager): Ten tag określa prefiksy adresów ruchu administracyjnego dedykowanych wdrożeń usługi Azure Active Directory Domain Services. Jeśli określisz *AzureActiveDirectoryDomainServices* wartości, ruch jest dozwolony lub zablokowany do AzureActiveDirectoryDomainServices. Ten tag jest zalecana dla reguły zabezpieczeń dla ruchu przychodzącego/wychodzącego.  
* **SqlManagement*** (tylko model Resource Manager): Ten tag określa prefiksy adresów ruchu administracyjnego dla programu SQL dedykowane wdrożeń. Jeśli określisz *SqlManagement* wartości, ruch jest dozwolony lub zablokowany do SqlManagement. Ten tag jest zalecana dla reguły zabezpieczeń dla ruchu przychodzącego/wychodzącego. 
* **CognitiveServicesManagement** (tylko model Resource Manager): Ten tag określa prefiksy adresów ruchu dla usług Cognitive Services. Jeśli określisz *CognitiveServicesManagement* wartości, ruch jest dozwolony lub zablokowany do CognitiveServicesManagement. Ten tag jest zalecane dla reguły zabezpieczeń dla ruchu wychodzącego.  
* **Dynamics365ForMarketingEmail** (tylko model Resource Manager): Ten tag określa prefiksy adresów, marketingowe wiadomości e-mail usługi Dynamics 365. Jeśli określisz *Dynamics365ForMarketingEmail* wartości, ruch jest dozwolony lub zablokowany do Dynamics365ForMarketingEmail. Jeśli chcesz zezwolić na dostęp do Dynamics365ForMarketingEmail w konkretnym [region](https://azure.microsoft.com/regions), możesz określić region, w następującym formacie Dynamics365ForMarketingEmail. [ Nazwa regionu].
* **AzurePlatformDNS** (tylko model Resource Manager): Ten tag określa DNS, czyli usłudze podstawowej infrastruktury. Jeśli określisz *AzurePlatformDNS* wartości, można wyłączyć domyślnie [brany pod uwagę platformę Azure](https://docs.microsoft.com/azure/virtual-network/security-overview#azure-platform-considerations) w systemie DNS. Prosimy o poświęcenie ostrożność przy użyciu tego tagu. Zaleca się testowanie, przed rozpoczęciem korzystania z tego tagu. 
* **AzurePlatformIMDS** (tylko model Resource Manager): Ten tag określa IMDS, czyli usłudze podstawowej infrastruktury. Jeśli określisz *AzurePlatformIMDS* wartości, można wyłączyć domyślnie [platformy Azure brany pod uwagę](https://docs.microsoft.com/azure/virtual-network/security-overview#azure-platform-considerations) dla IMDS. Prosimy o poświęcenie ostrożność przy użyciu tego tagu. Zaleca się testowanie, przed rozpoczęciem korzystania z tego tagu. 
* **AzurePlatformLKM** (tylko model Resource Manager): Ten tag określa licencji Windows lub Usługa zarządzania kluczami. Jeśli określisz *AzurePlatformLKM* wartości, można wyłączyć domyślnie [platformy Azure brany pod uwagę](https://docs.microsoft.com/azure/virtual-network/security-overview#azure-platform-considerations) licencjonowania. Prosimy o poświęcenie ostrożność przy użyciu tego tagu. Zaleca się testowanie, przed rozpoczęciem korzystania z tego tagu. 
...
### <a name="service-tags-in-on-premises"></a>Tagi usługi w lokalnych  
Możesz pobrać i zintegrować z lokalnej zaporze listę tagów usługi ze szczegółami prefiks następujące publikacje co tydzień na platformie Azure [publicznych](https://www.microsoft.com/download/details.aspx?id=56519), [Rządu Stanów Zjednoczonych](https://www.microsoft.com/download/details.aspx?id=57063), [(Chiny) ](https://www.microsoft.com/download/details.aspx?id=57062), i [Niemcy](https://www.microsoft.com/download/details.aspx?id=57064) chmury.

Możesz też programowo pobrać te informacje przy użyciu **interfejs API odnajdywania Tag usługi** (publiczna wersja zapoznawcza) — [REST](https://aka.ms/discoveryapi_rest), [programu Azure PowerShell](https://aka.ms/discoveryapi_powershell), i [ Interfejs wiersza polecenia Azure](https://aka.ms/discoveryapi_cli). 

> [!NOTE]
> Następujące publikacje co tydzień (stara wersja) dla platformy Azure [publicznych](https://www.microsoft.com/en-us/download/details.aspx?id=41653), [Chiny](https://www.microsoft.com/en-us/download/details.aspx?id=42064), i [(Niemcy)](https://www.microsoft.com/en-us/download/details.aspx?id=54770) chmury staną się przestarzałe przez 30 czerwca 2020 r. Uruchom przy użyciu zaktualizowanych publikacje, zgodnie z powyższym opisem. 
...
|---|---|---|---|---|---|---|
|65000|VirtualNetwork|0-65535|VirtualNetwork|0-65535|Wszyscy|Allow|

#### <a name="allowazureloadbalancerinbound"></a>AllowAzureLoadBalancerInBound

|Priorytet|Source|Porty źródłowe|Miejsce docelowe|Porty docelowe|Protocol|Access|
|---|---|---|---|---|---|---|
|65001|AzureLoadBalancer|0-65535|0.0.0.0/0|0-65535|Wszyscy|Allow|
...
|Priorytet|Source|Porty źródłowe| Miejsce docelowe | Porty docelowe | Protocol | Access |
|---|---|---|---|---|---|---|
| 65001 | 0.0.0.0/0 | 0-65535 | Internet | 0-65535 | Wszyscy | Allow |
