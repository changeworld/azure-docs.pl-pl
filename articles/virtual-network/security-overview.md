---
title: Omówienie grup zabezpieczeń platformy Azure
...
|Protocol     | TCP, UDP, ICMP lub dowolny.|
...
* **AzureCloud*** (tylko Menedżer zasobów): ten tag określa przestrzeń adresów IP dla platformy Azure, w tym wszystkie [publiczne adresy IP centrum danych](https://www.microsoft.com/download/details.aspx?id=41653). W przypadku określenia wartości *AzureCloud* dozwolony lub blokowany jest ruch do publicznych adresów IP platformy Azure. Jeśli chcesz zezwolić na dostęp tylko do AzureCloud w określonym [regionie](https://azure.microsoft.com/regions), możesz określić region w następującym formacie AzureCloud. [nazwa regionu]. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **AzureTrafficManager*** (tylko Menedżer zasobów): ten tag określa przestrzeń adresów IP dla adresów IP sondy usługi Azure Traffic Manager. Więcej informacji na temat adresów IP sondy usługi Traffic Manager można znaleźć w temacie [Usługa Azure Traffic Manager — często zadawane pytania](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs). Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu przychodzącego.  
...
* **AzureCosmosDB*** (tylko Menedżer zasobów): ten tag określa prefiksy adresów usługi Azure Cosmos Database. W przypadku określenia wartości *AzureCosmosDB* dozwolony lub blokowany jest ruch do usługi AzureCosmosDB. Jeśli chcesz zezwolić na dostęp do usługi AzureCosmosDB w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: AzureCosmosDB.[nazwa regionu]. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **AzureKeyVault*** (tylko Menedżer zasobów): ten tag określa prefiksy adresów usługi platformy Azure KeyVault. W przypadku określenia wartości *AzureKeyVault* dozwolony lub blokowany jest ruch do usługi AzureKeyVault. Jeśli chcesz zezwolić na dostęp do usługi AzureKeyVault w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: AzureKeyVault.[nazwa regionu]. Ten tag ma zależność od znacznika **usługi azureactivedirectory** . Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu wychodzącego.  
* **EventHub*** (tylko Menedżer zasobów): ten tag określa prefiksy adresów usługi Azure EventHub. W przypadku określenia wartości *EventHub* dozwolony lub blokowany jest ruch do usługi EventHub. Jeśli chcesz zezwolić na dostęp do usługi EventHub w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: EventHub.[nazwa regionu]. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **ServiceBus*** (tylko Menedżer zasobów): Ten tag określa prefiksy adresów usługi Azure ServiceBus przy użyciu warstwy usługi Premium. W przypadku określenia wartości *ServiceBus* dozwolony lub blokowany jest ruch do usługi ServiceBus. Jeśli chcesz zezwolić na dostęp do usługi ServiceBus w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: ServiceBus.[nazwa regionu]. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **MicrosoftContainerRegistry*** (tylko Menedżer zasobów): ten tag określa prefiksy adresów usługi Microsoft Container Registry. W przypadku określenia wartości *MicrosoftContainerRegistry* dozwolony lub blokowany jest ruch do usługi MicrosoftContainerRegistry. Jeśli chcesz zezwolić na dostęp do usługi MicrosoftContainerRegistry w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: MicrosoftContainerRegistry.[nazwa regionu]. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **AzureContainerRegistry*** (tylko Menedżer zasobów): ten tag określa prefiksy adresów usługi Azure Container Registry. W przypadku określenia wartości *AzureContainerRegistry* dozwolony lub blokowany jest ruch do usługi AzureContainerRegistry. Jeśli chcesz zezwolić na dostęp do usługi AzureContainerRegistry w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: AzureContainerRegistry.[nazwa regionu]. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **AppService*** (tylko Menedżer zasobów): ten tag określa prefiksy adresów usługi Azure AppService. W przypadku określenia wartości *AppService* dozwolony lub blokowany jest ruch do usługi AppService. Jeśli chcesz zezwolić na dostęp do usługi AppService w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: AppService.[nazwa regionu]. Ten tag jest zalecany w przypadku wychodzącej reguły zabezpieczeń do frontonów webapps.  
* **AppServiceManagement*** (tylko Menedżer zasobów): Ten tag określa prefiksy adresów dla ruchu związanego z zarządzaniem App Service Environment dedykowanych wdrożeniach. W przypadku określenia wartości *AppServiceManagement* dozwolony lub blokowany jest ruch do usługi AppServiceManagement. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu przychodzącego/wychodzącego. 
* **ApiManagement*** (tylko Menedżer zasobów): Ten tag określa prefiksy adresów ruchu zarządzania dla wdrożeń dedykowanych APIM. W przypadku określenia wartości *ApiManagement* dozwolony lub blokowany jest ruch do usługi ApiManagement. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu przychodzącego/wychodzącego. 
* **AzureConnectors*** (tylko Menedżer zasobów): Ten tag określa prefiksy adresów łączników Logic Apps dla połączeń sondy/zaplecza. W przypadku określenia wartości *AzureConnectors* dozwolony lub blokowany jest ruch do usługi AzureConnectors. Jeśli chcesz zezwolić na dostęp do usługi AzureConnectors w wybranym [regionie](https://azure.microsoft.com/regions), możesz określić ten region za pomocą następującego formatu: AzureConnectors.[nazwa regionu]. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu przychodzącego. 
* **GatewayManager** (tylko usługa Resource Manager): Ten tag określa prefiksy adresów ruchu zarządzania dla dedykowanych wdrożeń bram sieci VPN/aplikacji. W przypadku określenia wartości *GatewayManager* dozwolony lub blokowany jest ruch do usługi GatewayManager. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu przychodzącego. 
* **AzureDataLake*** (tylko Menedżer zasobów): ten tag określa prefiksy adresów usługi Azure Data Lake. W przypadku określenia wartości *AzureDataLake* dozwolony lub blokowany jest ruch do usługi AzureDataLake. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **Usługi azureactivedirectory*** (tylko Menedżer zasobów): ten tag określa prefiksy adresów usługi AzureActiveDirectory. W przypadku określenia wartości *AzureActiveDirectory* dozwolony lub blokowany jest ruch do usługi AzureActiveDirectory. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu wychodzącego.
* **AzureMonitor*** (tylko Menedżer zasobów): Ten tag określa prefiksy adresów Log Analytics, App Insights, AzMon i metryki niestandardowych (rdzeniowym procesorem Endpoints). Jeśli określisz *AzureMonitor* dla wartości, ruch jest dozwolony lub odrzucany do AzureMonitor. W przypadku Log Analytics ten tag ma zależność od tagu **magazynu** . Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu wychodzącego.
* **Servicefabric*** (tylko Menedżer zasobów): Ten tag oznacza prefiksy adresów usługi servicefabric. W przypadku określenia ** dla wartości elementu servicefabric, ruch jest dozwolony lub odrzucany przez sieć szkieletową. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **AzureMachineLearning*** (tylko Menedżer zasobów): Ten tag oznacza prefiksy adresów usługi AzureMachineLearning. Jeśli określisz *AzureMachineLearning* dla wartości, ruch jest dozwolony lub odrzucany do AzureMachineLearning. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **BatchNodeManagement*** (tylko Menedżer zasobów): Ten tag określa prefiksy adresów dla ruchu związanego z zarządzaniem Azure Batch dedykowanych wdrożeniach. Jeśli dla wartości określono wartość *BatchNodeManagement* , ruch jest dozwolony lub odrzucany przez usługę Batch do węzłów obliczeniowych. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu przychodzącego/wychodzącego. 
* **AzureBackup*** (tylko Menedżer zasobów): Ten tag oznacza prefiksy adresów usługi AzureBackup. Jeśli określisz *AzureBackup* dla wartości, ruch jest dozwolony lub odrzucany do AzureBackup. Ten tag ma zależność od znacznika **Storage** i **usługi azureactivedirectory** . Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu wychodzącego. 
* **AzureActiveDirectoryDomainServices*** (tylko Menedżer zasobów): Ten tag określa prefiksy adresów dla ruchu związanego z zarządzaniem Azure Active Directory Domain Services dedykowanych wdrożeniach. Jeśli określisz *AzureActiveDirectoryDomainServices* dla wartości, ruch jest dozwolony lub odrzucany do AzureActiveDirectoryDomainServices. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu przychodzącego/wychodzącego.  
...
* **CognitiveServicesManagement** (Tylko Menedżer zasobów): Ten tag oznacza prefiksy adresów dla Cognitive Services. Jeśli określisz *CognitiveServicesManagement* dla wartości, ruch jest dozwolony lub odrzucany do CognitiveServicesManagement. Ten tag jest zalecany dla reguły zabezpieczeń dla ruchu wychodzącego.  
* **Dynamics365ForMarketingEmail** (Tylko Menedżer zasobów): Ten tag określa prefiksy adresów usługi marketingowej poczty e-mail w usłudze Dynamics 365. Jeśli określisz *Dynamics365ForMarketingEmail* dla wartości, ruch jest dozwolony lub odrzucany do Dynamics365ForMarketingEmail. Jeśli chcesz zezwolić na dostęp tylko do Dynamics365ForMarketingEmail w określonym [regionie](https://azure.microsoft.com/regions), możesz określić region w następującym formacie Dynamics365ForMarketingEmail. [nazwa regionu].
* **AzurePlatformDNS** (Tylko Menedżer zasobów): Ten tag wskazuje system DNS, który jest podstawową usługą infrastruktury. W przypadku określenia *AzurePlatformDNS* dla wartości można wyłączyć domyślne zagadnienia dotyczące [platformy Azure](https://docs.microsoft.com/azure/virtual-network/security-overview#azure-platform-considerations) dla systemu DNS. Należy zachować ostrożność przy użyciu tego tagu. Testowanie jest zalecane przed użyciem tego tagu. 
* **AzurePlatformIMDS** (Tylko Menedżer zasobów): Ten tag wskazuje IMDS, który jest podstawową usługą infrastruktury. W przypadku określenia *AzurePlatformIMDS* dla wartości można wyłączyć domyślne zagadnienia dotyczące [platformy Azure dla usługi](https://docs.microsoft.com/azure/virtual-network/security-overview#azure-platform-considerations) IMDS. Należy zachować ostrożność przy użyciu tego tagu. Testowanie jest zalecane przed użyciem tego tagu. 
* **AzurePlatformLKM** (Tylko Menedżer zasobów): Ten tag oznacza Licencjonowanie systemu Windows lub usługę zarządzania kluczami. W przypadku określenia *AzurePlatformLKM* dla wartości można wyłączyć domyślne zagadnienia dotyczące [platformy Azure](https://docs.microsoft.com/azure/virtual-network/security-overview#azure-platform-considerations) na potrzeby licencjonowania. Należy zachować ostrożność przy użyciu tego tagu. Testowanie jest zalecane przed użyciem tego tagu. 
...
Możesz pobrać i zintegrować z lokalną zaporą Lista tagów usług z prefiksem szczegóły w następujących tygodniowych publikacjach dla chmur [publicznych](https://www.microsoft.com/download/details.aspx?id=56519), [administracji USA](https://www.microsoft.com/download/details.aspx?id=57063), [Chin](https://www.microsoft.com/download/details.aspx?id=57062)i [Niemiec](https://www.microsoft.com/download/details.aspx?id=57064) .

Możesz również programowo pobrać te informacje za pomocą **interfejsu API odnajdowania tagów usługi** (publiczna wersja zapoznawcza) — [rest](https://aka.ms/discoveryapi_rest), [Azure PowerShell](https://aka.ms/discoveryapi_powershell)i [interfejsu wiersza polecenia platformy Azure](https://aka.ms/discoveryapi_cli). 

> [!NOTE]
> Następujące publikacje tygodniowe (stara wersja) dla chmur [publicznych](https://www.microsoft.com/en-us/download/details.aspx?id=41653), [Chin](https://www.microsoft.com/en-us/download/details.aspx?id=42064)i [Niemiec](https://www.microsoft.com/en-us/download/details.aspx?id=54770) z platformy Azure będą przestarzałe do 30 czerwca 2020. Zacznij korzystać z zaktualizowanych publikacji zgodnie z powyższym opisem. 
...
|Priority|Source|Porty źródłowe|Miejsce docelowe|Porty docelowe|Protocol|Access|
|---|---|---|---|---|---|---|
...
|Priority|Source|Porty źródłowe|Miejsce docelowe|Porty docelowe|Protocol|Access|
|---|---|---|---|---|---|---|
|65500|0.0.0.0/0|0-65535|0.0.0.0/0|0-65535|Any|Zablokuj|
...
W kolumnach **Źródło** i **Obiekt docelowy** elementy *VirtualNetwork*, *AzureLoadBalancer* i *Internet* są [tagami usługi](#service-tags), a nie adresami IP. W kolumnie Protokół **wszystkie** obejmują protokoły TCP, UDP i ICMP. Podczas tworzenia reguły można określić protokół TCP, UDP, ICMP lub dowolny. Wartość *0.0.0.0/0* w kolumnach **Źródło** i **Obiekt docelowy** reprezentuje wszystkie adresy. Klienci, takie jak Azure Portal, interfejs wiersza polecenia platformy Azure lub program PowerShell, mogą używać dla tego wyrażenia znaku * lub dowolnego z nich.
...
> [!NOTE]
> Sieciowe grupy zabezpieczeń są skojarzone z podsieciami lub do maszyn wirtualnych i usług w chmurze wdrożonych w klasycznym modelu wdrażania, a także do podsieci lub interfejsów sieciowych w Menedżer zasobów model wdrażania. Aby dowiedzieć się więcej na temat modeli wdrażania platformy Azure, zapoznaj się z artykułem [Understand Azure deployment models](../azure-resource-manager/resource-manager-deployment-model.md?toc=%2fazure%2fvirtual-network%2ftoc.json) (Informacje na temat modeli wdrażania platformy Azure).
