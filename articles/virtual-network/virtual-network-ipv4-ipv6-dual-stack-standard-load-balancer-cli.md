---
title: Wdrażanie aplikacji IPv6 z dwoma stosami — standardowy moduł równoważenia obciążenia — interfejs wiersza polecenia
...
W tym artykule pokazano, jak wdrożyć aplikację z dwoma stosami (IPv4 + IPv6) przy użyciu standardowego modułu równoważenia obciążenia na platformie Azure, która zawiera sieć wirtualną z podwójnym stosem z podsiecią z dwoma stosami, standardowy moduł równoważenia obciążenia z dwoma (IPv4 + IPv6), maszyny wirtualne z kartami sieciowymi, które mają podwójną konfigurację IP, dwie reguły grupy zabezpieczeń sieci i podwójne publiczne adresy IP.
...
Utwórz standardowy moduł równoważenia obciążenia z [siecią az lb utwórz](https://docs.microsoft.com/cli/azure/network/lb?view=azure-cli-latest) o nazwie **dsLB,** która zawiera pulę frontonu o nazwie **dsLbFrontEnd_v4**, pulę zaplecza o nazwie **dsLbBackEndPool_v4** skojarzoną z publicznym adresem IP IPv4 **dsPublicIP_v4** utworzonym w poprzednim kroku. 
