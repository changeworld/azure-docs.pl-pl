---
title: Zabezpieczanie usługi Azure Key Vault — usługa Azure Key Vault | Dokumentacja firmy Microsoft
...
Uwierzytelnianie przy użyciu usługi Azure Active Directory — w tej sekcji wyjaśniono, jak obiekt wywołujący uwierzytelnia się za pomocą usługi Azure Active Directory do dostępu do magazynu kluczy za pośrednictwem płaszczyzny zarządzania i płaszczyzny danych. 
...
* **dostęp użytkownika i aplikacji**: Używane z aplikacji uzyskujących dostęp do usługi Key Vault w imieniu zalogowanego użytkownika. Program Azure PowerShell i witryny Azure portal to przykłady tego typu dostępu. Istnieją dwa sposoby udzielania dostępu użytkownikom:
...
* **dostęp tylko do aplikacji**: Używane z aplikacjami, które są uruchamiane jako demon usługi lub zadania w tle. Tożsamości aplikacji udzielany jest dostęp do magazynu kluczy.
...
* **Klucz magazynu**: Używane do uzyskania dostępu do konta magazynu.
