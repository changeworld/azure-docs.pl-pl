...
Aby przypisać użytkownika lub grupy do aplikacji przedsiębiorstwa, musi mieć odpowiednie uprawnienia do zarządzania aplikacji przedsiębiorstwa, a musi być administratorem globalnym katalogu. For Applications firmy Microsoft (takich jak aplikacje usługi Office 365) należy użyć programu PowerShell do przypisywania użytkowników do aplikacji przedsiębiorstwa.
...
1. Uruchom polecenie `$sp.AppRoles` do wyświetlenia dostępnych dla danej aplikacji Workplace Analytics ról. W tym przykładzie chcemy przypisać Britta Simon rola analityka (ograniczony dostęp).
   ...
   ![Pokazuje dostępne role dla użytkownika przy użyciu roli usługi Analytics miejsca pracy](./media/assign-user-or-group-access-portal/workplace-analytics-role.png)
