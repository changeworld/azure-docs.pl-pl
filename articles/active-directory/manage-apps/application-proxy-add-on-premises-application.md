...
1. Wybierz swoją nazwę użytkownika w prawym górnym rogu. Sprawdź, czy zalogowano do katalogu, który używa serwera Proxy aplikacji. Jeśli potrzebujesz zmienić katalogi, wybierz **Przełącz katalog** i wybierz katalog, który używa serwera Proxy aplikacji.
1. W panelu nawigacyjnym po lewej stronie wybierz **usługi Azure Active Directory**.
1. W obszarze **Zarządzaj**, wybierz opcję **serwera proxy aplikacji**.
1. Wybierz **Pobierz usługę łącznika**.

    ![Pobierz usługę łącznika, aby zobaczyć, warunki użytkowania usługi](./media/application-proxy-add-on-premises-application/application-proxy-download-connector-service.png)

1. Zapoznaj się z warunkami użytkowania usługi. Gdy wszystko będzie gotowe, wybierz **Zaakceptuj warunki i Pobierz**.
1. W dolnej części okna wybierz **Uruchom** instalacji łącznika. Zostanie otwarty kreator instalacji.
1. Postępuj zgodnie z instrukcjami w kreatorze, aby zainstalować usługę. Po wyświetleniu monitu o zarejestrowanie łącznika na serwerze proxy aplikacji w dzierżawie usługi Azure AD, wprowadź swoje poświadczenia administratora aplikacji.
...
1. W panelu nawigacyjnym po lewej stronie wybierz **usługi Azure Active Directory**, a następnie wybierz pozycję **serwera Proxy aplikacji** w obszarze **Zarządzaj** sekcji. Na stronie widać wszystkie łączniki i grupy łączników.
1. Wyświetl łącznika, aby sprawdzić jego szczegóły. Powinno zostać rozszerzone łączniki domyślnie. Jeśli nie jest rozwinięty łącznika którego chcesz wyświetlić, rozwiń łącznika Aby wyświetlić szczegóły. Zielona ikona oznacza, że łącznik jest aktywny i może łączyć się z usługą. Jednak nawet wtedy problem z siecią może uniemożliwiać łącznikowi odbieranie wiadomości.
...
1. Jeśli nie ma stanu dla usług **systemem**, kliknij prawym przyciskiem myszy, aby wybrać poszczególnych usług i wybierz **Start**.
...
1. W panelu nawigacyjnym po lewej stronie wybierz **usługi Azure Active Directory**.
1. Wybierz **aplikacje dla przedsiębiorstw**, a następnie wybierz pozycję **nową aplikację**.
1. Wybierz **aplikacje lokalne**.  
1. W **Dodaj własną aplikację w środowisku lokalnym** sekcji, podaj następujące informacje o aplikacji:
...
1. Wybierz **wprowadzenie**, a następnie wybierz pozycję **przypisać użytkownika do testowania**.
1. W obszarze **użytkowników i grup**, wybierz opcję **Dodaj użytkownika**.
1. W obszarze **Dodaj przypisanie**, wybierz opcję **użytkowników i grup**. **Użytkowników i grup** pojawi się sekcja.
1. Wybierz konto, które chcesz dodać.
1. Wybierz **wybierz**, a następnie wybierz pozycję **przypisać**.
...
1. Zaloguj się jako użytkownik, który został utworzony w poprzedniej sekcji.
