...
2.  Na lewy pasek nawigacyjny, wybierz **usługi Azure Active Directory**, a następnie wybierz **rejestracji aplikacji**.

3.  Wybierz aplikację, która używa usługi Azure AD działania dzienniki interfejsu API, wybierz **ustawienia**, a następnie w **ustawienia** bloku, wybierz opcję **wymagane uprawnienia**.

4.  W **wymagane uprawnienia** bloku, wybierz opcję **Windows Azure Active Directory** interfejsu API.

5.  W **delegowane uprawnienia** obszar **włączyć dostęp** bloku, zaznacz pole obok **odczytu katalogu** danych, a następnie wybierz **zapisać**.

    Powrócisz do **wymagane uprawnienia** bloku.

    ![Delegowane uprawnienia odczytu katalogu opcji](./media/whats-new/app-registration-delegate-read-directory.png)

6.  Wybierz **udzielić uprawnień**, a następnie wybierz **tak**.
...
### <a name="microsoft-graph-apis-for-administrative-scenarios-for-tou"></a>Microsoft Graph interfejsu API dla administracyjne scenariusze użytkowania
...
**Możliwości produktu:** środowisko dewelopera
 ...
 Dodaliśmy Microsoft Graph API dla operacji administracyjnych programu Azure AD warunki użytkowania. Masz możliwość tworzenie, aktualizowanie i usuwanie obiektu warunki użytkowania.
...
Klienci w Europie wymaga ich danych pozostanie w Europie i nie są replikowane poza Europejskiego centrów danych dla prywatności spotkania i ustawowe Europejskich. To [artykułu](https://go.microsoft.com/fwlink/?linkid=872328) udostępnia szczegółowe informacje, jakie informacje tożsamości będą przechowywane w Europie i udostępniają szczegółowe informacje o informacje, które będą przechowywane poza Europejskiego centrów danych. 
...
- [Podstawy na żądanie](https://docs.microsoft.com/en-us/azure/active-directory/active-directory-saas-cornerstone-ondemand-provisioning-tutorial)
...
Przegląda dostępu grup i aplikacji teraz jest ogólnie dostępna w ramach usługi Azure AD Premium P2.  Administratorzy będą mogli jej konfigurować dostępu przeglądy członkostwa w grupach i przypisania aplikacji, aby automatycznie powtarzać w regularnych odstępach czasu, takich jak co miesiąc lub co kwartał.
...
Azure Dzienniki aktywności AD, takich jak, dzienniki inspekcji i logowania, są teraz dostępne za pośrednictwem MS Graph. Firma Microsoft ma widoczne 2 punktów końcowych za pośrednictwem Graph MS dostępu do tych dzienników. Sprawdź, czy naszych [dokumentów](https://docs.microsoft.com/en-us/azure/active-directory/active-directory-reporting-api-getting-started-azure-portal) programistyczny dostęp do usługi Azure AD raportowania interfejsów API, aby rozpocząć pracę. 
...
**Tylko w czasie realizacji:** po udostępniania zasobu z użytkownika gościa za pomocą interfejsu API B2B — nie trzeba było wysłać wiadomość e-mail z zaproszeniem specjalnych. W większości przypadków użytkownika gościa można uzyskać dostęp do zasobów i zostaną wykonane przez środowisko realizacji tylko w czasie. Brak wpływu z powodu brakujących wiadomości e-mail. Nie ma więcej pytania użytkownika gościa "Czy możesz kliknąć łącze realizacji wysłania systemu, możesz?". Oznacza to, gdy SPO używa Menedżera zaproszenia — mętna załączniki mogą mieć ten sam adres URL kanonicznej dla wszystkich użytkowników — wewnętrznych i zewnętrznych — w dowolnym stanie realizacji.

**Realizacji nowoczesne środowisko:** ma więcej podzielić realizacji ekranu strona początkowa. Użytkownicy będą widzieć modern zgody doświadczenia z poufności organizacji zaproszenia, podobnie jak w przypadku aplikacji firm 3.
...
W 2018 maja dodano obsługę następujących 18 nowych aplikacji w galerii aplikacji z Federacją:
...
Możesz także znaleźć w dokumentacji wszystkie aplikacje [tutaj](https://aka.ms/appstutorial).

Aby uzyskać listę aplikacji w galerii aplikacji Azure AD, zobacz [wyświetlanie aplikacji w galerii aplikacji usługi Azure Active Directory](https://docs.microsoft.com/en-us/azure/active-directory/develop/active-directory-app-gallery-listing). 
...
### <a name="deployment-plans---guidance-to-get-you-started-with-azure-active-directory"></a>Plany wdrożenie — wskazówki ułatwiające rozpoczęcie pracy z usługą Azure Active Directory
...
Trwa wyszukiwanie wskazówki krok po kroku dotyczące wdrażania (SSPR, logowania jednokrotnego, urząd certyfikacji, serwer Proxy aplikacji, inicjowanie obsługi użytkowników, usług AD FS do PTA lub usług AD FS do PHS, który?). Mamy plany wdrażania, aby pomóc!

Przejdź do [Aka.ms/DeploymentPlans](http://Aka.ms/DeploymentPlans) której można uzyskać dostęp przewodniki kompleksowe, aby rozpocząć pobieranie wartości z tych możliwości.



- [Jak uzyskać planów wdrożenia](http://Aka.ms/DeploymentPlans)? 

- Opinię możesz przesłać [tutaj](http://aka.ms/DeploymentPlanFeedback)

- Pytania? Dotrzeć do: [IDGitDeploy@microsoft.com](mailto:IDGitDeploy@microsoft.com)
...
### <a name="public-preview-of-new-and-improved-sign-ins-user-experience-in-azure-portal"></a>Środowisko publicznej wersji zapoznawczej nowych i ulepszonych logowania użytkownika w portalu Azure
...
Możemy radością wprowadzenie nowych i ulepszonych logowania dzienniki środowisko użytkownika od w portalu usługi Azure AD. Z nowe środowisko logowania użytkownika klienci teraz mogą uzyskać następujące czynności:

- Ulepszone opóźnienie z 2 godziny w ciągu 5 minut. Zapoznaj się z naszym [opóźnienia](https://docs.microsoft.com/en-us/azure/active-directory/active-directory-reporting-latencies-azure-portal) dokumentów, aby uzyskać więcej informacji.

- Możliwość dodawania filtruje dynamicznie przy użyciu przycisku "Kolumny". Przez dodawanie kolumn do raportu logowania w środowiska użytkownika, można automatycznie zobaczyć je jako filtrów do użycia.

- Możliwość sortować według daty, nazwę użytkownika i aplikacji.

- Włączenie starszych uwierzytelnienia i możliwość filtrowania dla starszych uwierzytelnienia przy użyciu kolumny "Aplikacja klienta".

- Dołączenie do pobrania skryptu programu PowerShell, który jest dostosowana na podstawie filtru warunków wybrany w UX. Za pomocą tego skryptu programu PowerShell, można uzyskać dowolną liczbę wierszy danych w miarę chcesz (zgodnie z kryteriami filtru), który dostarcza dane wyjściowe w formacie CSV.

Aby uzyskać więcej szczegółów dotyczących tej funkcji, zobacz [logowania raporty aktywności w portalu usługi Azure Active Directory](https://docs.microsoft.com/en-us/azure/active-directory/active-directory-reporting-activity-sign-ins).
...
Przegląda dostępu grup i aplikacje znajdują się teraz ogólnie dostępna w ramach usługi Azure AD Premium P2. Administrator może skonfigurować, aby automatycznie zastosować zmian recenzenta do tej grupy lub aplikacji jako zakończeniu Przegląd dostępu. Administrator można również określić, co się dzieje z nieprzerwanego dostępu użytkownika osoby dokonujące przeglądu nie odpowiadać, usunięcie dostępu, program Access lub podjąć zalecenia systemu. 
...
Aplikacje utworzone na lub po 4/25/2018 nie będzie już można żądania **żądaniu** przy użyciu **zapytania** response_mode.  Dzięki temu udostępniają usługi Azure AD tekście razem ze specyfikacjami OIDC i pomaga zmniejszyć obszar ataków aplikacji.  Aplikacje utworzone przed 4/25/2018 nie jest blokowane przy użyciu **zapytania** response_mode z response_type z **żądaniu**.  Błąd zwrócony podczas żądania z usługi AAD, żądaniu **AADSTS70007: "query" nie jest obsługiwaną wartością "response_mode" podczas żądania tokenu**.

**Fragmentu** i **form_post** response_modes kontynuowanie pracy — podczas tworzenia nowych obiektów aplikacji (np. w przypadku użycia serwera Proxy aplikacji), użyj jednej z tych response_modes upewnij się przed utworzeniem nowego aplikacja.  
...
Obsługa polecenia cmdlet serwera Proxy aplikacji jest teraz modułu programu Powershell GA! Należy pamiętać, że to wymagać aktualne informacje o moduły programu Powershell — Jeśli staje się bardziej niż rok za niektóre polecenia cmdlet mogą przestać działać. 
...
### <a name="users-get-a-silent-sign-on-experience-with-seamless-sso-if-an-application-sends-sign-in-requests-to-azure-ads-tenanted-endpoints"></a>Użytkownicy logują się w trybie cichym za pomocą bezproblemowego logowania jednokrotnego, jeśli aplikacja wysyła żądania logowania do punktów końcowych w dzierżawach usługi Azure AD
...
W 2018 marca dodano obsługę następujących 15 nowych aplikacji w galerii aplikacji z Federacją:
...
Można znaleźć w dokumentacji wszystkie aplikacje w tym miejscu: [https://aka.ms/appstutorial](https://aka.ms/appstutorial)
...
Zaktualizowaliśmy Centrum usługi Azure AD administratora, aby dodać obsługę tej funkcji. Teraz klientów można przekonwertować istniejących grup z członkostwo dynamiczne przypisanych członkostwa i na odwrót. Nadal dostępne są również istniejące polecenia cmdlet programu PowerShell.
