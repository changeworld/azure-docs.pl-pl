...
|---------|---------|---------|---------|
| `accessTokenAcceptedVersion` | Int32 dopuszczającego wartość null | Określa wersję tokenu dostępu akceptowane dla bieżącego zasobu interfejsu API. Możliwe wartości to 1, 2, wartość null. Wartość domyślna to wartość null, która będzie traktowane jako 2. | `2` |
...
| `logoutUrl` | ciąg | Adres URL wylogować się z aplikacji. | `https://MyRegisteredAppLogout` |
...
| `parentalControlSettings` | ciąg | `countriesBlockedForMinors` Określa krajach, w których aplikacja jest zablokowana dla osoby nieletnie.<br>`legalAgeGroupRule` Określa pełnoletności reguły grupy, która ma zastosowanie do użytkowników aplikacji. Można ustawić `Allow`, `RequireConsentForPrivacyServices`, `RequireConsentForMinors`, `RequireConsentForKids`, lub `BlockMinors`.  | <code>{<br>&nbsp;&nbsp;&nbsp;"countriesBlockedForMinors":[],<br>&nbsp;&nbsp;&nbsp;"legalAgeGroupRule":"Allow"<br>} </code> |
