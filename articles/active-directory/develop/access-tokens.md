...
description: Akceptować tokeny dostępu emitowane przez 1.0 i 2.0 punktów końcowych usługi Azure AD.
...
| `unique_name` | Ciąg | Wyświetlane tylko w wersji 1.0 tokenów. Udostępnia ludzi, można odczytać wartość, która identyfikuje podmiotu tokenu. Ta wartość nie musi być unikatowa w ramach dzierżawy i należy używać tylko w celach wyświetlania. |
...
Poniższe oświadczenia zostaną uwzględnione w tokenów w wersji 1.0, jeśli ma to zastosowanie, ale nie są uwzględnione w wersji 2.0 tokenów domyślnie. Jeśli używasz wersji 2.0 i konieczności te oświadczenia zażądać ich przy użyciu [opcjonalnych oświadczeń](active-directory-optional-claims.md).
