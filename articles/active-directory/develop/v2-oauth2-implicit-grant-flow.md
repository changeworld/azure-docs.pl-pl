...
| id_token |Id_token, który zażądał aplikacji. Id_token można użyć do zweryfikowania tożsamości użytkownika i rozpocząć sesję z użytkownikiem. Szczegółowe informacje na temat id_tokens i ich zawartość znajduje się w [token odwołania do punktu końcowego v2.0](v2-id-and-access-tokens.md). |
...
Można wybrać sprawdzić poprawność `id_token` w kliencie kod, ale powszechną praktyką jest wysłanie `id_token` do serwera wewnętrznej bazy danych i zweryfikować istnieje. Po zweryfikowaniu podpis id_token, istnieje kilka oświadczenia, które będzie trzeba zweryfikować. Zobacz [odwołania do tokenu v2.0](v2-id-and-access-tokens.md) uzyskać więcej informacji, w tym [sprawdzania poprawności tokenów](v2-id-and-access-tokens.md#validating-tokens) i [ważnych informacji dotyczących podpisywania Przerzucanie klucza](v2-id-and-access-tokens.md#validating-tokens). Firma Microsoft zaleca korzystające z biblioteki do analizowania i sprawdzanie poprawności tokenów — Brak co najmniej jeden dostępny dla większości platform i języków.
...
Aby uzyskać więcej informacji na oświadczeniach w id_token, zobacz [token odwołania do punktu końcowego v2.0](v2-id-and-access-tokens.md).
...
Aby uzyskać więcej informacji na temat oświadczenia obecne w tokenie dostępu, zobacz [token odwołania do punktu końcowego v2.0](v2-id-and-access-tokens.md)
