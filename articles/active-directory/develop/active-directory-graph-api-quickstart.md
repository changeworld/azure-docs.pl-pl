...
W interfejs API programu Graph, aby uzyskać dostęp do danych i obiektów katalogu (innymi słowy, zasobów lub jednostek), dla których chcesz wykonywać operacje CRUD, możesz użyć adresów URL opartych na protokole Open Data (OData). Adresy URL używane w interfejs API programu Graph składają się z czterech głównych części: katalogu głównego usług, identyfikatora dzierżawy, ścieżki zasobów i opcji ciągu `https://graph.windows.net/{tenant-identifier}/{resource-path}?[query-parameters]`zapytania:. Zapoznaj się z następującym adresem URL: `https://graph.windows.net/contoso.com/groups?api-version=1.6`.
...
Na przykład `https://graph.windows.net/contoso.com/tenantDetails?api-version=1.6` zwraca informacje o firmie dotyczące katalogu contoso.com.
...
* Historia żądań.
* Obsługuje aliasy adresów "Me" i "weborganization". Można na przykład użyć `https://graph.windows.net/me?api-version=1.6` do zwrócenia obiektu użytkownika zalogowanego użytkownika lub `https://graph.windows.net/myorganization/users?api-version=1.6` zwrócenia wszystkich użytkowników w katalogu zalogowanego użytkownika.
* Obsługuje pełne operacje CRUD na własny katalog przy użyciu `POST` `PATCH` , `GET`i `DELETE`.
...
**Uzyskaj token dostępu**: Aby można było uzyskać dostęp do programu Azure AD Graph, klienci muszą najpierw uwierzytelnić się w usłudze Azure AD. Aby uzyskać więcej informacji, zobacz [scenariusze uwierzytelniania dla usługi Azure AD](authentication-scenarios.md).

**Twórz i uruchamiaj zapytanie**: Wykonaj następujące czynności:
...
4. W polu bezpośrednio poniżej wpisów ściągania wpisz następujący nagłówek HTTP:
   ...
   > [!NOTE]
   > Zastąp swój token&gt; dostępu tokenem dostępu do katalogu usługi Azure AD. &lt;
