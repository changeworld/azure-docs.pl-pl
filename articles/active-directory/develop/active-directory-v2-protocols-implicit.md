...
description: Tworzenie aplikacji sieci web przy użyciu usługi Azure AD v2.0 implementacji niejawny przepływ dla aplikacji jednostronicowych.
...
| id_token |Id_token, który zażądał aplikacji. Id_token można użyć do zweryfikowania tożsamości użytkownika i rozpocząć sesję z użytkownikiem. Szczegółowe informacje na temat id_tokens i ich zawartość znajduje się w [token odwołania do punktu końcowego v2.0](active-directory-v2-tokens.md). |
...
Można wybrać sprawdzić poprawność `id_token` w kliencie kod, ale powszechną praktyką jest wysłanie `id_token` do serwera wewnętrznej bazy danych i zweryfikować istnieje. Po zweryfikowaniu podpis id_token, istnieje kilka oświadczenia, które będzie trzeba zweryfikować. Zobacz [odwołania do tokenu v2.0](active-directory-v2-tokens.md) uzyskać więcej informacji, w tym [sprawdzania poprawności tokenów](active-directory-v2-tokens.md#validating-tokens) i [ważnych informacji dotyczących podpisywania Przerzucanie klucza](active-directory-v2-tokens.md#validating-tokens). Firma Microsoft zaleca korzystające z biblioteki do analizowania i sprawdzanie poprawności tokenów — Brak co najmniej jeden dostępny dla większości platform i języków.
...
Aby uzyskać więcej informacji na oświadczeniach w id_token, zobacz [token odwołania do punktu końcowego v2.0](active-directory-v2-tokens.md).

Po zweryfikowaniu całkowicie id_token, można rozpocząć sesji z użytkownikiem i korzystanie z oświadczeń id_token, aby uzyskać informacje o użytkowniku w swojej aplikacji. Te informacje mogą służyć do wyświetlania rekordów, autoryzacje itp.
...
| redirect_uri |zalecane |Redirect_uri aplikacji, gdzie odpowiedzi uwierzytelniania mogą być wysyłane i odbierane przez aplikację. Dokładnie musi odpowiadać jednej z redirect_uris, zarejestrowanych w portalu, z wyjątkiem musi być zakodowane w adresie url. |
| scope |wymagane |Rozdzielonej spacjami listy zakresów. W celu uzyskania tokenów, Uwzględnij wszystkie [zakresy](active-directory-v2-scopes.md) wymagają zasobu zainteresowania. |
...
Aby uzyskać więcej informacji na temat oświadczenia obecne w tokenie dostępu, zobacz [token odwołania do punktu końcowego v2.0](active-directory-v2-tokens.md)
