...
![Menu Start, za pomocą edytora reguł synchronizacji](./media/active-directory-aadconnectsync-change-the-configuration/startmenu2.png)
...
[Harmonogramu](active-directory-aadconnectsync-feature-scheduler.md) jest domyślnie uruchamiany co 30 minut. Upewnij się, że nie jest uruchomiona, podczas wprowadzania zmian i rozwiązywanie problemów z nowych zasad. Aby tymczasowo wyłączyć harmonogram, uruchom program PowerShell i uruchom `Set-ADSyncScheduler -SyncCycleEnabled $false`.

![Wyłącz harmonogram](./media/active-directory-aadconnectsync-change-the-configuration/schedulerdisable.png)  
...
3. Na **filtru Scoping** wpisz **givenName ISNOTNULL**.  
   ![Reguła filtru określania zakresu ruchu przychodzącego](./media/active-directory-aadconnectsync-change-the-configuration/scopingfilter.png)  
...
5. Na **przekształcenia** strony, zmień **dla przepływu** do **wyrażenie**. Aby uzyskać **atrybut docelowy**, wybierz opcję **givenName**. I **źródła**, wprowadź **PCase([givenName])**.
   ![Przekształcenia reguły dla ruchu przychodzącego](./media/active-directory-aadconnectsync-change-the-configuration/transformations.png)  
...
**(Wersja zapoznawcza) i pełną synchronizację na pojedynczy obiekt**  
   ...
   ![Zatwierdź (wersja zapoznawcza)](./media/active-directory-aadconnectsync-change-the-configuration/commitpreview.png)  
...
W tle od scenariusza, w tej sekcji, zobacz [kontrolowania procesu przepływu atrybutu](active-directory-aadconnectsync-understanding-declarative-provisioning.md#control-the-attribute-flow-process).

Istnieją dwa sposoby nie przepływu atrybutu. Pierwsza to za pomocą Kreatora instalacji, aby [usunąć wybrane atrybuty](active-directory-aadconnect-get-started-custom.md#azure-ad-app-and-attribute-filtering). Ta opcja działa, jeśli nigdy nie zostały zsynchronizowane atrybutu przed. Jeśli zostało rozpoczęte zsynchronizować ten atrybut i okaże się usunięcie go z tą funkcją, zatrzymuje aparat synchronizacji, zarządzanie atrybut i istniejącej wartości są pozostawiane w usłudze Azure AD.
...
![Zły rozszerzeń atrybuty](./media/active-directory-aadconnectsync-change-the-configuration/badextensionattribute.png)
...
2. Tworzenie przepływów atrybutów z **wyrażenie** dla **dla przepływu** i **AuthoritativeNull** dla **źródła**. Literał **AuthoritativeNull** wskazuje, że wartość powinna być pusta w magazynie metaverse programu nawet wtedy, gdy reguła synchronizacji niższego pierwszeństwa podejmie próbę wypełnienia wartość.
  ![Przekształcania dla atrybutów rozszerzenia](./media/active-directory-aadconnectsync-change-the-configuration/syncruletransformations.png)
...
4. Sprawdź, czy zamierzone zmiany do wyeksportowania, wyszukując przestrzeni łącznika.
  ![Usuń etapowe](./media/active-directory-aadconnectsync-change-the-configuration/deletetobeexported.png)
...
![PowerShell przed wprowadzeniem zmiany](./media/active-directory-aadconnectsync-change-the-configuration/powershell1.png)  
...
![Program PowerShell po zmianie](./media/active-directory-aadconnectsync-change-the-configuration/powershell2.png)  
...
![Dodaj atrybut źródłowy do sieci lokalnej schemat łącznika AD](./media/active-directory-aadconnectsync-change-the-configuration/usertype1.png)
...
![Dodaj atrybut źródłowy ze schematem łącznik usługi Azure AD](./media/active-directory-aadconnectsync-change-the-configuration/usertype2.png)
