---
title: 'Samouczek: Integracja usługi Azure Active Directory z zasobów banku | Dokumentacja firmy Microsoft'
...
1. Na **zasobów banku domena i adresy URL** sekcji, wykonaj następujące czynności:
    ...
    a. W **adres URL logowania** pole tekstowe, wpisz adres URL przy użyciu następującego wzorca: `https://<companyname>.assetbank-server.com`

    b. W **identyfikator** pole tekstowe, wpisz adres URL przy użyciu następującego wzorca: `https://<companyname>.assetbank-server.com/shibboleth`

    > [!NOTE] 
    > Te wartości są prawdziwe. Zaktualizuj te wartości przy użyciu rzeczywistego adresu URL logowania jednokrotnego i identyfikator. Skontaktuj się z pomocą [zespołem pomocy technicznej klienta banku zasobów](mailto:support@assetbank.co.uk) do uzyskania tych wartości. 
...
Brak elementu akcji dla Ciebie w tej sekcji. Nowy użytkownik jest tworzony podczas próby dostępu do zasobów banku, jeśli go jeszcze nie istnieje. 
