...
3. Na **wiersza działa domena i adresy URL** sekcji, wykonaj następujące czynności:
...
6. Na **konfiguracja działa linii** , kliknij przycisk **skonfigurować działa wiersza** otworzyć **Konfigurowanie logowania jednokrotnego** okna. Kopiuj **adres URL wylogowania, identyfikator jednostki języka SAML i SAML pojedynczego logowania jednokrotnego usługi adresu URL** z **krótki przewodnik po sekcji.**

    ![Konfiguracja działania linii](./media/worksmobile-tutorial/tutorial_lineworks_configure.png) 

7. Aby skonfigurować logowanie jednokrotne na **działa wiersza** stronie, musisz wysłać pobrany **certyfikatów, plik, adres URL wylogowania, identyfikator jednostki języka SAML i SAML pojedynczego logowania jednokrotnego adres URL usługi** do [obsługuje działania wiersza zespół](mailto:dl_ssoinfo@worksmobile.com). Ustawiają to ustawienie, aby były prawidłowo po obu stronach połączenia logowania jednokrotnego SAML.
