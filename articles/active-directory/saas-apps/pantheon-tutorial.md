...
3. **[Tworzenie użytkownika testowego Pantheon](#creating-a-pantheon-test-user)**  — aby odpowiednikiem Britta Simon w Pantheon połączonego z usługi Azure AD reprezentacja użytkownika.
...
3. Na **Pantheon domena i adresy URL** sekcji, wykonaj następujące czynności:
...
4. Aplikacja pantheon oczekuje dla asercji SAML w określonym formacie, który należy ustawić wartość atrybutu UserIdentifier przy użyciu adresu e-mail użytkownika. Domyślnie usługa Azure AD używa właściwości UserPrincipalName UserIdentifier atrybutu. Ale pomyślne integracji musisz dostosować tę wartość, aby dopasować przy użyciu adresu e-mail użytkownika. Integracja działa tylko po wykonaniu tej poprawne mapowania.
...
7. Na **konfiguracji Pantheon** , kliknij przycisk **skonfigurować Pantheon** otworzyć **Konfigurowanie logowania jednokrotnego** okna. Kopiuj **SAML pojedynczego logowania jednokrotnego usługi adresu URL** z **krótki przewodnik po sekcji.**

    ![Konfigurowanie logowania jednokrotnego](./media/pantheon-tutorial/tutorial_pantheon_configure.png) 

8. Aby skonfigurować logowanie jednokrotne na **Pantheon** stronie, musisz wysłać pobrany **certyfikatu** i **SAML pojedynczego logowania jednokrotnego usługi adresu URL** do [Pantheon zespołu pomocy technicznej](https://pantheon.io/docs/getting-support/).
