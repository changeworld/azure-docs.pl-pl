...
description: Dowiedz się, jak skonfigurować usługi Azure Active Directory, aby automatycznie zapewnianie i usuwanie kont użytkowników z produktu Workday.
...
* **Wynajem nowi pracownicy** — po dodaniu nowych pracowników do produktu Workday, konto użytkownika zostanie automatycznie utworzone w usłudze Active Directory, usługi Azure Active Directory i opcjonalnie usługi Office 365 i [innych obsługiwanych przez usługę Azure ADaplikacjiSaaS](../active-directory-saas-app-provisioning.md), z zapisu adres e-mail z produktu Workday.
...
* Istnieją użytkownicy, którzy muszą być przygotowana do usługi Active Directory i Azure Active Directory (na przykład "hybrydowe" użytkowników)?
* Istnieją użytkownicy, którzy muszą być przygotowana do usługi Azure Active Directory, ale nie Active Directory (na przykład "tylko w chmurze" użytkowników)?
...
![Galerii aplikacji usługi AAD](./media/workday-inbound-tutorial/WD_Gallery.PNG)
...
1. Zaloguj się do konta administratora dzierżawy produktu Workday. W **Workbench produktu Workday**, wprowadź użytkownika w polu wyszukiwania, a następnie kliknij polecenie **Tworzenie użytkownika System integracji**.
    ...
    ![Utwórz użytkownika](./media/workday-inbound-tutorial/IC750979.png "tworzenia użytkownika")
 ...
 * Pozostaw **minut limitu czasu sesji** z jego wartość domyślna 0, która uniemożliwi sesji użytkownika przedwcześnie limit czasu.
    ...
    ![Utwórz użytkownika System integracji](./media/workday-inbound-tutorial/IC750980.png "utworzyć integracji systemu użytkownika")
...
2. Dodaj nowego użytkownika system integracji do nowej grupy zabezpieczeń. 

    ![Grupa zabezpieczeń systemu](./media/workday-inbound-tutorial/IC750985.png "grupa zabezpieczeń systemu")  
...
1. Kliknij przycisk **Edytuj uprawnienia**, a następnie na **Edytuj uprawnienia** strony okna dialogowego Dodaj nową grupę zabezpieczeń do listy grup zabezpieczeń z **uzyskać** i **Put**  uprawnienia integracji.
    ...
    ![Edytuj uprawnienia](./media/workday-inbound-tutorial/IC750989.png "uprawnienia do edycji")  

1. Powtórz kroki 1 – 4 powyżej dla każdej z pozostałych zasady zabezpieczeń:
...
2. Rozpoczęcia zadania aktywować oczekujących zmian zasad zabezpieczeń, wprowadzając komentarz dla inspekcji, a następnie kliknij przycisk **OK**. 
    ...
    ![Aktywuj oczekujących zabezpieczeń](./media/workday-inbound-tutorial/IC750993.png "aktywować oczekujących zabezpieczeń")  
1. Ukończenie zadania na następnym ekranie, zaznaczając pole wyboru **Potwierdź**, a następnie kliknij przycisk **OK**.

    ![Aktywuj oczekujących zabezpieczeń](./media/workday-inbound-tutorial/IC750994.png "aktywować oczekujących zabezpieczeń")  
...
6. Aby zapisać mapowania, kliknij przycisk **zapisać** w górnej części sekcji mapowanie atrybutu.
...
13. W programie polecenia pasek z produktu Workday Studio wybierz **Plik > Otwórz plik...**  , a następnie otwórz plik XML został zapisany. Zostanie otwarte go w edytorze XML Studio produktu Workday.

    ![WORKDAY Studio](./media/workday-inbound-tutorial/WDstudio3.PNG)

14. W drzewie pliku nawigowania **/ENV: Envelope > env: treści > wd:Get_Workers_Response > wd:Response_Data > wd: procesu roboczego** celu znalezienia danych użytkownika. 

15. W obszarze **wd: proces roboczy**, Znajdź atrybut, który chcesz dodać i zaznacz je.

16. Skopiuj wyrażenie XPath dla wybranych atrybut poza **ścieżka dokumentu** pola.

1. Usuń **/env:Envelope / ENV / wd:Get_Workers_Response / wd:Response_Data /** prefiksu wyrażenia skopiowane.

18. Jeśli ostatniego elementu w wyrażeniu skopiowanych jest węzłem (przykład: "/ wd: Birth_Date"), następnie dołącz **/text()** na końcu wyrażenia. Nie jest to konieczne, jeśli ostatni element atrybutu (przykład: "/@wd: typu").
...
11. Wybierz **zapisać** powyżej, a następnie **tak** do okna dialogowego. Jeśli jest nadal otwarty, należy zamknąć ten ekran mapowanie atrybutu.
...
* [Informacje o sposobie konfigurowania rejestracji jednokrotnej między produktu Workday i Azure Active Directory](workday-tutorial.md)
