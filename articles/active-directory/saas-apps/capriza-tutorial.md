...
3. **[Tworzenie użytkownika testowego platformy Capriza](#creating-a-capriza-platform-test-user)**  — aby odpowiednikiem Britta Simon w platformę Capriza, która jest połączona z usługi Azure AD reprezentacja użytkownika.
...
3. Na **Capriza platformy domena i adresy URL** sekcji, wykonaj następujące czynności:

    ![Konfigurowanie logowania jednokrotnego](./media/capriza-tutorial/tutorial_caprizaplatform_url.png)

    W **adres URL logowania** pole tekstowe, wpisz adres URL przy użyciu następującego wzorca: `https://<companyname>.capriza.com/<tenantid>`
...
6. Na **konfigurację platformy Capriza** , kliknij przycisk **Konfigurowanie platformy Capriza** otworzyć **Konfigurowanie logowania jednokrotnego** okna. Kopiuj **adres URL wylogowania, identyfikator jednostki języka SAML i SAML pojedynczego logowania jednokrotnego usługi adresu URL** z **krótki przewodnik po sekcji.**

    ![Konfigurowanie logowania jednokrotnego](./media/capriza-tutorial/tutorial_caprizaplatform_configure.png) 

7. Aby skonfigurować logowanie jednokrotne na **platformy Capriza** stronie, musisz wysłać pobrany **certyfikatu**, **adres URL wylogowania**, **identyfikator jednostki SAML**i **SAML pojedynczego logowania jednokrotnego usługi adresu URL** do [zespołem pomocy technicznej platformy Capriza](mailTo:support@capriza.com). Ustawiają to ustawienie, aby były prawidłowo po obu stronach połączenia logowania jednokrotnego SAML.
...
Brak elementu akcji dla Ciebie w tej sekcji. Nowy użytkownik zostanie utworzony podczas próby dostępu Capriza, jeśli go jeszcze nie istnieje.
...
2. Na liście aplikacji wybierz **platformy Capriza**.
...
Po kliknięciu kafelka Capriza platformy w panelu dostępu, użytkownik powinien uzyskać automatycznie zalogowanych do aplikacji Capriza. Aby uzyskać więcej informacji na temat panelu dostępu, zobacz [wprowadzenie do panelu dostępu](../user-help/active-directory-saas-access-panel-introduction.md). 
