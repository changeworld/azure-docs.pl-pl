...
description: Ten dokument stanowi wprowadzenie nowego modułu programu ADConnectivity PowerShell i jak może służyć do rozwiązywania.
...
---
# <a name="troubleshoot-azure-ad-connectivity-with-the-adconnectivitytool-powershell-module"></a>Rozwiązywanie problemów z łącznością z usługi Azure AD przy użyciu modułu ADConnectivityTool PowerShell

Narzędzie ADConnectivity jest moduł programu PowerShell, używanym w jednym z następujących czynności:

- Gdy problem z łącznością sieciową uniemożliwiają pomyślne sprawdzanie poprawności podczas instalacji usługi Active Directory poświadczeń użytkownika podane w kreatorze.
