...
Usługa Azure Active Directory dostępu warunkowego można ograniczyć dostęp do aplikacji w chmurze do aplikacji klienckich, które obsługują zasady ochrony aplikacji usługi Intune. Na przykład można ograniczyć dostęp do usługi Exchange Online do aplikacji Outlook.
...
1. **Użytkownicy i grupy**: Wszystkie zasady dostępu warunkowego musi mieć co najmniej jednego użytkownika lub wybranej grupy.
1. **Aplikacje w chmurze:** Jak aplikacje w chmurze, musisz wybrać **Office 365 Exchange Online**.

   ![Dostęp warunkowy](./media/app-based-conditional-access/07.png)

1. **Warunki:** Jako **warunki**, należy skonfigurować **platform urządzeń** i **aplikacje klienckie**:
   1. Jako **platform urządzeń**, wybierz opcję **Android** i **iOS**.

      ![Dostęp warunkowy](./media/app-based-conditional-access/03.png)

   1. Jako **aplikacje klienckie (wersja zapoznawcza)** , wybierz opcję **aplikacje mobilne i aplikacje komputerowe** i **Nowocześni Klienci uwierzytelniania**.

      ![Dostęp warunkowy](./media/app-based-conditional-access/91.png)

1. Jako **kontrole dostępu**, musisz mieć **Wymagaj zatwierdzonej aplikacji klienckiej (wersja zapoznawcza)** wybrane.
...
1. **Warunki:** Jako **warunki**, należy skonfigurować **aplikacje klienckie (wersja zapoznawcza)** . 
   1. Jako **aplikacje klienckie (wersja zapoznawcza)** , wybierz opcję **aplikacje mobilne i klienci stacjonarni** i **klienci programu Exchange ActiveSync**.
...
- Próbuje zalogować się przy użyciu tych samych poświadczeń, poświadczenia aplikacji Outlook
...
1. **Nazwa** zasad dostępu warunkowego.
1. **Użytkownicy i grupy**: Wszystkie zasady dostępu warunkowego musi mieć co najmniej jednego użytkownika lub wybranej grupy.
1. **Aplikacje w chmurze:** Jak aplikacje w chmurze, musisz wybrać **Office 365 Exchange Online** i **Office 365 SharePoint Online**. 
...
1. **Warunki:** Jako **warunki**, należy skonfigurować **platform urządzeń** i **aplikacje klienckie**:
   ...
   1. Jako **aplikacje klienckie (wersja zapoznawcza)** , wybierz opcję **aplikacje mobilne i klienci stacjonarni** i **Nowocześni Klienci uwierzytelniania**.
...
1. **Nazwa** zasad dostępu warunkowego.
1. **Użytkownicy i grupy**: Wszystkie zasady dostępu warunkowego musi mieć co najmniej jednego użytkownika lub wybranej grupy.
1. **Aplikacje w chmurze:** Jak aplikacje w chmurze, musisz wybrać **Office 365 Exchange Online**. Online 

   ![Dostęp warunkowy](./media/app-based-conditional-access/07.png)

1. **Warunki:** Jako **warunki**, należy skonfigurować **aplikacje klienckie**:
   1. Jako **aplikacje klienckie (wersja zapoznawcza)** , wybierz opcję **aplikacje mobilne i klienci stacjonarni** i **klienci programu Exchange ActiveSync**.
...
- Niektórzy użytkownicy są już zarejestrowane (z lub bez urządzenia firmowe)
...
1. Jako **kontrole dostępu**, musisz mieć następujące wybrane:
