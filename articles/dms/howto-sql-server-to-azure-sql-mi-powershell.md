---
title: Użyj modułu usługi Azure Database Migration Service w systemie Microsoft Azure PowerShell do migracji programu SQL Server w środowisku lokalnym do bazy danych SQL Azure MI | Dokumentacja firmy Microsoft
...
Poniższy przykład tworzy `AzureRmDataMigrationDatabaseInfo` projektu dla **AdventureWorks** bazy danych i dodaje go do listy można podać jako parametr do utworzenia projektu.
...
Teraz należy utworzyć obiekt FileShare reprezentujący lokalny udział sieciowy SMB, że usługi Azure Database Migration Service może potrwać źródła kopii zapasowych bazy danych za pomocą polecenia cmdlet New-AzureRmDmsFileShare.
...
Następnym krokiem jest do wybrania źródłowej i docelowej bazy danych za pomocą polecenia cmdlet New-AzureRmDmsSelectedDB, jak pokazano w poniższym przykładzie:
...
- *TargetConnection*. Obiekt AzureRmDmsConnInfo reprezentujący docelowym wystąpienia zarządzanego Azure SQL Database połączenie.
