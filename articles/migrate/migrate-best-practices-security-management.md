...
Jako użytkownik planowania i projektowania dotyczące migracji, oprócz myśleć o migracji, należy zaplanować dla modelu zabezpieczeń i zarządzania na platformie Azure po migracji. W tym artykule opisano planowania oraz najlepsze rozwiązania dotyczące zabezpieczania wdrożenie systemu Azure, po przeprowadzeniu migracji i bieżących zadań, wdrożenie uruchomione na dowolnym poziomie optymalne. 

> [!IMPORTANT]
> Najlepsze rozwiązania i opinie opisanych w tym artykule opierają się na platformie Azure i usługi dostępne w czasie pisania funkcji. Funkcje i możliwości zmiany w czasie.

## <a name="secure-migrated-workloads"></a>Zabezpieczanie zmigrowanych obciążeń

Po zakończeniu migracji najważniejszych zadań jest zabezpieczanie zmigrowanych obciążeń przed zagrożeniami wewnętrznymi i zewnętrznymi. Najlepsze rozwiązania obejmują:

- Praca z Centrum zabezpieczeń Azure: Dowiedz się, jak pracować z monitorowaniem, ocenami oraz zalecenia podane przez usługę Azure Security Center
- **Szyfrowanie danych**: pobieranie najlepszych rozwiązań do szyfrowania danych na platformie Azure.
- **Konfigurowanie ochrony przed złośliwym oprogramowaniem**: ochrona maszyn wirtualnych przed złośliwym oprogramowaniem i złośliwych ataków.
- **Zabezpieczanie aplikacji sieci web**: Zachowaj informacje poufne zabezpieczone w migracji aplikacji sieci web.
- **Sprawdź subskrypcje**: Sprawdź, kto ma dostęp do subskrypcji platformy Azure i zasobów po zakończeniu migracji.
- **Praca z dziennikami**: Przejrzyj Azure inspekcji i zabezpieczeń dzienników w regularnych odstępach czasu.
- **Zaawansowane funkcje zabezpieczeń**: zrozumieć i ocenić innych funkcjach zabezpieczeń, które są oferowane na platformie Azure.
...
- **Dostęp tylko w czas (JIT)**: zmniejszyć swoje narażony na ataki sieciowe przy użyciu dokładnie na czas i kontrolowany dostęp do portów zarządzania na maszynach wirtualnych platformy Azure.
...
- **File Integrity Monitoring**: integralność plików uruchomione na maszynach wirtualnych.
...
Dowiedz się więcej o najlepszym rozwiązaniem jest: • Zarządzanie maszynie wirtualnej dostęp przy użyciu dokładnie na czas • funkcje adaptacyjnego sterowania aplikacjami kontrolki w usłudze Azure Security Center • File Integrity Monitoring w usłudze Azure Security Center
...
Szyfrowanie użytkownika ważną częścią rozwiązania w zakresie zabezpieczeń platformy Azure. Zapewnienie, że szyfrowanie jest włączone na wszystkich poziomach zapobiega nieupoważnione uzyskanie dostępu do poufnych danych, w tym przesyłanych i magazynowanych danych. 

### <a name="encryption-for-iaas"></a>Szyfrowanie dla IaaS

- **Maszyny wirtualne**: dla maszyn wirtualnych usługi Azure Disk Encryption umożliwia szyfrowanie dysków Windows i maszyn wirtualnych IaaS z systemem Linux.
...
- **Magazyn**: ochrona dane w stanie spoczynku w magazynie platformy Azure.
...
- **Zawsze szyfrowane**: Kreator zawsze szyfrowane w SQL Server Management Studio do ochrony danych magazynowanych.
...
- **Usługa Azure Key Vault**: już dziś deweloperom aplikacji sieci web są tworzone kroki, aby upewnić się, że poufne informacje, nie jest przeciek tych plików. Jedną z metod ochronę informacji jest prowadzenie plików i umieścić go w usłudze Azure Key Vault.
...
- **Środowisko App Service Environment**: Jeśli migrujesz dodatkową ochronę, można rozważyć dodanie środowiska App Service Environment i zapory aplikacji sieci Web, które chcesz chronić zasoby aplikacji.
...
- **Zapora aplikacji sieci Web**: funkcja usługi Application Gateway, która zapewnia scentralizowaną ochronę aplikacji sieci web.
...
- **Zarządzanie zasobami**: najlepsze rozwiązania dotyczące grupy zasobów platformy Azure i zasobów, w tym inteligentnego nazewnictwa, zapobieganie przypadkowemu usunięciu, zarządzanie uprawnień dotyczących zasobów i tagowania zasobów skuteczne.
- **Użyj plany**: uzyskać szybki przegląd dotyczące używania schematy dla przedsiębiorstw i zarządzania środowiska wdrażania.
- **Przegląd architektury**: Przejrzyj przykładowe architektur systemu Azure, aby dowiedzieć się więcej z podczas tworzenia wdrożenia po migracji.
- **Konfigurowanie grup zarządzania**: Jeśli masz wiele subskrypcji, możesz zebrać je do grupy zarządzania i zastosować ustawienia zarządzania do tych grup.
- **Konfigurowanie zasad dostępu**: stosowanie zasad zgodności do Twoich zasobów platformy Azure.
- **Wdrożenie strategii bcdr można wykorzystać**: Podsumowując firm ciągłość działalności biznesowej i odzyskiwanie po awarii (BCDR) strategii odzyskiwania na zapewnieniu bezpieczeństwa danych, odporne na błędy, środowiska i zasoby w górę i działa w przypadku wystąpienia awarii.
- **Zarządzanie maszynami wirtualnymi**: grupowanie maszyn wirtualnych do grupy dostępności w celu zapewnienia odporności i wysokiej dostępności. Korzystają z dysków zarządzanych w celu ułatwienia zarządzania dyskami i magazynu maszyny Wirtualnej.
- **Monitorowanie użycia zasobów**: Włączanie rejestrowania diagnostycznego dla zasobów platformy Azure, tworzyć alerty i elementy playbook do aktywnego rozwiązywania problemów i pulpitu nawigacyjnego platformy Azure na użytek spójny widok kondycji wdrożenia i stanu.
- **Zarządzanie pomocy technicznej i aktualizacjami**: zrozumieć Twój plan pomocy technicznej platformy Azure i sposobu jej wdrożenia, zawiera najlepsze rozwiązania dla aktualizowanie maszyn wirtualnych oraz zarządzanie zmianami put procesów w miejscu.
...
- [Odczyt](http://www.azurefieldnotes.com/2016/07/18/azure-resource-tagging-best-practices/) znakowanie najlepsze rozwiązania platformy Azure.
...
## <a name="best-practice---review-azure-reference-architectures"></a>Najlepszym rozwiązaniem jest - architektury referencyjne platformy Azure z przeglądu
...
## <a name="best-practice-manage-resources-with-management-groups"></a>Najlepsze rozwiązanie: Zarządzanie zasobami za pomocą grupy zarządzania
...
## <a name="best-practice-back-up-your-deployment"></a>Najlepsze rozwiązanie: tworzenie kopii zapasowej wdrożenia

W większości przypadków to obciążenie w środowisku lokalnym jest wycofywany po migracji, a musi być rozszerzony lub zastąpić strategii lokalne kopie zapasowe danych. Jeśli migrujesz całe centrum danych na platformie Azure, musisz zaprojektować i zaimplementować rozwiązanie pełnej kopii zapasowej przy użyciu technologii platformy Azure lub innych firm, zintegrowane rozwiązania. Wybrane rozwiązanie kopii zapasowych może się różnić, w zależności od wdrożenie systemu Azure. 


### <a name="back-up-an-iaas-deployment"></a>Tworzenie kopii zapasowej wdrożenia usługi IaaS


W przypadku obciążeń uruchomionych na maszynach wirtualnych IaaS platformy Azure, możesz rozważyć użycie następujących rozwiązania tworzenia kopii zapasowych:

- **Usługa Azure Backup**: zapewnia kopii zapasowych spójnych na poziomie aplikacji dla Windows Azure i maszyn wirtualnych systemu Linux.
- **Migawek magazynu**: migawek magazynu obiektów blob.
...
- **Bezpośrednie tworzenie kopii zapasowych w ustawieniach maszyny Wirtualnej**:
    - Usługa Azure Backup jest integrowany opcji maszyny Wirtualnej w witrynie Azure portal.
    - Tworzenie kopii zapasowej maszyny Wirtualnej raz dziennie i przywracanie dysku maszyny Wirtualnej. 
    - Usługa Azure Backup tworzy migawki danych uwzględniających aplikacji (w tle VSS).
    - Agent nie jest zainstalowany na maszynie Wirtualnej.
- **Bezpośrednie tworzenie kopii zapasowych w magazynie usługi Recovery Services**:
    - Wdrażanie magazyn usługi Azure Backup Recovery Services, a następnie wybierz maszyny wirtualne do tworzenia kopii zapasowych.
    - Ta metoda zapewnia pojedyncza lokalizacja umożliwiająca śledzenie i zarządzanie nimi kopii zapasowych i dodatkowych kopii zapasowych oraz opcje przywracania. 
    - Usługa Azure Backup instaluje agenta usługi Microsoft Azure Recovery Services (MARS) na maszynie Wirtualnej.
    - Kopia zapasowa jest maksymalnie trzy razy dziennie.
    - Kopia zapasowa znajduje się na poziomie pliku lub folderu, a nie jest świadomy aplikacji. Nie jest obsługiwanym w systemie Linux.
- **Usługa Azure Backup Server: Ochrona maszyn wirtualnych do usługi Azure Backup Server**:
   - Usługa Azure Backup Server znajduje się bezpłatnie przy użyciu usługi Azure Backup.
   - Maszyna wirtualna jest wykonywana kopia zapasowa do lokalnego magazynu kopii zapasowych Azure.
   - Możesz następnie utworzyć kopię zapasową serwera usługi Azure Backup na platformie Azure w magazynie usługi Recovery Services.
   - Kopia zapasowa jest aplikacji obsługujących, z dokładnością pełną za pośrednictwem często kopii zapasowej i przechowywania
   - Można utworzyć kopię na poziomie aplikacji. Na przykład przez tworzenie kopii zapasowej programu SQL Server lub SharePoint.

Usługa Azure Backup automatycznie przydziela i zarządza magazynem przy użyciu modelu zgodnie z rzeczywistym użyciem dla magazynu, które zostaną zużyte.
- Magazyn można przypisać jako LRS lub GRS.
- Usługa Azure Backup szyfruje dane, śledząc przy użyciu algorytmu AES 256, a następnie wysyła je za pośrednictwem protokołu HTTPS na platformie Azure. Kopia zapasowa danych przechowywanych na platformie Azure jest szyfrowana przy użyciu [szyfrowanie usługi Storage (SSE)](https://docs.microsoft.com/azure/storage/common/storage-service-encryption?toc=%2fazure%2fstorage%2fqueues%2ftoc.json)i dla transmisji i przechowywania.
...
- [Przegląd](https://docs.microsoft.com/azure/storage/common/storage-disaster-recovery-guidance?toc=%2fazure%2fstorage%2fblobs%2ftoc.json) co należy zrobić po wystąpieniu awarii usługi Azure Storage?
...
## <a name="best-practice-set-up-a-disaster-recovery-strategy"></a>Najlepsze rozwiązanie: Ustawianie strategii odzyskiwania po awarii
...
- **Aktywna replikacja geograficzna**: Wdrażanie aktywnej replikacji geograficznej dla odzyskiwania po awarii szybkie, jeśli wystąpi awaria centrum danych lub nie można nawiązać połączenia z bazą danych w podstawowej.
...
- [Uzyskaj wskazówki](https://docs.microsoft.com/azure/sql-database/sql-database-geo-replication-overview#best-practices-of-using-failover-groups-for-business-continuity) dla grupy trybu failover.
...
## <a name="best-practice---manage-updates"></a>Najlepsze praktyki — zarządzanie aktualizacjami
