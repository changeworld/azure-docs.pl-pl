---
title: Azure VMware Solutions (Automatyczna synchronizacja) — Optymalizuj chmurę prywatną do automatycznej synchronizacji z certyfikatem certyfikatu Oracle
...
---
# <a name="optimize-your-avs-private-cloud-for-installing-oracle-rac"></a>Zoptymalizuj chmurę prywatną do automatycznej synchronizacji, aby zainstalować certyfikat Oracle RAC

W środowisku chmury prywatnej do automatycznej synchronizacji można wdrożyć klastry usługi Oracle w rzeczywistości. W tym przewodniku opisano, jak wdrożyć nowy klaster i zoptymalizować maszynę wirtualną dla rozwiązania Oracle RAC. Po wykonaniu kroków opisanych w tym temacie można zainstalować i skonfigurować certyfikat Oracle RAC.
...
W poniższych sekcjach opisano sposób konfigurowania środowiska chmury prywatnej automatycznej synchronizacji dla programu Oracle RAC.

1. Najlepsze rozwiązania dotyczące konfiguracji dysków
2. Wdróż redyskową chmurę prywatną vSphere klaster
...
## <a name="deploy-avs-private-cloud-vsphere-cluster"></a>Wdróż redyskową chmurę prywatną vSphere klaster

Aby wdrożyć klaster vSphere w chmurze prywatnej automatycznej synchronizacji, wykonaj następujące czynności:

1. W portalu automatycznej synchronizacji [Utwórz chmurę prywatną do automatycznej synchronizacji](create-private-cloud.md). Funkcja automatycznej synchronizacji tworzy domyślnego użytkownika programu vCenter o nazwie "cloudowner" w nowo utworzonej chmurze prywatnej. Aby uzyskać szczegółowe informacje na temat domyślnej automatycznej synchronizacji użytkownika i modelu uprawnień chmury prywatnej, zobacz [Omówienie modelu uprawnień do automatycznej synchronizacji w chmurze](learn-private-cloud-permissions.md). Ten krok powoduje utworzenie podstawowego klastra zarządzania dla chmury prywatnej automatycznej synchronizacji.

2. W portalu automatycznej synchronizacji [Rozwiń chmurę prywatną automatycznej synchronizacji](expand-private-cloud.md) z nowym klastrem. Ten klaster będzie używany do wdrażania certyfikatu Oracle RAC. Wybierz liczbę węzłów na podstawie pożądanej odporności na uszkodzenia (co najmniej trzy węzły).

## <a name="set-up-networking-for-oracle-rac"></a>Konfigurowanie sieci dla programu Oracle RAC

1. W chmurze prywatnej automatycznej synchronizacji [Utwórz dwie sieci VLAN](create-vlan-subnet.md), jedną dla sieci publicznej Oracle i jedną dla sieci prywatnej Oracle, a następnie przypisz odpowiednie maski CIDR podsieci.
2. Po utworzeniu sieci VLAN Utwórz [rozproszone grupy portów w chmurze prywatnej chmury automatycznej](create-vlan-subnet.md#use-vlan-information-to-set-up-a-distributed-port-group-in-vsphere).
3. Skonfiguruj [maszynę wirtualną z serwerem DHCP i DNS](dns-dhcp-setup.md) w klastrze zarządzania dla środowiska Oracle.
4. [Skonfiguruj przekazywanie DNS na serwerze DNS](on-premises-dns-setup.md#create-a-conditional-forwarder) zainstalowanym w chmurze prywatnej automatycznej synchronizacji.
...
6. W sekcji **sieci vsan** > **Availability (dostępność** ) **nie** zaznaczaj opcji odporności na awarię lokacji. Aby nie można było tolerować niepowodzeń, wybierz opcję **dublowania RAID** dla żądanych FTT.
    ![](media/oracle-rac-storage-wizard-vsan.png)ustawień sieci vSAN.
...
1. W portalu automatycznej synchronizacji można [eskalować uprawnienia](escalate-private-cloud-privileges.md) użytkownika cloudowner.
