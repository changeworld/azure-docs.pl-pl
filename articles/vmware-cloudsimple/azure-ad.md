---
title: Azure VMware Solutions (Automatyczna synchronizacja) — używanie usługi Azure AD jako źródła tożsamości w chmurze prywatnej synchronizacji
description: Zawiera opis sposobu dodawania usługi Azure AD jako dostawcy tożsamości w chmurze prywatnej automatycznej synchronizacji w celu uwierzytelniania użytkowników uzyskujących dostęp do automatycznej synchronizacji na platformie Azure
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/15/2019
ms.topic: article
...
ms.locfileid: "77015927"
---
# <a name="use-azure-ad-as-an-identity-provider-for-vcenter-on-avs-private-cloud"></a>Korzystanie z usługi Azure AD jako dostawcy tożsamości dla programu vCenter w ramach automatycznej synchronizacji chmury prywatnej

Możesz skonfigurować program vCenter w chmurze prywatnej chmury do uwierzytelniania za pomocą usługi Azure Active Directory (Azure AD) dla administratorów oprogramowania VMware, aby uzyskać dostęp do programu vCenter. Po skonfigurowaniu źródła tożsamości logowania jednokrotnego użytkownik **cloudowner** może dodawać użytkowników ze źródła tożsamości do programu vCenter.  
...
2. [Jak skonfigurować źródło tożsamości w chmurze prywatnej chmury programu vCenter](#set-up-an-identity-source-on-your-avs-private-cloud-vcenter)
...
6.  Skonfiguruj bezpieczny protokół LDAP na Azure Active Directory Domain Services zgodnie z opisem w temacie [Konfigurowanie bezpiecznego protokołu LDAP (LDAPS) dla domeny zarządzanej Azure AD Domain Services](../active-directory-domain-services/tutorial-configure-ldaps.md).
    1. Przekaż certyfikat do użycia przez bezpieczny protokół LDAP zgodnie z opisem w temacie [dotyczącym platformy Azure uzyskaj certyfikat dla bezpiecznego protokołu LDAP](../active-directory-domain-services/tutorial-configure-ldaps.md#create-a-certificate-for-secure-ldap). Automatyczna synchronizacja zaleca użycie podpisanego certyfikatu wystawionego przez urząd certyfikacji, aby upewnić się, że program vCenter może ufać certyfikatowi.
...
## <a name="set-up-an-identity-source-on-your-avs-private-cloud-vcenter"></a>Konfigurowanie źródła tożsamości w chmurze prywatnej programu vCenter do automatycznej synchronizacji

1. [Eskalacja przywilejów](escalate-private-cloud-privileges.md) dla programu vCenter w chmurze prywatnej chmury.
...
3. Zaloguj się do swojej automatycznej wersji programu vCenter w chmurze prywatnej, gdy uprawnienia zostaną przełączone.
