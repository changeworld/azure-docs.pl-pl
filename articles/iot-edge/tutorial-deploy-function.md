...
4. W palecie poleceń wprowadź i uruchom polecenie **Azure IoT Edge: nowe rozwiązanie usługi IoT Edge**. Postępuj zgodnie z monitami wyświetlanymi na palecie poleceń, aby utworzyć rozwiązanie.
   ...
   5. Określ rejestr kontenerów platformy Azure utworzony w poprzedniej sekcji jako repozytorium obrazów dla pierwszego modułu. Zastąp ciąg **localhost:5000** skopiowaną wartością serwera logowania. Upewnij się, że nazwa modułu (na przykład /csharpfunction) stanowiąca część ciągu pozostanie niezmieniona. Ostateczny ciąg będzie wyglądał następująco: \<nazwa rejestru\>.azurecr.io/csharpfunction.
...
5. Zastąp zawartość pliku **CSharpFunction.cs** następującym kodem:
