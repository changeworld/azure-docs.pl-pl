...
Aby zmniejszyć przepustowość wykorzystywaną przez rozwiązanie IoT Edge, centrum IoT Edge optymalizuje, ile rzeczywistych połączeń odbywa się w chmurze. Usługa IoT Edge Hub pobiera logiczne połączenia z klientów, takich jak moduły lub urządzenia podrzędne, i łączy je z jednym połączeniem fizycznym z chmurą. Szczegóły tego procesu są niewidoczne w pozostałej części rozwiązania. Klienci wydaje się, że mają oni własne połączenia do chmury, nawet jeśli te są wszystkie są wysyłane za pośrednictwem tego samego połączenia. 
...
* **imagePullPolicy** — czy Agent IoT Edge próbuje pobrać najnowszy obraz dla modułu automatycznie. Jeśli nie określisz wartości, wartość domyślna to *OnCreate*. Możliwe wartości obejmują: 
   ...
   * `never` — Agent IoT Edge nigdy nie podejmie próby ściągnięcia obrazu modułu z rejestru kontenerów. Oczekuje się, że obraz modułu jest buforowany na urządzeniu, a wszystkie aktualizacje obrazu modułu są wprowadzane ręcznie lub zarządzane przez rozwiązanie innej firmy. 
