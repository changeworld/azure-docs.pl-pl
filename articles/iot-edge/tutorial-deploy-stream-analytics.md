...
1. Na karcie **trasy** należy określić sposób przekazywania komunikatów między modułami i IoT Hub. Komunikaty są konstruowane przy użyciu par nazwa/wartość. Zastąp domyślne `route` i `upstream` nazwy i wartości, używając par pokazanych w poniższej tabeli, następujących par nazwa/wartość, zastępując wystąpienia obiektu _{ModuleName}_ nazwą modułu Azure Stream Analytics.
