---
title: Konfigurowanie trwałości danych usługi Premium Azure Redis Cache
description: Dowiedz się, jak skonfigurować i zarządzać funkcji trwałości danych swoich wystąpień usługi Azure Redis Cache warstwy Premium
...
ms.locfileid: "38700887"
---
# <a name="how-to-configure-data-persistence-for-a-premium-azure-redis-cache"></a>Konfigurowanie trwałości danych usługi Premium Azure Redis Cache
Usługa Azure Redis Cache ma ofert różnych pamięci podręcznych, które zapewniają elastyczność przy wyborze rozmiar pamięci podręcznej i funkcji, takich jak funkcje warstwy Premium, takich jak klastrowanie, trwałość i obsługę sieci wirtualnej. W tym artykule opisano sposób konfigurowania trwałości w wystąpieniu usługi Azure Redis Cache — wersja premium.
...
Usługa Azure Redis Cache zapewnia trwałość pamięci podręcznej Redis przy użyciu następujących modeli:

* **Trwałość RDB** -trwałości po RDB (baza danych usługi Redis) jest skonfigurowany, usługi Azure Redis Cache migawkę pamięci podręcznej Redis w pamięci podręcznej Redis, format binarny na dysku, oparte na można skonfigurować częstotliwość wykonywania kopii zapasowych będzie się powtarzać. Sytuacji katastrofy wyłączającą podstawowej i replice pamięci podręcznej pamięć podręczna jest odtworzone przy użyciu najnowszej migawki. Dowiedz się więcej o [zalety](https://redis.io/topics/persistence#rdb-advantages) i [wady](https://redis.io/topics/persistence#rdb-disadvantages) trwałość RDB.
* **Trwałość AOF** -trwałości po AOF (Dołącz plik tylko) jest skonfigurowany, usługi Azure Redis Cache zapisuje każdej operacji zapisu w dzienniku, który jest zapisywany w co najmniej raz na sekundę do konta usługi Azure Storage. Sytuacji katastrofy wyłączającą podstawowej i replice pamięci podręcznej pamięć podręczna jest odtworzone przy użyciu operacji zapisu przechowywanych. Dowiedz się więcej o [zalety](https://redis.io/topics/persistence#aof-advantages) i [wady](https://redis.io/topics/persistence#aof-disadvantages) AOF trwałość.

Trwałość jest skonfigurowany z **nowej pamięci podręcznej Redis** bloku, podczas tworzenia pamięci podręcznej i na **menu zasobów** dla istniejące premie zapisuje w pamięci podręcznej.
...
Poniższa lista zawiera odpowiedzi na często zadawane pytania dotyczące usługi Azure Redis Cache trwałości.
