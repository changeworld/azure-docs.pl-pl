---
title: Jak rozwiązywać problemy z usługi Azure Redis Cache | Dokumentacja firmy Microsoft
description: Dowiedz się, jak rozwiązać typowe problemy z usługą Azure Redis Cache.
...
* [Rozwiązywanie problemów po stronie klienta](#client-side-troubleshooting) — ta sekcja zawiera wskazówki dotyczące identyfikowania i rozwiązywania problemów spowodowanych aplikacji łączenie z usługą Azure Redis Cache.
* [Rozwiązywanie problemów po stronie serwera](#server-side-troubleshooting) — ta sekcja zawiera wskazówki dotyczące identyfikowania i rozwiązywania problemów spowodowane po stronie serwera usługi Azure Redis Cache.
...
#### <a name="problem"></a>Problem
Dla niektórych oczekiwanych danych, aby były w Moje wystąpienie usługi Azure Redis Cache, ale go nie wydaje się, że dostępne.
...
#### <a name="resolution"></a>Rozwiązanie
* Przejrzyj wszelkie opinie i alerty wymienione w [Redis Cache Advisor](cache-configure.md#redis-cache-advisor).
...
9. Jeśli używasz `RedisSessionStateprovider`, upewnij się, limit czasu ponawiania zostały ustawione prawidłowo. `retrytimeoutInMilliseconds` powinien być większy niż `operationTimeoutinMilliseonds`, w przeciwnym razie wystąpić brak ponownych prób. W poniższym przykładzie `retrytimeoutInMilliseconds` jest równa 3000. Aby uzyskać więcej informacji, zobacz [dostawca stanu sesji ASP.NET dla usługi Azure Redis Cache](cache-aspnet-session-state-provider.md) i [sposobu korzystania z parametrów konfiguracji dostawcy stanu sesji i dostawca wyjściowej pamięci podręcznej](https://github.com/Azure/aspnet-redis-providers/wiki/Configuration).
...
* [Jak monitorować usługę Azure Redis Cache](cache-how-to-monitor.md)
