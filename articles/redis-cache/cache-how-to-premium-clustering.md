---
title: Konfigurowanie klastrowania Redis usługi Premium Azure Redis Cache | Dokumentacja firmy Microsoft
description: Dowiedz się, jak tworzyć i zarządzać nimi klaster w warstwie Premium usługi Azure Redis Cache wystąpienia usługi Redis
...
ms.locfileid: "38590464"
---
# <a name="how-to-configure-redis-clustering-for-a-premium-azure-redis-cache"></a>Konfigurowanie klastrowania Redis usługi Premium Azure Redis Cache
Usługa Azure Redis Cache ma pamięci podręcznej różnych ofert, które zapewniają elastyczność przy wyborze rozmiar pamięci podręcznej i funkcji, takich jak funkcje warstwy Premium, takich jak klastrowanie, trwałość i obsługę sieci wirtualnej. W tym artykule opisano konfigurowanie klastrowania w wystąpieniu usługi Azure Redis Cache — wersja premium.
...
Usługa Azure Redis Cache oferuje klastra pamięci podręcznej Redis jako [zaimplementowane w pamięci podręcznej Redis](http://redis.io/topics/cluster-tutorial). Za pomocą klaster pamięci podręcznej Redis możesz uzyskać następujące korzyści: 
...
Klaster nie zwiększa liczbę połączeń, które są dostępne dla klastra pamięci podręcznej. Aby uzyskać więcej informacji na temat rozmiaru, przepływności i przepustowości w pamięciach podręcznych premium, zobacz [jakie oferty pamięci podręcznej redis Cache i rozmiary użyć?](cache-faq.md#what-redis-cache-offering-and-size-should-i-use)

Na platformie Azure klastra pamięci podręcznej Redis jest oferowana jako model podstawowy i węzeł repliki, gdzie każdy fragment ma parę podstawowy i węzeł repliki za pomocą replikacji, gdy replikacja jest zarządzana przez usługę Azure Redis Cache. 

## <a name="clustering"></a>Klastrowanie
Klastrowanie jest włączona na **nowej pamięci podręcznej Redis** bloku podczas tworzenia pamięci podręcznej. 
...
> [!NOTE]
> Podczas gdy warstwa Premium usługi Azure Redis Cache została wydana w ogólnej dostępności, funkcja rozmiar klastra Redis jest obecnie w wersji zapoznawczej.
> 
> 
...
Poniższa lista zawiera odpowiedzi na często zadawane pytania dotyczące usługi Azure Redis Cache klastrowania.
...
Największy rozmiar pamięci podręcznej — wersja premium jest 53 GB. Można utworzyć do 10 fragmentów, dzięki czemu maksymalny rozmiar 530 GB. Jeśli potrzebujesz większego rozmiaru [Zażądaj więcej](mailto:wapteams@microsoft.com?subject=Redis%20Cache%20quota%20increase). Aby uzyskać więcej informacji, zobacz [cennik usługi Azure Redis Cache](https://azure.microsoft.com/pricing/details/cache/).
