---
title: Usługi Azure Redis Cache — często zadawane pytania | Dokumentacja firmy Microsoft
description: Dowiedz się więcej odpowiedzi na często zadawane pytania dotyczące, wzorce i najlepsze rozwiązania dla usługi Azure Redis Cache
...
* Jeśli chcesz utworzyć żądanie funkcji, można przesłać żądania i pomysłów dotyczących [usługi Azure Redis Cache User Voice](https://feedback.azure.com/forums/169382-cache).
...
## <a name="azure-redis-cache-basics"></a>Podstawy usługi Azure Redis Cache
Często zadawane pytania w tej sekcji opisano niektóre podstawowe informacje dotyczące usługi Azure Redis Cache.

* [Co to jest usługa Azure Redis Cache?](#what-is-azure-redis-cache)
* [Jak mogę rozpocząć pracę z usługą Azure Redis Cache?](#how-can-i-get-started-with-azure-redis-cache)

Poniższe często zadawane pytania obejmuje podstawowe pojęcia i pytania dotyczące usługi Azure Redis Cache i odpowiedzi są w innej sekcji często zadawane pytania.

* [Której oferty i jakiego rozmiaru usługi Redis Cache mam użyć?](#what-redis-cache-offering-and-size-should-i-use)
* [Jakie klientów pamięci podręcznej Redis można używać?](#what-redis-cache-clients-can-i-use)
* [Czy istnieje lokalnego emulatora usługi Azure Redis Cache?](#is-there-a-local-emulator-for-azure-redis-cache)
...
* [Której oferty i jakiego rozmiaru usługi Redis Cache mam użyć?](#what-redis-cache-offering-and-size-should-i-use)
* [Wydajność usługi Azure Redis Cache](#azure-redis-cache-performance)
* [W jakim regionie ma szukać przepełnieniu pamięci podręcznej?](#in-what-region-should-i-locate-my-cache)
* [Jak są rozliczane dla usługi Azure Redis Cache?](#how-am-i-billed-for-azure-redis-cache)
* [Czy można używać usługi Azure Redis Cache w chmurze Azure Government, Azure albo chmura lub Microsoft Azure (Niemcy)?](#can-i-use-azure-redis-cache-with-azure-government-cloud-azure-china-cloud-or-microsoft-azure-germany)
...
* [Dlaczego Azure Redis Cache nie ma MSDN odwołanie do biblioteki klas np. niektóre z innymi usługami platformy Azure?](#why-doesnt-azure-redis-cache-have-an-msdn-class-library-reference-like-some-of-the-other-azure-services)
* [Czy można używać usługi Azure Redis Cache, jako pamięci podręcznej sesji PHP?](#can-i-use-azure-redis-cache-as-a-php-session-cache)
...
Często zadawane pytania w tej sekcji obejmują typowe monitorowania i pytania dotyczące rozwiązywania problemów. Aby uzyskać więcej informacji dotyczących monitorowania i rozwiązywania problemów swoich wystąpień usługi Azure Redis Cache, zobacz [jak monitorować usługę Azure Redis Cache](cache-how-to-monitor.md) i [jak rozwiązywać problemy z usługi Azure Redis Cache](cache-how-to-troubleshoot.md).
...
Usługa Azure Redis Cache jest oparta na popularnej [pamięci podręcznej Redis](http://redis.io) typu „open source”. Daje ona dostęp do zabezpieczonej, dedykowanej pamięci podręcznej Redis, zarządzanej przez firmę Microsoft i dostępna z dowolnej aplikacji na platformie Azure. Aby uzyskać bardziej szczegółowym omówieniem, zobacz [usługi Azure Redis Cache](https://azure.microsoft.com/services/cache/) stronę produktu w witrynie Azure.com.

### <a name="how-can-i-get-started-with-azure-redis-cache"></a>Jak mogę rozpocząć pracę z usługą Azure Redis Cache?
Istnieje kilka sposobów, które możesz rozpocząć pracę z usługą Azure Redis Cache.
...
* Możesz obejrzeć [jak tworzyć wysokowydajne aplikacje przy użyciu Microsoft Azure Redis Cache](https://azure.microsoft.com/documentation/videos/how-to-build-high-performance-apps-using-microsoft-azure-cache/).
* Możesz zapoznać się w dokumentacji klienta dla klientów, którzy jest zgodny z językiem programowania projekt, aby zobaczyć sposób użycia usługi Redis. Istnieje wielu klientów Redis, które mogą być używane z usługi Azure Redis Cache. Aby uzyskać listę klientów usługi Redis, zobacz [ http://redis.io/clients ](http://redis.io/clients).
...
Każdej oferty usługi Azure Redis Cache zapewnia różne poziomy **rozmiar**, **przepustowości**, **wysokiej dostępności**, i **SLA** opcje.

Poniżej przedstawiono zagadnienia dotyczące wybierania oferty pamięci podręcznej.

* **Pamięć**: warstwy podstawowa i standardowa oferują 250 MB – 53 GB. Warstwa Premium zapewnia do 530 GB. Aby uzyskać więcej informacji, zobacz [cennik usługi Azure Redis Cache](https://azure.microsoft.com/pricing/details/cache/).
...
* **Wysoka dostępność/SLA**: Azure Redis Cache gwarantuje, że standardowa/Premium pamięć podręczna jest dostępna co najmniej 99,9% czasu. Aby dowiedzieć się więcej o umowach SLA, zobacz [cennik usługi Azure Redis Cache](https://azure.microsoft.com/support/legal/sla/cache/v1_0/). Umowa SLA obejmuje jedynie połączenia z punktami końcowymi pamięci podręcznej. Nie zapewnia ona ochrony przed utratą danych. Firma Microsoft zaleca używanie funkcji trwałości danych Redis w warstwie Premium, aby zwiększyć odporność na utratę danych.
* **Trwałość danych redis**: warstwa Premium umożliwia utrwalanie danych pamięci podręcznej na koncie usługi Azure Storage. W pamięci podręcznej podstawowa i standardowa wszystkie dane są przechowywane tylko w pamięci. W przypadku podstawowych problemy dotyczące infrastruktury może być możliwej utracie danych. Firma Microsoft zaleca używanie funkcji trwałości danych Redis w warstwie Premium, aby zwiększyć odporność na utratę danych. Usługa Azure Redis Cache oferuje RDB i AOF (wkrótce) opcje w stan trwały pamięci podręcznej Redis. Aby uzyskać więcej informacji, zobacz [Konfigurowanie trwałości dla wersji Premium usługi Azure Redis Cache](cache-how-to-premium-persistence.md).
* **Klaster redis**: do utworzenia buforuje większe niż 53 GB i współdzielenie danych między wieloma węzłami Redis, umożliwia klastrowanie Redis, która jest dostępna w warstwie Premium. Każdy węzeł składa się z pary pamięci podręcznej podstawowy i węzeł repliki w celu zapewnienia wysokiej dostępności. Aby uzyskać więcej informacji, zobacz temat [Konfigurowanie usługi klastrowania dla usługi Azure Redis Cache w warstwie Premium](cache-how-to-premium-clustering.md).
* **Ulepszone zabezpieczenia i sieci izolacji**: wdrożenie usługi Azure Virtual Network (VNET) zapewnia lepsze zabezpieczenia i izolację usługi Azure Redis Cache, a także podsieci, zasady kontroli dostępu i inne funkcje ograniczające dostęp. Więcej informacji można znaleźć w temacie [Konfigurowanie obsługi sieci wirtualnej dla usługi Azure Redis Cache w warstwie Premium](cache-how-to-premium-vnet.md).
...
### <a name="azure-redis-cache-performance"></a>Wydajność usługi Azure Redis Cache
W poniższej tabeli przedstawiono wartości maksymalnej przepustowości, przestrzegać podczas testowania różne rozmiary Standard i Premium zapisuje w pamięci podręcznej przy użyciu `redis-benchmark.exe` z maszyny Wirtualnej Iaas względem punktu końcowego usługi Azure Redis Cache. Przepływność SSL testu porównawczego usługi redis jest używany przy użyciu programu stunnel połączyć się z punktem końcowym usługi Azure Redis Cache.
...
Aby uzyskać najlepszą wydajność i najniższym opóźnieniu Znajdź usługę Azure Redis Cache w tym samym regionie co aplikacja klienta pamięci podręcznej.

<a name="cache-billing"></a>

### <a name="how-am-i-billed-for-azure-redis-cache"></a>Jak są rozliczane dla usługi Azure Redis Cache?
Cennik usługi Azure Redis Cache jest [tutaj](https://azure.microsoft.com/pricing/details/cache/). Wyświetla listę stronie cennika, ceny za godzinę. Pamięci podręczne są rozliczane na podstawie na minutę od czasu utworzenia pamięci podręcznej aż do czasu, które pamięć podręczna zostanie usunięta. Nie ma opcji dla zatrzymanie lub wstrzymanie Naliczanie opłat w pamięci podręcznej.

### <a name="can-i-use-azure-redis-cache-with-azure-government-cloud-azure-china-cloud-or-microsoft-azure-germany"></a>Czy można używać usługi Azure Redis Cache w chmurze Azure Government, Azure albo chmura lub Microsoft Azure (Niemcy)?
Tak, usługi Azure Redis Cache jest dostępna w chmurze Azure Government, w chmurze platformy Azure (Chiny) i Microsoft Azure (Niemcy). Adresy URL do uzyskiwania dostępu i zarządzania usługi Azure Redis Cache różnią się w tych chmurach w porównaniu z chmury publicznej platformy Azure. 
...
Aby uzyskać więcej informacji, uwagi dotyczące korzystania z usługi Azure Redis Cache za pomocą innych chmur zobacz poniższe linki.

- [Bazy danych platformy Azure dla instytucji rządowych — usługi Azure Redis Cache](../azure-government/documentation-government-services-database.md#azure-redis-cache)
- [Chmura platformy Azure (Chiny) — usługi Azure Redis Cache](https://www.azure.cn/documentation/services/redis-cache/)
- [Microsoft Azure (Niemcy)](https://azure.microsoft.com/overview/clouds/germany/)

Aby uzyskać informacje na temat korzystania z usługi Azure Redis Cache za pomocą programu PowerShell w chmurze Azure Government, w chmurze platformy Azure (Chiny) i Microsoft Azure (Niemcy), zobacz [jak połączyć się z innych chmur - pamięci podręcznej Redis Azure w programie PowerShell](cache-howto-manage-redis-cache-powershell.md#how-to-connect-to-other-clouds).
...
### <a name="what-redis-cache-clients-can-i-use"></a>Jakie klientów pamięci podręcznej Redis można używać?
Jedną z najważniejszych funkcji usługi Redis zakłada, że wielu klientów, obsługa wielu języków programowania różne. Aby uzyskać bieżącą listę klientów, zobacz [klienci Redis](http://redis.io/clients). Samouczki, które obejmują kilka różnych językach i klientów, zobacz [jak używać usługi Azure Redis Cache](cache-dotnet-how-to-use-azure-redis-cache.md) i kliknij żądany język z przełącznika język, w górnej części tego artykułu.

[!INCLUDE [redis-cache-create](../../includes/redis-cache-access-keys.md)]

<a name="cache-emulator"></a>

### <a name="is-there-a-local-emulator-for-azure-redis-cache"></a>Czy istnieje lokalnego emulatora usługi Azure Redis Cache?
Nie jest Brak lokalnego emulatora usługi Azure Redis Cache, ale można uruchomić wersji MSOpenTech redis-server.exe z [Redis narzędzia wiersza polecenia](https://github.com/MSOpenTech/redis/releases/) na lokalnym komputerze, a następnie nawiązać z nim, aby uzyskać podobne możliwości emulatora lokalnej pamięci podręcznej, jak pokazano w poniższym przykładzie:
...
Opcjonalnie można skonfigurować [redis.conf](http://redis.io/topics/config) pliku, aby lepiej dopasować [domyślne ustawienia pamięci podręcznej](cache-configure.md#default-redis-server-configuration) dla usługi online usługi Azure Redis Cache w razie potrzeby.

<a name="cache-commands"></a>

### <a name="how-can-i-run-redis-commands"></a>Jak uruchomić polecenia Redis
Można użyć dowolnego polecenia wymienione pod [polecenia Redis](http://redis.io/commands#) z wyjątkiem polecenia wymienione pod [polecenia nie są obsługiwane w usłudze Azure Redis Cache Redis](cache-configure.md#redis-commands-not-supported-in-azure-redis-cache). Istnieje kilka opcji, aby uruchamiać polecenia usługi Redis.
...
### <a name="why-doesnt-azure-redis-cache-have-an-msdn-class-library-reference-like-some-of-the-other-azure-services"></a>Dlaczego Azure Redis Cache nie ma MSDN odwołanie do biblioteki klas np. niektóre z innymi usługami platformy Azure?
Microsoft Azure Redis Cache jest oparta na popularnej "open source" Redis Cache i może zostać oceniony przez cały szereg [klienci Redis](http://redis.io/clients) dla wielu języków programowania. Każdy klient ma własny interfejs API, który sprawia, że wywołania do pamięci podręcznej Redis cache wystąpienia przy użyciu [polecenia Redis](http://redis.io/commands).

Ponieważ różni się każdego klienta, istnieje nie jeden scentralizowane odwołań do klas w witrynie MSDN, a każdy klient zachowuje własną dokumentację referencyjną. Oprócz dokumentacją referencyjną istnieje kilka samouczków przedstawiająca sposób rozpocząć pracę z usługą Azure Redis Cache przy użyciu różnych języków i klientów pamięci podręcznej. Aby uzyskać dostęp do tych samouczków, zobacz [jak używać usługi Azure Redis Cache](cache-dotnet-how-to-use-azure-redis-cache.md) i kliknij żądany język z przełącznika język, w górnej części tego artykułu.

### <a name="can-i-use-azure-redis-cache-as-a-php-session-cache"></a>Czy można używać usługi Azure Redis Cache, jako pamięci podręcznej sesji PHP?
Tak, aby używać usługi Azure Redis Cache jako pamięć podręczna sesji języka PHP, określ parametry połączenia do swojego wystąpienia usługi Azure Redis Cache w `session.save_path`.

> [!IMPORTANT]
> Korzystając z usługi Azure Redis Cache jako pamięci podręcznej sesji języka PHP, należy najpierw adresu URL kodowanie klucz zabezpieczeń używany do łączenia z pamięci podręcznej, jak pokazano w poniższym przykładzie:
>
...
Aby uzyskać więcej informacji o korzystaniu z pamięci podręcznej Redis jako pamięci podręcznej sesji PHP za pomocą klienta PhpRedis zobacz [obsługi sesji PHP](https://github.com/phpredis/phpredis#php-session-handler).
...
> [!NOTE]
> Korzystając z włączonym klastrowaniu Premium usługi Azure Redis Cache, tylko bazy danych 0 jest dostępna. To ograniczenie jest wewnętrzna ograniczenie pamięci podręcznej Redis i nie jest specyficzne dla usługi Azure Redis Cache. Aby uzyskać więcej informacji, zobacz [należy wprowadzać żadnych zmian Moja aplikacja kliencka, aby użyć klastrowania?](cache-how-to-premium-clustering.md#do-i-need-to-make-any-changes-to-my-client-application-to-use-clustering)
> 
> 
...
Redis server nie obsługuje natywnie protokołu SSL, ale nie usługi Azure Redis Cache. Jeśli łączysz się usługi Azure Redis Cache, a Twój klient obsługuje protokół SSL, takich jak StackExchange.Redis, należy używać protokołu SSL.

>[!NOTE]
>Port bez protokołu SSL jest domyślnie wyłączona, dla nowych wystąpień usługi Azure Redis Cache. Jeśli Twój klient nie obsługuje protokołu SSL, a następnie należy włączyć port bez protokołu SSL, postępując zgodnie z instrukcjami w [portów](cache-configure.md#access-ports) części [Konfigurowanie pamięci podręcznej w usłudze Azure Redis Cache](cache-configure.md) artykułu.
>
>
...
* Używane do testowania maszyny Wirtualnej klienta musi należeć do tego samego regionu wystąpienia pamięci podręcznej Redis.
...
* Upewnij się, że obciążenia, testowanie klienta i pamięć podręczna Redis znajdują się w tym samym regionie.
...
Microsoft Azure Redis Cache wystąpienia mogą być monitorowane w [witryny Azure portal](https://portal.azure.com). Można wyświetlić metryki, przypinać wykresy metryk do tablicy startowej, dostosować zakres dat i godzin, wykresy monitorowania, Dodaj i Usuń metryki z wykresów i ustawić alerty, gdy są spełnione określone warunki. Aby uzyskać więcej informacji, zobacz [monitora usługi Azure Redis Cache](cache-how-to-monitor.md).

Pamięć podręczna Redis **menu zasobów** zawiera także kilka narzędzi do monitorowania i rozwiązywania problemów pamięci podręczne.
...
Te narzędzia umożliwiają monitorowanie kondycji wystąpień usługi Azure Redis Cache i pomocne w zarządzaniu aplikacjami buforowania. Aby uzyskać więcej informacji, zobacz sekcję "Ustawienia dotyczące rozwiązywania problemów i pomocy technicznej" [sposób konfigurowania usługi Azure Redis Cache](cache-configure.md).
...
* Powoduje, że po stronie serwera
  * Na standardowa pamięci podręcznej, oferty usługi Azure Redis Cache zainicjować trybu failover z węzła podstawowego do węzła pomocniczego.
...
> [!IMPORTANT]
> Zgodnie z ostatniego roku [ogłoszenie](https://azure.microsoft.com/blog/azure-managed-cache-and-in-role-cache-services-to-be-retired-on-11-30-2016/), usługa Azure Managed Cache Service i pamięć podręczna oparta na roli Azure **został wycofany** 30 listopada 2016 r. Nasze zalecenie to użycie [usługi Azure Redis Cache](https://azure.microsoft.com/services/cache/). Aby uzyskać informacje na temat migracji, zobacz [migracja z usługi Managed Cache Service do usługi Azure Redis Cache](cache-migrate-to-redis.md).
>
>

### <a name="azure-redis-cache"></a>Azure Redis Cache
Usługa Azure Redis Cache jest ogólnie dostępna w rozmiarach do 53 GB i ma dostępność na poziomie 99,9%. Nowy [w warstwie premium](cache-premium-tier-intro.md) oferuje rozmiarach do 530 GB i pomoc techniczna dla klastra sieć Wirtualną i trwałości, z umową SLA 99,9%.

Usługa Azure Redis Cache zapewnia klientom możliwość korzystania z bezpiecznej, dedykowanej pamięci podręcznej Redis, zarządzanej przez firmę Microsoft. Ta oferta umożliwia skorzystanie z do rozbudowanego zestawu funkcji i ekosystemu udostępnianego udostępnianych przez Redis oraz niezawodnych oraz hostingu i monitorowania od firmy Microsoft.
...
Innym kluczowym aspektem powodzenia Redis jest ekosystemu dobrej kondycji, aktywny "open source" zbudowane wokół niej. Znajduje to odzwierciedlenie w różnorodnych klientów Redis są dostępne w wielu językach. Ten ekosystem i szerokiej gamy klientów umożliwiają usługi Azure Redis Cache ma być używany przez niemal dowolnego obciążenia, jakie można utworzyć w systemie Azure.

Aby uzyskać więcej informacji na temat rozpoczynania pracy z usługą Azure Redis Cache, zobacz [sposobu użycia usługi Azure Redis Cache](cache-dotnet-how-to-use-azure-redis-cache.md) i [dokumentacji usługi Azure Redis Cache](index.md).
