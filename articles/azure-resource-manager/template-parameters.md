---
title: Parametry w szablonach Azure Resource Manager
...
W szablonie odwołujesz się do wartości parametru za pomocą funkcji [Parameters](resource-group-template-functions-deployment.md#parameters) . W poniższym przykładzie wartość parametru służy do ustawiania jednostki SKU dla konta magazynu.
...
Nie można użyć funkcji [Reference](resource-group-template-functions-resource.md#reference) ani żadnej z funkcji [list](resource-group-template-functions-resource.md#list) w sekcji Parameters. Te funkcje uzyskują stan środowiska uruchomieniowego zasobu i nie można ich wykonać przed wdrożeniem, gdy parametry zostaną rozwiązane.
...
* Aby dowiedzieć się więcej o dostępnych właściwościach parametrów, zobacz [Opis struktury i składni szablonów Azure Resource Manager](resource-group-authoring-templates.md).
* Aby dowiedzieć się więcej o przekazywaniu wartości parametrów jako pliku, zobacz [Tworzenie pliku parametrów Menedżer zasobów](resource-manager-parameter-files.md).
