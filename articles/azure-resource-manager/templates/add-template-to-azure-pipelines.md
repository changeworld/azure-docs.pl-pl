...
* Skonfigurowano [połączenie usługi](/azure/devops/pipelines/library/connect-to-azure?view=azure-devops) z subskrypcją platformy Azure. Zadania w potoku są wykonywane w ramach tożsamości nazwy głównej usługi. Aby uzyskać instrukcje dotyczące tworzenia połączenia, zobacz [Tworzenie projektu DevOps](template-tutorial-use-azure-pipelines.md#create-a-devops-project).
...
Aby zapoznać się z procesem krok po kroku na temat używania Azure Pipelines z szablonami Menedżer zasobów, zobacz [Samouczek: Ciągła integracja szablonów Azure Resource Manager z Azure Pipelines](template-tutorial-use-azure-pipelines.md).
