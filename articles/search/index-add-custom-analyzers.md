---
title: Dodawanie niestandardowych analizatorów — Azure Search
description: Modyfikuj tokenizatory tekstowe i filtry znaków używane w Azure Search zapytaniach wyszukiwania pełnotekstowego.
ms.date: 08/08/2019
services: search
ms.service: search
ms.topic: conceptual
...
ms.locfileid: "68881534"
---
# <a name="add-custom-analyzers-to-an-azure-search-index"></a>Dodawanie niestandardowych analizatorów do indeksu Azure Search
...
## <a name="overview"></a>Przegląd
 ...
 W Azure Search można wybierać spośród zestawu wstępnie zdefiniowanych analizatorów języka niezależny od w tabeli analizatorów lub analizatorów charakterystycznych dla języka wymienionych w obszarze [analizatory języka &#40;Azure Search Interfejs&#41;API REST usługi](index-add-language-analyzers.md). [](#AnalyzerTable) Dostępna jest również opcja definiowania własnych analizatorów niestandardowych.  
...
- Łamanie w formacie ASCII. Dodaj standardowy filtr składania ASCII, aby znormalizować znaki diakrytyczne, takie jak ö lub ê, w terminach wyszukiwania.  
  ...
  Ta strona zawiera listę obsługiwanych analizatorów, tokenizatory, filtrów tokenów i filtrów znaków. Możesz również znaleźć opis zmian w definicji indeksu przy użyciu przykładu użycia. Aby uzyskać więcej informacji na temat podstawowej technologii wykorzystywanej w implementacji Azure Search, zobacz [Podsumowanie pakietu analizy (Lucene)](https://lucene.apache.org/core/6_0_0/core/org/apache/lucene/codecs/lucene60/package-summary.html). Przykłady konfiguracji analizatora znajdują się [w temacie Dodawanie analizatorów w Azure Search](search-analyzers.md#examples).
...
## <a name="create-custom-analyzers"></a>Tworzenie analizatorów niestandardowych
 ...
 `token_filter_name_1` `token_filter_name_2` `char_filter_name_2`Jestnazwą elementu [](#Tokenizers)tokenizatora i`char_filter_name_1` są nazwami filtrów tokenów, a i są nazwami filtrów char (zobacz tokenizatory, [filtry tokenów](#TokenFilters) i filtry znaków). `tokenizer_name` tabele dla prawidłowych wartości).
...
|Opcje|Muszą być prawidłowymi opcjami wstępnie zdefiniowanego analizatora [](#AnalyzerTable) wymienionymi w tabeli analizatorów poniżej.|  
...
|Tokenizer|Wymagana. Ustaw jeden z wstępnie zdefiniowanych tokenizatory wymienionych w poniższej tabeli [tokenizatory](#Tokenizers) lub Custom tokenizatora określony w definicji indeksu.|  
...
|-|-|  
|Name (Nazwa)|Musi zawierać tylko litery, cyfry, spacje, kreski i znaki podkreślenia, może zaczynać się i kończyć tylko znakami alfanumerycznymi i mieć ograniczone do 128 znaków.|  
|Type|Typ filtru char z listy obsługiwanych filtrów char. Zobacz kolumnę **char_filter_type** w tabeli [filtry znaków](#char-filters-reference) poniżej.|  
...
|-|-|  
|Name (Nazwa)|Musi zawierać tylko litery, cyfry, spacje, kreski i znaki podkreślenia, może zaczynać się i kończyć tylko znakami alfanumerycznymi i mieć ograniczone do 128 znaków.|  
|Type|Nazwa tokenizatora z listy obsługiwanych tokenizatory. Zobacz kolumnę **tokenizer_type** w tabeli [tokenizatory](#Tokenizers) poniżej.|  
...
|-|-|  
|Name (Nazwa)|Musi zawierać tylko litery, cyfry, spacje, kreski i znaki podkreślenia, może zaczynać się i kończyć tylko znakami alfanumerycznymi i mieć ograniczone do 128 znaków.|  
|Type|Nazwa filtru tokenu z listy obsługiwanych filtrów tokenu. Zobacz kolumnę **token_filter_type** w tabeli [filtry tokenu](#TokenFilters) poniżej.|  
...
|[pattern](https://lucene.apache.org/core/6_6_1/analyzers-common/org/apache/lucene/analysis/miscellaneous/PatternAnalyzer.html)|PatternAnalyzer|Elastycznie oddziela tekst do warunków za pośrednictwem wzorca wyrażenia regularnego.<br /><br /> **Opcje**<br /><br /> małe litery (Type: bool) — określa, czy warunki są małymi literami. Wartość domyślna to true.<br /><br /> [wzorzec](https://docs.oracle.com/javase/7/docs/api/java/util/regex/Pattern.html?is-external=true) (typ: String) — wzorzec wyrażenia regularnego do dopasowania do separatorów tokenów. Wartość domyślna to \w +.<br /><br /> [flagi](https://docs.oracle.com/javase/6/docs/api/java/util/regex/Pattern.html#field_summary) (typ: ciąg) — flagi wyrażenia regularnego. Wartość domyślna to pusty ciąg. Dozwolone wartości: CANON_EQ, CASE_INSENSITIVE, KOMENTARZE, DOTALL, LITERAL, WIELOWIERSZOWY, UNICODE_CASE, UNIX_LINES<br /><br /> Stop-słowa (typ: tablica ciągów) — lista Stop-słowa. Wartość domyślna to pusta lista.|  
...
|-|-|-|  
|[motyw](https://lucene.apache.org/core/6_6_1/analyzers-common/org/apache/lucene/analysis/standard/ClassicTokenizer.html)|ClassicTokenizer|Tokenizatora na podstawie gramatyki, które są odpowiednie do przetwarzania większości dokumentów w języku europejskim.<br /><br /> **Opcje**<br /><br /> maxTokenLength (typ: int) — Maksymalna długość tokenu. Domyślne: 255, maksimum: 300. Tokeny dłuższe niż maksymalna długość są dzielone.|  
...
|[keyword_v2](https://lucene.apache.org/core/6_6_1/analyzers-common/org/apache/lucene/analysis/core/KeywordTokenizer.html)|KeywordTokenizerV2|Emituje cały dane wejściowe jako jeden token.<br /><br /> **Opcje**<br /><br /> maxTokenLength (typ: int) — Maksymalna długość tokenu. Domyślne: 256, maksimum: 300. Tokeny dłuższe niż maksymalna długość są dzielone.|  
...
|[path_hierarchy_v2](https://lucene.apache.org/core/6_6_1/analyzers-common/org/apache/lucene/analysis/path/PathHierarchyTokenizer.html)|PathHierarchyTokenizerV2|Tokenizatora dla hierarchii podobnej do ścieżki.<br /><br /> **Opcje**<br /><br /> ogranicznik (typ: ciąg) — wartość domyślna: "/.<br /><br /> zastąpienie (typ: ciąg) — Jeśli jest ustawiony, zastępuje znak ogranicznika. Domyślnie taka sama jak wartość ogranicznika.<br /><br /> maxTokenLength (typ: int) — Maksymalna długość tokenu. Domyślne: 300, maksimum: 300. Ścieżki dłuższe niż maxTokenLength są ignorowane.<br /><br /> Odwróć (typ: bool) — Jeśli prawda, generuje token w odwrotnej kolejności. Wartość domyślna: false.<br /><br /> Skip (Type: bool) — początkowe tokeny do pominięcia. Wartość domyślna to 0.|  
...
|[standard_v2](http://lucene.apache.org/core/6_6_1/core/org/apache/lucene/analysis/standard/StandardTokenizer.html)|StandardTokenizerV2|Dzieli tekst po [regułach segmentacji tekstu Unicode](https://unicode.org/reports/tr29/).<br /><br /> **Opcje**<br /><br /> maxTokenLength (typ: int) — Maksymalna długość tokenu. Domyślne: 255, maksimum: 300. Tokeny dłuższe niż maksymalna długość są dzielone.|  
|[uax_url_email](https://lucene.apache.org/core/6_6_1/analyzers-common/org/apache/lucene/analysis/standard/UAX29URLEmailTokenizer.html)|UaxUrlEmailTokenizer|Tokenizes adresy URL i wiadomości e-mail jako jeden token.<br /><br /> **Opcje**<br /><br /> maxTokenLength (typ: int) — Maksymalna długość tokenu. Domyślne: 255, maksimum: 300. Tokeny dłuższe niż maksymalna długość są dzielone.|  
...
|[scandinavian_folding](https://lucene.apache.org/core/6_6_1/analyzers-common/org/apache/lucene/analysis/miscellaneous/ScandinavianFoldingFilter.html)|(typ ma zastosowanie tylko wtedy, gdy opcje są dostępne)  |Folds Scandinavian characters åÅäæÄÆ->a and öÖøØ->o. Odróżni się także od używania podwójnych samogłosek AA, AE, Ao, OE i oo, pozostawiając tylko pierwszy z nich. |  
...
## <a name="see-also"></a>Zobacz także  
 ...
 [Analizatory > Azure Search przykłady](search-analyzers.md#examples)    
