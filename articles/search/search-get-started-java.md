...
1. `src` WfolderzeDodaj`app` foldery i .`service`  >   `main`  >  `java` Aby to zrobić, wybierz `java` folder, naciśnij klawisze ALT + INSERT, a następnie wprowadź nazwę folderu.
1. `src` WfolderzeDodaj`app` foldery i .`service`  >   `main`  > `resources`
...
1. W oknie **projekt** rozwiń drzewo `src` źródłowe, aby uzyskać dostęp `resources`  > `main`  >    >  `config.properties` do folderu, a następnie Dodaj plik. `app` Aby to zrobić, wybierz `app` folder, naciśnij klawisze ALT + INSERT, wybierz pozycję **plik**, a następnie wprowadź nazwę pliku.
...
1. `src` Wfolderze`App`Dodaj klasę. `app`  >   `main`  >  `java`  >  Aby to zrobić, wybierz `app` folder, naciśnij klawisze ALT + INSERT, wybierz opcję **Klasa Java**, a następnie wprowadź nazwę klasy.
1. `App` Otwórz klasę i Zastąp zawartość następującym kodem. Ten kod zawiera `main` metodę. 
...
1. `src` Wfolderze`SearchServiceClient`Dodaj klasę. `service`  >   `main`  >  `java`  >  Aby to zrobić, wybierz `service` folder, naciśnij klawisze ALT + INSERT, wybierz opcję **Klasa Java**, a następnie wprowadź nazwę klasy.
1. `SearchServiceClient` Otwórz klasę i Zastąp zawartość następującym kodem. Ten kod zawiera operacje HTTP wymagane do korzystania z interfejsu API REST Azure Search. Dodatkowe metody tworzenia indeksu, przekazywania dokumentów i wysyłania zapytań do indeksu zostaną dodane w dalszej części.
...
![Wykonaj Maven cel: Weryfikuj exec: Java](media/search-get-started-java/java-quickstart-execute-maven-goal.png)
...
1. W oknie **projekt** rozwiń drzewo `src` źródłowe, aby uzyskać dostęp `resources`  > `main`  >    >  `index.json` do folderu, a następnie Dodaj plik. `service` Aby to zrobić, wybierz `app` folder, naciśnij klawisze ALT + INSERT, wybierz pozycję **plik**, a następnie wprowadź nazwę pliku.

1. `index.json` Otwórz plik i Wstaw następującą definicję indeksu.
    ...
    Pole w tym indeksie używa opcjonalnej `analyzer` właściwości do przesłania domyślnego analizatora języka Lucene. `Description` W `Description_fr` polu jest używany francuski Analizator `fr.lucene` Lucene, ponieważ jest on przechowywany w języku francuskim. `Description` Używa opcjonalnego narzędzia Microsoft Language Analyzer en. Lucene. Aby dowiedzieć się więcej na temat analizatorów, zobacz [analizatory do przetwarzania tekstu w Azure Search](search-analyzers.md).

1. Dodaj następujący kod do `SearchServiceClient` klasy. Te metody tworzą Azure Search adresy URL usługi REST, które tworzą i usuwają indeks, i określają, czy indeks istnieje. Metody również tworzą żądanie HTTP.
...
1. W oknie **projekt** rozwiń drzewo `src` źródłowe, aby uzyskać dostęp `resources`  > `main`  >    >  `hotels.json` do folderu, a następnie Dodaj plik. `service` Aby to zrobić, wybierz `app` folder, naciśnij klawisze ALT + INSERT, wybierz pozycję **plik**, a następnie wprowadź nazwę pliku.
...
1. Dodaj następujący kod do `SearchServiceClient` klasy. Ten kod kompiluje Azure Search adresy URL usługi REST, aby przeszukać indeksowane dane i wydrukował wyniki wyszukiwania.

    `SearchOptions` Klasa i`createSearchOptions` Metoda pozwalają określić podzestaw dostępnych opcji zapytania interfejsu API REST Azure Search. Aby uzyskać więcej informacji na temat opcji zapytania interfejsu API REST, zobacz [Wyszukiwanie dokumentów (usługa Azure Search API REST usługi)](/rest/api/searchservice/search-documents).
...
1. `App` W klasie Usuń komentarz z poniższego kodu. Ten kod konfiguruje pięć różnych zapytań, w tym tekst wyszukiwania, parametry zapytania i pola danych do zwrócenia. 
