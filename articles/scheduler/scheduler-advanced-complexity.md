...
W tym artykule opisano przykład zadań, można utworzyć za pomocą harmonogramu i i [interfejsu API REST usługi Azure Scheduler](https://docs.microsoft.com/rest/api/schedule)i zawiera definicję JavaScript Object Notation (JSON) dla każdego harmonogramu. 
...
* Uruchom, a następnie powtarzać wiele razy dla określonego okresu. Na przykład co miesiąc na poniedziałek i ostatni piątek lub codziennie o 5:15:00 i o godzinie 17:15.
...
Aby utworzyć harmonogram podstawowe z [interfejsu API REST usługi Azure Scheduler](https://docs.microsoft.com/rest/api/schedule), wykonaj następujące kroki:
...
* Daty i godziny w zadania usługi Scheduler obejmują daty i godziny, postępuj zgodnie z [specyfikacji ISO 8601](http://en.wikipedia.org/wiki/ISO_8601), zakłada że jest czasem UTC, gdy nie określono żadnych przesunięcie czasu UTC. 
...
|-----------|---------------|-------------------------|--------------------------|
| **Brak godziny rozpoczęcia** | Uruchom raz od razu. | Uruchom raz od razu. Kolejne wykonania obliczonym na podstawie ostatniego czasu wykonania. | Uruchom raz od razu. Kolejne wykonania opierają na harmonogramie cyklu. | 
| **Godzina rozpoczęcia w przeszłości** | Uruchom raz od razu. | Oblicz przyszłość pierwszego uruchamiania czas po czasie rozpoczęcia i w tym czasie. <p>Kolejne wykonania obliczonym na podstawie ostatniego czasu wykonania. <p>Zobacz przykład tą tabelą. | Uruchamianie zadania *nie wcześniej niż* w określonym czasie rozpoczęcia. Pierwsze wystąpienie opiera się na harmonogramie obliczonym na podstawie czasu rozpoczęcia. <p>Kolejne wykonania opierają na harmonogramie cyklu. | 
| **Godzina rozpoczęcia w przyszłości lub bieżący czas** | Uruchamiany raz o określonej godzinie. | Uruchamiany raz o określonej godzinie. <p>Kolejne wykonania obliczonym na podstawie ostatniego czasu wykonania. | Uruchamianie zadania *nie wcześniej niż* w określonym czasie rozpoczęcia. Pierwsze wystąpienie opiera się na harmonogramie obliczonym na podstawie czasu rozpoczęcia. <p>Kolejne wykonania opierają na harmonogramie cyklu. |
||||| 
...
* Bieżąca data i godzina to "2015-04-08 13:00".

* Data i godzina rozpoczęcia jest "2015-04-07 14:00", czyli przed bieżącą datą i godziną.

* Cykl jest określony na 2 dni.

1. W tych warunkach pierwsze wykonanie następuje o 2015-04-09 o 14:00. 
   ...
   W tym przypadku **startTime** włączeniu 2015-04-07 o 14:00, więc kolejne wystąpienie ma miejsce dwa dni po tym czasie, czyli 09-04-2015 r. o 14:00.

   Pierwszego wykonania jest taka sama czy **startTime** to 2015-04-05-14:00 lub 2015-04-01 14:00. Po pierwszym wykonaniu kolejne wykonania są obliczane na podstawie harmonogramu. 
...
Możesz użyć **harmonogram** do *limit* liczby wykonań zadania. Na przykład, jeśli zadanie **częstotliwość** "Month" ma harmonogram uruchamiany tylko w dniu 31, zadanie będzie uruchamiane tylko w miesiącach, które mają trzydzieści jeden dni.
...
Jeśli określisz wielu elementów harmonogramu, kolejność obliczania następuje od największego do najmniejszego: numer tygodnia, miesiąc, dzień, dzień tygodnia, godziny i minuty.
...
| `{"minutes":[0], "hours":[6], "monthDays":[28]}` |Uruchamiany o 6: 00 dwudziestego ósmego dnia każdego miesiąca (zakładając, że **częstotliwość** "Month"). |
...
| `{monthDays":[1,14]}` |Uruchom na pierwszego i czternastego dnia każdego miesiąca o godzinie rozpoczęcia. |
