# [Dokumentacja usługi Scheduler](index.md)

# Omówienie
## [Czym jest Azure Scheduler?](scheduler-intro.md)

# Rozpoczęcie pracy
## [Tworzenie zadań i zarządzanie nimi — portal](scheduler-get-started-portal.md)
## [Migrowanie z usługi Scheduler do usługi Logic Apps](migrate-from-scheduler-to-logic-apps.md)

# Pojęcia
## [Pojęcia, terminy i jednostki](scheduler-concepts-terms.md)

# Przewodniki z instrukcjami
## Planowanie i projektowanie
### [Plany i rozliczanie](scheduler-plans-billing.md)
### [Limity, limity przydziału i wartości domyślne](scheduler-limits-defaults-errors.md)
### [Wysoka dostępność i niezawodność](scheduler-high-availability-reliability.md)

## Programowanie
### [Tworzenie złożonych i zaawansowanych harmonogramów cyklicznych](scheduler-advanced-complexity.md)
### [Tworzenie harmonogramów i zarządzanie nimi — Azure PowerShell](scheduler-powershell-reference.md)

## Bezpieczeństwo
### [Uwierzytelnianie połączeń wychodzących](scheduler-outbound-authentication.md)

# Dokumentacja
## [Azure PowerShell](/powershell/module/azurerm.scheduler)
## [Interfejs API REST](/rest/api/scheduler)
## [Szablon usługi Resource Manager](/azure/templates/microsoft.scheduler/allversions)

# Zasoby
## [Harmonogram działania usługi Azure Scheduler](https://azure.microsoft.com/updates/?product=scheduler)
## [Harmonogram działania dla platformy Azure](https://azure.microsoft.com/updates/)
## [Forum MSDN](https://social.msdn.microsoft.com/Forums/home?forum=azurescheduler)
## [Stack Overflow](https://stackoverflow.com/questions/tagged/azure-scheduler)
## [Cennik](https://azure.microsoft.com/pricing/details/scheduler/)
## [Kalkulator cen](https://azure.microsoft.com/pricing/calculator/)
## [Aktualizacje usług](https://azure.microsoft.com/updates/?product=scheduler)
## [Filmy wideo](https://azure.microsoft.com/documentation/videos/index/?services=scheduler)