...
W tym artykule omówiono sposób dostosowywania interfejsu użytkownika Azure AD B2C z *zawartością dynamiczną* przy użyciu zasad niestandardowych. Aby rozpocząć, zobacz [Dostosowywanie interfejsu użytkownika w zasadach niestandardowych](custom-policy-ui-customization.md).

>[!NOTE]
>W Azure AD B2C artykule [Skonfiguruj dostosowanie interfejsu użytkownika w zasadach niestandardowych](custom-policy-ui-customization.md), omówiono następujące zagadnienia:
...
W artykule [Konfigurowanie dostosowywania interfejsu użytkownika w ramach zasad niestandardowych](custom-policy-ui-customization.md) można przekazać pliki HTML5 do usługi Azure Blob Storage. Te pliki HTML5 są statyczne i renderują tę samą zawartość HTML dla każdego żądania.
...
3. Nadaj nazwę aplikacji (na przykład *contoso. AADB2C. UI*), a następnie wybierz **przycisk OK**.
    ...
    ![Utwórz nowy projekt programu Visual Studio](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project1.png)

4. Wybierz szablon **aplikacji sieci Web** .

5. Ustaw uwierzytelnianie na **Brak uwierzytelniania**.

    ![Wybieranie szablonu aplikacji sieci Web](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project2.png)
...
1. Kliknij prawym przyciskiem myszy widok/folder macierzysty, a następnie **dodaj** > **nowy element**.
    ...
    ![Element menu Dodaj nowy element w programie Visual Studio](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view1.png)
...
5. Wybierz pozycję **Dodaj**.
    ...
    ![Okno dialogowe Dodawanie nowego elementu w programie Visual Studio z wyróżnioną stroną widoku MVC](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view2.png)
...
10. Skopiuj zawartość pliku i wklej go poniżej definicji układu. Kod powinien wyglądać następująco:
    ...
    ![ujednolicony plik. cshtml po dodaniu HTML5](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-edit-view1.png)
...
![img, element z niestandardową background_background_image wartość src](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-static-background.png)
...
1. Otwórz **Controllers\HomeController.cs**i Dodaj następującą metodę:
    ...
    ![Zmień kontroler, aby renderować widok](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-controller-view.png)
...
1. W **Eksplorator rozwiązań**kliknij prawym przyciskiem myszy projekt **contoso. AADB2C. UI** , a następnie wybierz pozycję **Publikuj**.
    ...
    ![Publikuj w Microsoft Azure App Service](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish1.png)

2. Wybierz kafelek **App Service Microsoft Azure** , a następnie wybierz pozycję **Publikuj**.

    ![Utwórz nowy App Service Microsoft Azure](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish2.png)
...
4. Wybierz pozycję **Utwórz**, aby rozpocząć tworzenie zasobów platformy Azure.
    ...
    ![Podaj właściwości App Service](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish3.png)
...
1. W [Azure Portal](https://portal.azure.com/)wybierz pozycję **App Services**, a następnie wybierz nazwę aplikacji interfejsu API.
    ...
    ![Wybierz aplikację interfejsu API w Azure Portal](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS1.png)

2. W sekcji **Ustawienia** w obszarze **interfejs API** wybierz pozycję **CORS**.

    ![Element menu CORS wyróżniony w menu App Service w Azure Portal](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS2.png)
...
4. Wybierz pozycję **Zapisz**.
    ...
    ![Strona ustawień CORS z wyróżnioną gwiazdką w dozwolonych źródłach](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS3.png)
...
6. Zmień wartość `LoadUri` z _~/tenant/default/Unified_ na _https://< APP_NAME >. azurewebsites. net/Home/Unified_.
    ...
    ![Przykład fragmentu kodu XML z wyróżnionym elementem LoadUri](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-content-definition.png)
...
2. Otwórz **B2C_1A_signup_signin**, załadowane zasady niestandardowe jednostki uzależnionej (RP), a następnie wybierz pozycję **Uruchom teraz**.
    ...
    ![Zasady rejestracji lub logowania](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo1.png)
...
2. Znajdź `<img>` element z IDENTYFIKATORem `background_background_image`i Zastąp wartość `src` `@ViewData["background"]`.
    ...
    ![element img z wyróżnioną wartością src ](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-dynamic-background.png)
...
5. Dodaj parametr ciągu zapytania _campaignId_ do identyfikatora URI. Na przykład Dodaj `&campaignId=hawaii`, jak pokazano na poniższej ilustracji:
    ...
    ![Wyróżniono identyfikator URI przy użyciu parametru ciągu zapytania campaignId](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-campaignId-param.png)

6. Wybierz **klawisz ENTER** , aby wyświetlić obraz tła Hawaje.

    ![Strona logowania przy użyciu konta z niestandardowym tłem obrazu Hawaje](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo2.png)

7. Zmień wartość na *Tokio*, a następnie wybierz **klawisz ENTER**.
    Przeglądarka wyświetla tło Tokio.

    ![Strona logowania do rejestracji z niestandardowym tłem obrazu w postaci Tokio](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo3.png)
...
* Po ukończeniu przewodnika [wprowadzenie do zasad niestandardowych](custom-policy-get-started.md) zalecamy utworzenie scenariusza przy użyciu własnych niestandardowych plików zasad. W odniesieniu do Twojej dokumentacji udostępniono [przykładowe pliki zasad](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/scenarios/aadb2c-ief-ui-customization).
