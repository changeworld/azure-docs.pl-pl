---
title: Często zadawane pytania dotyczące usługi Azure Active Directory B2C
...
Usługa Azure AD B2C dodano również obsługę [zasady niestandardowe](active-directory-b2c-overview-custom.md). Zasady te niestandardowe umożliwiają deweloperom tworzenie własnych zasad za pomocą dowolnego dostawcy tożsamości, która obsługuje [OpenID Connect](https://openid.net/specs/openid-connect-core-1_0.html) lub SAML.
...
1. Usuń wszystkie **aplikacje** zarejestrowany w dzierżawie usługi Azure AD B2C.
1. Następnie zaloguj się do [witryny Azure portal](https://portal.azure.com/) jako Administrator subskrypcji. Użyj tego samego pracy lub konta służbowego lub tego samego konta Microsoft, którego użyto podczas tworzenia konta na platformie Azure.
1. Przełącz do dzierżawy usługi Azure AD B2C chcesz usunąć.
1. Wybierz **usługi Azure Active Directory** w menu po lewej stronie.
1. W obszarze **Zarządzaj** wybierz pozycję **Użytkownicy**.
1. Zaznacz każdy użytkownik z kolei (Wyklucz użytkownika administratora subskrypcji, który jest aktualnie zalogowany jako). Wybierz **Usuń** w dolnej części strony i wybierz pozycję **tak** po wyświetleniu monitu.
1. W obszarze **Zarządzaj**, wybierz opcję **rejestracje aplikacji** (lub **rejestracje aplikacji (starsza wersja)** ).
1. Wybierz **Wyświetl wszystkie aplikacje**
1. Wybierz aplikację o nazwie **b2c-extensions-app**, wybierz opcję **Usuń**, a następnie wybierz pozycję **tak** po wyświetleniu monitu.
1. W obszarze **Zarządzaj**, wybierz opcję **ustawienia użytkownika**.
1. W obszarze **połączenia konta usługi LinkedIn**, wybierz opcję **nie**, a następnie wybierz **Zapisz**.
1. W obszarze **Zarządzaj**, wybierz opcję **właściwości**
1. W obszarze **Access management dla zasobów platformy Azure**, wybierz opcję **tak**, a następnie wybierz pozycję **Zapisz**.
1. Wyloguj się z witryny Azure portal, a następnie zaloguj się ponownie do odświeżania dostęp do usługi.
1. Wybierz **usługi Azure Active Directory** w menu po lewej stronie.
1. Na **Przegląd** wybierz opcję **usunąć katalog**. Postępuj zgodnie z wyświetlanymi instrukcjami, aby ukończyć proces.
