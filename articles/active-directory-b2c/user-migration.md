...
![Część pliku UsersData. JSON przedstawiająca bloki JSON dwóch użytkowników](./media/user-migration/pre-migration-data-file.png)
...
![Okno wiersza polecenia przedstawiające dane wyjściowe polecenia UserMigration. exe](./media/user-migration/pre-migration-demo.png)
...
- Aby pobrać użytkownika przy użyciu adresu e-mail logowania, użyj przykładowej aplikacji:
   ...
   1. Otwórz plik UserProfile. JSON w edytorze JSON, aby wyświetlić informacje o użytkowniku.
      ...
      ![Plik UserProfile. JSON jest otwarty w edytorze Visual Studio Code](./media/user-migration/pre-migration-get-by-email2.png)
...
Aby uzyskać link do zasad resetowania hasła, wykonaj następujące kroki. W tej procedurze przyjęto założenie, że wcześniej utworzono [zasady niestandardowe](custom-policy-get-started.md)resetowania hasła.
...
1. Skopiuj adres URL wyświetlany w polu tekstowym **Uruchom teraz punkt końcowy** , a następnie wyślij go do użytkowników.
    ...
    ![Strona zasad resetowania hasła z wyróżnionym punktem końcowym Uruchom teraz](./media/user-migration/pre-migration-policy-uri.png)
...
Gdy użytkownicy próbują zalogować się bez wcześniejszego resetowania hasła, zasady powinny zwrócić przyjazny komunikat o błędzie. Przykład:
...
1. Wprowadź poświadczenia jednego z zmigrowanych użytkowników, a następnie wybierz pozycję **Zaloguj się**. Interfejs API REST powinien zgłosić następujący komunikat o błędzie:
    ...
    ![Strona rejestracji logowania z komunikatem o błędzie zmiany hasła](./media/user-migration/pre-migration-error-message.png)
...
1. Wybierz pozycję **Zapisz**.
    ...
    ![Strona konfiguracji dzienników diagnostycznych w Azure Portal](./media/user-migration/pre-migration-diagnostic-logs.png)
