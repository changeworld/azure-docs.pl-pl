...
| rolling_refresh_token_lifetime_secs | Nie | Jeśli ustawiono `true`, token odświeżania oknem kroczącym okres istnienia nigdy nie wygasa. |
...
| issuer_refresh_token_key | Yes | X509 certyfikat (zestawu kluczy RSA) ma być używany do szyfrowania tokenu odświeżania. Możesz skonfigurować `B2C_1A_TokenEncryptionKeyContainer` w [wprowadzenie do zasad niestandardowych](active-directory-b2c-get-started-custom.md) |
