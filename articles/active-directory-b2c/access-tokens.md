...
Jeśli parametr **response_type** w żądaniu `/authorize` zawiera `token`, parametr **SCOPE** musi zawierać co najmniej jeden zakres zasobów inny niż `openid` i `offline_access`, który zostanie przyznany. W przeciwnym razie żądanie `/authorize` zakończy się niepowodzeniem.

## <a name="request-a-token"></a>Żądanie tokenu

Aby zażądać tokenu dostępu, musisz mieć kod autoryzacji. Poniżej znajduje się przykład żądania do punktu końcowego `/authorize` dla kodu autoryzacji. Domeny niestandardowe nie są obsługiwane w przypadku tokenów dostępu. Użyj domeny tenant-name.onmicrosoft.com w adresie URL żądania.
...
W przypadku użycia https://jwt.ms do sprawdzenia zwróconego tokenu dostępu powinien zostać wyświetlony komunikat podobny do następującego:
