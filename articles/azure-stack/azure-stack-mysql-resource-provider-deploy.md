...
- **Serwery obsługujące serwer MySQL**, które zapewniają pojemność baz danych, które są nazywane serwerów hosta. Należy utworzyć wystąpień MySQL, samodzielnie lub zapewniają dostęp do zewnętrznych wystąpień programu SQL. Odwiedź stronę [galerii Szybki Start Azure stosu](https://github.com/Azure/AzureStack-QuickStart-Templates/tree/master/mysql-standalone-server-windows) przykład szablonu, który można: 
    - Tworzy serwer MySQL. 
...
- Baza danych: Utwórz, Porzuć 
- Logowania: Tworzenie, ustawić, porzucić, przydzielić, odwołać 
...
6. Uruchom **DeployMySqlProvider.ps1** skryptu. Skrypt wykonuje następujące czynności: 
    ...
    - Wdraża używane maszyny Wirtualnej systemu Windows Server 2016 Azure stosu obrazu z witryny marketplace, a następnie instaluje dostawcę zasobów. 
...
2. Wybierz **zasobów administracyjnych** > **MySQL serwerów hosta** > **+ Dodaj**. Na **Hosting serwerów MySQL** bloku można nawiązać dostawcy zasobów serwer MySQL bieżące wystąpienia serwera MySQL służące jako wewnętrzna dostawcy zasobów. 
...
4. W miarę dodawania serwerów, należy je przypisać do nowej lub istniejącej jednostki SKU umożliwiają rozróżnienie ofert usług. Na przykład można mieć wystąpieniem przedsiębiorstwa dostarczanie: 
...
## <a name="update-the-mysql-resource-provider-adapter-integrated-systems-only"></a>Zaktualizuj kartę Dostawca zasobów MySQL (tylko systemy zintegrowanym)
...
Problem: W dziennikach rotacją kluczy tajnych nie są automatycznie zbierane Jeśli skrypt zakończy się niepowodzeniem, po jego uruchomieniu. 
 
Obejście problemu: Należy użyć polecenia cmdlet Get-AzsDBAdapterLogs zbierać wszystkie dzienniki dostawcy zasobów, w tym AzureStack.DatabaseAdapter.SecretRotation.ps1_*.log, w obszarze C:\Logs. 
...
7. Uruchom ponownie skrypt wdrażania, korzystając z następujących parametrów: 
