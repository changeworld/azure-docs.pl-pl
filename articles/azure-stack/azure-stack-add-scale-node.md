...
2. Skonfiguruj prawidłowego adresu IP w kontroler zarządzania płytą główną (BMC) i Zastosuj wszystkie ustawienia systemu BIOS na dokumentacji dostarczonego przez producenta OEM.
3. Zastosuj bieżącą bazową oprogramowania układowego dla wszystkich składników przy użyciu narzędzia, które zostały udostępnione przez producenta sprzętu i uruchamianych na HLH.
4. Uruchamianie operacji dodawania węzła w portalu administracyjnym usługi Azure Stack.
5. Sprawdź, czy operacji dodawania node zakończy się pomyślnie. Aby to zrobić, należy sprawdzić [ **stan** jednostki skalowania](#monitor-add-node-operations). 

## <a name="add-the-node"></a>Dodaj węzeł 
Aby dodać nowe węzły, można użyć konsoli administracyjnej lub programu PowerShell. Operacja węźle Dodaj najpierw dodaje nowy węzeł jednostki skalowania jako możliwości obliczeniowych dostępnych i automatycznie rozszerza pojemność magazynu. Pojemność automatycznie rozszerza się, ponieważ usługa Azure Stack to system hiperzbieżnego gdzie *obliczenia* i *magazynu* skalowane razem.

### <a name="use-the-admin-console"></a>Za pomocą konsoli administratora
1. Zaloguj się do portalu administracyjnego usługi Azure Stack jako operatorów usługi Azure Stack.
2. Przejdź do **zarządzanie regionami** > **jednostek skalowania**, a następnie wybierz jednostkę skalowania, która ma być zwiększenia wydajności.  
   ![Wybierz jednostkę skalowania](media/azure-stack-add-scale-node/select-node1.png)

   ![Wyświetl szczegóły jednostki skalowania](media/azure-stack-add-scale-node/select-node2.png)
 
3. Wymagane są następujące parametry:  
   - **ADRES IP kontrolera BMC** dla każdego węzła jednostki skalowania, możesz dodać. (Jeden adres IP dla każdego wiersza.) ![Dodaj węzeł](media/azure-stack-add-scale-node/add-node.png)

### <a name="use-powershell"></a>Korzystanie z programu PowerShell

Użyj **New AzsScaleUnitNodeObject** polecenia cmdlet, aby dodać jeden lub więcej węzłów.  
...
## <a name="monitor-add-node-operations"></a>Monitorowanie operacji Dodaj węzeł 
...
Przegląd [węzła akcji](azure-stack-node-actions.md) 
