---
title: Tworzenie i przekazywanie Red Hat Enterprise Linux wirtualnego dysku twardego do użycia w stosie Azure | Dokumentacja firmy Microsoft
...
ms.locfileid: "34076619"
---
# <a name="prepare-a-red-hat-based-virtual-machine-for-azure-stack"></a>Przygotowywanie maszyny wirtualnej z systemem Red Hat Azure stosu
W tym artykule dowiesz się, jak przygotować maszyny wirtualnej Red Hat Enterprise Linux (RHEL) do użycia w stosie Azure. Wersje RHEL, które zostały omówione w tym artykule są 7.1 +. Funkcje hypervisor w celu przygotowania, które zostały omówione w tym artykule są maszyny wirtualnej funkcji Hyper-V, na podstawie jądra (KVM) i VMware.
...
* Stos Azure nie obsługuje formatu VHDX. Azure obsługuje tylko stałe wirtualnego dysku twardego. Menedżer funkcji Hyper-V umożliwia Konwertuj dysk na format wirtualnego dysku twardego, lub można użyć polecenia cmdlet convert-vhd. Jeśli używasz VirtualBox, wybierz **stały rozmiar** zamiast domyślnego dynamicznie przydzielane opcji podczas tworzenia dysku.
* Stos Azure obsługuje tylko maszyny wirtualne generacji 1. Maszyny wirtualne generacji 1 można przekonwertować z VHDX do formatu pliku wirtualnego dysku twardego i dynamicznie powiększające się na dysku o stałym rozmiarze. Nie można zmienić generację maszyny wirtualnej. Aby uzyskać więcej informacji, zobacz [należy tworzyć maszyny wirtualne generacji 1 lub 2 w funkcji Hyper-V?](https://technet.microsoft.com/windows-server-docs/compute/hyper-v/plan/should-i-create-a-generation-1-or-2-virtual-machine-in-hyper-v).
...
* Wszystkie wirtualne dyski twarde na platformie Azure muszą mieć rozmiar wirtualny wyrównany do 1MB. Podczas konwersji z pierwotnych dysku VHD musi upewnij się, że rozmiar dysku pierwotnych jest wielokrotnością liczby 1MB przed konwersją. Więcej szczegółów można znaleźć w poniższych krokach.
* Stos Azure nie obsługuje inicjowania chmury. Użytkownik maszyny Wirtualnej musi być skonfigurowany z obsługiwaną wersją systemu Windows Azure Linux Agent (WALA).
...
Teraz możesz używać wirtualnego dysku twardego Red Hat Enterprise Linux do tworzenia nowych maszyn wirtualnych Azure stosu. Jeśli jest to, że przesyłana pliku VHD stosu Azure po raz pierwszy, zobacz [Marketplace toolkit umożliwia tworzenie i publikowanie elementów marketplace](azure-stack-marketplace-publisher.md).
