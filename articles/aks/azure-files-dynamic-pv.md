...
Podczas dynamicznego tworzenia udziału plików na platformę Azure jako wolumin Kubernetes, dowolne konto magazynu może służyć tak długo, jak jest AKS **węzła** grupy zasobów. Jest to jeden z `MC_` prefiks, który został utworzony przez Inicjowanie obsługi zasobów klastra AKS. Pobierz nazwę grupy zasobów z [Pokaż zasobów az] [ az-resource-show] polecenia.
...
> Usługa pliki Azure tylko obecnie współpracujesz z magazynu w warstwie standardowa. Jeśli używasz magazyn w warstwie premium woluminu zakończy się niepowodzeniem, aby udostępnić.
...
Utwórz plik o nazwie `azure-file-sc.yaml` i skopiuj następujące manifestu. Aktualizacja `storageAccount` z nazwą konta magazynu docelowego. Zobacz sekcję [opcje instalacji] Aby uzyskać więcej informacji o `mountOptions`.
