...
## <a name="why-can-i-not-set-maxpods-below-30"></a>Dlaczego I nie można ustawić `maxPods` poniżej 30?

AKS obsługuje ustawienie `maxPods` wartości w czasie tworzenia klastra za pomocą szablonów interfejsu wiersza polecenia platformy Azure i usługi Azure Resource Manager. Istnieje jednak *minimalną wartość* (zweryfikowany w czasie tworzenia) dla wtyczki Kubenet i wtyczki Azure CNI pokazano poniżej:
...
Jak AKS to zarządzana usługa, firma Microsoft udostępnia dodatków i zasobników, możemy wdrożyć i zarządzania nimi jako część klastra. W przeszłości, użytkownicy mogą definiować `maxPods` wartości mniejszej niż wartość wymaganą dla zarządzanych zasobników uruchomić (przykład: 30) AKS teraz oblicza minimalną liczbę zasobników za pośrednictwem: ((maxPods lub (maxPods * vm_count)) > minimum zasobników zarządzanych dodatku.

Użytkownicy nie mogą zastępować minimum `maxPods` sprawdzania poprawności.
