...
| eTag | string | Wartość, której można użyć do warunkowego wykonywania operacji. |
...
| url | sring | Ścieżka do obiektu BLOB. <br>Jeśli klient używa interfejsu API REST usługi BLOB, ten adres URL ma strukturę:  *\<Storage-account-name\>.\<BLOB.Core.Windows.NET/Container-\>/\<name\>* . <br>Jeśli klient używa interfejsu API REST Data Lake Storage, ten adres URL ma następującą strukturę:  *\<Storage-account-name\<\>\>./\<DFS.Core.Windows.NET/File-Name File-Name \>* . |
