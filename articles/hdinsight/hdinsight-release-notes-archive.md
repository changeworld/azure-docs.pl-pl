---
title: Informacje o starszych wersjach dla usługi Azure HDInsight
...
- **Aktualizowanie Apache Hadoop i innych projektów typu open-source** — oprócz ponad 1000 poprawki w projektach typu open-source ponad 20, ta aktualizacja zawiera nową wersję platformy Apache Spark (2.3) i platformy Apache Kafka (1.0).
...
| Aktualizacje platformy Spark | Wersja platformy Spark 2.1.1. Aby uzyskać więcej informacji, zobacz [Apache Spark wersji 2.1.1](https://spark.apache.org/releases/spark-release-2-1-1.html). | Usługa | platforma Spark | |
...
* **Obsługa widoku Hive w wersji 2.0**. Należy to poprawić środowisko użytkownika Interactive Hive. Aby uzyskać więcej informacji, zobacz [dokumentacji Hortonworks](https://docs.hortonworks.com/HDPDocuments/Ambari-2.5.0.3/bk_ambari-views/content/ch_using_hive_view.html).
...
* **Gałąź obsługi interfejsu wiersza polecenia**: Gałąź interfejsu wiersza polecenia jest wycofywany, a klientów zachęca się na korzystanie z usługi Beeline zamiast tego. Aby uzyskać więcej informacji, zobacz [dokumentacji oprogramowania Apache](https://cwiki.apache.org/confluence/display/Hive/Replacing+the+Implementation+of+Hive+CLI+Using+Beeline). Aby uzyskać instrukcje na temat korzystania z usługi Beeline z HDInsight, zobacz [korzystać z usługi Beeline przy użyciu usługi HDInsight Hadoop clusters](hadoop/apache-hadoop-use-hive-beeline.md).

* **Nowe funkcje w Apache Phoenix i bazy danych HBase**.
    * Obsługa limitu przydziału pamięci masowej: Często używane w środowiskach wielodostępnych, dzięki czemu ograniczoną ilość miejsca na każdej tabeli i na poziomie przestrzeni nazw.
    * Phoenix indeksowania ulepszenia: Tworzenie indeksu przyrostowych i ponownej kompilacji/wznowień indeksowanie z wcześniejszych niepowodzeń.
    * Phoenix narzędzia integralności danych: Obsługuje sprawdzanie poprawności schematu, indeksów i innych metadanych.


* **Problem z bazą danych HBase**: Podczas uruchamiania przekazywania masowego CSV zadania MapReduce, następująca składnia może spowodować błąd.
...
* Partycjonowanie Spark SQL są teraz obsługiwane za pomocą nowego mechanizmu obsługi skalowalnych partycji. Zobacz więcej szczegółów [tutaj](https://spark.apache.org/releases/spark-release-2-1-0.html) dotyczącymi uaktualniania.
...
* Dostęp do magazynu usługi Azure Data Lake z klastrami Spark 2.1 nie jest obsługiwana w tej wersji zapoznawczej.
