...
1. Skonfiguruj ustawienia klastra Spark, wykonując następujące czynności: 
    ...
    1. Ustaw `spark.security.credentials.hiveserver2.enabled` na`false` dla trybu wdrażania klienta przędzenia.
    1. Ustaw `spark.hadoop.hive.zookeeper.quorum` na dozorcy kworum klastra LLAP. Aby znaleźć kworum dozorcy dla klastra LLAP, Wyszukaj Właściwość **Hive. dozorcy. kworum** w interfejsie użytkownika Ambari dla klastra LLAP w obszarze**zaawansowana** > **zaawansowana witryna**Hive programu **Hive** > . Wartość będzie wyglądać podobnie do następującego ciągu:
...
Aby przetestować konfigurację łącznika magazynu Hive, wykonaj kroki opisane w sekcji [łączenie i uruchamianie zapytań](#connecting-and-running-queries).
...
1. Węzła głównego dla klastra za pośrednictwem protokołu SSH. Aby uzyskać więcej informacji na temat nawiązywania połączenia z klastrem przy użyciu protokołu SSH, zobacz [nawiązywanie połączenia z usługą HDInsight (Apache Hadoop) przy użyciu protokołu SSH](../../hdinsight/hdinsight-hadoop-linux-use-ssh-unix.md).
1. Przejdź do poprawnego katalogu, `cd /usr/hdp/current/hive_warehouse_connector` wpisując lub podaj pełną ścieżkę do wszystkich plików jar używanych jako parametry w poleceniu Spark-Shell.
...
1. Zobaczysz komunikat powitalny i `scala>` monit, gdzie można wprowadzać polecenia.
...
1. Postępuj zgodnie z początkowymi krokami 1 i 2 w obszarze [łączenie i uruchamianie zapytań](#connecting-and-running-queries).
...
1. Zapisz dane przesyłane strumieniowo do nowo utworzonej tabeli przy użyciu następującego polecenia:
...
1. Dane wstawione do tabeli można wyświetlić za pomocą następującego polecenia:
