...
1. Aby uzyskać pełny adres domyślny magazyn, należy użyć następującego polecenia. Ten adres jest używany w pliku konfiguracji, który zostanie utworzony w następnym kroku.
    ...
    > [!NOTE]  
    > Jeśli klaster HDInsight używa usługi Azure Storage jako magazynu domyślnego `<value>` zawartość elementu zaczynają się od `wasb://`. Jeśli usługi Azure Data Lake Storage Gen1 jest używana zamiast tego, rozpoczyna się od `adl://`. Użycie usługi Azure Data Lake Storage Gen2 zaczyna się od `abfs://`.
...
* [Za pomocą usług oparte na czasie koordynatora programu Oozie Apache HDInsight][hdinsight-oozie-coordinator-time]
