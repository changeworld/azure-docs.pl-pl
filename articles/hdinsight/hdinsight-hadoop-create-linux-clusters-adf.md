...
3. Wybierz nazwę grupy zasobów utworzoną w skrypcie programu PowerShell. Użyj filtru, jeśli na liście znajduje się zbyt wiele grup zasobów.
4. Na kafelku **zasoby** zostanie wyświetlony jeden zasób na liście, o ile nie zostanie udostępniona Grupa zasobów z innymi projektami. Ten zasób jest kontem magazynu o podanej wcześniej nazwie. Wybierz nazwę konta magazynu.
5. Wybierz kafelki **obiektów BLOB** .
6. Wybierz kontener **adfgetstarted** . Zobaczysz folder o nazwie **hivescripts**.
...
2. W menu po lewej stronie przejdź do **+ Utwórz** > **Data Factory** **analizy** > zasobów.
...
3. Wprowadź lub wybierz następujące wartości dla nowego kafelka usługi **Data Factory** :
    ...
    |Właściwość  |Value  |
    |---------|---------|
    |Name | Wprowadź nazwę fabryki danych. Ta nazwa musi być unikatowa w skali globalnej.|
    |Subscription | Wybierz swoją subskrypcję platformy Azure. |
    |Resource group | Wybierz pozycję **Użyj istniejącej** , a następnie wybierz grupę zasobów utworzoną za pomocą skryptu programu PowerShell. |
    |Version | Pozostaw w **wersji 2**. |
    |Location | Lokalizacja jest automatycznie ustawiana na lokalizację określoną podczas tworzenia grupy zasobów. W tym samouczku lokalizacja jest ustawiona na **Wschodnie stany USA**. |

    ![Tworzenie Azure Data Factory przy użyciu Azure Portal](./media/hdinsight-hadoop-create-linux-clusters-adf/create-data-factory-portal.png "Tworzenie Azure Data Factory przy użyciu Azure Portal")
...
6. Wybierz pozycję **utwórz & monitor** , aby uruchomić Azure Data Factory Portal tworzenia i monitorowania.
    ...
    ![Przegląd Azure Data Factory](./media/hdinsight-hadoop-create-linux-clusters-adf/data-factory-portal-overview.png "Przegląd Azure Data Factory")
...
4. W oknie **Nowa połączona usługa** wprowadź następujące wartości i pozostaw resztę jako domyślną:
    ...
    | Wybierz grupę zasobów | Wybierz grupę zasobów utworzoną w ramach użytego wcześniej skryptu programu PowerShell.|
    ...
    | Typ systemu operacyjnego/nazwa użytkownika klastra | Wprowadź nazwę `admin`użytkownika klastra. |
    | Typ systemu operacyjnego/hasło użytkownika klastra | Podaj hasło dla użytkownika klastra. |
...
3. Upewnij się, że wybrano działanie Hive, wybierz kartę **klaster HDI** , a następnie z listy rozwijanej **połączona Usługa HDInsight** wybierz połączoną usługę utworzoną wcześniej, **HDinightLinkedService**, dla usługi HDInsight.
