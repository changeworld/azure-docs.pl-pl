...
- **Tryb alokacji puli** Podczas tworzenia konta usługi Batch można wybrać jeden z dwóch trybów alokacji puli: **usługa wsadowa** lub **subskrypcja użytkownika**. W większości przypadków należy użyć domyślnego trybu usługi batch, w którym pule są przydzielane za kulisami w subskrypcjach zarządzanych przez partię. W alternatywnym trybie subskrypcji użytkownika maszyny wirtualne i inne zasoby usługi Batch są tworzone bezpośrednio w Twojej subskrypcji po utworzeniu puli. Konta subskrypcji użytkowników są używane przede wszystkim do włączania ważnego, ale mały podzbiór scenariuszy. Więcej informacji o trybie subskrypcji użytkownika można przeczytać w sekcji [Dodatkowa konfiguracja dla trybu subskrypcji użytkownika](batch-account-create-portal.md#additional-configuration-for-user-subscription-mode).
...
- **Ponowne tworzenie puli** W podobnym tonie nie zaleca się codziennego usuwania i tworzenia pul. Zamiast tego utwórz nową pulę, zaktualizuj istniejące zadania, aby wskazywały nową pulę. Po przeniesieniu wszystkich zadań do nowej puli usuń starą pulę.

- **Wydajność puli i rozliczenia** Sama partia nie wiąże się z dodatkowymi opłatami, ale opłaty są naliczane za użyte zasoby obliczeniowe. Naliczane są naliczane płatności za każdy węzeł obliczeniowy w puli, niezależnie od stanu, w jakiej się znajduje. Obejmuje to wszelkie opłaty wymagane do uruchomienia węzła, takie jak koszty magazynu i sieci. Aby poznać więcej najlepszych rozwiązań, zobacz [Analiza kosztów i budżety usługi Azure Batch](budget.md).
...
- **Umieszczanie wielu zadań w zadaniu** Używanie zadania do uruchamiania pojedynczego zadania jest nieefektywne. Na przykład bardziej efektywne jest użycie pojedynczego zadania zawierającego 1000 zadań, zamiast tworzenia 100 zadań zawierających po 10 zadań. Uruchamianie 1000 zadań, z których każde z jednym zadaniem, byłoby najmniej wydajnym, najwolniejszym i najdroższym podejściem do podjęcia.
...
- **Okres istnienia zadania** Zadanie wsadowe ma nieokreślony okres istnienia, dopóki nie zostanie usunięty z systemu. Stan zadania określa, czy może akceptować więcej zadań do planowania, czy nie. Zadanie nie jest automatycznie przesunięty do stanu ukończone, chyba że jawnie zakończone. Może to być automatycznie wyzwalane przez [onAllTasksComplete](https://docs.microsoft.com/dotnet/api/microsoft.azure.batch.common.onalltaskscomplete?view=azure-dotnet) właściwości lub [maxWallClockTime](https://docs.microsoft.com/rest/api/batchservice/job/add#jobconstraints).
...
- **Unikanie tworzenia połączeń katalogów w systemie Windows** Połączenia katalogowe, czasami nazywane twardymi łączami katalogu, są trudne do rozwiązania podczas oczyszczania zadań i zadań. Użyj dowiązań symbolicznych (linków miękkich), a nie linków twardych.

- **Zbieranie dzienników agenta wsadowego, jeśli występuje problem** Jeśli zauważysz problem związany z zachowaniem węzła lub zadań uruchomionych w węźle, zaleca się zbieranie dzienników agenta usługi Batch przed przydzieleniem danych węzłów. Dzienniki agenta usługi Batch mogą być zbierane przy użyciu interfejsu API dzienników usługi przekazywania wsadowego. Te dzienniki mogą być dostarczane jako część biletu pomocy technicznej do firmy Microsoft i pomoże w rozwiązywaniu problemów i rozwiązywaniu problemów.
...
## <a name="connectivity-to-the-batch-service"></a>Łączność z usługą wsadową

### <a name="network-security-groups-nsgs-and-user-defined-routes-udrs"></a>Sieciowe grupy zabezpieczeń (NSG) i trasy zdefiniowane przez użytkownika (UDR)

Podczas inicjowania obsługi [administracyjnej pule usługi Batch w sieci wirtualnej](batch-virtual-network.md)upewnij się, że ściśle przestrzegasz wytycznych dotyczących używania tagu `BatchNodeManagement` usługi, portów, protokołów i kierunku reguły.
Użycie tagu usługi jest wysoce zalecane, a nie podstawowe adresy IP usługi Batch, ponieważ mogą się zmieniać w czasie. Korzystanie z adresów IP usługi Batch bezpośrednio może objawiać się jako niestabilność, przerwy lub awarie dla pul partii, jak usługa Batch aktualizuje adresy IP używane w czasie. Jeśli obecnie używasz adresów IP usługi batch w regułach sieciowej listy adresów sieciowych, zaleca się przełączenie się na używanie tagu usługi.

W przypadku tras zdefiniowanych przez użytkownika upewnij się, że w tabeli adresów IP usługi wsadowej jest okresowo aktualizowany adresy IP usługi wsadowej, ponieważ zmieniają się one w czasie. Aby dowiedzieć się, jak uzyskać listę adresów IP usługi wsadowej, zobacz [Tagi usługi lokalnie](../virtual-network/service-tags-overview.md). Adresy IP usługi wsadowej `BatchNodeManagement` będą skojarzone z tagiem usługi (lub wariantem regionalnym, który pasuje do regionu konta usługi Batch).

### <a name="honoring-dns"></a>Honorowanie systemu DNS

Upewnij się, że systemy są honorujące DNS Time-to-Live (TTL) dla adresu URL usługi konta usługi usługi batch. Ponadto upewnij się, że klienci usługi Batch i inne mechanizmy łączności z usługą Batch nie są zależne od adresów IP.
...
### <a name="retrying-requests-automatically"></a>Automatyczne ponawianie żądań

Upewnij się, że klienci usługi Batch mają odpowiednie zasady ponawiania próby w miejscu, aby automatycznie ponowić próbę żądania, nawet podczas normalnej pracy, a nie wyłącznie w okresach konserwacji usługi. Te zasady ponawiania próby powinny obejmować interwał co najmniej 5 minut. Funkcje automatycznego ponawiania prób są dostarczane z różnymi plikami SDK partii, takimi jak [klasa .NET RetryPolicyProvider](https://docs.microsoft.com/dotnet/api/microsoft.azure.batch.retrypolicyprovider?view=azure-dotnet).
