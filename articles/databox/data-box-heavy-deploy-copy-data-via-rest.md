...
W tym samouczku opisano procedury łączenia się z magazynem obiektów blob usługi Azure Data Box za pośrednictwem protokołu *HTTP* lub *HTTPS* przy użyciu interfejsów API REST. Po nawiązaniu połączenia opisano kroki wymagane do kopiowania danych do magazynu obiektów Blob pola danych.
...
4. Masz dostęp do komputera hosta, zawierającej dane, które mają do skopiowania do duże pole danych. Na komputerze hosta wymagane jest:
...
> [!IMPORTANT]
> Dla danych pole bardzo duże należy powtórzyć dane instrukcje połączenia do łączenia się z drugim węzłem.
...
-  Podczas kopiowania danych, upewnij się, że rozmiar danych jest zgodny ze limity rozmiaru opisanego w [usługi Azure storage i dane pole duże limity](data-box-limits.md).
- Jeśli dane, które jest przekazywany przez duże pole danych, jednocześnie jest przekazywany przez inne aplikacje poza duże pole danych, może to spowodować przekazywania zadania awarii i uszkodzenia danych.
...
> [!IMPORTANT]
> Dla danych pole bardzo duże należy powtórzyć dane instrukcje kopiowania do skopiowania danych do drugiego węzła.
