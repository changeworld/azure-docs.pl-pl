...
Podczas tworzenia serwera za pomocą polecenia `az postgres server create`, parametr `--geo-redundant-backup` decyduje o opcji nadmiarowości kopii zapasowej. W przypadku `Enabled`są pobierane geograficznie nadmiarowe kopie zapasowe. Lub jeśli `Disabled` lokalnie nadmiarowe kopie zapasowe są wykonywane. 

Okres przechowywania kopii zapasowych jest ustawiany przez `--backup-retention-days`parametru. 
...
Aby utworzyć serwer przy użyciu geograficznie nadmiarowej kopii zapasowej, użyj interfejsu wiersza polecenia platformy Azure `az postgres server georestore`.
