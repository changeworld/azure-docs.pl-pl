...
## <a name="resource-manager-template-for-metric-alert-that-monitors-multiple-resources"></a>Szablon usługi Resource Manager alert dotyczący metryki, które monitoruje wiele zasobów

W poprzedniej sekcji opisano przykładowe szablony usługi Azure Resource Manager w celu tworzenia alertów dotyczących metryk, które monitorują pojedynczy zasób. Usługa Azure Monitor teraz obsługuje monitorowania wielu zasobów przy użyciu jednej reguły alertów metryk. Tę funkcję wersji zapoznawczej jest obecnie dostępna wyłącznie za pośrednictwem usługi Azure Resource Manager i interfejsu API REST i jest obsługiwany tylko w przypadku maszyn wirtualnych. Wkrótce zostanie udostępniona Obsługa portalu do tworzenia i edytowania tych zasad.
...
### <a name="all-virtual-machines-in-one-or-more-resource-groups"></a>Wszystkie maszyny wirtualne w co najmniej jedną grupę zasobów

Ten szablon utworzy regułę alertu metryki, który monitoruje procentowe użycie procesora CPU dla wszystkich maszyn wirtualnych (w jednym regionie platformy Azure) w co najmniej jedną grupę zasobów.

Zapisz poniższe dane json jako wszystkie — maszyn wirtualnych w resource-group.json na potrzeby tego przewodnika.
...
Zapisz i zmodyfikować poniższe dane json, jeśli wszystkie — maszyn wirtualnych w resource-group.parameters.json na potrzeby ten przewodnik.
...
### <a name="all-virtual-machines-in-a-subscription"></a>Wszystkie maszyny wirtualne w ramach subskrypcji

Ten szablon utworzy regułę alertu metryki, który monitoruje procentowe użycie procesora CPU dla wszystkich maszyn wirtualnych (w jednym regionie platformy Azure) w ramach subskrypcji.

Zapisz poniższe dane json jako all-vms w subscription.json na potrzeby tego przewodnika.
...
Używając powyższego szablonu przy użyciu pliku parametrów poniżej.
Zapisz i zmodyfikować poniższe dane json, jeśli wszystkie vms w subscription.parameters.json na potrzeby ten przewodnik.
...
### <a name="a-list-of-virtual-machines"></a>Listę maszyn wirtualnych

Ten szablon utworzy regułę alertu metryki, który monitoruje procentowe użycie procesora CPU, aby uzyskać listę maszyn wirtualnych (w jednym regionie platformy Azure) w ramach subskrypcji.

Zapisz poniższe dane json jako listy z vms.json na potrzeby tego przewodnika.
...
Używając powyższego szablonu przy użyciu pliku parametrów poniżej.
Zapisz i zmodyfikuj je zgodnie z listy z vms.parameters.json na potrzeby ten przewodnik poniższe dane json.
...
* Przeczytaj więcej na temat [alertów na platformie Azure](monitoring-overview-alerts.md)
