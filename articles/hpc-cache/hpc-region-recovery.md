---
title: Regionalna nadmiarowość i odzyskiwanie do trybu failover za pomocą usługi Azure HPC cache (wersja zapoznawcza)
...
1. Planując utworzenie wystąpienia podstawowej pamięci podręcznej, należy również przygotować się do replikowania tego procesu konfiguracji w drugim regionie. Uwzględnij następujące elementy:
   ...
   > [!NOTE]
   > Pamięć podręczną platformy Azure HPC można utworzyć programowo przy użyciu [szablonu Azure Resource Manager](../azure-resource-manager/template-deployment-overview.md) lub przez bezpośredni dostęp do jego interfejsu API. Aby uzyskać szczegółowe informacje, skontaktuj się z zespołem usługi Azure HPC cache.
