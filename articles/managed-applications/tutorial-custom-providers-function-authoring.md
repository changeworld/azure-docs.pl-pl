---
title: Tworzenie punktu końcowego RESTful dla dostawców niestandardowych
...
- **UMIEŚĆ**: Utwórz nowy zasób
- **Pobierz (wystąpienie)** : Pobieranie istniejącego zasobu
...
Zaktualizowana Metoda **Run** obejmuje teraz powiązanie danych wejściowych *TableStorage* dodane do usługi Azure Table Storage. Pierwsza część metody odczytuje `x-ms-customproviders-requestpath` nagłówek i `Microsoft.Azure.Management.ResourceManager.Fluent` używa biblioteki, aby przeanalizować wartość jako identyfikator zasobu. `x-ms-customproviders-requestpath` Nagłówek jest wysyłany przez niestandardowego dostawcę i określa ścieżkę żądania przychodzącego.
