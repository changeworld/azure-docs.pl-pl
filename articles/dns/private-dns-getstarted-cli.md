...
description: W tym przewodniku szybki start utworzysz i testujesz prywatną strefę DNS i rekord w Azure DNS. Jest to przewodnik krok po kroku dotyczący tworzenia pierwszej prywatnej strefy i rekordu DNS przy użyciu interfejsu wiersza polecenia platformy Azure oraz zarządzania nim.
services: dns
author: vhorne
ms.service: dns
ms.topic: quickstart
...
Aby wyliczyć strefy DNS, użyj `az network private-dns zone list`. Aby uzyskać pomoc, zobacz `az network dns zone list --help`.
...
Aby utworzyć rekord DNS, użyj polecenia `az network private-dns record-set [record type] add-record`. Aby uzyskać pomoc dotyczącą dodawania rekordów na przykład, zobacz `az network private-dns record-set A add-record --help`.
