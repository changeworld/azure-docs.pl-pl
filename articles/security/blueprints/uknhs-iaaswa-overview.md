---
title: Zabezpieczenia platformy Azure i zgodności planu — aplikacja sieci Web IaaS dla NHS Zjednoczone Królestwo
description: Zabezpieczenia platformy Azure i zgodności planu — aplikacja sieci Web IaaS dla NHS Zjednoczone Królestwo
services: security
author: jomolesk
...
Aby zwiększyć bezpieczeństwo wszystkie zasoby w tym rozwiązaniu są zarządzane jako grupa zasobów za pomocą usługi Azure Resource Manager. Usługa Azure Active Directory, kontrola dostępu oparta na rolach jest używana do kontrolowania dostępu do wdrożonych zasobów, w tym do kluczy w usłudze Azure Key Vault. Kondycja systemu jest monitorowany za pośrednictwem usługi Azure Security Center i Azure Monitor. Klienci, skonfigurować zarówno usług monitorowania do przechwytywania dzienników i wyświetlania kondycji systemu w jednym, łatwo można nawigować pulpitu nawigacyjnego. Usługa Azure Application Gateway jest konfigurowana jako zapora w trybie zapobiegania i nie zezwalają na ruch, który nie jest zabezpieczeń TLSv1.2.
...
![Aplikacja internetowa IaaS dla Wielkiej Brytanii NHS referencyjny diagram architektury](images/uknhs-iaaswa-architecture.png?raw=true "aplikacja internetowa IaaS dla Wielkiej Brytanii NHS referencyjny diagram architektury")
...
**Monitor w chmurze**: [monitora w chmurze](https://docs.microsoft.com/windows-server/failover-clustering/whats-new-in-failover-clustering#BKMK_CloudWitness) to typ monitora kworum klastra trybu Failover w systemie Windows Server 2016, która korzysta z platformy Azure jako punktu rozstrzygania. Monitor w chmurze, takich jak każdy inny monitor kworum ma głos i można uczestniczyć w obliczeniach kworum, ale używa standardowych publicznej usługi Azure Blob Storage. Eliminuje to obciążenie obsługi dodatkowych machiness wirtualnych hostowanych w chmurze publicznej.
...
Diagram przepływu danych dla tej architektury referencyjnej jest dostępne dla [Pobierz](https://aka.ms/uknhs-iaaswa-tm) lub znajdują się poniżej. Ten model może pomóc klientom w zrozumieniu punkty potencjalne ryzyko w ramach infrastruktury systemu podczas wprowadzania zmian.

![Aplikacja internetowa IaaS dla Wielkiej Brytanii NHS model zagrożeń](images/uknhs-iaaswa-threat-model.png?raw=true "aplikacja internetowa IaaS dla Wielkiej Brytanii NHS model zagrożeń")
...
[Zabezpieczeń platformy Azure i plan zgodność — UK NHS IaaS sieci Web aplikacji implementacji macierzy](https://aka.ms/uknhs-iaaswa-cim) udostępnia informacje, na które NHS UK wymagania są rozwiązywane przez IaaS architektury aplikacji sieci web, w tym szczegółowe Opis sposobu implementacji spełnia wymagania dotyczące każdego z nich obejmuje zasady.
