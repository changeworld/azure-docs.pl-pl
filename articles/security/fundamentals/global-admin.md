...
Ponadto włączenie [uwierzytelniania wieloskładnikowego systemu Azure (MFA)](https://docs.microsoft.com/azure/multi-factor-authentication/multi-factor-authentication) dla administratorów powoduje dodanie drugiej warstwy zabezpieczeń do logowania i transakcji użytkownika. Usługa Azure MFA pomaga również ograniczyć prawdopodobieństwo, że naruszone poświadczenie będzie miało dostęp do danych organizacji.
...
Alternatywą dla organizacji, które chcą zachować całą kontrolę uwierzytelniania lokalnie, jest korzystanie z [serwera usługi Azure](https://docs.microsoft.com/azure/multi-factor-authentication/multi-factor-authentication-get-started-server)MFA, nazywanego również "uwierzytelnianiem lokalnym". Korzystając z tej metody, nadal będzie można wymusić uwierzytelnianie wieloskładnikowe, zachowując jednocześnie lokalny serwer usługi MFA.
...
Przejrzyj sposób działania usługi [MFA](https://docs.microsoft.com/azure/multi-factor-authentication/multi-factor-authentication-whats-next) przed kontynuowaniem.
...
Aby rozpocząć korzystanie z usługi Azure MFA, należy skorzystać z jednej z procedur wymienionych w temacie [jak wymagać weryfikacji](https://docs.microsoft.com/azure/multi-factor-authentication/multi-factor-authentication-get-started-user-states) dwuetapowej dla użytkownika lub grupy. Można wybrać opcję wymuszania weryfikacji dwuetapowej dla wszystkich logowań lub można utworzyć zasady dostępu warunkowego, aby wymagać weryfikacji dwuetapowej tylko wtedy, gdy jest to ważne.
