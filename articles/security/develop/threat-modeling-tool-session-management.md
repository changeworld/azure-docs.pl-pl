...
| **Faza SDL**               | Kompilacja |  
| **Odpowiednie technologie** | Ogólnego |
| **Attributes**              | EnvironmentType-lokalnego |
...
| **Attributes**              | EnvironmentType-lokalnego |
| **Wołują**              | ND  |
| **Kroki** | Gdy aplikacja sieci Web jest jednostką uzależnioną, a dostawcy tożsamości to serwer ADFS, bezpieczny atrybut tokenu FedAuth można skonfigurować przez ustawienie requireSSL na true w sekcji `system.identityModel.services` pliku Web. config:|
...
| **Kroki** | Formularze CSRF i ASP.NET MVC — Użyj metody pomocnika `AntiForgeryToken` w widokach; Umieść `Html.AntiForgeryToken()` w formularzu, na przykład|
...
* Te wartości plików cookie i `Request.Form` są zgodne z założeniem, że wszystkie są prawidłowo, żądanie przechodzi przez zwykły sposób. W przeciwnym razie błąd autoryzacji z komunikatem "wymagany token chroniący przed fałszerstwem nie został dostarczony lub był nieprawidłowy". 
...
| **Odpowiednie technologie** | Ogólnego |
| **Attributes**              | ND  |
| **Wołują**              | [HttpSessionState. Timeout — właściwość](https://msdn.microsoft.com/library/system.web.sessionstate.httpsessionstate.timeout(v=vs.110).aspx) |
...
| **Faza SDL**               | Kompilacja |  
| **Odpowiednie technologie** | MVC5, MVC6 |
| **Attributes**              | Dostawca tożsamości — ADFS, dostawca tożsamości — Azure AD |
