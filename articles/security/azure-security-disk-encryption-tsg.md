...
ms.locfileid: "39390725"
---
# <a name="azure-disk-encryption-troubleshooting-guide"></a>Usługa Azure Disk Encryption przewodnik rozwiązywania problemów

Ten przewodnik jest przeznaczony dla specjalistów IT, analityków zabezpieczeń informacji oraz administratorów chmury, w których organizacje używają usługi Azure Disk Encryption. Ten artykuł jest zapewnienie wskazówki dotyczące rozwiązywania problemów dotyczących szyfrowania dysku.
...
- Nie wyłączono Linux zwiększonych zabezpieczeń (SELinux) przed włączeniem szyfrowania, dlatego krok odinstalowywania zakończy się niepowodzeniem. Może być reenabled SELinux, po zakończeniu szyfrowania.
- Dysk systemu operacyjnego wykorzystuje schemat Menedżer woluminów logicznych (LVM). Ograniczona obsługa dysku danych LVM jest dostępne, nie jest dyskiem systemu operacyjnego LVM.
...
W następujących przypadkach zaleca się przywrócenia maszyny Wirtualnej do migawki lub kopii zapasowej wykonanej bezpośrednio przed szyfrowania:
   - Jeśli sekwencji rozruchu opisanego wcześniej nie jest realizowane.
...
Przed rozpoczęciem następnej próby właściwości maszyny Wirtualnej to ponowne ocenienie i upewnij się, że są spełnione wszystkie wymagania wstępne.
...
W czasie wykonywania usługa Azure Disk Encryption dla systemu Linux opiera się na system zarządzania pakietami dystrybucji docelowej, aby zainstalować wstępnie wymagane składniki potrzebne przed włączeniem szyfrowania. Jeśli ustawienia zapory uniemożliwić maszyny Wirtualnej do pobrania i zainstalowania tych składników, oczekiwane są kolejne błędy. Kroki, aby skonfigurować ten system zarządzania pakietami można różnią się zależnie od dystrybucji. W systemie Red Hat Jeśli serwer proxy jest wymagany, upewnij się, że Menedżer subskrypcji i yum są prawidłowo skonfigurowane. Aby uzyskać więcej informacji, zobacz [rozwiązywania problemów Menedżera subskrypcji i yum](https://access.redhat.com/solutions/189533).  
...
Jeśli stan oczekiwany szyfrowania nie jest zgodny, co to jest zgłaszany w portalu, zobacz następujący artykuł pomocy technicznej: [stanu szyfrowania jest wyświetlany niepoprawnie w portalu zarządzania systemu Azure](https://support.microsoft.com/en-us/help/4058377/encryption-status-is-displayed-incorrectly-on-the-azure-management-por)
...
- [Szyfrowanie maszyny wirtualnej z platformy Azure](../security-center/security-center-disk-encryption.md)
