...
W tym temacie opisano wymagania dotyczące uwierzytelniania, konfigurować i kontrolować [aktywnej replikacji geograficznej](sql-database-geo-replication-overview.md) oraz kroków wymaganych do skonfigurowania dostępu użytkowników do pomocniczej bazy danych. Opisano również sposób włączania dostępu do odzyskanej bazy danych po zakończeniu korzystania z [geoprzywracanie](sql-database-recovery-using-backups.md#geo-restore). Aby uzyskać więcej informacji na temat opcji odzyskiwania, zobacz [omówienie ciągłości działania](sql-database-business-continuity.md).

> [!NOTE]
> [Aktywna replikacja geograficzna](sql-database-geo-replication-overview.md) jest teraz dostępna dla wszystkich baz danych we wszystkich warstwach usługi.
>  
...
#### <a name="1-determine-logins-with-access-to-the-primary-database"></a>1. Określ nazwy logowania z uprawnieniami do podstawowej bazy danych:
...
#### <a name="2-find-the-sid-for-the-logins-identified-in-step-1"></a>2. Znajdź identyfikator SID dla nazw logowania w kroku 1:
...
* Aby uzyskać więcej informacji na temat zarządzania dostęp do bazy danych i nazwami logowania, zobacz [zabezpieczeń bazy danych SQL Database: Zarządzanie zabezpieczeniami dostępu i logowania w bazie danych](sql-database-manage-logins.md).
...
* Aby uzyskać informacje o używaniu i konfigurowanie aktywnej replikacji geograficznej, zobacz [aktywnej replikacji geograficznej](sql-database-geo-replication-overview.md)
