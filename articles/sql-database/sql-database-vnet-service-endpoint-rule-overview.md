...
> [!IMPORTANT]
> This article applies to Azure SQL server, and to both SQL Database and SQL Data Warehouse databases that are created on the Azure SQL server. Dla uproszczenia usługi SQL Database i SQL Data Warehouse są łącznie nazywane usługą SQL Database. This article does *not* apply to a **managed instance** deployment in Azure SQL Database because it does not have a service endpoint associated with it.
...
*Error description:* The client is in a subnet that has virtual network server endpoints. Serwer usługi Azure SQL Database nie ma jednak reguły sieci wirtualnej, która przyznaje podsieci prawo do komunikowania się z usługą SQL Database.
...
*Error description:* The client is trying to connect from an IP address that is not authorized to connect to the Azure SQL Database server. Zapora serwera nie ma reguły adresu IP zezwalającej klientowi na komunikację z usługą SQL Database z tego adresu IP.

*Error resolution:* Enter the client's IP address as an IP rule. W tym celu użyj okienka Zapora w witrynie Azure Portal.
