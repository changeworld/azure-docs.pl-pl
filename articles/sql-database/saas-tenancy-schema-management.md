...
1. **W programie POWERSHELL ISE**Otwórz... \\Learning modules @ No__t-2Schema Management @ no__t-3*demo-SchemaManagement. ps1*.
...
1. Otwórz narzędzie SSMS i Połącz się z serwerem dzierżawy: *tenants1-DPT-@no__t -1user&gt;.database.windows.net*
1. Aby upewnić się, że **nie są** obecnie uwzględniane wyścigi i klubu z *motocykla* , przejdź do bazy danych _contosoconcerthall_ na serwerze *tenants1-DPT-&lt;User @ no__t-6* i zapytaj o *VenueTypes* tabele.
...
1. W programie SSMS Połącz się z serwerem wykazu: *Catalog-DPT-@no__t -1user&gt;.database.windows.net* Server 
1. W programie SSMS Otwórz plik... \\Learning modules @ no__t-1Schema Management\\DeployReferenceData.sql
1. Modyfikowanie instrukcji: Ustaw @wtpUser = &lt;user @ no__t-2 i Zastąp wartość użytkownika używaną podczas wdrażania aplikacji Wingtip biletów SaaS Database for dzierżawca
...
* **SP @ no__t-1add @ no__t-2target @ no__t-3group** tworzy nazwę grupy docelowej DemoServerGroup.
* **SP @ no__t-1add @ no__t-2target @ no__t-3group @ no__t-4member** służy do definiowania zestawu docelowych baz danych.  Najpierw zostanie dodany serwer _tenants1-DPT-&lt;user @ no__t-2_ .  Dodanie serwera jako obiektu docelowego powoduje, że bazy danych na tym serwerze w momencie wykonania zadania zostaną uwzględnione w zadaniu. Następnie baza danych _basetenantdb_ i baza danych *adhocreporting* (używane w późniejszym samouczku) zostaną dodane jako elementy docelowe.
* **SP @ no__t-1add @ no__t-2job** tworzy zadanie o nazwie _wdrożenie danych referencyjnych_.
* **SP @ no__t-1add @ no__t-2jobstep** tworzy krok zadania zawierający tekst polecenia t-SQL w celu zaktualizowania tabeli referencyjnej, VenueTypes.
...
Po zakończeniu skryptu możesz sprawdzić, czy dane referencyjne zostały zaktualizowane.  W programie SSMS przejdź do bazy danych *contosoconcerthall* na serwerze *tenants1-DPT-&lt;user @ no__t-3* i zbadaj tabelę *VenueTypes* .  Sprawdź, czy **są** teraz obecne wyścigi i *kluby* w *motocyklach* .
...
1. Otwórz narzędzie SSMS i Połącz się z _katalogiem DPT-@no__t -1user&gt;.database.windows.net_ Server
1. Otwórz plik _... \\Learning modules @ no__t-2Schema Management\\OnlineReindex.sql_
1. Kliknij prawym przyciskiem myszy, wybierz opcję połączenie i nawiąż połączenie z serwerem _wykazu-DPT-@no__t -1user&gt;.database.windows.net_ , jeśli nie jest jeszcze połączony
1. Upewnij się, że nawiązano połączenie z bazą danych _jobagent_ , a następnie naciśnij klawisz **F5** , aby uruchomić skrypt

Obserwuj następujące elementy w skrypcie _OnlineReindex. SQL_ :
* **SP @ no__t-1add @ no__t-2job** tworzy nowe zadanie o nazwie "online reindex PK @ no__t-3 @ No__t-4VenueTyp @ no__t-5 @ NO__T-6265E44FD7FD4C885"
* **SP @ no__t-1add @ no__t-2jobstep** tworzy krok zadania zawierający tekst polecenia t-SQL w celu zaktualizowania indeksu
