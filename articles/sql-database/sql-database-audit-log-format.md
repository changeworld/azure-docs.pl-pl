...
Dzienniki inspekcji replik tylko do odczytu są przechowywane w tym samym kontenerze. Hierarchia katalogów w kontenerze ma postać `<ServerName>/<DatabaseName>/<AuditName>/<Date>/RO/`. Nazwa pliku obiektu BLOB ma ten sam format.
...
| connection_id | ND | Identyfikator połączenia na serwerze | IDENT | ND |
...
| sequence_group_id | sequence_group_id_g | Unikatowy identyfikator | liczby | IDENT |
...
| server_principal_sid | server_principal_sid_s | Bieżący identyfikator SID logowania | liczby | string |
...
| target_server_principal_sid | target_server_principal_sid_s | Identyfikator SID docelowej nazwy logowania. Wartość NULL, jeśli nie dotyczy | liczby | string |
