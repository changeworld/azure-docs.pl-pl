...
* **Dodawanie baz danych do grupy trybu failover**: kilka baz danych na serwerze lub w ramach puli elastycznej można umieścić w tej samej grupy trybu failover. Jeśli dodasz pojedynczej bazy danych do grupy, automatycznie tworzy pomocniczą bazę danych przy użyciu tego samego rozmiaru edition i mocy obliczeniowej. Jeśli podstawowa baza danych znajduje się w puli elastycznej, pomocnicza jest automatycznie tworzony w elastycznej puli o takiej samej nazwie. Po dodaniu bazy danych, która ma już pomocniczej bazy danych serwera pomocniczego, że replikacja geograficzna jest dziedziczona przez grupę.
