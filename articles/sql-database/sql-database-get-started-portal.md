...
1. W formularzu **SQL Database** wpisz lub wybierz następujące wartości: 
...
1. Wybierz pozycję **Serwer**, a następnie opcję **Utwórz nowy serwer**. 
   
1. W formularzu **Nowy serwer** wpisz lub wybierz następujące wartości: 
   
   - **Nazwa serwera**: wpisz *mysqlserver*.
...
1. Wpisz informacje dotyczące logowania, a następnie wybierz przycisk **OK**.
...
Zachowaj tę grupę zasobów, serwer SQL i bazę danych SQL, jeśli chcesz przejść do sekcji [Następne kroki](#next-steps) i dowiedzieć się, jak połączyć się z bazą danych i wykonywać w niej zapytania przy użyciu różnych metod. 
...
1. Wpisz w polu ciąg *myResourceGroup*, a następnie wybierz opcję **Usuń**.

## <a name="next-steps"></a>Następne kroki

- Należy utworzyć regułę zapory na poziomie serwera w celu nawiązania połączenia z bazą danych SQL platformy Azure za pomocą narzędzi lokalnych lub zdalnych. Aby uzyskać więcej informacji, zobacz temat [Tworzenie reguły zapory na poziomie serwera](sql-database-get-started-portal-firewall.md).
- Po utworzeniu reguły zapory na poziomie serwera można [nawiązać połączenie z bazą danych i wykonywać zapytania](sql-database-connect-query.md) przy użyciu różnych narzędzi i języków. 
