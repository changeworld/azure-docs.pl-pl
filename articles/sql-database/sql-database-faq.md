...
## <a name="is-moving-to-the-vcore-based-model-required"></a>Zostanie przeniesione do modelu opartego na rdzeniach wirtualnych, wymagane
...
## <a name="do-the-new-vcore-based-service-tiers-offer-the-compute-sizes-compatible-with-all-existing-compute-sizes"></a>Nowe warstwy usług oparte na rdzeniach wirtualnych oferują zgodny z istniejącymi wszystkich rozmiarów wystąpień obliczeniowych obliczenia rozmiarów
...
Ponieważ model rdzenia wirtualnego umożliwia niezależną kontrolę nad ilość zainicjowanych zasobów obliczeniowych i magazynu, można efektywniej zarządzać związane z nimi koszty, ułatwiając atrakcyjny miejsce docelowe dla baz danych Premium RS. Ponadto [korzyść użycia hybrydowego platformy Azure dla programu SQL Server](https://azure.microsoft.com/pricing/hybrid-benefit/) zapewnia znaczny rabat w wysokości, gdy model oparty na rdzeniach wirtualnych jest używany.
...
## <a name="what-if-a-single-database-is-active-for-less-than-an-hour-or-uses-a-higher-service-tier-for-less-than-an-hour"></a>Co zrobić, jeśli jedna baza danych jest aktywna krócej niż godzinę lub korzystanie z wyższej warstwy usługi dla mniej niż godzinę
...
## <a name="what-tools-are-available-to-monitor-the-replication-lag-between-the-primary-database-and-geo-secondary"></a>Jakie narzędzia są dostępne do monitorowania opóźnienie replikacji między podstawowej bazy danych i pomocnicza geograficzna
