...
>[!NOTE]
> Platforma Azure udostępnia zestaw w pełni zarządzanych rozwiązań do równoważenia obciążenia dla swoich scenariuszy. Jeśli szukasz kończenie żądań protokołu zabezpieczeń TLS (Transport Layer) ("odciążanie protokołu SSL") lub żądania dla protokołu HTTP/HTTPS, warstwy aplikacji, przetwarzania, zapoznaj się z [Application Gateway](../application-gateway/application-gateway-introduction.md). Jeśli szukasz globalnego DNS równoważenia obciążenia, przejrzyj [usługi Traffic Manager](../traffic-manager/traffic-manager-overview.md). Scenariuszy end-to-end mogą korzystać z łączenia tych rozwiązań, zgodnie z potrzebami.
...
* **Sondy kondycji**
    ...
    Obsługiwane są trzy typy sondy:

    - **Niestandardowe sondy HTTP**: to sondowanie umożliwia tworzenie własnej niestandardowej logiki można określić kondycji wystąpienie puli serwerów zaplecza. Moduł równoważenia obciążenia regularnie sondy punktu końcowego (co 15 sekund, domyślnie). Wystąpienie jest uważana za dobrej kondycji, jeśli odpowiada za pomocą protokołu HTTP 200 przed upływem limitu czasu (domyślnie 31 sekund). Dowolny stan innych niż HTTP 200 powoduje, że sonda nie powiedzie się. To sondowanie jest również przydatne w przypadku implementowania własnej logiki, aby usunąć wystąpienia z rotacji modułu równoważenia obciążenia. Na przykład można skonfigurować wystąpienia próbę zwrócenia stanu – 200, jeśli wystąpienie jest większa niż 90 procent procesora CPU.  To sondowanie zastąpienia domyślnej funkcji badania agenta gościa.

    - **Niestandardową sondę TCP**: to sondowanie zależy od tego, ustanawiania pomyślne sesji TCP na porcie sondowania zdefiniowane. Tak długo, jak określony odbiornik na maszynie Wirtualnej istnieje, to sondowanie zakończy się pomyślnie. W przypadku odmowy połączenia sondy kończy się niepowodzeniem. To sondowanie zastąpienia domyślnej funkcji badania agenta gościa.

    - **Sondowanie agenta gościa**: moduł równoważenia obciążenia może również korzystać z agenta gościa wewnątrz maszyny Wirtualnej. Agent gościa wykrywa i odpowiada za pomocą odpowiedź HTTP 200 OK, tylko wtedy, gdy wystąpienie jest w stanie gotowe. Jeśli agent nie odpowiada przy użyciu protokołu HTTP 200 OK, moduł równoważenia obciążenia oznacza wystąpienie odpowiada i przestaje wysyłać ruch do tego wystąpienia. Moduł równoważenia obciążenia w dalszym ciągu próbują uzyskać dostęp z wystąpieniem. Jeśli agent gościa odpowiada za pomocą protokołu HTTP 200, moduł równoważenia obciążenia wysyła ruch do tego wystąpienia ponownie. Sondy agenta gościa są _ostatnia czynność i nie jest zalecane_ podczas konfiguracji niestandardowej sondy HTTP lub TCP są możliwe. 
