...
- **Konta wieloregionowe z regionem pojedynczego zapisu (awaria regionu zapisu):**
  ...
  - Klienci mogą również wybrać użycie **ręcznego trybu failover** i monitorować ich cosmos napisać adres URL punktu końcowego się przy użyciu agenta utworzonego siebie. Dla klientów z złożonych i zaawansowanych potrzeb monitorowania kondycji, może to zapewnić zmniejszenie RTO w przypadku wystąpienia awarii w regionie zapisu.
  - Gdy region, którego to dotyczy, jest z powrotem w trybie online, wszystkie dane zapisu, które nie zostały przywrócone, gdy region nie powiódł się, są udostępniane za pośrednictwem [kanału danych o konfliktach](how-to-manage-conflicts.md#read-from-conflict-feed). Aplikacje mogą odczytywać źródła danych konfliktów, rozwiązywać konflikty na podstawie logiki specyficznej dla aplikacji i zapisywać zaktualizowane dane z powrotem do kontenera usługi Azure Cosmos odpowiednio.
  - Po odzyskaniu wcześniej dotkniętego regionu zapisu staje się on automatycznie dostępny jako region odczytu. Można przełączyć się z powrotem do odzyskanego regionu jako region zapisu. Regiony można przełączać za pomocą [interfejsu wiersza polecenia platformy Azure lub witryny Azure portal.](how-to-manage-database-account.md#manual-failover) Nie **ma żadnych danych lub utraty dostępności** przed, w trakcie lub po przełączeniu regionu zapisu i aplikacja nadal jest wysoce dostępna.

- **Konta wieloregionowe z regionem pojedynczego zapisu (awaria regionu odczytu):**
  - Podczas awarii regionu odczytu te konta pozostaną wysoce dostępne dla odczytów i zapisów.
  ...
  - Żadne zmiany nie są wymagane w kodzie aplikacji do obsługi awarii regionu odczytu. Po pewnym czasie, gdy region, na który ma wpływ, zostanie ponownie udostępniony w trybie online, poprzednio dotknięty region odczytu zostanie automatycznie zsynchronizowany z bieżącym regionem zapisu i będzie ponownie dostępny do obsługi żądań odczytu.
...
Nadmiarowość strefy można włączyć podczas dodawania regionu do nowych lub istniejących kont usługi Azure Cosmos. Aby włączyć nadmiarowość strefy na koncie usługi `isZoneRedundant` Azure `true` Cosmos, należy ustawić flagę dla określonej lokalizacji. Tę flagę można ustawić we właściwości locations. Na przykład następujący fragment kodu programu PowerShell umożliwia nadmiarowość strefy dla regionu "Azja Południowo-Wschodnia":
