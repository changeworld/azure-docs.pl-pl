...
Działania w poniższej tabeli służą do uzyskiwania dostępu do zmiennych w elementach Runbook i konfiguracjach DSC. Różnica między `Get-AzAutomationVariable` i `Get-AutomationVariable` została omówiona dla zmiennych szyfrowanych na początku tego artykułu.
...
> [!NOTE]
> Należy unikać używania zmiennych w `Name` parametr `Get-AutomationVariable` w elemencie Runbook lub konfiguracji DSC. Użycie tego parametru może komplikuje odnajdywanie zależności między elementami Runbook lub konfiguracjami DSC a zmiennymi automatyzacji w czasie projektowania.
...
Skrypt używa polecenia cmdlet `New-AzAutomationVariable`, aby utworzyć nową zmienną i ustawić jej początkową wartość. Następnie może pobrać wartość przy użyciu `Get-AzAutomationVariable`. Jeśli wartość jest typu prostego, wówczas pobierany jest ten sam typ. Jeśli jest to typ złożony, pobierany jest typ `PSCustomObject`.
...
Użyj działania `Set-AutomationVariable`, aby ustawić wartość zmiennej automatyzacji w elemencie Runbook programu PowerShell lub konfiguracji DSC, a `Get-AutomationVariable` ją pobrać. Nie należy używać `Set-AzAutomationVariable` i `Get-AzAutomationVariable` poleceń cmdlet ani ich odpowiedników modułu AzureRM w elemencie Runbook lub konfiguracji DSC, ponieważ są one mniej wydajne niż działania przepływu pracy. 

Należy zauważyć, że nie można pobrać wartości bezpiecznej zmiennej z `Get-AzAutomationVariable` lub jej odpowiednikiem modułu AzureRM. 
...
Następujące przykładowe polecenia pokazują, jak ustawić i pobrać zmienną w tekstowym elemencie Runbook. W tym przykładzie przyjęto założenie, że tworzone są zmienne typu Integer o nazwie `NumberOfIterations` i `NumberOfRunnings` i zmienną ciągu o nazwie `SampleMessage`.
