...
Element runbook został przetestowany i opublikowany, ale jak do tej pory nie wykonuje on żadnych użytecznych czynności. Powinien zarządzać zasobami platformy Azure. Nie można tego zrobić, jeśli nie masz uwierzytelnienia przy użyciu poświadczeń, które są określone w [wymaganiach wstępnych](#prerequisites). Można to zrobić za pomocą polecenia cmdlet **Connect-AzureRmAccount** .
...
4. Wpisz lub skopiuj i wklej poniższy kod, który będzie obsługiwać uwierzytelnianie za pomocą konta Uruchom jako w usłudze Automation:
   ...
   > [!IMPORTANT]
   > Do programu **Connect-AzureRmAccount**są teraz aliasy **Add-AzureRmAccount** i **login-AzureRmAccount** . Jeśli polecenie cmdlet **Connect-AzureRMAccount** nie istnieje, możesz użyć polecenia **Add-AzureRMAccount** lub **login-AzureRMAccount**lub [zaktualizować moduły](automation-update-azure-modules.md) na koncie usługi Automation do najnowszych wersji.
...
1. Kliknij opcję **Uruchom**, aby rozpocząć test. Po zakończeniu powinny pojawić się dane wyjściowe podobne do poniższych, zawierające podstawowe informacje powiązane z kontem. Ta akcja potwierdza, że poświadczenie jest prawidłowe.
   ...
   ![Uwierzytelnij](media/automation-first-runbook-textual/runbook-auth-output.png)
