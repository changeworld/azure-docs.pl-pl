---
title: Zarządzanie wystąpieniami zastrzeżone Azure - rozliczenia Azure | Dokumentacja firmy Microsoft
...
## <a name="split-a-single-reserved-instance-into-two-reserved-instances"></a>Podziel pojedynczego wystąpienia zarezerwowane na dwa wystąpienia zastrzeżone
 Po zakupie więcej niż jedno wystąpienie można przypisać wystąpień w wystąpieniu zarezerwowane do różnych subskrypcji. Domyślnie wszystkie wystąpienia (określone podczas zakupu ilość) mają jeden zakres - albo jedną subskrypcją lub udostępnionego. Na przykład zakupione 10 standardowe maszyn wirtualnych D2 i określony zakres być subskrypcji A. Teraz można zmienić zakres siedmiu wystąpień zarezerwowane do subskrypcji A oraz pozostałych 3 do subskrypcji B. dzielenia zastrzeżone wystąpienia umożliwia dystrybucję wystąpień dla szczegółowych zakres zarządzania. Można uprościć alokacji do subskrypcji, wybierając zakres udostępniony. Jednak do celów zarządzania lub budżetu kosztów, możesz przydzielić ilości do określonej subskrypcji.

 Można podzielić wystąpienia zarezerwowane na dwa wystąpienia zastrzeżone jednak programu PowerShell, interfejsu wiersza polecenia, lub za pośrednictwem interfejsu API.
...
2. Pobieranie szczegółów zastrzeżone wystąpienie:
    ...
    ```
3. Podzielone dwa wystąpienia zastrzeżone i dystrybucja wystąpień:
...
Możesz delegować Zarządzanie zastrzeżone wystąpienia przez dodanie użytkowników do ról w wystąpieniu zastrzeżone. Domyślnie osoba, z którą zakupione wystąpienia zastrzeżone i administratora konta mają rolę właściciela wystąpienia zastrzeżone. 
...
1.  Zaloguj się do witryny [Azure Portal](https://portal.azure.com).
2.  Wybierz **wszystkie usługi** > **rezerwacji** do zastrzeżonego wystąpień, które ma dostęp do listy.
