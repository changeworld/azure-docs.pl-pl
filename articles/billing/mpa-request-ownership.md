...
\* Opłata za subskrypcję w ramach usługi Enterprise DevTest będzie naliczana według stawki z płatnością zgodnie z rzeczywistym użyciem po przeniesieniu, rabat oferowany w ramach oferty DevTest w przedsiębiorstwie w ramach umowy EA klienta nie będzie dostępny dla partnera programu CSP.
...
Przejście nie wpływa na dostęp do istniejących użytkowników, grup lub jednostek usługi, które zostały przypisane za pomocą [kontroli dostępu opartej na rolach platformy Azure](../role-based-access-control/overview.md). Partner nie uzyska nowego dostępu do subskrypcji za pomocą kontroli dostępu opartej na rolach.  
