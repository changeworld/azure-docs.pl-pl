...
Skonfiguruj liczniki wydajności z [danych menu Ustawienia usługi Analiza dzienników](log-analytics-data-sources.md#configuring-data-sources).
...
![Skonfiguruj liczniki wydajności Windows](media/log-analytics-data-sources-performance-counters/configure-windows.png)
...
![Skonfiguruj liczniki wydajności systemu Linux](media/log-analytics-data-sources-performance-counters/configure-linux.png)
...
| wystąpienie\_wyrażeń regularnych |  A *wyrażenia regularnego* Definiowanie które wystąpienia do zbierania. Wartość: `.*` określa wszystkie wystąpienia. Aby zbierać metryki procesora tylko \_łączna liczba wystąpień, można określić `_Total`. Aby zbierać metryki tylko crond lub sshd wystąpień procesu, można określić: `(crond\|sshd)`. |
...
Poniższa tabela zawiera listę obiektów i liczników, które można określić w pliku konfiguracji.  Dostępne są dodatkowe liczniki dla określonych aplikacji zgodnie z opisem w [zbierania liczników wydajności dla aplikacji systemu Linux w usłudze Log Analytics](log-analytics-data-sources-linux-applications.md).
...
| Perf &#124; gdzie komputer == "Mój komputer" |Wszystkie dane dotyczące wydajności z określonego komputera |
...
| Perf &#124; w przypadku, gdy CounterName == "% czasu procesora" &#124; summarize AggregatedValue = max(Max) według komputera |Maksymalne wykorzystanie procesora CPU na wszystkich komputerach |
