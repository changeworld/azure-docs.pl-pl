...
- **Finanse** — osoby odpowiedzialne za zatwierdzanie żądań budżetowych w organizacji w oparciu o prognozy wydatków w chmurze. One zapłacić rachunku odpowiedniego i przypisać koszty do różnych zespołów na zapewnienie odpowiedzialności.
- **Kierownicy** — osoby podejmujące decyzje biznesowe w organizacji, które muszą zrozumieć wydatki na chmurę, aby znaleźć najlepsze wyniki.
- **Zespoły aplikacji** — inżynierowie zarządzający zasobami w chmurze w codziennym zakresie, opracowując usługi spełniające potrzeby organizacji. Te zespoły muszą mieć możliwość dostarczania największych korzyści z ich budżetów zdefiniowane.
...
Aby dowiedzieć się więcej o różnych opcjach, odwiedź stronę [Jak kupić platformę Azure](https://azure.microsoft.com/pricing/purchase-options/). Kilka typowych modeli rozliczeń są identyfikowane poniżej.
...
Aby uzyskać więcej informacji, zobacz [Azure Migrate](../migrate/migrate-overview.md).
...
- **Szacowane koszty bieżącego miesiąca** — ile poniesiono do tego miesiąca? Będą przechowywane w ramach budżetu?
- **Zbadaj anomalie** — sprawdza rutynowo, aby upewnić się, że koszty pozostają w ramach rozsądnego zakresu normalnego użytkowania. Jakie są trendy? Czy istnieją jakiekolwiek elementy odstające?
- **Uzgadnianie faktur** — czy mój ostatni wystawiony koszt jest większy niż poprzedni miesiąc? Jak wydatków nawyków zmienić miesiąc do miesiąca?
- **Wewnętrzny obciążenia zwrotnego** — teraz wiemy, jak dużo opłaty są naliczane, jak należy je podzielić na moją organizację?
...
Aby uzyskać więcej informacji na temat eksportowania danych dotyczących rozliczeń, zobacz [Tworzenie eksportowanych danych i zarządzanie nimi](tutorial-export-acm-data.md).
...
Aby uzyskać więcej informacji, zobacz artykuł dotyczący [budżetów platformy Azure](tutorial-acm-create-budgets.md).

Aby uzyskać więcej informacji na temat automatyzacji opartego na budżecie, zobacz [Automatyzacja oparta na budżetach](../billing/billing-cost-management-budget-scenario.md).
...
Aby uzyskać więcej informacji, zobacz [Azure IaaS: właściwa wielkość i koszt](https://azure.microsoft.com/resources/videos/azurecon-2015-azure-iaas-proper-sizing-and-cost/).
...
Aby uzyskać więcej informacji, zobacz [co to są Azure Reservations?](../billing/billing-save-compute-costs-reservations.md).

#### <a name="use-azure-hybrid-benefit"></a>Korzystaj z zalet użycia hybrydowego platformy Azure

Jeśli już masz licencje systemu Windows Server lub SQL Server we wdrożeniach w środowisku lokalnym, można użyć programu korzyści użycia hybrydowego platformy Azure, można zapisać na platformie Azure. Korzyści systemu Windows Server z każdej licencji po awarii obejmuje koszt systemu operacyjnego (maksymalnie dwóch maszynach wirtualnych), a ponosisz tylko koszty według podstawowej stawki obliczeniowej. Możesz korzystać z istniejących licencji na program SQL Server, aby zaoszczędzić do 55% na opcjach usługi SQL Database opartych na rdzeniach wirtualnych. Opcje obejmują program SQL Server w usłudze Azure Virtual Machines i usług SQL Server Integration Services.

Aby uzyskać więcej informacji, zobacz [Kalkulator oszczędności korzyść użycia hybrydowego platformy Azure](https://azure.microsoft.com/pricing/hybrid-benefit/).

### <a name="other-resources"></a>Inne zasoby

Platforma Azure ma to usługa, która umożliwia tworzenie usług, które wykorzystują nadwyżkę wydajności na platformie Azure dla obniżonych stawek. Aby uzyskać więcej informacji, zobacz [Korzystanie z maszyn wirtualnych o niskim priorytecie w usłudze Batch](../batch/batch-low-pri-vms.md).

## <a name="next-steps"></a>Następne kroki
- Jeśli jesteś nowym do Cost Management, Przeczytaj [co to jest Azure Cost Management?](overview-cost-mgt.md) , aby dowiedzieć się, jak ułatwia monitorowanie i kontrolowanie wydatków platformy Azure oraz optymalizowanie użycia zasobów.
