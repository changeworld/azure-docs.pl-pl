...
1. Kliknij przycisk **Dostosuj:** Aby zmodyfikować ustawienia replikacji.
   ...
   - W przypadku przejścia w tryb failover wszystkie maszyny w grupie replikacji będą korzystać ze współdzielonych punktów odzyskiwania, spójnych na poziomie awarii i aplikacji. Włączenie spójności wielu maszyn wirtualnych może wpłynąć na wydajność obciążeń (ponieważ jest intensywnie korzystających z procesora CPU) i powinna być używana tylko wtedy, gdy maszynach są uruchomione to samo obciążenie i jest wymagana spójność między wieloma maszynami. Na przykład jeśli aplikacja ma 2 maszyny wirtualne sql i serwery sieci web 2 następnie dodawaj tylko maszyny wirtualne sql jako część grupy replikacji.
   - Można mieć maksymalny 16 maszyn wirtualnych w grupie replikacji.
