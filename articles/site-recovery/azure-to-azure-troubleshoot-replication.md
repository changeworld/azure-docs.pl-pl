...
W tym artykule opisano typowe problemy w usłudze Azure Site Recovery podczas replikowania i odzyskiwania maszyn wirtualnych platformy Azure z jednego regionu do innego regionu. Wyjaśniono również, jak rozwiązać typowe problemy. Aby uzyskać więcej informacji na temat obsługiwanych konfiguracji, zobacz [macierz obsługi replikacji maszyn wirtualnych platformy Azure](site-recovery-support-matrix-azure-to-azure.md).
...
Komunikat o błędzie: "Brak punktu odzyskiwania zgodnej z awarią dostępne dla maszyny Wirtualnej w ciągu ostatnich 60 minut."</br>
Identyfikator błędu: 153007

W poniższych sekcjach opisano przyczyny i rozwiązania.

## <a name="high-data-change-rate-on-the-source-virtual-machine"></a><a name="high-data-change-rate-on-the-source-virtal-machine"></a>Wysoki współczynnik zmian danych na źródłowej maszynie wirtualnej
...
Powinno zostać wyświetlone zdarzenie "Szybkość zmiany danych poza obsługiwanymi limitami":

![Strona usługi Azure Site Recovery z zbyt wysokim wskaźnikiem zmiany danych, który jest zbyt wysoki](./media/site-recovery-azure-to-azure-troubleshoot/data_change_event.png)

Jeśli wybierzesz zdarzenie, powinieneś zobaczyć dokładne informacje o dysku:

![Strona ze szczegółami zdarzenia szybkości zmiany szybkości danych](./media/site-recovery-azure-to-azure-troubleshoot/data_change_event2.png)
...
![Strona, która pokazuje trzyetapowy proces znajdowania szybkości zmiany danych](./media/site-recovery-azure-to-azure-troubleshoot/churn.png)
...
- Zmień warstwę dysku magazynu odzyskiwania po awarii: Ta opcja jest możliwa tylko wtedy, gdy zmiana danych dysku jest mniejsza niż 20 MB/s. Załóżmy, że maszyna wirtualna z dyskiem P10 ma współczynnik danych większy niż 8 MB/s, ale mniejszy niż 10 MB/s. Jeśli klient może użyć dysku P30 do przechowywania docelowego podczas ochrony, problem można rozwiązać. To rozwiązanie jest możliwe tylko dla komputerów korzystających z dysków zarządzanych w wersji Premium. Wykonaj następujące kroki:
...
## <a name="network-connectivity-problems"></a><a name="Network-connectivity-problem"></a>Problemy z łącznością sieciową
...
**Jak naprawić**: Zapoznaj się z artykułem [Nieskszlionkowe kopie zapasowe VSS, takie jak zadania odzyskiwania witryny Azure, kończą się niepowodzeniem na serwerach obsługujących wystąpienia programu SQL Server z AUTO_CLOSE db](https://support.microsoft.com/help/4504104/non-component-vss-backups-such-as-azure-site-recovery-jobs-fail-on-ser).


#### <a name="known-issue-in-sql-server-2016-and-2017"></a>Znany problem w programie SQL Server w wersjach 2016 i 2017

**Jak naprawić**: Zapoznaj się z [artykułem Błąd występuje podczas tworzenia kopii zapasowej maszyny wirtualnej z nieskładkową kopią zapasową w programie SQL Server 2016 i 2017](https://support.microsoft.com/en-us/help/4508218/cumulative-update-16-for-sql-server-2017).
...
**Jak naprawić:** Usługa Azure Site Recovery nie może utworzyć spójnego punktu odzyskiwania aplikacji dla bezpośredniej konfiguracji miejsca do magazynowania. [Skonfiguruj zasady replikacji](https://docs.microsoft.com/azure/site-recovery/azure-to-azure-how-to-enable-replication-s2d-vms).
...
1. Ponowne instalowanie dostawcy usługi VSS: C:\Program Files (x86)\Microsoft Azure Site Recovery\agent\InMageVSSProvider_Install.cmd
