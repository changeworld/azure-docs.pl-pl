---
title: Usługi Azure Cloud Services Def. Workerrole — schemat | Dokumentacja firmy Microsoft
...
|vmsize|ciąg|Opcjonalny. Ustaw tę wartość, aby zmienić rozmiar maszyny wirtualnej, który jest przydzielony do tej roli. Wartość domyślna to `Small`. Aby uzyskać listę rozmiarów możliwości maszyn wirtualnych i ich atrybuty, zobacz [rozmiarów maszyn wirtualnych dla usług w chmurze](cloud-services-sizes-specs.md).|
...
|protokół|ciąg|Wymagany. Protokół transportu jest zewnętrzny punkt końcowy. Rola procesu roboczego, możliwe wartości to `HTTP`, `HTTPS`, `UDP`, lub `TCP`.|
