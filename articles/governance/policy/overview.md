...
Organizacja nadzoru sprawdza, czy organizacja może osiągnąć swoje cele dzięki efektywnemu i efektywnemu wykorzystaniu it. W tym celu jest tworzone jasne połączenie między celami biznesowymi i projektami IT.
...
Azure Policy to usługa platformy Azure, która umożliwia tworzenie i przypisywanie zasad oraz zarządzanie nimi. Te zasady wymuszają różne reguły i efekty dotyczące zasobów, dzięki czemu zasoby te pozostają zgodne ze standardami firmy i umowami dotyczącymi poziomu usług. Usługa Azure Policy spełnia to wymaganie, oceniając zasoby pod kątem niezgodności z przypisanymi zasadami. Wszystkie dane przechowywane przez usługę Azure Policy są szyfrowane w stanie spoczynku.
...
Istnieje kilka kluczowych różnic między zasadami platformy Azure a kontrolą dostępu opartą na rolach (RBAC). RBAC koncentruje się na działaniach użytkownika w różnych zakresach. Użytkownik może zostać dodany do roli współautora dla grupy zasobów, aby mógł wprowadzać zmiany w tej grupie zasobów. Usługa Azure Policy koncentruje się na właściwościach zasobów podczas wdrażania i już istniejących zasobach. Usługa Azure Policy służy do kontrolowania właściwości, takich jak typy lub lokalizacje zasobów. W przeciwieństwie do RBAC, zasady platformy Azure jest domyślny system zezwalania i jawne odmowy.
...
- **Dozwolony typ zasobu:** Definiuje typy zasobów, które można wdrożyć. Jej efektem jest odrzucanie wszystkich zasobów, które nie należą do tej zdefiniowanej listy.
...
- **Dozwolone jednostki SKU maszyny wirtualnej:** Określa zestaw jednostek SKU maszyny wirtualnej, które można wdrożyć.
- **Dodaj znacznik do zasobów:** Stosuje wymagany tag i jego wartość domyślną, jeśli nie jest określony przez żądanie wdrożenia.
- **Wymuszanie znacznika i jego wartości:** Wymusza wymagany znacznik i jego wartość do zasobu.
- **Niedozwolone typy zasobów:** Zapobiega wdrażaniu listy typów zasobów.
...
Podobnie jak przypisanie zasad, przypisanie inicjatywy to definicja inicjatywy przypisana do określonego zakresu. Przypisania inicjatyw ograniczają potrzebę tworzenia różnych definicji inicjatyw dla każdego zakresu. Zakres ten może również wahać się od grupy zarządzania do pojedynczego zasobu.
...
- Przekaż wartości parametrom definicji zasad w ramach tej definicji inicjatywy. W tym przykładzie można podać listę lokalizacji do **policyA parametr - dozwoloneLocations** i **policyB parametr - allowedSingleLocation**. Wartości można przekazać również podczas przypisywania tej inicjatywy.
