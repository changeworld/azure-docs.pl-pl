...
Zwraca kąt w radianach między dodatnią osią x płaszczyzny i punktem podanym przez współrzędne
...
Na podstawie zmiennych warunków stosuje jedną wartość lub drugą. Jeśli liczba danych wejściowych jest parzysta, druga jest domyślnie zerowa dla ostatniego warunku
...
Oblicza wartość skrótu CRC32 zestawu kolumn o różnych typach danych pierwotnych o długości bitowej, która może zawierać tylko wartości 0 (256), 224, 256, 384, 512. Może służyć do obliczania odcisku palca dla wiersza
...
Pobiera dzień miesiąca, w którym nadana została Data
...
Oblicza rangę wartości w grupie wartości. Wynik to jeden i liczba wierszy poprzedzających lub równych bieżącemu wierszowi w kolejności partycji. Wartości nie będą powodować przerw w sekwencji. Gęsta ranga działa nawet wtedy, gdy dane nie są sortowane i wyszukują zmiany wartości
...
Sprawdza, czy ciąg jest zakończony podanym ciągiem
...
Pobiera pierwszą wartość grupy kolumn. Jeśli drugi parametr ignoreNulls jest pominięty, zakłada się, że jest fałszywy
...
Mapuje każdy element tablicy do nowego elementu przy użyciu podanego wyrażenia. Mapa oczekuje odwołania do jednego elementu w funkcji wyrażenia jako #item i odwołania do indeksu elementu jako #index
...
W oparciu o warunek stosuje jedną wartość lub drugą. Jeśli inne nie określono, jest uznawane za NULL. Obie wartości muszą być zgodne (numeryczne, ciąg...)
...
Sprawdza, czy wartość nie jest RÓWNa NULL, i zwraca alternatywę. Sprawdza wszystkie dane wejściowe do momentu znalezienia pierwszej wartości innej niż null
...
Konwertuje pierwszą literę każdego wyrazu na wielkie litery. Wyrazy są identyfikowane jako oddzielone odstępem
...
Sprawdza, czy wiersz jest oznaczony do usunięcia. W przypadku transformacji z więcej niż jednym strumieniem wejściowym można przekazać indeks (od 1 do) strumienia. Wartość domyślna indeksu strumienia to 1
...
Sprawdza, czy wiersz jest oznaczony jako błąd. W przypadku transformacji z więcej niż jednym strumieniem wejściowym można przekazać indeks (od 1 do) strumienia. Wartość domyślna indeksu strumienia to 1
...
Sprawdza, czy wiersz jest oznaczony jako ignorowany. W przypadku transformacji z więcej niż jednym strumieniem wejściowym można przekazać indeks (od 1 do) strumienia. Wartość domyślna indeksu strumienia to 1
...
Sprawdza, czy wiersz jest oznaczony do wstawienia. W przypadku transformacji z więcej niż jednym strumieniem wejściowym można przekazać indeks (od 1 do) strumienia. Wartość domyślna indeksu strumienia to 1
...
Sprawdza, czy wiersz jest dopasowany podczas wyszukiwania. W przypadku transformacji z więcej niż jednym strumieniem wejściowym można przekazać indeks (od 1 do) strumienia. Wartość domyślna indeksu strumienia to 1
...
Sprawdza, czy wiersz jest oznaczony do aktualizacji. W przypadku transformacji z więcej niż jednym strumieniem wejściowym można przekazać indeks (od 1 do) strumienia. Wartość domyślna indeksu strumienia to 1
...
Pobiera ostatnią wartość grupy kolumn. Jeśli drugi parametr ignoreNulls jest pominięty, zakłada się, że jest fałszywy
...
Porównanie z operatorem mniejszym niż lub równym. Taki sam jak <= operator
...
Oblicza wartość dziennika. Opcjonalna wartość bazowa może być podana w innym przypadku, jeśli jest używana
...
Oblicza skrót MD5 zestawu kolumn o różnych typach danych pierwotnych i zwraca ciąg szesnastkowy o 32 znaku. Może służyć do obliczania odcisku palca dla wiersza
...
Pobiera średnią wartości w kolumnie. Takie same jak średnia
...
Odejmuje liczby. Odejmij od daty i liczby dni. Substract czas trwania z sygnatury czasowej. Substract dwa sygnatury czasowe, aby uzyskać różnicę w milisekundach. Analogicznie jak operator
...
Porównanie operatora not Equals. Analogicznie jak operator! =
...
Zwraca wartość NULL. Użyj składni funkcji (null ()), jeśli istnieje kolumna o nazwie "null". Każda operacja, która używa, spowoduje utratę wartości NULL
...
Logiczny operator LUB. Analogicznie jak | |
...
Oblicza rangę wartości w grupie wartości. Wynik to jeden i liczba wierszy poprzedzających lub równych bieżącemu wierszowi w kolejności partycji. Wartości będą generować przerwy w sekwencji. Ranga działa nawet wtedy, gdy dane nie są sortowane i wyszukują zmiany wartości
...
Gromadzi elementy w tablicy. Funkcja ograniczania oczekuje odwołania do akumulowana i jednego elementu w pierwszej funkcji wyrażenia jako #acc i #item i oczekuje wartości, która będzie używana jako #result do użycia w drugiej funkcji wyrażenia
...
Wyodrębnij pasujący podciąg dla danego wzorca wyrażenia regularnego. Ostatni parametr identyfikuje grupę dopasowania i domyślnie ma wartość 1 w przypadku pominięcia. Użyj "<regex>" (cudzysłów tylny), aby dopasować ciąg bez ucieczki
...
Sprawdza, czy ciąg jest zgodny z danym wzorcem wyrażenia regularnego. Użyj "<regex>" (cudzysłów tylny), aby dopasować ciąg bez ucieczki
...
Zamień wszystkie wystąpienia podciągu na inny podciąg w podanym ciągu. Jeśli ostatni parametr zostanie pominięty, jest to domyślny ciąg pusty
...
Czas trwania (w milisekundach) dla liczby sekund
...
Oblicza skrót SHA-1 zestawu kolumn o różnych typach danych pierwotnych i zwraca ciąg szesnastkowy znaku 40. Może służyć do obliczania odcisku palca dla wiersza
...
Oblicza skrót SHA-2 zestawu kolumn o różnych typach danych pierwotnych o długości bitowej, która może zawierać tylko wartości 0 (256), 224, 256, 384, 512. Może służyć do obliczania odcisku palca dla wiersza
...
Wyodrębnia podzbiór tablicy z położenia. Pozycja ma 1 na podstawie. Jeśli długość zostanie pominięta, zostanie ona domyślnie zakończona na końcu ciągu
...
Wyodrębnia podciąg o określonej długości z położenia. Pozycja ma 1 na podstawie. Jeśli długość zostanie pominięta, zostanie ona domyślnie zakończona na końcu ciągu
...
Konwertuje wartość ('t ', 'true ', ' y ', ' yes ', ' 1 ') na true i ("f", "false", "n", "No", "0") na wartość false i wartość NULL dla dowolnej innej wartości
...
Konwertuje wartość liczbową lub ciąg na liczbę całkowitą. Do konwersji można użyć opcjonalnego formatu dziesiętnego języka Java. Obcina wszystkie długie, zmiennoprzecinkowe, podwójne
...
Konwertuje dowolną liczbę lub ciąg na wartość długą. Do konwersji można użyć opcjonalnego formatu dziesiętnego języka Java. Obcina wszystkie zmiennoprzecinkowe, podwójne
...
Konwertuje dowolną liczbę lub ciąg na krótką wartość. Do konwersji można użyć opcjonalnego formatu dziesiętnego języka Java. Obcina liczbę całkowitą, Long, float, Double
...
Pobiera wariancję niebias kolumny
...
Pobiera tydzień roku z danego dnia.
