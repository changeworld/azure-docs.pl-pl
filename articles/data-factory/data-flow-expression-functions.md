...
Konwertuje sygnaturę czasową od czasu UTC. Można opcjonalnie przekazać strefy czasowej w formie 'GMT', "PST", "UTC", "Ameryka/Kajmany". Ustawiana domyślnie do bieżącej strefy czasowej * ``fromUTC(currentTimeStamp()) -> 12-12-2030T19:18:12``
...
Sprawdza, czy jeśli wiersz jest oznaczona do usunięcia. Dla przekształceń trwa więcej niż jeden strumień wejściowy można przekazać indeks strumienia (od 1). Wartość domyślna dla indeks strumienia to 1 * ``isDelete() -> true``
* ``isDelete(1) -> false``
*********************************
<code>isError</code>
==============================
<code><b>isError([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Sprawdza, czy jeśli wiersz jest oznaczona jako błąd. Dla przekształceń trwa więcej niż jeden strumień wejściowy można przekazać indeks strumienia (od 1). Wartość domyślna dla indeks strumienia to 1 * ``isError() -> true``
* ``isError(1) -> false``
*********************************
<code>isIgnore</code>
==============================
<code><b>isIgnore([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Sprawdza, czy jeśli wiersz jest oznaczony do zignorowania. Dla przekształceń trwa więcej niż jeden strumień wejściowy można przekazać indeks strumienia (od 1). Wartość domyślna dla indeks strumienia to 1 * ``isIgnore() -> true``
* ``isIgnore(1) -> false``
*********************************
<code>isInsert</code>
==============================
<code><b>isInsert([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Sprawdza, czy jeśli wiersz jest oznaczony do wstawienia. Dla przekształceń trwa więcej niż jeden strumień wejściowy można przekazać indeks strumienia (od 1). Wartość domyślna dla indeks strumienia to 1 * ``isInsert() -> true``
* ``isInsert(1) -> false``
*********************************
<code>isMatch</code>
==============================
<code><b>isMatch([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Sprawdza, czy jeśli wiersz jest dopasowywany w wyszukiwaniu. Dla przekształceń trwa więcej niż jeden strumień wejściowy można przekazać indeks strumienia (od 1). Wartość domyślna dla indeks strumienia to 1 * ``isMatch() -> true``
* ``isMatch(1) -> false``
*********************************
<code>isNull</code>
==============================
...
Sprawdza, czy jeśli wiersz jest oznaczony do aktualizacji. Dla przekształceń trwa więcej niż jeden strumień wejściowy można przekazać indeks strumienia (od 1). Wartość domyślna dla indeks strumienia to 1 * ``isUpdate() -> true``
* ``isUpdate(1) -> false``
*********************************
<code>kurtosis</code>
==============================
...
Pobiera wartość pierwszych wierszy n parametr oceniane przed bieżącego wiersza. Drugi parametr jest liczba wierszy Wstecz, a wartość domyślna to 1. Jeśli nie są dowolną liczbę wierszy, które zostanie zwrócona wartość null, jeśli nie określono wartości domyślnej * ``lag(amount, 2) -> 60``
...
Pobiera wartość pierwszych wierszy n parametr oceniane po bieżącego wiersza. Drugi parametr jest liczba wierszy do wyszukiwania do przodu, a wartość domyślna to 1. Jeśli nie są dowolną liczbę wierszy, które zostanie zwrócona wartość null, jeśli nie określono wartości domyślnej * ``lead(amount, 2) -> 60``
...
Zwraca wartość NULL. Użyj syntax(null()) — funkcja, przypadku nazwę kolumny o nazwie "null". Każdej operacji, która używa spowoduje o wartości NULL * ``custId = NULL (for derived field)``
* ``custId == NULL -> NULL``
* ``'nothing' + NULL -> NULL``
* ``10 * NULL -> NULL'``
* ``NULL == '' -> NULL'``
...
Oblicza Skrót SHA-2 zestawu kolumn o różnym poziomie pierwotne typy danych, biorąc pod uwagę długość w bitach, które mogą być tylko 0(256) wartości 224, 256, 384, 512. Może służyć do obliczania odcisku palca dla wiersza * ``sha2(256, 'gunchus', 8.2, 'bojjus', true, toDate('2010-4-4')) -> 'd3b2bff62c3a00e9370b1ac85e428e661a7df73959fa1a96ae136599e9ee20fd'``
*********************************
<code>sin</code>
==============================
...
Konwertuje ciąg na wartość typu date, biorąc pod uwagę format daty opcjonalne. Zobacz Java SimpleDateFormat dla wszystkich możliwych formatów. W przypadku pominięcia format daty następujących kombinacji są akceptowane. [rrrr, rrrr-[M] M, rrrr-[M] min [d]-d, rrrr-[M] M-[d] d, rrrr-[M] M-[d] d, rrrr-[M] M-[d] dT *] * ``toDate('2012-8-8') -> 2012-8-8``
...
Konwertuje element podstawowy datatype. W przypadku liczb i dat można określić formatu. Jeśli nie zostanie podany, domyślny system jest pobierany. Format dziesiętny języka Java jest używana w przypadku liczb. Można znaleźć Java SimpleDateFormat wszystkie formaty daty możliwe; domyślny format to RRRR MM-dd * ``toString(10) -> '10'``
...
Konwertuje ciąg na wartość typu date podany format znacznika czasu opcjonalne. Zobacz Java SimpleDateFormat dla wszystkich możliwych formatów. W przypadku pominięcia sygnaturę czasową Domyślny wzorzec rrrr-[M] M-[d] d hh: mm: [. f...] jest używany * ``toTimestamp('2016-12-31 00:12:00') -> 2012-8-8T00:12:00``
...
==============================
<code><b>log(<i>&lt;value1&gt;</i> : number, [<i>&lt;value2&gt;</i> : number]) => double</b></code><br/><br/>
Oblicza wartość funkcji dziennika. Opcjonalne podstawowy może być inne podany numer Eulera, jeśli używana * ``log(100, 10) -> 2``
*********************************
<code>log10</code>
==============================
<code><b>log10(<i>&lt;value1&gt;</i> : number) => double</b></code><br/><br/>
