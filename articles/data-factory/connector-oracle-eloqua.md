---
title: Kopiowanie danych z programu Oracle Eloqua przy użyciu Azure Data Factory (wersja zapoznawcza)
...
| endpoint | Punkt końcowy serwera Eloqua. Usługa Eloqua obsługuje wiele centrów danych, aby określić punkt końcowy, zaloguj się do https://login.eloqua.com z poświadczeniami, a następnie skopiuj **podstawową część adresu URL** z przekierowanego adresu URL ze wzorcem `xxx.xxx.eloqua.com`. | Tak |
| nazwa użytkownika | Nazwa witryny i nazwa użytkownika konta Eloqua w formularzu: `SiteName\Username` np. `Eloqua\Alice`.  | Tak |
