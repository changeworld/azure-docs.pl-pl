...
![Zrzut ekranu przedstawiający relacje między kontem, kontenery i stronicowe obiekty BLOB](./media/storage-blob-pageblob-overview/storage-blob-pageblob-overview-figure1.png)
...
Aby zmienić rozmiar stronicowych obiektów blob, po utworzeniu, należy użyć [rozmiar](/dotnet/api/microsoft.azure.storage.blob.cloudpageblob.resize) metody. Żądany rozmiar powinien być wielokrotnością liczby 512 bajtów.
...
Na poniższej ilustracji pokazano operację odczytu z przesunięciem 256 i rozmiar zakresu 4352. Dane zwracane jest wyróżniany kolorem pomarańczowym. Dla stron NUL są zwracane zer.

![](./media/storage-blob-pageblob-overview/storage-blob-pageblob-overview-figure3.png)

Jeśli słabo wypełnionych obiektów blob, można po prostu pobrać regionów prawidłową stronę, aby uniknąć płacenia za egressing zero bajtów i zmniejszyć opóźnienia pobierania.  Aby określić, które strony są wspierane przez dane, użyj [CloudPageBlob.GetPageRanges](/dotnet/api/microsoft.azure.storage.blob.cloudpageblob.getpageranges). Można wyliczyć zwrócone zakresów i pobrać dane w każdym zakresie. 
...
Magazyn w warstwach standardowa i premium są trwałego magazynu, gdzie dane obiektów blob strony są zawsze replikowane w celu zapewnienia trwałości i wysokiej dostępności. Aby uzyskać więcej informacji na temat nadmiarowości magazynu platformy Azure, zobacz ten [dokumentacji](../common/storage-redundancy.md). Azure stale dostarczał trwałości korporacyjnej dla dysków IaaS i stronicowe obiekty BLOB, dzięki wiodącej w branży zera [wycena współczynnik błędów](https://en.wikipedia.org/wiki/Annualized_failure_rate).
