...
Dla celów szyfrowania po stronie klienta za pomocą języka Java, zobacz [szyfrowania po stronie klienta za pomocą języka Java dla usługi Microsoft Azure Storage](storage-client-side-encryption-java.md).
...
Biblioteka klienta obsługuje obecnie szyfrowanie tylko całe obiektów blob. Ściślej mówiąc, szyfrowanie jest obsługiwane, gdy użytkownicy używają **UploadFrom*** metody lub **OpenWrite** metody. Pliki do pobrania, zarówno pełne i pobrania zakresu są obsługiwane.
...
> [!WARNING]
> W przypadku edytowania lub przekazywanie metadane obiektu blob, należy upewnić się, że te metadane są zachowywane. Jeśli załadujesz nowymi metadanymi bez takich metadanych, CEK opakowanej, IV i inne metadane, zostaną utracone, a zawartość obiektu blob nigdy nie być możliwe do pobierania.
> 
> 
...
4. Ze względu na te dodatkowe właściwości zastrzeżonych wymagana na potrzeby szyfrowania użytkownicy mogą teraz mieć tylko 250 właściwości niestandardowych zamiast 252. Całkowity rozmiar jednostki musi być mniejszy niż 1 MB.
...
* Microsoft.Azure.KeyVault.Core zawiera klucz Instrumentacji i IKeyResolver. To niewielki pakiet bez zależności. Biblioteki klienta usługi storage dla platformy .NET definiuje ją jako zależność.
* Microsoft.Azure.KeyVault zawiera klienta Key Vault REST.
...
> * Jeśli ustawisz metadane w obiekcie blob zaszyfrowanego może zastąpić metadane związane z szyfrowaniem, wymaganego do odszyfrowywania, ponieważ ustawienie metadanych nie jest dodatek. Dotyczy to również migawki Unikaj określania metadanych podczas tworzenia migawki obiektu blob zaszyfrowany. Jeśli należy ustawić metadane, należy wywołać **FetchAttributes** metodą najpierw uzyskać bieżące metadane szyfrowania i uniknąć współbieżne operacje podczas ustawiania metadanych.
> * Włącz **RequireEncryption** właściwości domyślne opcje żądania dla użytkowników, którzy powinny działać tylko w przypadku zaszyfrowanych danych. Aby uzyskać więcej informacji, zobacz poniżej.
> 
> 
...
Przykłady kodu, w tym artykule pokazują, ustawić zasady dotyczące szyfrowania i pracą z nimi za pomocą zaszyfrowanych danych, ale nie wskazują pracy z usługą Azure Key Vault. [Przykłady szyfrowania](https://github.com/Azure/azure-storage-net/tree/master/Samples/GettingStarted/EncryptionSamples) w serwisie GitHub pokazują bardziej szczegółowe scenariusz end-to-end dla obiektów blob, kolejek i tabel, a wraz z integracją usługi Key Vault.

### <a name="requireencryption-mode"></a>Tryb RequireEncryption
Użytkownicy mogą włączyć opcjonalnie tryb działania, w której muszą być szyfrowane wszystkie przekazywanie i pobieranie. W tym trybie próby przekazania danych bez zasady szyfrowania lub pobrać dane, które nie są szyfrowane w usłudze zakończy się niepowodzeniem na komputerze klienckim. **RequireEncryption** właściwość obiektu opcje żądania określa to zachowanie. Jeśli aplikacja spowoduje zaszyfrowanie wszystkich obiektów przechowywanych w usłudze Azure Storage, a następnie można ustawić **RequireEncryption** właściwości domyślne opcje żądanie dotyczące obiektu klienta usługi. Na przykład ustawić **CloudBlobClient.DefaultRequestOptions.RequireEncryption** do **true** do wymagania szyfrowania danych dla wszystkich obiektów blob operacji wykonywanych za pośrednictwem tego obiektu klienta.
...
Oprócz tworzenia zasady szyfrowania i ustawienie opcji żądania, należy określić **EncryptionResolver** w **TableRequestOptions**, lub ustawić na wartość atrybutu [EncryptProperty] Jednostka.
...
Jak wspomniano powyżej, jeśli jednostka implementuje TableEntity, następnie właściwości mogą być dekorowane za pomocą atrybutu [EncryptProperty] zamiast określania **EncryptionResolver**.
...
* [Samouczek: Szyfrowanie i odszyfrowywanie obiektów blob w Microsoft Azure Storage przy użyciu usługi Azure Key Vault](../blobs/storage-encrypt-decrypt-blobs-key-vault.md)
* Pobierz [biblioteki klienta usługi Azure Storage dla pakietu NuGet programu .NET](https://www.nuget.org/packages/WindowsAzure.Storage)
* Pobierz NuGet magazynu usługi Azure Key [Core](http://www.nuget.org/packages/Microsoft.Azure.KeyVault.Core/), [klienta](http://www.nuget.org/packages/Microsoft.Azure.KeyVault/), i [rozszerzenia](http://www.nuget.org/packages/Microsoft.Azure.KeyVault.Extensions/) pakietów  
