---
title: Publikowanie ofert usług zarządzanych w witrynie Azure Marketplace
...
---
# <a name="publish-a-managed-services-offer-to-azure-marketplace"></a>Publikowanie ofert usług zarządzanych w witrynie Azure Marketplace

W tym artykule dowiesz się, jak opublikować ofertę publicznych lub prywatnych usług zarządzanych w [portalu Azure Marketplace](https://azuremarketplace.microsoft.com) przy użyciu portalu cloud [partner,](https://cloudpartner.azure.com/)umożliwiając klientowi, który kupuje ofertę, wbudowane zasoby do zarządzania zasobami delegowanymi platformy Azure.

> [!NOTE]
> Aby utworzyć i opublikować te oferty, musisz mieć [prawidłowe konto w Centrum partnerskim.](../../marketplace/partner-center-portal/create-account.md) Jeśli nie masz jeszcze konta, [proces rejestracji](https://aka.ms/joinmarketplace) doprowadzi Cię przez etapy tworzenia konta w Centrum partnerów i rejestrowania się w programie Marketplace komercyjnych. Identyfikator sieci Partner Network (MPN) firmy Microsoft zostanie [automatycznie skojarzony](../../billing/billing-partner-admin-link-started.md) z publikowanych ofertami w celu śledzenia wpływu na zaangażowanie klientów.
>
...
Publikowanie oferty usług zarządzanych jest podobne do publikowania dowolnego innego typu oferty w portalu Azure Marketplace. Aby dowiedzieć się więcej o tym procesie, zobacz [Przewodnik publikowania w portalu Azure Marketplace i AppSource](../../marketplace/marketplace-publishers-guide.md) oraz zarządzanie [ofertami platformy Azure i portalu AppSource Marketplace.](../../marketplace/cloud-partner-portal/manage-offers/cpp-manage-offers.md) Należy również zapoznać się z [zasadami certyfikacji rynku komercyjnego,](https://docs.microsoft.com/legal/marketplace/certification-policies)w szczególności sekcją [Usługi zarządzane.](https://docs.microsoft.com/legal/marketplace/certification-policies#700-managed-services)

Gdy klient doda ofertę, będzie mógł delegować jedną lub więcej określonych subskrypcji lub grup zasobów, które następnie będą [dołączane do zarządzania zasobami delegowanymi platformy Azure.](#the-customer-onboarding-process) Należy zauważyć, że zanim subskrypcja (lub grupy zasobów w ramach subskrypcji) mogą być dołączane, subskrypcja musi być autoryzowana do dołączania przez ręczne zarejestrowanie dostawcy zasobów **Microsoft.ManagedServices.**

> [!IMPORTANT]
> Każdy plan w ofercie usług zarządzanych zawiera sekcję **Szczegóły manifestu,** w której definiujesz jednostki usługi Azure Active Directory (Azure AD) w dzierżawie, które będą miały dostęp do delegowanych grup zasobów i/lub subskrypcji dla klientów, którzy zakupią ten plan. Ważne jest, aby mieć świadomość, że każda grupa (lub użytkownik lub usługodawca), który zostanie w tym dołączeniu, będzie miała takie same uprawnienia dla każdego klienta, który kupuje plan. Aby przypisać różne grupy do pracy z każdym klientem, musisz opublikować oddzielny plan prywatny, który jest dostępny wyłącznie dla każdego klienta.

## <a name="create-your-offer-in-the-cloud-partner-portal"></a>Tworzenie oferty w portalu Cloud Partner Portal
...
Po delegowania subskrypcji klienta (lub co najmniej jednej grupy zasobów w ramach subskrypcji), dostawca zasobów **Microsoft.ManagedServices** zostanie zarejestrowany dla tej subskrypcji, a użytkownicy w dzierżawie będą mogli uzyskać dostęp do zasobów delegowanych zgodnie z autoryzacjami w ofercie.
