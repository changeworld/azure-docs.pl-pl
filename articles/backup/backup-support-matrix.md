---
title: Macierz obsługi Azure Backup
description: Zawiera podsumowanie ustawień i ograniczeń pomocy technicznej dla usługi Azure Backup.
author: dcurwin
manager: carmonm
ms.service: backup
ms.topic: conceptual
...
**Ona** | **Uzyskać**
--- | ---
**Magazyny w subskrypcji** | Do 500 magazynów Recovery Services w ramach jednej subskrypcji.
...
**Źródła danych w magazynie magazynu** | Maksymalna 54 400 GB. Nie ma limitu dla kopii zapasowych maszyny wirtualnej platformy Azure.
...
**Modyfikuj typ magazynu magazynów** | Możesz zmodyfikować typ replikacji magazynu (magazyn Geograficznie nadmiarowy lub Magazyn lokalnie nadmiarowy) dla magazynu przed zapisaniem kopii zapasowych. Po rozpoczęciu wykonywania kopii zapasowych w magazynie nie można modyfikować typu replikacji.

## <a name="on-premises-backup-support"></a>Obsługa kopii zapasowych w środowisku lokalnym

Oto nowości obsługiwane, jeśli chcesz utworzyć kopię zapasową maszyn lokalnych:

**Maszynie** | **Co to jest kopia zapasowa** | **Lokalizacja** | **Funkcje**
--- | --- | --- | ---
...
**Rozmiar dysku danych maszyny wirtualnej platformy Azure** | Obsługuje tworzenie kopii zapasowych maszyn wirtualnych przy użyciu każdego rozmiaru dysku do 30 TB i maksymalnie 256 TB połączonego ze wszystkimi dyskami w maszynie wirtualnej.

### <a name="azure-vm-backup-options"></a>Opcje kopii zapasowej maszyny wirtualnej platformy Azure
...
**Maszyna wirtualna platformy Azure z programem DPM** | Pliki, foldery, woluminy, stan systemu, dane aplikacji | Wykonaj kopię zapasową do lokalnego magazynu maszyny wirtualnej platformy Azure, na którym działa program DPM. Program DPM następnie tworzy kopię zapasową w magazynie. | Migawki z obsługą aplikacji.<br/><br/> Pełny stopień szczegółowości tworzenia kopii zapasowych i odzyskiwania.<br/><br/> System Linux jest obsługiwany w przypadku maszyn wirtualnych (Hyper-V/VMware).<br/><br/> Oracle nie jest obsługiwany.
**Maszyna wirtualna platformy Azure z usługą serwera usługi MAB** | Pliki, foldery, woluminy, stan systemu, dane aplikacji | Utwórz kopię zapasową do lokalnego magazynu maszyny wirtualnej platformy Azure z systemem serwera usługi MAB. SERWERA usługi MAB następnie tworzy kopię zapasową w magazynie. | Migawki z obsługą aplikacji.<br/><br/> Pełny stopień szczegółowości tworzenia kopii zapasowych i odzyskiwania.<br/><br/> System Linux jest obsługiwany w przypadku maszyn wirtualnych (Hyper-V/VMware).<br/><br/> Oracle nie jest obsługiwany.

## <a name="linux-backup-support"></a>Obsługa kopii zapasowych w systemie Linux

W tym miejscu nowości są obsługiwane, jeśli chcesz utworzyć kopię zapasową maszyn z systemem Linux:

**Typ kopii zapasowej** | **Linux (potwierdzony przez platformę Azure)**
--- | ---
...
Obsługa deduplikacji dysków jest następująca:
...
- Deduplikacja nie jest obsługiwana na platformie Azure dla żadnego składnika kopii zapasowej. Po wdrożeniu programu DPM i serwera usługi MAB na platformie Azure dyski magazynu dołączone do maszyny wirtualnej nie mogą być deduplikowane.
...
- Dane kopii zapasowej są wysyłane za pośrednictwem bezpiecznego linku HTTPS.
- Dane kopii zapasowej są przechowywane w magazynie Recovery Services w postaci zaszyfrowanej.
- Tylko ty masz hasło do odblokowania tych danych. Firma Microsoft nie może odszyfrować danych kopii zapasowej w dowolnym momencie.

    > [!WARNING]
    > Po skonfigurowaniu magazynu tylko ty masz dostęp do klucza szyfrowania. Firma Microsoft nigdy nie utrzymuje kopii i nie ma dostępu do klucza. Jeśli klucz jest zagubiony, firma Microsoft nie może odzyskać danych kopii zapasowej.
...
- Azure Backup obsługuje Azure Disk Encryption, który używa funkcji BitLocker na maszynach wirtualnych z systemem Windows i **dm-crypt** na maszynach wirtualnych z systemem Linux.
...
**Maszynie** | **Podczas przesyłania** | **W spoczynku**
--- | --- | ---
...
**Maszynie** | **Kompresuj do serwera usługi MAB/DPM (TCP)** | **Kompresuj do magazynu (HTTPS)**
--- | --- | ---
**Bezpośrednia kopia zapasowa lokalnych maszyn z systemem Windows** | POTRĄCON | ![Tak][green]
**Tworzenie kopii zapasowych maszyn wirtualnych platformy Azure przy użyciu rozszerzenia maszyny wirtualnej** | POTRĄCON | POTRĄCON
...
**Maksymalna częstotliwość tworzenia kopii zapasowych w programie DPM/serwera usługi MAB** | Co 15 minut dla SQL Server<br/><br/> Raz na godzinę dla innych obciążeń
...
**Maksymalny okres przechowywania** | Zależy od częstotliwości tworzenia kopii zapasowej
