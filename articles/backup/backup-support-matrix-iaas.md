...
- [Macierz obsługi ogólne](backup-support-matrix.md) dla Backkup platformy Azure
- [Macierz obsługi](backup-support-matrix-mabs-dpm.md) usługi Kopia zapasowa Microsoft Azure backup server/modelu DOM
- [Macierz obsługi](backup-support-matrix-mars-agent.md) do utworzenia kopii zapasowej za pomocą agenta usług MARS
...
--- | ---
Włącz wykonywanie kopii zapasowej po utworzeniu maszyny Wirtualnej platformy Azure z systemem Windows | Obsługiwane w przypadku:  Windows Server 2016 (Datacenter/Datacenter Core); Windows Server 2012 R2 DataCenter; Windows Server 2008 R2 SP1
...
Tworzenie kopii zapasowych dysków zarządzanych, po włączeniu blokady grupy zasobów | Nieobsługiwane.<br/><br/> Kopia zapasowa nie można usunąć starsze punkty zasobów i kopie zapasowe będą kończyć się niepowodzeniem, jeśli osiągnięto maksymalny limit punktów przywracania.
Modyfikowanie zasad tworzenia kopii zapasowej dla maszyny Wirtualnej | Obsługiwane.<br/><br/> Maszyna wirtualna będzie można kopii zapasowej przy użyciu ustawień harmonogram i okres przechowywania w nowych zasad. Jeśli ustawienia przechowywania są przedłużony, istniejące punkty odzyskiwania są oznaczone i przechowywane. W przypadku są one istniejących punktów odzyskiwania będzie można przeczyścić w ramach następnego zadania oczyszczania, a ostatecznie usunięta.
...
Kopie zapasowe dziennie (za pośrednictwem rozszerzenia maszyny Wirtualnej platformy Azure) | Jeden zaplanowane tworzenie kopii zapasowej na dzień.<br/><br/> Może potrwać maksymalnie cztery kopii zapasowych na żądanie na dzień.
...
Korekta automatyczna zegara | Nieobsługiwane.<br/><br/> Usługa Azure Backup nie przyjmuje automatyczne dostosowanie zegara zmiany letniego pod uwagę podczas tworzenia kopii zapasowej maszyny Wirtualnej.<br/><br/>  Zmodyfikuj zasady ręcznego zgodnie z potrzebami.
[Funkcje zabezpieczeń dla hybrydowych kopii zapasowych](https://docs.microsoft.com/azure/backup/backup-azure-security-feature) |  Wyłączanie funkcji nie jest obsługiwane.



## <a name="operating-system-support-windows"></a>Obsługa systemu operacyjnego (Windows)

Tabela zawiera podsumowanie obsługiwanych systemów operacyjnych, podczas wykonywania kopii zapasowych maszyn wirtualnych Windows.

**Scenariusz** | **Obsługa systemu operacyjnego** 
--- | ---
Tworzenie kopii zapasowej za pomocą rozszerzenia agenta maszyny Wirtualnej platformy Azure | Klient Windows: Nieobsługiwane<br/><br/> Windows Server: Windows Server 2008 R2 lub nowszy 
...
Tworzenie kopii zapasowej za pomocą programu DPM/serwera usługi Mab | Obsługiwane systemy operacyjne dla kopii zapasowych z użyciem [serwera usługi Mab](backup-mabs-protection-matrix.md) i [DPM](https://docs.microsoft.com/system-center/dpm/dpm-protection-matrix?view=sc-dpm-1807).
...
Dla kopii zapasowych systemu Linux maszyny Wirtualnej platformy Azure, Azure Backup obsługuje systemu Linux [dystrybucje zalecane dla platformy Azure](https://docs.microsoft.com/azure/virtual-machines/linux/endorsed-distros). Należy pamiętać, że:
...
- Inne dystrybucje systemu Linux bring your own może działać tak długo, jak [agenta maszyny Wirtualnej platformy Azure dla systemu Linux](https://docs.microsoft.com/azure/virtual-machines/extensions/agent-linux) jest dostępna na maszynie Wirtualnej, i obsługuje język Python.
...
**Ustawienie** | **Limity** 
--- | --- 
Maksymalna liczba punktów odzyskiwania dla każdego chronionego wystąpienia (/ obciążenie maszyny | 9999
...
W tabeli przedstawiono obsługę kopii zapasowych podczas zadania zarządzania maszyny Wirtualnej, takie jak dodawanie lub zastępowanie dysków maszyny Wirtualnej.
...
Przywracanie do konta magazynu używających magazynu ZRS | Nieobsługiwane
...
Przywracanie kopii zapasowej niezarządzanych maszyn wirtualnych po uaktualnieniu do zarządzanego | Obsługiwane.<br/><br/> Przywróć dyski i następnie utworzyć zarządzanej maszyny Wirtualnej.
...
Przywracanie kontrolera domeny (DC) maszyny Wirtualnej, która jest częścią konfiguracji kontrolera domeny multi, za pośrednictwem portalu | Obsługiwane w przypadku przywracania dysku i utworzenia maszyny Wirtualnej przy użyciu programu PowerShell.
Przywróć maszynę Wirtualną w innej sieci wirtualnej |  Obsługiwane.<br/><br/> Sieć wirtualna musi być w tej samej subskrypcji i regionu.
...
Tworzenie kopii zapasowych maszyn wirtualnych w [zestawów dostępności](https://docs.microsoft.com/azure/virtual-machines/windows/regions-and-availability#availability-sets) | Obsługiwane.<br/><br/> Nie można przywrócić Maszynę wirtualną w zestawie dostępności przy użyciu opcji, aby szybko utworzyć Maszynę wirtualną. Zamiast tego podczas przywracania maszyny Wirtualnej należy do przywracania dysku i użyj go, aby wdrożyć Maszynę wirtualną, lub przywrócić dysk i użyć go do zastąpienia istniejącego dysku. 
...
Tworzenie kopii zapasowych maszyn wirtualnych wdrożonych przy użyciu [korzyść użycia hybrydowego (HUB)](https://docs.microsoft.com/azure/virtual-machines/windows/hybrid-use-benefit-licensing) | Obsługiwane.
Tworzenie kopii zapasowych maszyn wirtualnych wdrożonych w [zestawu skalowania](https://docs.microsoft.com/azure/virtual-machine-scale-sets/overview) |   Nieobsługiwane   
Tworzenie kopii zapasowych maszyn wirtualnych wdrażanych za pomocą [portalu Azure Marketplace](https://azuremarketplace.microsoft.com/en-us/marketplace/apps?filters=virtual-machine-images)<br/><br/> (Opublikowane przez firmę Microsoft i innych firm) |   Obsługiwane.<br/><br/> Maszyna wirtualna musi działać obsługiwany system operacyjny.<br/><br/> W przypadku odzyskiwania plików na maszynie Wirtualnej, można przywrócić tylko do zgodnych systemów operacyjnych (nie poprzedni system operacyjny).
Tworzenie kopii zapasowych maszyn wirtualnych wdrożonych za pomocą niestandardowego obrazu (innej firmy) |    Obsługiwane.<br/><br/> Maszyna wirtualna musi działać obsługiwany system operacyjny.<br/><br/> W przypadku odzyskiwania plików na maszynie Wirtualnej, można przywrócić tylko do zgodnych systemów operacyjnych (nie poprzedni system operacyjny).
Tworzenie kopii zapasowych maszyn wirtualnych z migracji na platformę Azure   | Obsługiwane.<br/><br/> Aby utworzyć kopię zapasową maszyny Wirtualnej, musi być zainstalowany agent maszyny Wirtualnej, na zmigrowanej maszynie. 
...
Rozmiar dysku danych | Każdy dysk może mieć rozmiar do 4095 GB.<br/><br/> Jeśli używasz najnowszej wersji kopii zapasowej maszyny Wirtualnej platformy Azure (nazywanych, natychmiastowe przywracanie) dysków o rozmiarach do 4TB są obsługiwane. [Dowiedz się więcej](backup-instant-restore-capability.md).
Typ magazynu | Standardowa HDD i SSD w warstwie standardowa, premium SSD <br/><br/> SSD w warstwie standardowa jest obsługiwany, jeśli używasz najnowszej wersji kopii zapasowej maszyny Wirtualnej platformy Azure (nazywanych, natychmiastowe przywracanie), SSD w warstwie standardowa jest obsługiwany. [Dowiedz się więcej](backup-instant-restore-capability.md).
Dyski zarządzane | Obsługiwane
Zaszyfrowanych dysków | Obsługiwane.<br/><br/> Maszyny wirtualne platformy Azure, włączone za pomocą szyfrowania dysków Azure (ADE) utworzeniem kopii zapasowej (z lub bez aplikację usługi Azure AD).<br/><br/> Nie można odzyskać zaszyfrowanych maszyn wirtualnych na poziomie plików/folderów. Musisz odzyskać całą maszynę Wirtualną.<br/><br/> Użytkownik może włączyć szyfrowanie na maszynach wirtualnych, które są już chronione przez usługę Azure Backup.
...
**Składnik** | **Pomoc techniczna**
--- | ---
Liczba kart sieciowych | Maksymalnie maksymalna liczba kart sieciowych obsługiwanych dla określonego rozmiaru maszyny Wirtualnej platformy Azure.<br/><br/> Karty sieciowe są tworzone podczas tworzenia maszyny Wirtualnej podczas procesu przywracania.<br/><br/> Liczba kart sieciowych do przywróconej maszyny wirtualnej odzwierciedla liczbę kart sieciowych na maszynie Wirtualnej, po włączeniu ochrony. Usuwanie kart sieciowych po włączeniu nie wpływa na liczbę.
Zewnętrzne/wewnętrznego modułu równoważenia obciążenia |   Obsługiwane. <br/></br> [Dowiedz się więcej](backup-azure-arm-restore-vms.md#restore-vms-with-special-network-configurations) dotyczące przywracania maszyn wirtualnych przy użyciu ustawień sieciowych.
Wiele zastrzeżonego adresu IP |  Obsługiwane. <br/></br> [Dowiedz się więcej](backup-azure-arm-restore-vms.md#restore-vms-with-special-network-configurations) dotyczące przywracania maszyn wirtualnych przy użyciu ustawień sieciowych.
Maszyny wirtualne z wieloma kartami sieciowymi  | Obsługiwane. <br/></br> [Dowiedz się więcej](backup-azure-arm-restore-vms.md#restore-vms-with-special-network-configurations) dotyczące przywracania maszyn wirtualnych przy użyciu ustawień sieciowych.
Maszyny wirtualne z publicznych adresów IP    | Obsługiwane.<br/><br/> Musisz skojarzyć istniejącego publicznego adresu IP z karty Sieciowej, lub Utwórz adres i skojarzyć ją z kartą Sieciową, po zakończeniu przywracania.
...
Punkty końcowe usługi sieci wirtualnej  | Obsługiwane.<br/><br/> Ustawienia zapory i sieci wirtualnej konta magazynu powinien zezwalać na dostęp ze wszystkich sieci. 



## <a name="vm-securityencryption-support"></a>Obsługa szyfrowania/zabezpieczeń maszyny Wirtualnej
