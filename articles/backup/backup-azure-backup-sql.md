---
title: 19.00.
description: Wprowadzenie do tworzenia kopii zapasowych baz danych programu SQL Server przy użyciu usługi Azure Backup
ms.topic: conceptual
ms.date: 01/30/2019
...
---
# <a name="back-up-sql-server-to-azure-as-a-dpm-workload"></a>19.00.

W tym artykule można przejść przez kroki konfiguracji do tworzenia kopii zapasowych baz danych programu SQL Server przy użyciu usługi Azure Backup.

Aby wykonać tworzenie kopii zapasowych baz danych programu SQL Server na platformie Azure, potrzebujesz konta platformy Azure. Jeśli go nie masz, możesz utworzyć bezpłatne konto w ciągu zaledwie kilku minut. Aby uzyskać więcej informacji, zobacz [Tworzenie bezpłatnego konta platformy Azure](https://azure.microsoft.com/pricing/free-trial/).

Aby zrobić kopii zapasowej bazy danych programu SQL Server na platformie Azure i odzyskać ją z platformy Azure:
...
Przed rozpoczęciem upewnij się, że spełniasz [wymagania wstępne dotyczące](backup-azure-dpm-introduction.md#prerequisites-and-limitations) korzystania z usługi Azure Backup w celu ochrony obciążeń. Oto niektóre z zadań wstępnych: 
* Utwórz magazyn kopii zapasowych.
* Pobierz poświadczenia magazynu. 
* Zainstaluj agenta usługi Azure Backup.
* Zarejestruj serwer w przechowalni.
...
1. Na serwerze Menedżera ochrony danych (DPM) wybierz obszar roboczy **Ochrona.**
...
1. Rozwiń komputer programu SQL Server, na którym znajdują się bazy danych, których chcesz szesnastą. Zostaną wyświetlonych źródeł danych, których kopię zapasową można wyw. Rozwiń **rozwiń wszystkie udziały SQL,** a następnie wybierz bazy danych, których chcesz udostępnić. W tym przykładzie wybieramy ReportServer$MSDPM2012 i ReportServer$MSDPM2012TempDB. Następnie wybierz **przycisk Dalej**.
...
1. Wybierz **pozycję Dalej**. Program DPM pokazuje ogólną dostępną przestrzeń dyskową. Pokazuje również potencjalne wykorzystanie miejsca na dysku.

    ![Konfigurowanie alokacji dysku](./media/backup-azure-backup-sql/pg-storage.png)

    Domyślnie program DPM tworzy jeden wolumin na źródło danych (baza danych programu SQL Server). Wolumin jest używany do początkowej kopii zapasowej. W tej konfiguracji Menedżer dysków logicznych (LDM) ogranicza ochronę programu DPM do 300 źródeł danych (baz danych programu SQL Server). Aby obejść to ograniczenie, wybierz **opcję Co-locate data in DPM Storage Pool**. Jeśli używasz tej opcji, program DPM używa jednego woluminu dla wielu źródeł danych. Ta konfiguracja umożliwia programowi DPM ochronę do 2000 baz danych programu SQL Server.

    Jeśli **wybierzesz opcję Automatycznie zwiększaj woluminy,** program DPM może uwzględnić zwiększony wolumin kopii zapasowej w miarę wzrostu danych produkcyjnych. Jeśli nie **wybierzesz opcji Automatycznie zwiększaj woluminy,** program DPM ograniczy magazyn kopii zapasowych do źródeł danych w grupie ochrony.
...
1. Jeśli jesteś administratorem, możesz wybrać harmonogramy tworzenia kopii zapasowych i zasady przechowywania, które odpowiadają zasadom organizacji.
    ...
    >
    > Użyj programu DPM, aby zaplanować tworzenie kopii zapasowych platformy Azure po zakończeniu tworzenia kopii zapasowych na dysku lokalnym. Zgodnie z tą praktyką najnowsza kopia zapasowa dysku jest kopiowana na platformę Azure.
    >
...
1. Program DPM pokazuje szczegóły punktu odzyskiwania. Wybierz **pozycję Dalej**. Aby zastąpić bazę danych, wybierz typ odzyskiwania **Recover to original instance of SQL Server**. Następnie wybierz **przycisk Dalej**.
    ...
    W tym przykładzie program DPM umożliwia odzyskanie bazy danych do innego wystąpienia programu SQL Server lub do autonomicznego folderu sieciowego.
