---
title: Zainstaluj aktualizację 1.1 na macierz wirtualną StorSimple | Dokumentacja firmy Microsoft
description: Opisuje sposób stosowania aktualizacji za pomocą witryny Azure portal i lokalnego internetowego interfejsu użytkownika dla rozwiązania StorSimple Virtual Array
services: storsimple
...
Możesz zastosować aktualizacje oprogramowania lub poprawek w celu zapewnienia aktualności rozwiązania StorSimple Virtual Array. Przed zastosowaniem aktualizacji zaleca się wykonanie woluminy lub udziały w tryb offline na hoście pierwszy i następnie urządzenia. W ten sposób możliwości uszkodzenie danych. Po woluminy lub udziały są w trybie offline, należy również wziąć ręcznego tworzenia kopii zapasowych urządzenia.

> [!IMPORTANT]
> - Aktualizacja 1.1 odnosi się do **10.0.10307.0** wersję oprogramowania na urządzeniu. Aby uzyskać informacji na temat nowości w tej aktualizacji, przejdź do [informacje o wersji 1.1 aktualizacji](storsimple-virtual-array-update-11-release-notes.md).
>
...
>
> - Aktualizacja 1.1 jest dostępna w witrynie Azure portal, tylko wtedy, gdy macierz wirtualna jest uruchomiona Update 1. Dla macierzy wirtualnych aktualizacji Update 0.6 wersjami należy najpierw zainstalować aktualizację 1.0, a następnie Zastosuj aktualizację 1.1.
...
Jeśli z aktualizacją Update 0,2 lub nowszy, zaleca się zainstalowanie aktualizacji w witrynie Azure portal. Procedury portalu wymaga od użytkownika skanowania, pobieranie i instalowanie aktualizacji. W zależności od wersji oprogramowania, którego macierz wirtualna jest uruchomiona, stosowania aktualizacji za pośrednictwem witryny Azure portal jest inny.
 ...
 - Macierz wirtualna jest uruchomiona Update 1, witryny Azure portal bezpośrednio instaluje aktualizację 1.1 (10.0.10307.0) na urządzeniu. Ta procedura trwa około 10 minut.
 - Jeśli macierz wirtualna jest uruchomiona w aktualizacji Update 0.6, aktualizacja jest wykonywane w dwuetapowy. Witryny Azure portal najpierw instaluje aktualizację 1.0 (10.0.10296.0) na urządzeniu. Ponowne uruchomienie macierzy wirtualnej, a następnie instaluje aktualizację 1.1 (10.0.10307.0) na urządzeniu z systemem w portalu. Ta procedura trwa około 15 minut.
...
Po zakończeniu instalacji przejdź do usługi Menedżer urządzeń StorSimple. Wybierz **urządzeń** a następnie wybierz i kliknij urządzenie właśnie został zaktualizowany. Przejdź do **Ustawienia > Zarządzaj > aktualizacje urządzeń**. Wersja oprogramowania wyświetlane powinna być **10.0.10307.0**.
...
> [!IMPORTANT] 
> **Kontynuuj tę aktualizację tylko wtedy, gdy są uruchomione Update 1 (10.0.10296.0). W przypadku korzystania z aktualizacji 0.6, [zainstaluj aktualizację 1](storsimple-virtual-array-install-update-1.md) na urządzeniu z systemem pierwsze, a następnie zastosować aktualizację 1.1.**

### <a name="download-the-update-or-the-hotfix"></a>Pobieranie aktualizacji lub poprawki

Wykonaj poniższe kroki, aby pobrać aktualizację 1.1 z wykazu usługi Microsoft Update.
...
3. W polu wyszukiwania wykazu usługi Microsoft Update wprowadź numer bazy wiedzy Knowledge Base (KB) poprawki, którą chcesz pobrać. Wprowadź **4337628** aktualizacji 1.1, a następnie kliknij polecenie **wyszukiwania**.
   
    Lista poprawek zawierająca pojawia się na przykład **1.1 aktualizacji macierzy wirtualnej StorSimple**.
...
6. Otwórz folder, w którym znajdują się pliki.
    ...
    - Plik, który zawiera aktualizacje zbiorcze w czerwcu `Windows8.1-KB4284815-x64`. Aby uzyskać więcej informacji o to, co jest uwzględnione w tym pakiecie zbiorczym, przejdź do [czerwca miesięczne zabezpieczeń zbiorczy](https://support.microsoft.com/help/4284815/windows-81-update-kb4284815).
...
Przed instalacją aktualizacji lub poprawki upewnij się, że:
 ...
 - Macierz wirtualna jest uruchomiona Update 1 (10.0.10296.0). Jeśli korzystasz z aktualizacji 0.6, [zainstaluj aktualizację 1](storsimple-virtual-array-install-update-1.md) pierwszy, a następnie zainstaluj aktualizację 1.1.
...
1. W lokalnym internetowym interfejsie użytkownika, przejdź do **konserwacji** > **aktualizacji oprogramowania**. Zanotuj wersji oprogramowania, które są uruchomione. **Kontynuuj tę aktualizację tylko wtedy, gdy są uruchomione Update 1 (10.0.10296.0). W przypadku korzystania z aktualizacji 0.6, [zainstaluj aktualizację 1](storsimple-virtual-array-install-update-1.md) na urządzeniu z systemem pierwsze, a następnie zastosować aktualizację 1.1.**
...
5. Po ponownym uruchomieniu, nastąpi przekierowanie do **Zaloguj** strony. Aby zweryfikować, że oprogramowanie urządzenia został zaktualizowany w lokalnym internetowym interfejsie użytkownika, przejdź do **konserwacji** > **aktualizacji oprogramowania**. Wersja oprogramowania wyświetlane powinna być **10.0.0.0.0.10307** 1.1 aktualizacji.
   
   > [!NOTE]
   > Firma Microsoft raport wersje oprogramowania w nieco inny sposób w lokalnym internetowym interfejsie użytkownika, jak i portalu Azure. Na przykład raporty lokalnego Interfejsu w przeglądarce **10.0.0.0.0.10307** i Azure portal raporty **10.0.10307.0** dla tej samej wersji.
   
    ![aktualizowanie urządzenia](./media/storsimple-virtual-array-install-update-11/update6m.png)

6. Powtórz kroki 2 – 4, aby zainstalować poprawkę zabezpieczeń Windows przy użyciu pliku `Windows8.1-KB4284815-x64`. Macierz wirtualna ponownego uruchomienia po zakończeniu instalacji i musisz zalogować się do lokalnego Interfejsu w przeglądarce.
