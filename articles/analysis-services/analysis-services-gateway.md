...
- **Pobierz i uruchom Instalatora** — ten krok powoduje zainstalowanie usługi bramy na komputerze w organizacji. Logujesz się również na platformie Azure przy użyciu konta w [](/previous-versions/azure/azure-services/jj573650(v=azure.100)#what-is-an-azure-ad-tenant) usłudze Azure AD swojej dzierżawy. Konta B2B (gość) platformy Azure nie są obsługiwane.
...
Brama tworzy połączenie wychodzące do Azure Service Bus. Komunikuje się on na portach wychodzących: TCP 443 (domyślnie), 5671, 5672, 9350 za pośrednictwem 9354.  Brama nie wymaga portów przychodzących.

Może być konieczne dozwolonych adresów IP dla obszaru danych w zaporze. Możesz pobrać [listę adresów IP Microsoft Azure centrum](https://www.microsoft.com/download/details.aspx?id=41653)danych. Ta lista jest aktualizowana co tydzień. Adresy IP wymienione na liście adresów IP centrum danych platformy Azure są w notacji CIDR. Aby dowiedzieć się więcej, zobacz [Routing bezklasowy między domenami](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing).
