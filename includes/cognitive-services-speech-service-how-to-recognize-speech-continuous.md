---
author: wolfma61
ms.service: cognitive-services
ms.topic: include
origin.date: 07/27/2018
ms.date: 04/01/2019
ms.author: v-biyu
ms.openlocfilehash: 8c8b8548436a9e2e2f49e8b40dc88cc4df28ff64
ms.sourcegitcommit: 3102f886aa962842303c8753fe8fa5324a52834a
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 04/23/2019
ms.locfileid: "60535798"
---
## <a name="continuous-speech-recognition-from-a-file"></a>Rozpoznawanie mowy z pliku

Poniższy fragment kodu rozpoznaje stale wejście mowy z pliku audio w języku domyślnym (en US). Obsługiwany format to WAV jednego kanału (mono) / PCM o częstotliwości próbkowania 16 kHz.

[!INCLUDE [Sample audio](cognitive-services-speech-service-sample-audio.md)]

