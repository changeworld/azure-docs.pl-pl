...
ms.locfileid: "75942793"
---
Rodzina maszyn wirtualnych serii B umożliwia wybranie rozmiaru maszyny wirtualnej, który zapewnia niezbędną wydajność na poziomie podstawowym dla obciążenia, dzięki czemu można nawiązać cykl wydajności procesora CPU do 100% procesora Intel® Broadwell E5-2673 v4 2,3 GHz lub procesor Intel® Haswell 2,4 GHz E5-2673 v3 vCPU.
...
| Rozmiar             | vCPU  | Pamięć: GiB | Magazyn tymczasowy (SSD): GiB | Podstawowa wydajność procesora CPU maszyny wirtualnej | Maksymalna wydajność procesora dla maszyny wirtualnej | Kredyty początkowe | Uzyskane kredyty/godzinę | Maksymalne dochody | Maksymalna liczba dysków danych | Maksymalna przepływność magazynu w pamięci podręcznej i tymczasowej: IOPS/MB/s | Maksymalna przepływność niebuforowanych dysków: liczba operacji we/wy na sekundę / MB/s | Maksymalna liczba kart interfejsów sieciowych |          
|---------------|-------------|----------------|----------------------------|-----------------------|--------------------|--------------------|--------------------|----------------|----------------------------------------|-------------------------------------------|-------------------------------------------|----------|
...
![Wykres danych ruchu godzinowego](./media/virtual-machines-common-b-series-burstable/office-workload.png)

| Scenariusz | Czas | Użycie procesora CPU (%) | Środki skumulowane<sup>1</sup> | Dostępne kredyty |
| --- | --- | --- | --- | --- |
| Wdrożenie B16ms | Wdrażanie | Wdrażanie  | 480 (końcowe środki) | 480 |
...
Odp **.: będziemy**wprowadzać 2 nowe metryki w nadchodzących tygodniach, dzięki czemu Metryka **kredytowa** umożliwi wyświetlenie liczby kredytów uzyskanych przez maszynę wirtualną, a Metryka **CONSUMEDCREDIT** pokazuje, ile kredytów CPU zużywał przez maszynę wirtualną od banku.    Te metryki będą widoczne w okienku metryki w portalu lub programowo za pośrednictwem interfejsów API Azure Monitor.

Aby uzyskać więcej informacji na temat uzyskiwania dostępu do danych metryk dla platformy Azure, zobacz [Omówienie metryk w Microsoft Azure](../articles/monitoring-and-diagnostics/monitoring-overview-metrics.md).
...
### <a name="q-what-happens-if-i-run-out-of-credits"></a>P: co się stanie, jeśli wykorzystałem kredyty?
Odp **.: Jeśli** maszyna wirtualna spędza wszystkie dostępne kredyty, wówczas może korzystać tylko z bazowego procesora CPU i nie będzie mogła przekroczyć tej linii bazowej. 
