...
| Sieć wirtualna {nazwa_sieci_wirtualnej} nie istnieje. |Może to się zdarzyć, jeśli sieć wirtualna została utworzona w nowej witrynie Azure Portal. Rzeczywista nazwa sieci wirtualnej jest \<zgodna ze wzorcem "Grupa * nazwa sieci wirtualnej>" |
...
| Migracja jest niedozwolona dla usługi hostowanej {nazwa_usługi_hostowanej}, ponieważ ma ona maszynę wirtualną {nazwa_maszyny_wirtualnej} w stanie: RoleStateUnknown. Migracja jest dozwolona tylko wtedy, gdy maszyna wirtualna jest w jednym z następujących stanów: Uruchomiona, Zatrzymana, Zatrzymana bez alokacji. |Maszyna wirtualna może być w trakcie przejścia stanu, co zwykle dzieje się podczas operacji aktualizacji na HostedService, takich jak ponowne uruchomienie, instalacja rozszerzenia itp. Zaleca się, aby operacja aktualizacji została ukończona w usłudze HostedService przed próbą migracji. |
...
| Migracja wdrożenia {nazwa_wdrożenia} w usłudze hostowanej {nazwa_usługi_w_chmurze} nie jest dozwolona, ponieważ nie znajduje się ono w zakresie regionalnym. Aby przenieść to\/wdrożenie do zakresu regionalnego, należy zapoznać się z adresem https: /aka.ms/regionalscope. | W 2014 r. zespół platformy Azure zapowiedział, że zasoby sieciowe zostaną przeniesione z zakresu poziomu klastra do zakresu regionalnego. Zobacz [https://aka.ms/regionalscope](https://aka.ms/regionalscope) więcej szczegółów . Ten błąd zdarza się, gdy migrowane wdrożenie nie zostało poddane operacji aktualizacji, która automatycznie przenosi je do zakresu regionalnego. Najlepszym obejściem tego problemu jest dodanie punktu końcowego lub dysku danych do maszyny wirtualnej, a następnie ponowienie próby przeprowadzenia migracji. <br> Zobacz [How to set up endpoints on a classic Windows virtual machine in Azure](/previous-versions/azure/virtual-machines/windows/classic/setup-endpoints#create-an-endpoint) (Jak skonfigurować punkty końcowe na klasycznej maszynie wirtualnej z systemem Windows na platformie Azure) lub [Dołączanie dysku danych do maszyny wirtualnej systemu Windows przy użyciu klasycznego modelu wdrażania](../articles/virtual-machines/windows/classic/attach-disk.md)|
| Migracja nie jest obsługiwana dla sieci wirtualnej {vnet-name}, ponieważ ma ona wdrożenia paaS bez bramy. | Ten błąd występuje, gdy masz nie-bramy paaS wdrożeń, takich jak brama aplikacji lub usługi zarządzania interfejsami API, które są połączone z siecią wirtualną.|
