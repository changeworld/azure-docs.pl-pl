...
| Maszyna wirtualna {nazwa_maszyny_wirtualnej} w usłudze hostowanej {nazwa_usługi_hostowanej} zawiera rozszerzenie VMSnapshot/VMSnapshotLinux, które nie jest obecnie obsługiwane dla migracji. Odinstaluj je z maszyny wirtualnej i dodaj ponownie za pomocą usługi Azure Resource Manager po zakończeniu migracji |Jest to scenariusz, w którym maszyna wirtualna jest skonfigurowana na potrzeby usługi Azure Backup. Ponieważ aktualnie jest to nieobsługiwany scenariusz, wykonaj rozwiązania na https://aka.ms/vmbackupmigration |
...
| Migracja wdrożenia {nazwa_wdrożenia} w usłudze hostowanej {nazwa_usługi_w_chmurze} nie jest dozwolona, ponieważ nie znajduje się ono w zakresie regionalnym. Zapoznaj się z http://aka.ms/regionalscope przenoszenia to wdrożenie do zakresu regionalnego. | W 2014 r. zespół platformy Azure zapowiedział, że zasoby sieciowe zostaną przeniesione z zakresu poziomu klastra do zakresu regionalnego. Zobacz [http://aka.ms/regionalscope] więcej szczegółów (http://aka.ms/regionalscope). Ten błąd zdarza się, gdy migrowane wdrożenie nie zostało poddane operacji aktualizacji, która automatycznie przenosi je do zakresu regionalnego. Najlepszym obejściem tego problemu jest dodanie punktu końcowego lub dysku danych do maszyny wirtualnej, a następnie ponowienie próby przeprowadzenia migracji. <br> Zobacz [How to set up endpoints on a classic Windows virtual machine in Azure](../articles/virtual-machines/windows/classic/setup-endpoints.md#create-an-endpoint) (Jak skonfigurować punkty końcowe na klasycznej maszynie wirtualnej z systemem Windows na platformie Azure) lub [Dołączanie dysku danych do maszyny wirtualnej systemu Windows przy użyciu klasycznego modelu wdrażania](../articles/virtual-machines/windows/classic/attach-disk.md)|
| Migracja nie jest obsługiwana dla sieci wirtualnej {vnet-name}, ponieważ zawiera ona wdrożenia PaaS innych niż bramy. | Ten błąd występuje, gdy masz wdrożeń typu PaaS-brama, takie jak usługi interfejsu API zarządzania lub bramy aplikacji, które są podłączone do sieci wirtualnej.|
